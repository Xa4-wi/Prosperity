...
V50:
Works: Yes
Improvement: Fresh clean-slate bot built from scratch around the reusable patterns extracted from public Prosperity writeups rather than by patching the old family. The architecture is intentionally explicit: raw book parsing -> reusable feature/state layer -> memory -> regime -> fair value -> target position -> reservation price -> execution -> final orders. EMERALDS uses a stable-product fixed-fair market maker with zero-edge clearing and inside-spread quoting; TOMATOES uses a persistent wall-fair / large-quote estimator, a small rolling state store, a simple regime classifier, one main reservation-price inventory control, and explicit one-sided execution in directional states.
Notes: This is a foundation bot, not yet a top-score challenger, but it is already viable. After one grounded EMERALDS quoting fix, Rust PnL reached 12'181 with EMERALDS at 7'717 and TOMATOES at 4'464 over 828 trades. Officially it transferred to 1'989.4375 with EMERALDS still perfect at 1'050.0 and TOMATOES at 939.4375. That confirms the architecture is sound, but also that the clean first-cut TOMATOES engine is much too slow and too polite compared with the stronger official family: it traded a lot, but at weak prices and with too little directional edge capture. So `V50` is a real clean baseline, but not yet close to `V39.4`'s 2'640.875 / 1'590.875 line.
PnL: Official 1'989.4375 | Rust 12'181

V50.1:
Works: Yes
Improvement: First reaction-speed and execution-feedback patch on top of `V50`. TOMATOES now uses a faster `fast_fair - slow_fair` trend signal, lower regime thresholds, less eager trend-aligned inventory clearing, explicit reversal-cover behavior when caught on the wrong side of a move, and a lightweight fill-feedback memory that tracks recent own-trade markouts and adverse selection.
Notes: Structurally coherent, but weaker in the first pass. Rust PnL fell to 11'487 with EMERALDS unchanged at 7'717 and TOMATOES dropping to 3'770 over 698 trades. So the diagnosis behind the patch was reasonable, but this implementation over-corrected: TOMATOES became less active overall and the new feedback / reversal logic likely widened or vetoed too much before the simplified engine had enough alpha quality to earn it back. The clean takeaway is that `V50` still needs faster TOMATOES state and stronger execution, but those controls need to be added more selectively rather than all at once.
PnL: Official N/A | Rust 11'487

V50 Narrow Variant Sweep:
Works: Yes
Improvement: Controlled follow-up experiment around the clean `V50` baseline, splitting the broad `V50.1` patch into narrower TOMATOES-only variants to isolate which ideas actually helped. The tested variants were: `V50.2` faster trend signal plus lower thresholds only, `V50.3` that same state upgrade plus strict strong-trend reversal cover, `V50.4` the same state upgrade plus selective strong-trend inventory holding, `V50.5` a more aggressive combined version, `V50.6` mild threshold loosening on top of the original `V50` state, `V50.7` that same mild threshold loosen plus strict reversal cover, `V50.8` that same mild threshold loosen plus selective strong-trend holding, and `V50.9` a tighter trend-side execution version on top of the original state.
Notes: The results were surprisingly clean. `V50.2`, `V50.3`, and `V50.4` all landed identically at Rust 11'459 with EMERALDS unchanged at 7'717 and TOMATOES at 3'742, which means the faster `fast_fair - slow_fair` state change was the thing that hurt while the added strict reversal and hold logic were effectively inert at that strength. The more aggressive combined branch `V50.5` was clearly worst at Rust 11'051 / TOMATOES 3'334. The second pass around the original `V50` state was much healthier: `V50.6`, `V50.7`, and `V50.8` all tied at Rust 12'068 / TOMATOES 4'351, showing that mild threshold loosening helps a bit while the narrow reversal / hold tweaks again do almost nothing. The most targeted trend-execution variant, `V50.9`, nearly matched the original baseline at Rust 12'153 / TOMATOES 4'436, but still did not beat `V50` at 12'181 / 4'464. So the clear conclusion is: the clean `V50` line should stay closer to its original state and execution structure; faster state, reversal cover, and selective strong-trend holding are not the missing edge yet. The next likely gain on this clean family is better fair estimation or quote calibration, not more gating logic.
PnL: Best Rust 12'181 (`V50`) | Best Variant Rust 12'153 (`V50.9`)

V50 Pricing Calibration Sweep:
Works: Yes
Improvement: Narrow fair-model and execution-calibration sweep around the original `V50` state core, explicitly avoiding more regime or gating changes. The tested variants were: `V50.10` a tiny TOMATOES fair-value rebalance toward microprice, `V50.11` a stronger wall-fair anchor, `V50.12` globally tighter quote/take edges, `V50.13` slightly larger passive size with a neutral-inventory bump, and `V50.14` a combined light-touch version of the fair, edge, and size changes.
Notes: This sweep was much more informative than the earlier state experiments. `V50.10` was the clear winner at Rust 12'231 with EMERALDS unchanged at 7'717 and TOMATOES improving to 4'514 over 819 trades, which means the clean `V50` architecture did have edge left in pricing quality after all. `V50.11` was respectable but weaker at Rust 12'160 / TOMATOES 4'443, so pushing harder into wall-fair did not help as much as simply giving microprice a bit more weight. `V50.13` nearly tied the best old variants at Rust 12'155 / TOMATOES 4'438, which suggests size was not the main bottleneck but also was not harmful when added mildly. By contrast, `V50.12` and the combined `V50.14` both lost clearly at Rust 11'956 / TOMATOES 4'239 and Rust 11'987 / TOMATOES 4'270, so broad quote/take tightening still seems to give up too much bread-and-butter spread capture on this clean line. The practical takeaway is strong: on the `V50` family, fair estimation matters more than new gating logic, and the best next path is to refine the fair model in small steps rather than to make the bot globally more aggressive.
PnL: Best Rust 12'231 (`V50.10`) | Best Other Rust 12'160 (`V50.11`)

V50.10:
Works: Yes
Improvement: First successful pricing-calibration winner on the clean `V50` family. TOMATOES keeps the same state spine, regime logic, inventory control, and execution rules as `V50`, but the fair-value blend is nudged slightly away from the slow wall anchor and toward the current microprice. Specifically, wall-fair weight falls from 0.58 to 0.55, mid weight from 0.17 to 0.15, and microprice weight rises from 0.15 to 0.20 while the flow term stays unchanged.
Notes: This is the first `V50` follow-up that actually beats the baseline instead of just rearranging the same edge. Rust PnL rose to 12'231 with EMERALDS unchanged at 7'717 and TOMATOES up to 4'514 over 819 trades, beating `V50`'s 12'181 / 4'464. The result is modest rather than dramatic, but it is strategically important because it validates the conclusion from the earlier sweep: the clean `V50` line does not mainly need more gating, reversal logic, or state complexity. It needs slightly better TOMATOES pricing quality. `V50.10` is therefore the right promotion candidate from the clean-slate family so far.
PnL: Official N/A | Rust 12'231

V50.10x Overshoot Follow-up:
Works: Yes
Improvement: Correctness and local-calibration follow-up around `V50.10`, focused on fixing the target-overshoot bug and then testing only tiny nearby fair/quote changes. The shared correction was to remove the old `max(1, ...)` sizing floor in trend-side taker and passive logic and replace it with proper zero-clamped desired sizing, so the bot no longer forces an extra lot in the target direction when it is already at or beyond target. The tested variants were: `V50.10.1` bug-fix only, `V50.10.2` bug-fix plus a slightly stronger microprice tilt, `V50.10.3` bug-fix plus a slightly stronger wall anchor, and `V50.10.4` bug-fix plus a very light quote/take edge tightening.
Notes: This was a useful but slightly uncomfortable result. The overshoot fix is clearly the right correctness change, but locally it did not improve PnL on day `-1`. `V50.10.1` landed at Rust 12'173 with EMERALDS unchanged at 7'717 and TOMATOES at 4'456 over 808 trades, which is clean and close but still below `V50.10`'s 12'231 / 4'514. The nearby fair variants were weaker still: `V50.10.2` reached 12'099 / TOMATOES 4'382, `V50.10.3` reached 12'120 / TOMATOES 4'403, and the light quote/take tweak `V50.10.4` reached 12'111 / TOMATOES 4'394. So the conclusion is nuanced: the overshoot bug should still be treated as a real execution correctness issue, but in the current local replay it was apparently contributing a small amount of accidental aggressiveness that helped TOMATOES. That means the next clean step is not more local fair drift around the corrected version. It is likely to be a compensating execution tweak that restores a little of that lost participation without reintroducing the bug.
PnL: Best Rust 12'173 (`V50.10.1`) | Baseline Rust 12'231 (`V50.10`)

V50.11 (Aligned Execution):
Works: Yes
Improvement: Fresh continuation built from the corrected `V50.10.1` base, adding only a very small aligned execution boost for TOMATOES. The change lowers taker thresholds slightly when the side matches the current trend direction and the bot still has room to move toward target, adds a tiny near-flat passive size bonus on the aligned side, and trims quote edge a touch in calm/range states when inventory pressure is low. This is intentionally much narrower than the older archived `V50.11` wall-anchor sweep candidate referenced above.
Notes: The result was completely flat to `V50.10.1`. Rust PnL came in at 12'173 with EMERALDS unchanged at 7'717 and TOMATOES at 4'456 over 808 trades, exactly matching the pure overshoot-fix branch on this replay. So the idea is safe, but at this strength it is effectively inert: it neither recovers the lost edge from fixing the overshoot bug nor harms the clean execution core. The practical takeaway is that the next gain probably needs a slightly stronger or more directly targeted participation change, not just a tiny uniform aligned-side nudge.
PnL: Official N/A | Rust 12'173

V51:
Works: Yes
Improvement: Structural TOMATOES rebuild on top of the clean `V50` family, importing only the high-signal pieces from the stronger old hybrid line. The new module keeps the `V50` pipeline shape, but adds wall-strength persistence, rolling history storage, regression fit and predicted edge, a separate hybrid alpha, guarded conflict damping, fit-aware regime classification, fit-aware target sizing, and volatility/fit-aware take and quote calibration. Inventory control stays clean through one reservation-price shift and explicit target sizing rather than being re-spread across the whole module.
Notes: Architecturally this is much closer to the right long-term design, but the first calibration did not beat the simpler `V50.10` line. Rust PnL reached 12'167 with EMERALDS unchanged at 7'717 and TOMATOES at 4'450 over 945 trades. That is close to the corrected `V50.10.1` / `V50.11` level, but still below `V50.10`'s 12'231 / 4'514. So the old-family imports look directionally sensible, but in this first pass they mainly increased activity without improving enough execution quality. The key lesson is that `V51` likely needs tighter calibration, not a wholesale rollback: the structure is promising, but the take/quote layer still needs to be made more selective so the richer alpha stack translates into better prices instead of just more trades.
PnL: Official N/A | Rust 12'167

V51.1:
Works: Yes
Improvement: Selective-calibration follow-up on `V51`, aimed specifically at fixing the over-eager first pass without throwing away the stronger TOMATOES structure. The changes are intentionally local: lower alpha blend weight, higher fit threshold for trend classification, higher taker thresholds when fit quality is weak in trend states, slightly wider passive quote edges in trend / strong-trend modes, and smaller passive sizing when fit quality is mediocre. In other words, the richer `V51` alpha stack stays intact, but it now has to earn its trades more often.
Notes: This is the first real success on the `V51` branch. Rust PnL improved to 12'234 with EMERALDS unchanged at 7'717 and TOMATOES up to 4'517 over 937 trades. That beats both `V51` at 12'167 / 4'450 and the previous clean-family leader `V50.10` at 12'231 / 4'514, though only by a narrow margin. The result is strategically important because it validates the intended direction: the stronger wall-strength / regression / guarded-alpha architecture can beat the simpler clean bot, but only once the take/quote layer is made more selective. So `V51.1` is now the best local clean-family bot in the repo, albeit by a very small edge that still needs official confirmation.
PnL: Official N/A | Rust 12'234

V51 Ablation Sweep:
Works: Yes
Improvement: Narrow follow-up set created to decompose the bigger `V51` merge into isolated imports on top of the corrected `V50.10.1` base. The tested variants were: `V51.2` wall-strength logic only, `V51.3` regression fair term only, `V51.4` hybrid alpha only with fixed momentum and live `flow_ema` usage, and `V51.5` quote / clear behavior tweaks only with corrected clear-inventory sign logic.
Notes: The ablations were very informative. `V51.2` was the weakest at Rust 12'037 / TOMATOES 4'320, which means wall-strength filtering by itself removed too much useful wall-fair participation. `V51.3` reached 12'165 / 4'448 and `V51.5` reached 12'127 / 4'410, so regression fair alone and quote/clear tweaks alone were both directionally okay but not enough to move the line. The strongest isolated import was `V51.4` at Rust 12'187 / TOMATOES 4'470 over 831 trades. That is still below `V51.1` and `V50.10`, but it clearly says the most valuable old-bot idea in this clean framework is the hybrid-alpha layer, especially once the momentum bug is fixed and `flow_ema` is used rather than just stored. So the next best clean-family move is to keep building around `V51.1`, with hybrid alpha as the main import and the other pieces added only very selectively.
PnL: Best Rust 12'187 (`V51.4`) | Best Full Hybrid Rust 12'234 (`V51.1`)

V51.6:
Works: Yes
Improvement: Clean promoted follow-up on `V51.1` that keeps the same architecture and fair structure but with a more selective TOMATOES execution calibration: slightly lower `ALPHA_BLEND_WEIGHT`, slightly lower `FAIR_ALPHA_WEIGHT`, slightly higher `INVENTORY_SKEW`, slightly wider `BASE_QUOTE_EDGE`, slightly higher `BASE_TAKE_EDGE`, slightly stricter `FIT_THRESHOLD` / `TREND_EDGE_THRESHOLD`, and a larger passive quote size.
Notes: This is the strongest local version of the clean-family branch so far. Day `-1` Rust PnL rose to 12'305 with EMERALDS unchanged at 7'717 and TOMATOES up to 4'588 over 932 trades. That beats `V51.1` at 12'234 / 4'517 and `V50.10` at 12'231 / 4'514. The important practical read is that the gain came from tighter TOMATOES quote/take calibration, not from adding more structure. So `V51.6` remains the current clean promoted line, while the raw optimizer artifacts are no longer needed in the repo.
PnL: Official N/A | Rust 12'305

V80:
Works: Yes
Improvement: New intentionally risk-seeking clean-structure bot built from the original `V50` architecture. EMERALDS stays as the stable fixed-fair market maker, while TOMATOES is turned into a much more aggressive directional engine: faster trend-state updates, lower trend thresholds, a fair-value blend shifted more toward microprice and flow, larger trend/strong-trend target bands, lower inventory skew, lower taker and quote edges, larger take/passive sizes, stronger one-sided participation in trend, and less eager clearing of aligned inventory.
Notes: This did exactly what a “very risky” clean bot usually does in a first pass: it traded a lot more but paid heavily for the aggression. Rust PnL landed at 11'035 with EMERALDS unchanged structurally and TOMATOES doing most of the damage, over 880 trades. So the branch is valid and clearly much more risk-on than the earlier clean family, but this first calibration overshoots badly and turns aggression into expensive noise rather than extra edge. The useful part is structural: the bot remains clean and easy to tune, which makes `V80` a good base for future “risk-on but not reckless” follow-ups rather than a promotion candidate in its current form.
PnL: Official N/A | Rust 11'035

V60 Chaos Overlay Test:
Works: Yes
Improvement: Practical chaos-inspired trust filter layered on top of the strong `V60` TOMATOES execution engine. The added feature measures local instability from microprice drift, imbalance drift, flow acceleration, spread instability, and a short-window entropy proxy built from recent return/micro/imbalance/flow signs. The first branch `V60.1` uses that chaos score only as an execution overlay: orderly markets quote a touch tighter, chaotic markets widen quotes, require more take edge, and cut passive size. The second branch `V60.2` strengthens the same idea by lowering the chaos thresholds and forcing chaotic execution states into `volatile` handling without changing the core fair-value engine.
Notes: This was a useful negative result. `V60.1` tied the base bot exactly at Rust 15'163 with EMERALDS 7'723 and TOMATOES 7'440 over 682 own trades, which means the light chaos overlay was structurally safe but too weak to alter behavior on the local replay. The stronger `V60.2` did change behavior, but it clearly hurt: Rust PnL fell to 14'905 with TOMATOES dropping to 7'182 over 643 own trades. So the broad conclusion is that the “chaos as instability filter” idea is directionally sensible, but `V60` already has enough volatility/toxicity structure that an extra chaos brake is redundant unless it is calibrated extremely carefully. In this first application it did not add edge, and the stronger version mostly just made the bot too cautious.
PnL: Base Rust 15'163 (`V60`) | Chaos Rust 15'163 (`V60.1`) | Stronger Chaos Rust 14'905 (`V60.2`)

V51 Black-Scholes Fair Experiment:
Works: Yes
Improvement: Black-Scholes-inspired nonlinear fair-value overlay tested on the clean `V51.6` structure. Because Black-Scholes is naturally an option-pricing model rather than a spot fair-value model, the implementation uses it as a short-horizon moneyness/volatility bias term inside TOMATOES fair calculation rather than pretending it is a literal spot fair. The strike anchor is a blend of wall fair and recent average; realized volatility is converted into a bounded relative sigma; and the resulting `N(d1)` directional bias is turned into a small extra fair component. `V51.7` is the mild version and `V51.7.1` is the stronger version.
Notes: The result was mixed and not robust enough to promote. On day `-1`, the mild branch `V51.7` fell to Rust 12'268 with EMERALDS unchanged at 7'717 and TOMATOES at 4'551, which is clearly below `V51.6`. The stronger branch `V51.7.1` did slightly beat `V51.6` on day `-1`, reaching Rust 12'311 with TOMATOES at 4'594. But the follow-up on day `-2` reversed that gain: `V51.6` scored 12'008 while `V51.7.1` fell to 11'939. So the honest read is that a Black-Scholes-style nonlinear bias can nudge the fair calculation in an interesting way, but for these spot products it is not a robust improvement and behaves more like a replay-specific transform than a reliable pricing upgrade.
PnL: Base Rust 12'305 / 12'008 (`V51.6`) | Mild BS Rust 12'268 (`V51.7`) | Stronger BS Rust 12'311 / 11'939 (`V51.7.1`)

V60 Black-Scholes Fair Experiment:
Works: Yes
Improvement: The same Black-Scholes-style nonlinear fair-value transform was applied to the strongest current local trunk `V60`. Here the spot fair engine already has a richer execution and state stack, so the experiment only adds the nonlinear moneyness/volatility bias as an extra fair component, without changing execution or regime logic. `V60.3` is the mild version and `V60.4` is the stronger version.
Notes: On the strongest bot, the idea was still only marginally useful and not robust. `V60.3` was completely inert, tying `V60` exactly at Rust 15'163 on day `-1` with EMERALDS 7'723 and TOMATOES 7'440. The stronger branch `V60.4` did improve day `-1` slightly to Rust 15'168, with TOMATOES rising from 7'440 to 7'445 while EMERALDS stayed flat. But the day `-2` check gave that gain back and a bit more: base `V60` scored 15'276.5 while `V60.4` scored 15'270.5. So even on the strongest bot, the Black-Scholes-style fair term is at best a tiny replay-specific nudge rather than a robust new edge. The richer `V60` structure neither breaks from the idea nor meaningfully benefits from it.
PnL: Base Rust 15'163 / 15'276.5 (`V60`) | Mild BS Rust 15'163 (`V60.3`) | Stronger BS Rust 15'168 / 15'270.5 (`V60.4`)

Queue-Reactive / Order-Flow State Test Series:
Works: Yes
Improvement: First structured cross-base experiment using the same market-microstructure idea on three different trunks: `V39.4`, `V51.6`, and `V60`. The shared idea was to treat TOMATOES more like interacting queues than a simple mid/micro/imbalance process. Each variant adds a lightweight queue-reactive signal built from top-of-book depletion/refill, best-price stepping, depth skew across the first three levels, and a simple trade-pressure proxy from market trades. That signal is then used as a small extra input into TOMATOES alpha/fair calculation plus local quote/take/size calibration. The tested bots were `V39.7`, `V51.8`, and `V60.5`.
Notes: This was one of the clearer cross-family tests so far because the same idea affected each trunk differently. On the `V39.4` family it worked well and transferred across both local days: base `V39.4` scored Rust 15'573 / 15'490 while `V39.7` improved to 15'595 / 15'555.5, entirely through better TOMATOES (`7'850 -> 7'872` on day `-1`, `8'322 -> 8'387.5` on day `-2`). On the clean `V51.6` family it was clearly harmful: base `V51.6` scored 12'305 / 12'008 while `V51.8` fell to 12'243 / 12'123, so the extra queue-reactive layer added noise rather than useful structure there. On the stronger `V60` family the picture was mixed: `V60.5` improved day `-1` from 15'163 to 15'249 but slipped on day `-2` from 15'276.5 to 15'244. So the practical conclusion is that queue-reactive/order-flow state logic is a real and useful idea, but it seems to fit the simpler wall-fair / hybrid-alpha family best. It improves `V39.4` robustly, is replay-specific on `V60`, and does not suit the already-clean `V51.6` line in its current form.
PnL: `V39.4` 15'573 / 15'490 -> `V39.7` 15'595 / 15'555.5 | `V51.6` 12'305 / 12'008 -> `V51.8` 12'243 / 12'123 | `V60` 15'163 / 15'276.5 -> `V60.5` 15'249 / 15'244

Hawkes-Style Burst Detection Test Series:
Works: Yes
Improvement: Second structured cross-base experiment using the same idea on `V39.4`, `V51.6`, and `V60`. The shared layer is a lightweight Hawkes-style burst detector for TOMATOES: recent market-trade arrival pressure, one-sided top-of-book depletion/refill, and price-step bursts are fused into a signed self-exciting signal with short memory. That burst score is then fed into TOMATOES alpha/fair plus take/quote/size calibration, rather than replacing the existing architecture. The tested bots were `V39.8`, `V51.9`, and `V60.6`.
Notes: This idea transferred better across the three trunks than the queue-reactive layer. On the `V39.4` family, `V39.8` improved day `-1` from 15'573 to 15'615 but gave some of that back on day `-2`, falling from 15'490 to 15'456.5, so the combined two-day result still improved slightly from 31'063 to 31'071.5. On the clean `V51.6` family, the pattern inverted versus the queue test: `V51.9` lost a little on day `-1` (12'305 to 12'294) but improved meaningfully on day `-2` (12'008 to 12'074), giving a stronger two-day total of 24'368 versus 24'313. On the strong `V60` trunk, the result was the clearest: `V60.6` improved day `-1` from 15'163 to 15'242 and only slipped slightly on day `-2` from 15'276.5 to 15'266.5, leaving the best combined two-day gain of the three bases at 30'508.5 versus 30'439.5. The practical conclusion is that Hawkes-style burst detection is a more generally compatible improvement than the queue-reactive layer. It seems especially well matched to the richer `V60` execution engine, while still being directionally helpful to `V51.6` on a multi-day view.
PnL: `V39.4` 15'573 / 15'490 -> `V39.8` 15'615 / 15'456.5 | `V51.6` 12'305 / 12'008 -> `V51.9` 12'294 / 12'074 | `V60` 15'163 / 15'276.5 -> `V60.6` 15'242 / 15'266.5

Online Change-Point Detection Test Series:
Works: Yes
Improvement: Third structured cross-base experiment on `V39.4`, `V51.6`, and `V60`, using a lightweight online change-point layer for TOMATOES. Instead of recomputing regime from scratch only from current indicators, each variant tracks fast-versus-slow state breaks in mean, flow, and volatility, then smooths those into a break probability and directional change score. That signal is used only for regime gating and execution calibration: trend activation after a detected directional break, wider quotes in non-directional breaks, and slightly stricter/looser take behavior depending on whether the detected break aligns with the trade side. The tested bots were `V39.9`, `V51.10`, and `V60.7`.
Notes: This was mostly a negative result. On the `V39.4` family the layer was completely inert: `V39.9` matched `V39.4` exactly at Rust 15'573 / 15'490. On the strong `V60` family it was also fully inert, with `V60.7` tying `V60` exactly at 15'163 / 15'276.5. On the clean `V51.6` family it was actively harmful, dropping from 12'305 / 12'008 to 12'201 / 11'965. So in this implementation, online change-point detection did not add useful edge. The likely read is that the added break logic was either too weak to change already-rich trunks (`V39.4`, `V60`) or too conservative on the cleaner `V51.6` line, where it cut TOMATOES participation instead of improving state transitions.
PnL: `V39.4` 15'573 / 15'490 -> `V39.9` 15'573 / 15'490 | `V51.6` 12'305 / 12'008 -> `V51.10` 12'201 / 11'965 | `V60` 15'163 / 15'276.5 -> `V60.7` 15'163 / 15'276.5

Explicit Quote Action Set Test Series:
Works: Yes
Improvement: Fourth structured cross-base experiment on `V39.4`, `V51.6`, and `V60`, testing an explicit TOMATOES quote-action state machine: `both`, `buy_only`, `sell_only`, or `none`. The implementation was intentionally narrow and applied only to the passive quoting layer, leaving fair value, target logic, and taker behavior unchanged. The intended effect was to let the bot abstain in toxic conditions and quote one side only in sufficiently directional states instead of always defaulting to two-sided MM. The tested bots were `V39.10`, `V51.11`, and `V60.8`.
Notes: This implementation was fully inert across both local days on all three trunks. `V39.10` matched `V39.4` exactly at Rust 15'573 / 15'490, `V51.11` matched `V51.6` exactly at 12'305 / 12'008, and `V60.8` matched `V60` exactly at 15'163 / 15'276.5. The practical conclusion is not that the idea is bad, but that in this form it never actually changed the final realized quoting behavior of these bots. Their existing regime / passive logic already seems to collapse into effectively the same action choices, so adding an explicit action-set wrapper on top did not move any orders. If we revisit this idea later, it would need to be made more central to execution, not just layered around the existing quote logic.
PnL: `V39.4` 15'573 / 15'490 -> `V39.10` 15'573 / 15'490 | `V51.6` 12'305 / 12'008 -> `V51.11` 12'305 / 12'008 | `V60` 15'163 / 15'276.5 -> `V60.8` 15'163 / 15'276.5

Target / Execution Separation Test Series:
Works: Yes
Improvement: Fifth structured cross-base experiment on `V39.4`, `V51.6`, and `V60`, making the separation between desired exposure and book execution explicit. The existing target logic was left in place, but each TOMATOES variant gained a separate execution-planning layer that classified the current task as things like aggressive accumulation, passive accumulation, risk reduction, or neutral holding. That execution plan then drove taker threshold biases, passive size bonuses, and whether same-side passive quoting was even allowed, instead of leaving those choices fully distributed across the old quote/take heuristics. The tested bots were `V39.11`, `V51.12`, and `V60.9`.
Notes: This was a clear negative result. On the `V39.4` family, `V39.11` collapsed from Rust 15'573 / 15'490 to 13'628 / 13'640.5, with TOMATOES falling from 7'850 / 8'322 to 5'905 / 6'472.5. On the clean `V51.6` family, `V51.12` dropped from 12'305 / 12'008 to 11'819 / 11'363, again through weaker TOMATOES. On the strong `V60` trunk, `V60.9` fell from 15'163 / 15'276.5 to 14'097 / 14'200.5. So although the architectural idea is sound in principle, this first implementation over-controlled execution and suppressed too much ordinary passive participation. The likely lesson is that these bots already encode execution intent implicitly quite well, and a naive explicit planner can easily double-count caution, resulting in fewer good fills without enough improvement in adverse-selection control.
PnL: `V39.4` 15'573 / 15'490 -> `V39.11` 13'628 / 13'640.5 | `V51.6` 12'305 / 12'008 -> `V51.12` 11'819 / 11'363 | `V60` 15'163 / 15'276.5 -> `V60.9` 14'097 / 14'200.5

Order-Book Filtration / Directional-Signal Extraction Test Series:
Works: Yes
Improvement: Sixth structured cross-base experiment on `V39.4`, `V51.6`, and `V60`, testing a cleaner order-book directional-signal extraction layer for TOMATOES instead of slower chart-style indicators. Each variant adds a filtered LOB signal built from top-level imbalance, multi-level depth skew, book-shape asymmetry, and microprice premium relative to half-spread, then smooths that into a short-memory directional estimate. That signal is used only as a mild extra input into TOMATOES alpha, fair value, quote edge, and take edge, without changing the underlying architecture. The tested bots were `V39.12`, `V51.13`, and `V60.10`.
Notes: This idea produced one clear winner and two clear negatives. On the `V39.4` family it worked well on both local days: base `V39.4` scored Rust 15'573 / 15'490 while `V39.12` came in at 15'570 / 15'598.5. That is a tiny day `-1` giveback but a much stronger day `-2`, leaving the best net two-day gain of the series at 31'168.5 versus 31'063. On the clean `V51.6` family it was mildly harmful overall: `V51.13` scored 12'252 / 11'984 versus base `V51.6` at 12'305 / 12'008, so the filtered LOB term added a little TOMATOES noise without improving the line. On the strong `V60` trunk it was clearly negative, falling from 15'163 / 15'276.5 to 14'998 / 15'224.5, which suggests `V60` already has enough microstructure information and the extra directional extraction mostly double-counted it. The practical conclusion is that this cleaner LOB filtration idea fits the simpler `V39.4` hybrid family best, where it improves the two-day result, but it is not a good add-on for the richer `V60` trunk and does not help the `V51.6` clean family in this form.
PnL: `V39.4` 15'573 / 15'490 -> `V39.12` 15'570 / 15'598.5 | `V51.6` 12'305 / 12'008 -> `V51.13` 12'252 / 11'984 | `V60` 15'163 / 15'276.5 -> `V60.10` 14'998 / 15'224.5

Robust / Uncertainty-Aware Quoting Test Series:
Works: Yes
Improvement: Seventh structured cross-base experiment on `V39.4`, `V51.6`, and `V60`, testing a TOMATOES-only robust quoting layer that reacts to uncertainty in the existing signal stack instead of trying to optimize one locally best quote edge. Each variant builds a lightweight uncertainty score from fit weakness, disagreement between regression and hybrid alpha, directional conflict with imbalance or flow, and elevated spread / volatility. That score is then used only to widen passive quote edges, raise take thresholds slightly, and trim passive size, without changing the core fair-value or target logic. The tested bots were `V39.13`, `V51.14`, and `V60.11`.
Notes: This was one of the more informative host-dependent tests in the series. On the `V39.4` family it was clearly harmful overall: `V39.13` jumped strongly on day `-1` from 15'573 to 15'758, but then gave back much more on day `-2`, falling from 15'490 to 15'131.5. That leaves the two-day result worse at 30'889.5 versus 31'063, which suggests the older `V39.4` line was already tuned to exploit short-horizon opportunity aggressively and the extra uncertainty brake suppressed too much useful TOMATOES edge when conditions changed. On the clean `V51.6` family the result was excellent and robust: `V51.14` improved from 12'305 / 12'008 to 12'477 / 12'308, entirely through stronger TOMATOES, for a two-day gain of 472. That is the strongest relative improvement in the whole series and fits the intuition behind the idea very well: the cleaner `V51.6` trunk benefits from being made more robust about when it trusts its alpha. On the strong `V60` trunk the result was small but positive on both days, moving from 15'163 / 15'276.5 to 15'175 / 15'285.5. So the practical conclusion is that robust / uncertainty-aware quoting is a genuinely useful idea, but it belongs much more on the clean `V51.6` family than on the older `V39.4` hybrid. It also seems compatible with `V60`, though only as a small refinement rather than a major new edge source.
PnL: `V39.4` 15'573 / 15'490 -> `V39.13` 15'758 / 15'131.5 | `V51.6` 12'305 / 12'008 -> `V51.14` 12'477 / 12'308 | `V60` 15'163 / 15'276.5 -> `V60.11` 15'175 / 15'285.5

V81 Inventory Management Test Series:
Works: Yes
Improvement: Focused follow-up on the `V39.4` family, testing more sophisticated TOMATOES inventory management designs rather than changing alpha or fair estimation. Three designs were tried. `V81` adds nonlinear target-gap pressure: the farther inventory drifts from target, the more aggressively fair, reservation, quotes, and passive size react, with a curved rather than linear penalty. `V81.1` adds inventory-age / stale-carry memory, making older same-side inventory more expensive to keep and easier to unwind. `V81.2` adds regime-aware carry limits, with dynamic effective soft limits that allow more carry in aligned trends and less in range / volatile states.
Notes: The result was quite clean locally. `V81` was the only meaningful winner, improving `V39.4` from Rust 15'573 / 15'490 to 15'675 / 15'485.5. That gives a better two-day total of 31'160.5 versus 31'063, entirely through stronger day `-1` TOMATOES while preserving the day `-2` profile almost exactly. So the useful part of inventory sophistication here is not more memory or more dynamic soft-limit machinery; it is simply making the penalty for being far from target more nonlinear. By contrast, `V81.1` underperformed clearly at 15'594 / 15'163.0, which suggests stale-inventory memory mostly over-tightened execution and reduced good TOMATOES participation. `V81.2` was also worse overall at 15'461 / 15'490.0: the regime-aware carry-limit idea was safe on day `-2`, but too restrictive on day `-1`, so the richer soft-limit shaping did not improve the branch. The official replay did not transfer, though: `V81` came back at 2'549.164 versus `V39.4` at 2'640.875. EMERALDS was identical at 1'050.0, so the entire miss was TOMATOES, which dropped from 1'590.875 to 1'499.164. Trade activity was only slightly lower, but execution quality was worse on the sell side in particular: average TOMATOES sell price fell from 4995.952 to 4995.721 while average buy price was basically unchanged. The practical takeaway is that `V39.4` does benefit from a better inventory curve locally, but this nonlinear target-gap pressure over-shaped TOMATOES inventory on the official path and should stay a research branch rather than replace `V39.4`.
PnL: Rust `V39.4` 15'573 / 15'490 -> `V81` 15'675 / 15'485.5 | `V81.1` 15'594 / 15'163.0 | `V81.2` 15'461 / 15'490.0 || Official `V39.4` 2'640.875 -> `V81` 2'549.164

V81 Combination Follow-Ups:
Works: Yes
Improvement: Next iteration on the useful `V81` nonlinear inventory curve, now combining it with the two `V39.4`-family add-ons that had previously shown positive multi-day local results. `V81.3` adds the queue-reactive / order-flow state layer on top of `V81`. `V81.4` adds the filtered order-book directional signal. `V81.5` combines both together with the nonlinear target-gap inventory pressure. The goal was to see whether better TOMATOES directional context could make the stronger inventory curve productive instead of over-shaping exits.
Notes: This was the strongest local inventory series so far. `V81.3` improved both days versus `V81`, moving from 15'675 / 15'485.5 to 15'713 / 15'578.0. `V81.4` was only marginal, at 15'628 / 15'533.5, so the LOB-filter idea alone did not add much beyond the inventory curve. The best result was `V81.5`, which combined both directional layers with the nonlinear gap pressure and reached 15'785 / 15'523.5. That gives the best two-day total in the whole `V39.4` inventory branch at 31'308.5, clearly above `V39.4` at 31'063 and above `V81` at 31'160.5. The practical read is that inventory shaping works much better when the bot also gets slightly better short-horizon directional context; the problem with plain `V81` was not just “too much inventory logic,” but “inventory logic acting without enough help about when TOMATOES pressure is real.” The official replay partly confirmed that direction but not enough to dethrone `V39.4`: `V81.5` improved on plain `V81`, rising from 2'549.164 to 2'561.164, but still stayed below `V39.4` at 2'640.875. Again the miss was entirely TOMATOES. Compared with `V81`, buys improved slightly and average sell price recovered a bit, but not enough to close the full transfer gap.
PnL: Rust `V81` 15'675 / 15'485.5 -> `V81.3` 15'713 / 15'578.0 | `V81.4` 15'628 / 15'533.5 | `V81.5` 15'785 / 15'523.5 || Official `V81` 2'549.164 -> `V81.5` 2'561.164 (`V39.4` 2'640.875)

V81 Inventory Refinement Follow-Ups:
Works: Yes
Improvement: Focused refinement on top of the new local winner `V81.5`, testing two more inventory-only ideas aimed at the official `V81` failure mode. `V81.6` adds signal-aware carry relief, reducing nonlinear inventory pressure when current inventory is aligned with the local order-flow / LOB directional state and increasing it when inventory is wrong-way. `V81.7` adds a deadzone before nonlinear gap pressure activates, so small target gaps are left mostly unmanaged and the stronger curve only engages once inventory drifts meaningfully away from target.
Notes: These two refinements did not create a new level. `V81.6` came back identical to `V81.5` at 15'785 / 15'523.5, which means the extra carry-relief logic was effectively inert at this calibration on the local paths. `V81.7` was clearly worse at 15'686 / 15'530.5: the deadzone softened inventory control too much on day `-1` without buying enough on day `-2`. So the current best read is that the winning part of this branch is still the `V81.5` combination itself, not a more sophisticated second layer of inventory modulation. The next meaningful filter for this branch is now official testing rather than more local reshaping of the inventory curve.
PnL: `V81.5` 15'785 / 15'523.5 | `V81.6` 15'785 / 15'523.5 | `V81.7` 15'686 / 15'530.5

V82 Transfer-Aware Inventory Follow-Ups:
Works: Yes
Improvement: Further inventory research on top of `V81.5`, now guided by two outside inspirations. From the Fishbowl inventory-management article, the translated ideas were safety-stock / reorder-point style inventory bands: do not over-manage small inventory drift, but become more urgent once inventory moves into an excess tier. From the HFT inventory paper “Inventory Management for High-Frequency Trading with Imperfect Competition” (arXiv:1808.05169), the translated idea was a signal-driven mean-reverting inventory anchor: inventory targets should be pulled by short-lived signal advantage and then decay back as the edge evaporates. Four variants were tested. `V82` added confidence-aware target compression plus asymmetric exit urgency for wrong-way inventory. `V82.1` kept just the exit-urgency logic. `V82.2` implemented the safety-stock / reorder-point inventory banding. `V82.3` implemented the signal-driven mean-reverting shadow inventory target.
Notes: None of these beat `V81.5`, but the ranking was informative. The best of the four was `V82.1` at 15'785 / 15'426.0, followed closely by `V82` at 15'794 / 15'412.0. Both improved day `-1` slightly but gave back too much on day `-2`, so the transfer-aware unwind ideas were directionally sensible but not yet calibrated well enough. The warehouse-style safety-stock / reorder-point band in `V82.2` held up better on day `-2` at 15'536.5, but it lost too much on day `-1`, leaving it below the branch leader overall. The paper-inspired mean-reverting signal anchor in `V82.3` was the weakest result at 15'550 / 15'211, which suggests that on this bot the extra target dynamics mostly added noise rather than helping inventory decay cleanly. The practical conclusion is that the next useful inventory direction still looks like asymmetric exit handling, not banded stock logic or a shadow inventory process, but `V81.5` remains the best local branch for now.
PnL: `V82` 15'794 / 15'412.0 | `V82.1` 15'785 / 15'426.0 | `V82.2` 15'588 / 15'536.5 | `V82.3` 15'550 / 15'211.0 | current branch best `V81.5` 15'785 / 15'523.5

V83 Paper-Driven Inventory Rebuild:
Works: Yes
Improvement: Full TOMATOES inventory-management rebuild inspired by the HFT inventory paper “Inventory Management for High-Frequency Trading with Imperfect Competition” (arXiv:1808.05169). Instead of starting from the old nonlinear target-gap machinery, this series replaced the inventory core with a signal-driven, fast mean-reverting inventory anchor. The guiding idea was that desired inventory should be tied to short-lived signal advantage and decay quickly as that advantage fades, rather than being managed mainly through static gap penalties. `V83` implemented the rebuilt anchor model directly. `V83.1` added an anti-overtrading / turnover-penalty layer on top. `V83.2` added faster anchor decay in weak-signal or range states.
Notes: This rebuild was clearly too weak on the local paths. `V83` scored only 15'249 / 15'045.5, with TOMATOES falling to 7'526 / 7'877.5. `V83.1` was identical, which means the turnover layer was effectively inert once the core rebuild had already weakened the branch. `V83.2` was almost identical as well at 15'249 / 15'047.5, so faster signal decay did not rescue the design. The key takeaway is useful: on this bot family, tearing out the existing inventory machinery and replacing it with a pure signal-anchor process loses too much of the practical execution discipline that `V39.4` and `V81.5` rely on. So the paper is still helpful directionally, but its ideas fit better as selective refinements than as a full inventory-management rewrite.
PnL: `V83` 15'249 / 15'045.5 | `V83.1` 15'249 / 15'045.5 | `V83.2` 15'249 / 15'047.5

V84 Selective Paper-Informed V39.4 Rebuild:
Works: Yes
Improvement: Selective rebuild on top of `V39.4` that keeps the proven wall-fair / hybrid-alpha / execution spine, but only imports the parts of the inventory paper that looked compatible with this bot family. `V84` adds a decaying signal-driven inventory anchor plus confidence-aware exit urgency, so desired carry is gently pulled by short-lived signal conviction and wrong-way excess inventory gets unwound more assertively. `V84.1` is the lighter version: it keeps the decaying anchor and small anchor-based reservation/target blend, but drops the extra exit-urgency execution layer. Both variants were also corrected before testing so the new anchor state is advanced only once per decision cycle rather than being mutated again inside reservation logic.
Notes: The lighter blend was clearly better. `V84` came in at 15'437 / 15'165.5, which is well below both `V39.4` and `V81.5`; the extra exit-urgency layer made the bot too eager to unwind and gave back too much TOMATOES edge on both days. `V84.1` was much healthier at 15'522 / 15'004.0. That still does not beat `V39.4` at 15'573 / 15'490 or `V81.5` at 15'785 / 15'523.5, but it is a useful result: the paper-inspired decaying anchor can coexist with the old spine if it stays mild, while the heavier execution-side urgency overlay is too much. So the best interpretation is that the paper helps as a small target/reservation refinement, not as a second execution controller.
PnL: `V84` 15'437 / 15'165.5 | `V84.1` 15'522 / 15'004.0

V85 Asymmetric Anchor Inventory Hybrid:
Works: Yes
Improvement: Another attempt to merge the paper-inspired inventory ideas into the stronger `V81.5` trunk rather than the plain `V39.4` spine. The key design change was to stop treating the paper anchor as a second execution engine and instead use it only as an asymmetric inventory modifier. `V85` keeps the winning nonlinear gap pressure plus the queue-reactive and filtered-LOB directional context from `V81.5`, then adds a small decaying signal-driven inventory anchor that blends into target selection and reservation pressure. The reservation effect is asymmetric: wrong-way inventory gets a stronger extra penalty, while aligned inventory gets a softer one. `V85.1` is the lighter version and drops the extra quote-edge anchor pressure, keeping the anchor only in target and reservation.
Notes: This was a meaningful improvement over the plain `V39.4` line, but not over the best local inventory combo. `V85` scored 15'783 / 15'530.5, which is strong and very close to `V81.5` on day `-2`, but still slightly below it overall. The more important result is `V85.1`: it reached 15'914 / 15'255.0, beating `V39.4` on both local days and giving a much stronger day `-1` than `V81.5`. At the same time, it still trails `V81.5` on the two-day total because day `-2` softened too much. That makes the lesson pretty crisp: the paper-style anchor does seem useful when it only shapes target and reservation asymmetrically, but once it also starts touching quote pressure (`V85`) the extra control is unnecessary. So the best new direction from this series is the lighter `V85.1` design, which looks like a cleaner and more transfer-friendly variant of the paper hybrid than `V84.1`, even though `V81.5` remains the top local branch overall.
PnL: `V85` 15'783 / 15'530.5 | `V85.1` 15'914 / 15'255.0

V86 Clean Fast-Switch Passive/Aggressive Hybrid:
Works: Yes
Improvement: New clean-structure bot built largely from scratch to test a fast switch between passive market making and aggressive taking. The architecture is deliberately simple: one state layer estimates TOMATOES fair value, trend strength, and desired target exposure; a separate execution layer then chooses between `market_make`, `hybrid`, `aggressive`, or `defensive` behavior. In range it quotes both sides; in trends it becomes one-sided and will also take to catch up toward target; in strong trends it lowers take thresholds further and prioritizes faster entry into the move. The intent was to see whether a clean MM-plus-taking split could react faster than the older mixed heuristics without inheriting the same code complexity.
Notes: Structurally the bot works, but the first calibration is too eager and clearly underperforms. `V86` scored only 11'003 / 10'638, with TOMATOES at 3'286 / 3'486 despite EMERALDS holding up at 7'717 / 7'152. That means the fast-switch idea is not broken as an architecture, but this first setting gives away too much TOMATOES edge by taking too much low-quality flow and not harvesting enough quiet passive PnL in between. So `V86` is best treated as a clean research baseline for this hybrid execution style, not as a promotion candidate yet.
PnL: `V86` 11'003 / 10'638

V86 Follow-Up Sweep:
Works: Yes
Improvement: First focused refinement pass on the clean fast-switch branch, aimed specifically at extracting more TOMATOES PnL without abandoning the clean architecture. `V86.1` made execution calmer: higher take thresholds, smaller take clips, lower target cap, slower target EMA, and more selective switching into hybrid/aggressive mode. `V86.2` added the same calmer execution but also introduced a persistent wall-fair anchor into fair value and trend estimation. `V86.3` kept the original aggressive execution personality and instead upgraded only the TOMATOES fair/trend spine with the same wall-fair anchor.
Notes: None of the follow-ups improved the branch. `V86.1` came back at 10'925 / 10'538 and `V86.2` at 10'905 / 10'589, which tells us that simply calming the execution did not rescue the idea; it mostly reduced TOMATOES even further. `V86.3` was the clearest diagnosis tool: at 10'715 / 10'452 it was worst of all, meaning the wall-fair anchor by itself did not fix the problem and actually weakened the current fast-switch state logic further. So the clean takeaway is that this branch is not just “a bit too aggressive.” Its core TOMATOES state/alpha quality is still not good enough to support a fast passive/aggressive switch, and tightening or anchoring the current version does not solve that. For now the best bot in the `V86` family remains the original `V86`, even though it is still far from competitive with the stronger existing trunks.
PnL: `V86.1` 10'925 / 10'538 | `V86.2` 10'905 / 10'589 | `V86.3` 10'715 / 10'452 | family best remains `V86` 11'003 / 10'638

V87 Tomatoes Mean-Reversion Overlay:
Works: Yes
Improvement: New TOMATOES branch built on the `V85.1` family, adding an explicit mean-reversion signal using the three requested tools: moving-average deviation, Bollinger-style z-score, and RSI. A persistent TOMATOES price window is stored in memory; from that the bot computes short-vs-long moving-average spread, long-window standard deviation, Bollinger-style z-score, and RSI. Those are combined into a bounded mean-reversion signal that is then used mainly in range / weak-trend states to shape target position, fair value, take thresholds, and quote width. `V87` is the stronger first pass, while `V87.1` is the lighter overlay version with reduced target/fair/quote/take influence.
Notes: The first pass was clearly too strong. `V87` traded TOMATOES much more and did earn real reversion PnL, but not efficiently enough, finishing at 14'125 / 13'730.5 with TOMATOES only 6'402 / 6'562.5. The lighter `V87.1` is the meaningful result: it improved to 15'433 / 15'648.5, with TOMATOES at 7'710 / 8'480.5. That does not beat the best local inventory-combo branch `V81.5` at 15'785 / 15'523.5, but it does beat plain `V39.4` on the two-day total and gives a stronger day `-2` than both `V39.4` and `V81.5`. So the clean takeaway is that the mean-reversion idea itself is viable for TOMATOES, but it has to stay a light overlay on top of the stronger existing trunk rather than becoming a dominant controller.
PnL: `V87` 14'125 / 13'730.5 | `V87.1` 15'433 / 15'648.5

V87 Follow-Up Official-Led Tightening:
Works: Yes
Improvement: Follow-up pass driven by the official `V87.1` log rather than only the local Rust result. The log shape suggested that the mean-reversion overlay was real but too eager early in the day, so the next variants focused on one idea only: make TOMATOES mean reversion more selective. `V87.2`, `V87.3`, and `V87.4` all add an activation gate based on stretched Bollinger-style z-score and RSI, then damp the overlay when it conflicts with provisional directional state or short-horizon book signals. `V87.3` and `V87.4` also reduced the direct target / fair / quote / take influence to different degrees. `V87.5` is the balanced version: it keeps most of `V87.1`'s productive overlay, but only activates it meaningfully when the deviation is large enough and the rest of the book is not strongly arguing continuation.
Notes: The sweep gave a very clean answer. The strict variants helped day `-1` but gave up too much day `-2`: `V87.2` scored 15'527 / 15'434.5, `V87.3` 15'803 / 15'199.5, and `V87.4` 15'625 / 15'402.5. The middle-ground `V87.5` was the real winner. It reached 15'866 / 15'791.5, with TOMATOES at 8'143 / 8'623.5. That beats `V87.1` on both local days and materially improves the two-day total, which is exactly the pattern we were looking for after the official log review: fewer weak early mean-reversion entries without throwing away the strong later-session reversion edge. The practical takeaway is that the mean-reversion idea was good, but it needed an activation gate and conflict damping to become a stronger branch rather than a noisy overlay.
PnL: `V87.2` 15'527 / 15'434.5 | `V87.3` 15'803 / 15'199.5 | `V87.4` 15'625 / 15'402.5 | `V87.5` 15'866 / 15'791.5

V87.6 Mean-Reversion Execution Rollback:
Works: Yes
Improvement: Direct follow-up to the official `V87.5` replay analysis. The diagnosis there was that the improved activation gate had fixed the worst early mean-reversion entries, but the branch still seemed to over-recycle TOMATOES by letting mean reversion directly shape take thresholds and quote width. `V87.6` tested the cleanest possible fix: keep the stronger `V87.5` activation and conflict-damping logic, but remove the direct mean-reversion influence from the execution layer so that the overlay affects target and fair value, not take/quote eagerness.
Notes: This did not improve the branch. `V87.6` came back at 15'864 / 15'373.0, which is nearly identical to `V87.5` on day `-1` but clearly worse on day `-2`. That means the execution-side mean-reversion adjustments were not the main reason the branch lagged official `V39.4`; removing them mostly cut some of the good late-session reversion monetization without solving the deeper transfer issue. The useful takeaway is that the current weakness is probably not “mean reversion touches execution at all,” but rather that the branch still lacks the stronger exit quality and trade selection that `V39.4` gets from its core TOMATOES execution spine.
PnL: `V87.6` 15'864 / 15'373.0

V88 MR-First Cleanup Branch:
Works: Yes
Improvement: New mean-reversion-first follow-up built from the `V87.5` line using the cleanup list directly rather than another small parameter nudge. To avoid colliding with the older unrelated `V86` family, this was created as `V88`. The branch caches wall fair, queue-reactive bias, and filtered LOB signal once per tick inside `run()` and passes those cached values through the decision stack, removing repeated in-tick state mutation from the live engine. It also removes the dead `queue_activity_ema` state from live use. Structurally, the branch is intentionally more MR-first: hybrid clutter was cut back by reducing `GAP_FAIR_WEIGHT`, `QUEUE_FAIR_WEIGHT`, `LOB_FAIR_WEIGHT`, `FAIR_ALPHA_WEIGHT`, and the heavier anchor/gap reservation terms; wall fair was kept only as a lighter stabilizer; trend-side fair/take/quote bonuses were reduced; and mean reversion was strengthened in `range` through larger `MR_RANGE_TARGET_SCALE`, `MR_FAIR_WEIGHT`, `MR_TAKE_SHIFT`, and `MR_QUOTE_SHIFT`. In addition, queue/LOB influence in `range` execution was damped so they act more like confirmation signals than co-pilots.
Notes: This version is a real improvement over the recent MR branch. `V88` scored 15'803 / 15'890.0, with TOMATOES at 8'080 / 8'722.0. Compared with `V87.5` at 15'866 / 15'791.5, it gives up some day `-1` but more than earns it back on day `-2`, improving the two-day total overall. That is a healthy sign for this design direction: the cleaner MR-first structure did not collapse the branch, and the stronger `range` ownership appears to monetize the better reversion day more effectively. The current read is that this is a better organized and slightly stronger mean-reversion contender than `V87.5`, though it still needs official validation before we can say it transfers better.
PnL: `V88` 15'803 / 15'890.0

V89 Tuned Mean-Reversion Stabilization:
Works: Yes
Improvement: New stabilization branch on the weaker `V89` family, trying to reduce the very large TOMATOES drawdowns without changing the broad architecture. `V89_tuned` keeps the same general mean-reversion / hybrid structure, but makes the branch materially less aggressive: lower base take edge and quote edge were paired with smaller mean-reversion target scale and smaller direct MR influence on fair, take, and quote behavior. The intention was to make TOMATOES less swervy and more survivable, even if that meant giving up some upside.
Notes: This branch did what it was supposed to do stylistically, but not competitively. It became much steadier than the original `V89` line and removed the huge negative TOMATOES collapse, but it also became underpowered. Local Rust results were only 8'696 / 7'873, with TOMATOES at 973 / 705. So the tuning made the branch safer, but the cost was simply too much lost edge. This is a useful baseline for later debugging because it shows that the `V89` family can be stabilized, but that caution alone does not make it good.
PnL: `V89_tuned` 8'696 / 7'873

V90.2 Cross-Day TOMATOES Reset:
Works: Yes
Improvement: Narrow infrastructure fix on top of the `V89` family, testing the hypothesis that TOMATOES mean-reversion state might be contaminated across day boundaries. `V90.2` keeps EMERALDS unchanged and keeps the `V89` TOMATOES logic intact, but adds a reset for TOMATOES memory and mid-history when a new session starts. Because the Python `TradingState` in this repo does not expose a day field, the reset is implemented using timestamp rollback: if the next run starts with a smaller timestamp than the stored `last_timestamp`, the TOMATOES memory is cleared so the new day relearns its own MR baseline rather than trusting the previous day’s wall / queue / LOB / MR state.
Notes: Mechanically this was the right fix, and a direct sanity check confirmed the reset really triggers when timestamp rolls back. But the local backtests were completely unchanged: `V90.2` matched `V89` exactly at 10'379 / 5'319 on isolated day runs and also matched it on the local `--carry` replay at 16'159 total. The reason is important: isolated day runs already start clean, and the local backtester’s `--carry` mode normalizes timestamps into one continuous stream, so there is no rollback for the bot to detect. So this is still a useful official-transfer safeguard, but it is not the main explanation for the `V89` family’s swervy TOMATOES behavior.
PnL: `V89` 10'379 / 5'319 | `V90.2` 10'379 / 5'319 | carry both 16'159

V90.3 Range Continuation Veto:
Works: Yes
Improvement: First attempt to directly fix the swerviness of the stabilized `V89_tuned` line by trading fewer bad TOMATOES fades rather than changing day-reset logic. `V90.3` adds a stricter range-only mean-reversion gate: if queue bias, filtered LOB, microprice, and predicted edge all continue to point against the fade, the range MR signal is blocked or heavily damped. It also adds a small target deadzone around flat and reduces passive size in `range` so tiny MR targets do not create churn.
Notes: This was informative but negative. `V90.3` clearly traded less than `V89_tuned` on both days, but performance fell slightly as well, landing at 8'682 / 7'856 versus 8'696 / 7'873. That means the branch is not mainly losing because it is taking obviously bad range fades. By the time the bot is in the `V89_tuned` shape, it is already too weak and too low-capture; adding more brakes only cuts TOMATOES further. So the issue is deeper than entry filtering.
PnL: `V89_tuned` 8'696 / 7'873 | `V90.3` 8'682 / 7'856

V90.4 Exit-Quality Patience Test:
Works: Yes
Improvement: Second focused attempt on the stabilized `V89_tuned` line, now testing the opposite hypothesis: perhaps TOMATOES entries are acceptable, but the branch gives too much back by exiting good mean-reversion inventory too early. `V90.4` therefore leaves the entry logic alone and adds only modest extra patience for MR-aligned inventory in `range`. On strong enough MR setups, taker exits require a bit more edge and passive unwind quotes are shifted slightly farther out so the bot tries to monetize inventory better instead of immediately recycling it.
Notes: This turned out to be completely inert. `V90.4` matched `V89_tuned` exactly at 8'696 / 7'873, with identical TOMATOES PnL and trade counts. That tells us the current weakness of the branch is not primarily an exit-timing problem either. The local `V89_tuned` / `V90.x` experiments together give a clear conclusion: this family is not suffering from one isolated bug or one simple execution leak. Its TOMATOES state/alpha quality is just too weak, and small reset / filter / exit overlays do not change the realized behavior enough to matter.
PnL: `V90.4` 8'696 / 7'873

V90.5 Upside-Preserving Range Clamp:
Works: Yes
Improvement: New hybrid attempt built from the original `V89` branch instead of the already over-braked `V89_tuned` line. The goal was to keep the strong upside bursts of `V89` while trimming only the worst range overextension. `V90.5` adds a narrow range-only control layer: weak mean-reversion signals are zeroed, clearly opposed range fades are damped rather than fully blocked, tiny range targets are snapped back to flat, and both taker and passive TOMATOES orders in `range` are capped to a small buffer around the current MR target. Outside `range`, the original `V89` trend behavior stays intact.
Notes: This is the first `V90.x` variant that actually behaves like the intended mix. Instead of flattening the whole branch, it materially improves both days: `V90.5` scored 10'209 / 8'942.5, versus `V89` at 10'379 / 5'319. Day `-1` gives up a little upside, but day `-2` improves dramatically, which is exactly the trade we wanted: keep most of the bulls and remove a meaningful share of the bears. TOMATOES rose from 2'656 / -1'849 in `V89` to 2'486 / 1'774.5 in `V90.5`. That means the branch is still lively, but it no longer lets range inventory spiral as badly on the weak path. This looks like a genuinely stronger overall combination than the earlier `V90.x` safety overlays.
PnL: `V89` 10'379 / 5'319 | `V90.5` 10'209 / 8'942.5

V90.6 Range Long-Hold Momentum Overlay:
Works: Yes
Improvement: Follow-up on `V90.5` inspired by the official replay feedback that TOMATOES might still be monetizing good longs too cheaply. `V90.6` adds a narrow range-only momentum hold layer on top of the `V90.5` structure. When the bot is already long in `range` and short-horizon continuation still looks positive through predicted edge, local momentum, queue bias, filtered LOB signal, and microprice, the branch slightly boosts the range target, lifts passive sell quotes, and requires a bit more edge before taking a sell exit.
Notes: This turned out to be completely inert in local testing. `V90.6` matched `V90.5` exactly at 10'209 / 8'942.5, with identical TOMATOES PnL and trade counts. That suggests the current `V90.5` local behavior is not mainly limited by prematurely selling good longs; the real remaining issue is probably upstream in regime ownership or signal quality rather than a simple exit-hold tweak.
PnL: `V90.5` 10'209 / 8'942.5 | `V90.6` 10'209 / 8'942.5
