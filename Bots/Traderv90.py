...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "EARLY_READINESS_MIN": 0.35,
    "INV_WEAK_CAP": 10.0,
    "INV_MEDIUM_CAP": 18.0,
    "INV_STRONG_CAP": 28.0,
    "INV_EXCEPTIONAL_CAP": 40.0,
    "INV_MAX_CAP": 60.0,
    "INV_LEAN_ZONE": 28.0,
    "INV_UNWIND_ZONE": 42.0,
    "INV_EMERGENCY_ZONE": 56.0,
    "INV_ADD_CONFIDENCE": 0.62,
    "INV_HOLD_CONFIDENCE": 0.55,
    "INV_UNWIND_TARGET": 18.0,
    "INV_UNWIND_FLATTEN_BIAS": 0.55,
    "INV_EMERGENCY_FLATTEN_BIAS": 0.95,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def directional_confidence(
        ...
        confidence = self.signal_confidence(predicted_edge, fit_quality, queue_bias, lob_signal)
        half_spread = max(1.0, float(self.spread) / 2.0)
        trend_score = abs(predicted_edge) / max(1.0, 2.0 * half_spread)
        mr_score = abs(mr_signal) * max(0.25, mr_strength) / max(1.0, self.MR_SIGNAL_CAP)
        score = confidence * (0.65 * min(1.0, trend_score) + 0.35 * min(1.0, mr_score))
        direction = 0
        if regime == "range":
            if mr_signal > 0.06:
                direction = 1
    ...
    def take_orders(
        ...
        if (
            ...
            if regime != "range" and self.projected_position() >= target_position:
                ...
                if quantity > 0:
                    quantity = max(0, int(round(quantity * self.readiness_scale())))
                    before = self.buy_capacity
                    self.add_buy(int(self.best_ask), quantity)
                    took_buy = self.buy_capacity < before
