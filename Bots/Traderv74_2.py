...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "FUSED_DEPTH_MEMORY_ALPHA": 0.28,
    "FUSED_DEPTH_FAST_WEIGHT": 0.48,
    "FUSED_DEPTH_SLOW_WEIGHT": 0.38,
    "FUSED_DEPTH_MEMORY_WEIGHT": 0.12,
    "FUSED_DEPTH_BLEND_WEIGHT": 0.07,
    "FUSED_DEPTH_FAIR_WEIGHT": 0.04,
    "CHANNEL_WINDOW": 18,
    "CHANNEL_POSITION_WEIGHT": 0.08,
    "CHANNEL_BREAKOUT_BONUS": 0.0,
    "CHANNEL_REVERT_DAMP": 0.65,
    "CHANNEL_CONFIRM_IMBALANCE": 0.08,
    "FORCE_SPRING_WEIGHT": 0.55,
    "FORCE_DAMPING_WEIGHT": 0.40,
    "FORCE_ACCEL_WEIGHT": 0.32,
    "FORCE_FLOW_WEIGHT": 0.70,
    "FORCE_PRESSURE_WEIGHT": 0.55,
    "FORCE_DEPTH_WEIGHT": 0.35,
    "FORCE_BREAKOUT_WEIGHT": 0.20,
    "FORCE_CAP": 2.20,
    "FORCE_BLEND_WEIGHT": 0.35,
    "FORCE_FAIR_WEIGHT": 0.10,
    "GRAVITY_LEVELS": 3,
    "GRAVITY_WEIGHT": 0.28,
    "GRAVITY_FAIR_WEIGHT": 0.08,
    "GRAVITY_CAP": 1.40,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def fused_depth_signal(self, slow_depth_alpha: float) -> Dict[str, float]:
        half_spread = max(1.0, float(self.spread) / 2.0)
        top_alpha = (float(self.micro) - float(self.mid)) + (0.60 * half_spread * float(self.imbalance))
        alpha = float(self.FUSED_DEPTH_MEMORY_ALPHA)
        fast_prev = float(self.product_memory.get("fused_depth_fast_ema", top_alpha))
        slow_prev = float(self.product_memory.get("fused_depth_slow_ema", slow_depth_alpha))
        fast_ema = ((1.0 - alpha) * fast_prev) + (alpha * top_alpha)
        slow_ema = ((1.0 - alpha) * slow_prev) + (alpha * slow_depth_alpha)
        self.product_memory["fused_depth_fast_ema"] = fast_ema
        self.product_memory["fused_depth_slow_ema"] = slow_ema
        memory_component = 0.5 * (fast_ema + slow_ema)
        fused = (
            self.FUSED_DEPTH_FAST_WEIGHT * top_alpha
            + self.FUSED_DEPTH_SLOW_WEIGHT * slow_depth_alpha
            + self.FUSED_DEPTH_MEMORY_WEIGHT * memory_component
        )
        if top_alpha * slow_depth_alpha < 0:
            agreement = 0.0
            fused *= 0.72
        else:
            agreement = min(1.0, (abs(top_alpha) + abs(slow_depth_alpha)) / max(1.0, 2.0 * half_spread))
            if agreement < 0.35:
                fused *= 0.72
            else:
                fused += math.copysign(0.05 * agreement * abs(memory_component), fused if fused != 0 else top_alpha)
        fused = max(-self.ALPHA_CAP, min(self.ALPHA_CAP, fused))
        return {
            "top": top_alpha,
            "slow": slow_depth_alpha,
    ...
    def physics_force(
        ...
        half_spread = max(1.0, float(self.spread) / 2.0)
        displacement = (float(self.mid) - float(self.recent_average)) / half_spread
        velocity = flow_metrics["mid_drift"]
        acceleration = flow_metrics["flow_acceleration"]
        depth_signal = depth_alpha / half_spread

        spring = -self.FORCE_SPRING_WEIGHT * displacement
        damping = -self.FORCE_DAMPING_WEIGHT * velocity
        driving = (
            self.FORCE_ACCEL_WEIGHT * acceleration
            + self.FORCE_FLOW_WEIGHT * flow_metrics["bias"]
            + self.FORCE_PRESSURE_WEIGHT * pressure_bias
            + self.FORCE_DEPTH_WEIGHT * depth_signal
            + self.FORCE_BREAKOUT_WEIGHT * breakout_score
        )
        net = spring + damping + driving
        net = max(-self.FORCE_CAP, min(self.FORCE_CAP, net))
        return {
    ...
    def liquidity_gravity(self) -> float:
        half_spread = max(1.0, float(self.spread) / 2.0)
        support_pull = 0.0
        resistance_pull = 0.0

        for index, (price, volume) in enumerate(self.buy_levels[: int(self.GRAVITY_LEVELS)]):
            mass = max(0.0, float(volume) - 1.0) / (index + 1.0)
            if mass <= 0.0:
                continue
            distance = max(1.0, (float(self.mid) - float(price)) / half_spread)
            support_pull += mass / (distance * distance)

        for index, (price, volume) in enumerate(self.sell_levels[: int(self.GRAVITY_LEVELS)]):
            mass = max(0.0, float(volume) - 1.0) / (index + 1.0)
            if mass <= 0.0:
                continue
            distance = max(1.0, (float(price) - float(self.mid)) / half_spread)
            resistance_pull += mass / (distance * distance)

        scale = max(
            1.0,
            float(self.best_bid_volume) + float(self.best_ask_volume),
            3.0 * half_spread,
        )
        gravity = (support_pull - resistance_pull) / scale
        return max(-self.GRAVITY_CAP, min(self.GRAVITY_CAP, gravity))
    ...
    def fair_value(
        ...
        pressure_bias: float,
        depth_alpha: float,
        fused_depth: Dict[str, float],
        channel_position: float,
        channel_direction: float,
        physics_force: Dict[str, float],
    ) -> float:
        ...
        fair += self.FUSED_DEPTH_FAIR_WEIGHT * fused_depth["net"]
        fair += self.PRESSURE_BIAS_SCALE * pressure_bias * (0.65 if regime == "range" else 1.0)
        fair += self.FORCE_FAIR_WEIGHT * physics_force["net"] * half_spread
        fair += self.GRAVITY_FAIR_WEIGHT * physics_force.get("gravity", 0.0) * half_spread
    ...
    def run(self) -> List[Order]:
        ...
        fused_depth = self.fused_depth_signal(depth_alpha)
        ...
        if pressure_bias * regression_edge < 0:
            pressure_bias *= 0.60
        physics_force = self.physics_force(flow_metrics, pressure_bias, depth_alpha, breakout_score)
        if physics_force["net"] * regression_edge < 0:
            physics_force["net"] *= 0.70
        gravity_bias = self.liquidity_gravity()
        if gravity_bias * regression_edge < 0 and gravity_bias * breakout_score < 0:
            gravity_bias *= 0.65
        elif gravity_bias * regression_edge < 0:
            gravity_bias *= 0.85
        physics_force["gravity"] = gravity_bias
        fused_contrib = self.FUSED_DEPTH_BLEND_WEIGHT * fused_depth["net"]
        if fused_depth["agreement"] < 0.40:
            fused_contrib *= 0.55
        ...
        provisional_edge = (
            ...
            + (0.20 * pressure_bias)
            + fused_contrib
            + self.FORCE_BLEND_WEIGHT * physics_force["net"]
            + self.GRAVITY_WEIGHT * gravity_bias
        )
        ...
        self.diagnostic_context = {
            ...
            "fused_depth": fused_depth["net"],
            "pressure_bias": pressure_bias,
            "hybrid_alpha": hybrid_alpha,
            "physics_force": physics_force["net"],
