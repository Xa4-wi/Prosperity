...
class AshLocalFairTrader:
    ...
    RECYCLE_START = 34
    RECYCLE_STRONG = 46
    RECYCLE_TAKE_FADE = 0.12
    RECYCLE_QUOTE_FADE = 0.10
    RECYCLE_TAKE_RELIEF = 0.10
    RECYCLE_QUOTE_RELIEF = 0.10
    RECYCLE_JOIN_BONUS = 0.10
    VACUUM_SIZE = 3
    VACUUM_GAP = 4
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        recycler_base = (
            max(bid_level, ask_level) <= 1
            and book.spread <= 18.0
            and abs(quote_alpha) <= 0.35
        )
        long_recycler = (
            recycler_base
            and mgr.projected() >= self.RECYCLE_START
            and ask_level <= 1
            and take_alpha < self.RECYCLE_TAKE_FADE
            and quote_alpha < self.RECYCLE_QUOTE_FADE
        ...
        strong_long_recycler = (
            long_recycler
            and mgr.projected() >= self.RECYCLE_STRONG
            and take_alpha < 0.04
        )
        strong_short_recycler = (
            short_recycler
            and mgr.projected() <= -self.RECYCLE_STRONG
            and take_alpha > -0.04
        )
        ...
        if long_recycler:
            buy_need += 0.04
            sell_need = max(0.70, sell_need - self.RECYCLE_TAKE_RELIEF)
            if strong_long_recycler and take_alpha <= 0.02:
                can_take_buy = False
        elif short_recycler:
            sell_need += 0.04
            buy_need = max(0.70, buy_need - self.RECYCLE_TAKE_RELIEF)
            if strong_short_recycler and take_alpha >= -0.02:
                can_take_sell = False
        ...
        if neutral_drip:
            ...
            sell_join_edge += 0.10
        if long_recycler:
            buy_edge += 0.12 + (0.06 if strong_long_recycler else 0.0)
            sell_edge -= self.RECYCLE_QUOTE_RELIEF
            sell_join_edge += self.RECYCLE_JOIN_BONUS
        elif short_recycler:
            sell_edge += 0.12 + (0.06 if strong_short_recycler else 0.0)
            buy_edge -= self.RECYCLE_QUOTE_RELIEF
            buy_join_edge += self.RECYCLE_JOIN_BONUS
        ...
        if long_recycler:
            sell_size += 1
            buy_size = max(2, buy_size - (2 if strong_long_recycler else 1))
        elif short_recycler:
            buy_size += 1
            sell_size = max(2, sell_size - (2 if strong_short_recycler else 1))
        if buy_markout < self.SOFT_BAD_MARKOUT and book.imbalance <= 0.0:
        ...
        if strong_long_recycler and pos > self.RECYCLE_STRONG - 2:
            allow_bid = False
        if strong_short_recycler and pos < -(self.RECYCLE_STRONG - 2):
            allow_ask = False
