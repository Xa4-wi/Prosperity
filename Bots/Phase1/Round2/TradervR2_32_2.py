...
PORTFOLIO_RISK_PARAMS = {
    ...
    "DRAWDOWN_CAUTION": 320.0,
    "DRAWDOWN_DEFENSE": 520.0,
    "DRAWDOWN_KILL": 760.0,
    "SHOCK_CAUTION": 140.0,
    "SHOCK_DEFENSE": 220.0,
    "AGGRESSIVE_SCALE": 0.70,
    "INCREASE_SCALE": 0.88,
    "LATE_ASH_PROGRESS": 0.97,
    "LATE_ASH_POS": 28,
}
...
class Trader:
    ...
    def _update_portfolio_risk(self, state: TradingState, memory: dict) -> Tuple[dict, dict]:
        ...
        for product in ("ASH_COATED_OSMIUM", "INTARIAN_PEPPER_ROOT"):
            ...
            total_cash += cash
            marks[product] = self._mark_price(product, state, risk_state)
        ...
        mode = "normal"
        if drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_KILL"]):
            mode = "kill"
        elif drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_DEFENSE"]) or shock_ema >= float(PORTFOLIO_RISK_PARAMS["SHOCK_DEFENSE"]):
            mode = "defense"
        elif drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_CAUTION"]) or shock_ema >= float(PORTFOLIO_RISK_PARAMS["SHOCK_CAUTION"]):
            mode = "caution"

        progress = clamp(timestamp / 999900.0 if timestamp > 0 else 0.0, 0.0, 1.0)
        if progress >= float(PORTFOLIO_RISK_PARAMS["LATE_ASH_PROGRESS"]) and abs(int(state.position.get("ASH_COATED_OSMIUM", 0))) >= int(PORTFOLIO_RISK_PARAMS["LATE_ASH_POS"]):
            if mode == "normal":
                mode = "caution"
            elif mode == "caution":
                mode = "defense"
            ...
            for order in orders:
                ...
                if mode == "caution":
                    if aggressive and increases_abs:
                        out_qty = self._scaled_qty(qty, float(PORTFOLIO_RISK_PARAMS["AGGRESSIVE_SCALE"]))
                    elif increases_abs:
                        out_qty = self._scaled_qty(qty, float(PORTFOLIO_RISK_PARAMS["INCREASE_SCALE"]))
                elif mode == "defense":
                    if aggressive and increases_abs:
                        keep = False
                    elif increases_abs:
