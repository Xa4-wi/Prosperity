...
DEFAULT_ASH_PARAMS = {
    ...
    "TERMINAL_RECYCLE_START": 0.86,
    "TERMINAL_RECYCLE_POS": 18,
    "TERMINAL_RECYCLE_SIGNAL_MAX": 0.10,
    "TERMINAL_RECYCLE_CONVICTION_MAX": 0.55,
    "TERMINAL_RECYCLE_EDGE_BONUS": 0.12,
    "TERMINAL_RECYCLE_JOIN_BONUS": 0.06,
    "TERMINAL_RECYCLE_FRONT_BONUS": 1,
    "TERMINAL_RECYCLE_SELL_EDGE_PENALTY": 0.18,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        late_short_recycle = (
            mode == "normal"
            and pos <= -int(self.p["TERMINAL_RECYCLE_POS"])
            and progress >= float(self.p["TERMINAL_RECYCLE_START"])
            and ask_level == 0
            and quote_signal >= -float(self.p["TERMINAL_RECYCLE_SIGNAL_MAX"])
            and signal_conviction <= float(self.p["TERMINAL_RECYCLE_CONVICTION_MAX"])
        )
        late_long_recycle = (
            mode == "normal"
            and pos >= int(self.p["TERMINAL_RECYCLE_POS"])
        ...
        if late_short_recycle:
            ...
            sell_qe += float(self.p["TERMINAL_RECYCLE_SELL_EDGE_PENALTY"])
        elif late_long_recycle:
            sell_qe -= float(self.p["TERMINAL_RECYCLE_EDGE_BONUS"])
            buy_qe += float(self.p["TERMINAL_RECYCLE_SELL_EDGE_PENALTY"])
        if refill_bias > 0:
