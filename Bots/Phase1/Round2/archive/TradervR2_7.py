...
DEFAULT_IPR_PARAMS = {
    ...
    "SCHEDULE_END": 0.72,
    "CATCHUP_RELIEF": 0.35,
    "CATCHUP_FRONT_BONUS": 2,
    "CATCHUP_FRONT_PUSH": 1,
    "SHOCK_BLOCK_Z": 0.80,
    "LATE_TRIM_START": 0.82,
    "LATE_TRIM_Z": 0.75,
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        carry_target = max(0, target_int)
        schedule_frac = clamp(
            (progress - float(self.p["SCHEDULE_START"]))
            / max(1e-6, float(self.p["SCHEDULE_END"]) - float(self.p["SCHEDULE_START"])),
            0.0,
            1.0,
        )
        schedule_target = int(round(carry_target * schedule_frac))
        behind_schedule = mgr.projected() < max(0, schedule_target - 4)
        ahead_carry = mgr.projected() > max(schedule_target + 6, carry_target)
        favorable_shock = zscore <= 0.0
        chase_risk = zscore >= float(self.p["SHOCK_BLOCK_Z"])
        ...
        if behind_schedule and favorable_shock:
            buy_te -= float(self.p["CATCHUP_RELIEF"])
        elif behind_schedule and chase_risk:
            buy_te += 0.18
        if progress >= float(self.p["LATE_TRIM_START"]) and zscore >= float(self.p["LATE_TRIM_Z"]) and ahead_carry:
            sell_te -= 0.15
        ...
        if bullish and pos < target_int:
            ...
            front_buy = max(front_buy, book.best_bid + 1)
        if behind_schedule and favorable_shock and book.best_bid + int(self.p["CATCHUP_FRONT_PUSH"]) < book.best_ask:
            front_buy = max(front_buy, book.best_bid + int(self.p["CATCHUP_FRONT_PUSH"]))
        ...
        if behind_schedule and favorable_shock:
            buy_front_sz += int(self.p["CATCHUP_FRONT_BONUS"])
