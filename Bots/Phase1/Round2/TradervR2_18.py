...
class AshLocalFairTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        deep_imbalance_gap = half_spread * deep_imbalance

        take_alpha = (
            0.50 * micro_gap
            + 0.25 * deep_micro_gap
            + 0.20 * imbalance_gap
            + 0.15 * deep_imbalance_gap
            + 0.10 * stable_gap
        )
        quote_alpha = 0.50 * stable_gap + 0.15 * micro_gap + 0.10 * deep_micro_gap

        stable_sign = sign(stable_gap, 0.20)
        imbalance_sign = sign(imbalance_gap + 0.50 * deep_imbalance_gap, 0.12)
        if stable_sign != 0 and stable_sign == imbalance_sign:
            take_alpha += 0.15 * stable_gap
            quote_alpha += 0.10 * stable_gap
