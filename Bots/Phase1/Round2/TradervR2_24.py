...
DEFAULT_ASH_PARAMS = {
    ...
    "TERMINAL_RECYCLE_START": 0.78,
    "TERMINAL_RECYCLE_HARD_START": 0.92,
    "TERMINAL_RECYCLE_POS": 14,
    "TERMINAL_RECYCLE_SIGNAL_MAX": 0.18,
    "TERMINAL_RECYCLE_STABLE_MAX": 0.75,
    "TERMINAL_RECYCLE_CONVICTION_MAX": 0.70,
    "TERMINAL_RECYCLE_COVER_RELIEF": 0.22,
    "TERMINAL_RECYCLE_EDGE_BONUS": 0.18,
    "TERMINAL_RECYCLE_JOIN_BONUS": 0.10,
    "TERMINAL_RECYCLE_FRONT_BONUS": 2,
    "TERMINAL_RECYCLE_SELL_THROTTLE": 0.35,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        stable_gap = self._stable_mid(book) - book.mid
        trade_confirm = self._trade_confirmation(state, book)
        ...
        recycle_start = float(self.p["TERMINAL_RECYCLE_START"])
        recycle_hard_start = float(self.p["TERMINAL_RECYCLE_HARD_START"])
        recycle_pos = int(self.p["TERMINAL_RECYCLE_POS"])
        recycle_signal_max = float(self.p["TERMINAL_RECYCLE_SIGNAL_MAX"])
        recycle_stable_max = float(self.p["TERMINAL_RECYCLE_STABLE_MAX"])
        recycle_conviction_max = float(self.p["TERMINAL_RECYCLE_CONVICTION_MAX"])
        late_short_recycle = (
            mode == "normal"
            and pos <= -recycle_pos
            and progress >= recycle_start
            and ask_level <= 1
            and quote_signal >= -recycle_signal_max
            and stable_gap >= -recycle_stable_max
            and signal_conviction <= recycle_conviction_max
        )
        late_long_recycle = (
            mode == "normal"
            and pos >= recycle_pos
        ...
        hard_short_recycle = late_short_recycle and progress >= recycle_hard_start and pos <= -(recycle_pos + 6)
        hard_long_recycle = late_long_recycle and progress >= recycle_hard_start and pos >= recycle_pos + 6
        ...
        if late_short_recycle:
            buy_need = max(0.70, buy_need - float(self.p["TERMINAL_RECYCLE_COVER_RELIEF"]))
            sell_need += 0.5 * float(self.p["TERMINAL_RECYCLE_SELL_THROTTLE"])
            if hard_short_recycle:
                buy_need = max(0.70, buy_need - 0.12)
                sell_need += 0.18
        elif late_long_recycle:
            sell_need = max(0.70, sell_need - float(self.p["TERMINAL_RECYCLE_COVER_RELIEF"]))
            buy_need += 0.5 * float(self.p["TERMINAL_RECYCLE_SELL_THROTTLE"])
            if hard_long_recycle:
                sell_need = max(0.70, sell_need - 0.12)
                buy_need += 0.18
        if refill_bias > 0:
        ...
        if buy_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance <= 0.0:
            buy_need += 0.5 * float(self.p["MARKOUT_EDGE_PENALTY"])
        if sell_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance >= 0.0:
            sell_need += 0.5 * float(self.p["MARKOUT_EDGE_PENALTY"])
        ...
        if late_short_recycle:
            buy_qe -= float(self.p["TERMINAL_RECYCLE_EDGE_BONUS"])
            sell_qe += float(self.p["TERMINAL_RECYCLE_SELL_THROTTLE"])
            if hard_short_recycle:
                buy_qe -= 0.08
                sell_qe += 0.10
        elif late_long_recycle:
            sell_qe -= float(self.p["TERMINAL_RECYCLE_EDGE_BONUS"])
            buy_qe += float(self.p["TERMINAL_RECYCLE_SELL_THROTTLE"])
            if hard_long_recycle:
                sell_qe -= 0.08
                buy_qe += 0.10
        if refill_bias > 0:
        ...
        if buy_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance <= 0.0:
            buy_qe += 0.5 * float(self.p["MARKOUT_EDGE_PENALTY"])
        if sell_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance >= 0.0:
            sell_qe += 0.5 * float(self.p["MARKOUT_EDGE_PENALTY"])
        ...
        if late_short_recycle or late_long_recycle:
            join_edge += float(self.p["TERMINAL_RECYCLE_JOIN_BONUS"])
        ...
        if sell_markout < float(self.p["HARD_BAD_MARKOUT"]) and book.imbalance >= 0.0 and ask_level >= 2:
            allow_ask = False
        if hard_short_recycle and quote_signal >= -0.05 and signal_conviction <= 0.55:
            allow_ask = False
        if hard_long_recycle and quote_signal <= 0.05 and signal_conviction <= 0.55:
            allow_bid = False
        ...
        if late_short_recycle:
            buy_front_sz += int(self.p["TERMINAL_RECYCLE_FRONT_BONUS"])
            sell_front_sz = max(2, sell_front_sz - 1)
            if hard_short_recycle:
                buy_front_sz += 1
                sell_back_sz = max(1, sell_back_sz - 1)
        elif late_long_recycle:
            sell_front_sz += int(self.p["TERMINAL_RECYCLE_FRONT_BONUS"])
            buy_front_sz = max(2, buy_front_sz - 1)
            if hard_long_recycle:
                sell_front_sz += 1
                buy_back_sz = max(1, buy_back_sz - 1)
        if refill_bias > 0:
            buy_front_sz += int(self.p["REFILL_FRONT_BONUS"])
        elif refill_bias < 0:
            sell_front_sz += int(self.p["REFILL_FRONT_BONUS"])
        if buy_reactivation:
            buy_front_sz += 1 + int(bars_since_buy_fill >= 10)
            if micro_nibble:
                buy_front_sz += 1
        elif sell_reactivation:
            sell_front_sz += 1 + int(bars_since_sell_fill >= 10)
            if micro_nibble:
                sell_front_sz += 1
        if access_buy:
