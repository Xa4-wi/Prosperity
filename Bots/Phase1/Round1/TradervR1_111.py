...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        prev_react_signal_dir = int(ash_state.get("react_signal_dir", 0))
        prev_react_persist = int(ash_state.get("react_persist_ticks", 0))
        prev_react_signal_ema = float(ash_state.get("react_signal_ema", 0.0))
        ...
        react_signal_dir = sign(quote_signal, 0.08)
        react_signal_ema = ema(prev_react_signal_ema, quote_signal, 0.35)
        if react_signal_dir != 0 and react_signal_dir == prev_react_signal_dir:
            react_persist_ticks = prev_react_persist + 1
        elif react_signal_dir != 0:
            react_persist_ticks = 1
        else:
            react_persist_ticks = max(0, prev_react_persist - 1)
        ...
        reactivation = (
            ...
            and react_signal_dir != 0
            and react_persist_ticks >= 3
            and max(abs(quote_signal), abs(react_signal_ema)) >= 0.08
        )
        reactivation_strength = 0.0
        if reactivation:
            reactivation_strength = clamp(
                0.35
                + 0.05 * min(5, max(0, react_persist_ticks - 3))
                + 0.20 * min(1.0, max(abs(quote_signal), abs(react_signal_ema)) / 0.30),
                0.0,
                1.0,
            )
            extra = min(0.35, 0.03 * (no_fill_ticks - 8) + 0.03 * max(0, react_persist_ticks - 3))
            if react_signal_dir > 0:
                buy_qe -= 0.15 + extra
                sell_qe += 0.05 * reactivation_strength
            elif react_signal_dir < 0:
                sell_qe -= 0.15 + extra
                buy_qe += 0.05 * reactivation_strength
        ...
        if reactivation:
            if react_signal_dir > 0 and bid_level == 0:
                if reactivation_strength >= 0.70 and book.best_bid + 1 < book.best_ask:
                    front_buy = max(front_buy, book.best_bid + 1)
                else:
                    front_buy = max(front_buy, book.best_bid)
            elif react_signal_dir < 0 and ask_level == 0:
                if reactivation_strength >= 0.70 and book.best_ask - 1 > book.best_bid:
                    front_sell = min(front_sell, book.best_ask - 1)
                else:
                    front_sell = min(front_sell, book.best_ask)
        ...
        if reactivation:
            if react_signal_dir > 0:
                buy_front_sz += 1 + int(reactivation_strength >= 0.65)
            elif react_signal_dir < 0:
                sell_front_sz += 1 + int(reactivation_strength >= 0.65)
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
        ...
        ash_state["last_fill_ts"] = int(last_fill_ts)
        ash_state["no_fill_ticks"] = int(no_fill_ticks)
        ash_state["react_signal_dir"] = int(react_signal_dir)
        ash_state["react_persist_ticks"] = int(react_persist_ticks)
        ash_state["react_signal_ema"] = round(react_signal_ema, 4)
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
