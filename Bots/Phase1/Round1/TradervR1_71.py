...
class AshCoatedOsmiumTrader:
    ...
    def _smoothed_toxicity_levels(
        ...
        return bid_level, ask_level, bid_score, ask_score, memory
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        bid_level, ask_level, bid_score, ask_score, memory = self._smoothed_toxicity_levels(
        ...
        long_exit_boost = clamp((bid_score - 0.85) / 1.95, 0.0, 1.0) if pos > 6 else 0.0
        short_exit_boost = clamp((ask_score - 0.85) / 1.95, 0.0, 1.0) if pos < -6 else 0.0
        ...
        if ask_level == 2 and pos < -6:
            can_take_sell = False
        if long_exit_boost > 0.85 and pos > 10:
            can_take_buy = False
        if short_exit_boost > 0.85 and pos < -10:
            can_take_sell = False
        ...
        if long_exit_boost > 0.0:
            buy_need += 0.10 + 0.30 * long_exit_boost
            sell_need = max(0.70, sell_need - (0.12 + 0.28 * long_exit_boost))
        if short_exit_boost > 0.0:
            sell_need += 0.10 + 0.30 * short_exit_boost
            buy_need = max(0.70, buy_need - (0.12 + 0.28 * short_exit_boost))
        ...
        if mode == "normal":
            ...
            if pos > 0:
                ...
                sell_qe += 0.90
        if long_exit_boost > 0.0:
            buy_qe += 0.15 + 0.45 * long_exit_boost
            sell_qe -= 0.10 + 0.35 * long_exit_boost
        if short_exit_boost > 0.0:
            sell_qe += 0.15 + 0.45 * short_exit_boost
            buy_qe -= 0.10 + 0.35 * short_exit_boost
        ...
        buy_join_edge = join_edge
        sell_join_edge = join_edge
        if long_exit_boost > 0.0:
            buy_join_edge = max(0.20, buy_join_edge - (0.08 + 0.10 * long_exit_boost))
            sell_join_edge += 0.08 + 0.22 * long_exit_boost
        if short_exit_boost > 0.0:
            sell_join_edge = max(0.20, sell_join_edge - (0.08 + 0.10 * short_exit_boost))
            buy_join_edge += 0.08 + 0.22 * short_exit_boost
        ...
        if pos > 6 and bid_level >= 1:
            ...
            buy_front_sz += 1
        if long_exit_boost > 0.0:
            sell_front_sz += max(1, int(round(2.0 * long_exit_boost)))
            buy_front_sz = max(2, buy_front_sz - int(round(long_exit_boost)))
        if short_exit_boost > 0.0:
            buy_front_sz += max(1, int(round(2.0 * short_exit_boost)))
            sell_front_sz = max(2, sell_front_sz - int(round(short_exit_boost)))
