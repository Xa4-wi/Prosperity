...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        bid_level, ask_level, memory = self._smoothed_toxicity_levels(
            ...
            memory,
            timestamp,
            quiet_non_toxic_ticks=bars_since_fill if raw_bid_level == 0 and raw_ask_level == 0 else 0,
        )
        attack = clamp(
        ...
        if reactivation:
            extra = min(0.30, 0.05 * max(0, bars_since_fill - 8))
            if react_side > 0:
                buy_qe -= 0.15 + extra
                if micro_nibble:
                    buy_qe -= 0.05
        ...
        if reactivation and react_side > 0:
            if book.best_bid + 1 < book.best_ask:
                front_buy = max(front_buy, book.best_bid + 1)
            else:
                front_buy = max(front_buy, book.best_bid)
        elif reactivation and react_side < 0:
        ...
        if reactivation:
            if react_side > 0:
                buy_front_sz += 1 + int(bars_since_fill >= 12)
                if micro_nibble:
                    buy_front_sz += 1
            elif react_side < 0:
                sell_front_sz += 1 + int(bars_since_fill >= 12)
                if micro_nibble:
                    sell_front_sz += 1
        if neutral_drip:
