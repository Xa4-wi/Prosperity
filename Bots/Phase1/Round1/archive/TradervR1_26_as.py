...
# Archetype: Avellaneda-Stoikov style anchored MM with reactive reservation/spread
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    ...
    "AS_TAU": 1.0,
    "AS_SIGMA_BASE": 0.55,
    "AS_SIGMA_SPREAD_WEIGHT": 0.085,
    "AS_SIGMA_MICRO_WEIGHT": 0.70,
    "AS_K_BASE": 1.35,
    "AS_K_DEPTH_WEIGHT": 0.020,
    "AS_MIN_HALF_SPREAD": 4.7,
    "JOIN_EDGE": 2.0,
...
class AshCoatedOsmiumTrader:
    """
    Avellaneda-Stoikov flavored Osmium engine.
    - Fair stays anchored to 10000 with small imbalance/micro nudges
    - Reservation and half-spread react to volatility and depth via A-S proxies
    - Toxic book logic still gates passive exposure
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        fair = ref + self.p["IMBALANCE_WEIGHT"] * book.imbalance + self.p["MICRO_WEIGHT"] * (book.micro - book.mid)

        depth = max(1.0, float(book.best_bid_vol + book.best_ask_vol))
        sigma2 = (
            float(self.p["AS_SIGMA_BASE"])
            + float(self.p["AS_SIGMA_SPREAD_WEIGHT"]) * (book.spread_val ** 2)
            + float(self.p["AS_SIGMA_MICRO_WEIGHT"]) * abs(book.micro - book.mid)
        )
        gamma = float(self.p["AS_GAMMA"])
        tau = float(self.p["AS_TAU"])
        k = float(self.p["AS_K_BASE"]) + float(self.p["AS_K_DEPTH_WEIGHT"]) * depth
        inv_term = gamma * mgr.projected() * sigma2 * tau
        reservation = fair - inv_term
        half_spread = 0.5 * gamma * sigma2 * tau + math.log1p(gamma / max(0.2, k)) / gamma
        half_spread = max(float(self.p["AS_MIN_HALF_SPREAD"]), half_spread)
        ...
        # Dynamic quote edges start from the A-S half-spread.
        buy_qe = sell_qe = half_spread
