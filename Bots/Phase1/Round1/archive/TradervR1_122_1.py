...
DEFAULT_ASH_PARAMS = {
    ...
    "MARKOUT_ALPHA": 0.16,
    "SOFT_BAD_MARKOUT": -0.50,
    "HARD_BAD_MARKOUT": -1.00,
    "MARKOUT_EDGE_PENALTY": 0.18,
    "MARKOUT_SIZE_PENALTY": 0.14,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if buy_reactivation:
            extra = min(0.35, 0.06 * max(0, bars_since_buy_fill - 8))
            buy_qe -= 0.18 + extra
            if micro_nibble:
                buy_qe -= 0.05
        elif sell_reactivation:
            extra = min(0.35, 0.06 * max(0, bars_since_sell_fill - 8))
            sell_qe -= 0.18 + extra
            if micro_nibble:
                sell_qe -= 0.05
