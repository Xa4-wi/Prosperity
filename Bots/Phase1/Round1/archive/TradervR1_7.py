...
class AshCoatedOsmiumBot:
    ...
    IMBALANCE_FAIR_WEIGHT = 3.8
    MICRO_FAIR_WEIGHT = 0.40
    INVENTORY_SKEW = 0.08
    BASE_EDGE = 7.4
    JOIN_EDGE = 2.0
    FRONT_ORDER_SIZE = 8
    ...
    def take_orders(self, reservation: float) -> None:
        ...
        position = self.manager.projected_position()
        buy_edge_needed = 1.7 if self.bid_toxic() else 1.2
        sell_edge_needed = 1.7 if self.ask_toxic() else 1.2
        ...
        for distance, clip in self.TAKE_LEVELS:
            if buy_edge >= max(distance, buy_edge_needed):
                buy_size = clip
        if buy_edge >= self.DISLOCATION_EDGE:
            buy_size = max(buy_size, 18)
        if buy_size > 0:
            if position >= self.SOFT_LIMIT:
        ...
        for distance, clip in self.TAKE_LEVELS:
            if sell_edge >= max(distance, sell_edge_needed):
                sell_size = clip
        if sell_edge >= self.DISLOCATION_EDGE:
            sell_size = max(sell_size, 18)
        if sell_size > 0:
            if position <= -self.SOFT_LIMIT:
