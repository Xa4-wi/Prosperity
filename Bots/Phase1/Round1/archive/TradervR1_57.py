...
DEFAULT_ASH_PARAMS = {
    ...
    "NORMAL_TAKE_EDGE": 1.40,
    "TOXIC_TAKE_EDGE": 2.10,
...
class AshCoatedOsmiumTrader:
    ...
    def _size_mult(
        ...
        if mode == "calm_mm":
            mult = 1.20
        elif mode == "toxic_clear":
            mult = 0.75
        else:
            mult = 1.0

        if mode == "toxic_clear":
            if side == "buy" and (bid_toxic or pos >= soft):
                mult *= 0.90
            if side == "sell" and (ask_toxic or pos <= -soft):
                mult *= 0.90

        return clamp(mult, 0.55, 1.20)
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        if mode == "calm_mm":
            l1 = max(0.35, float(self.p["TAKE_L1_EDGE"]) - 0.25)
            l2 = max(l1, float(self.p["TAKE_L2_EDGE"]) - 0.40)
            take_levels = [
                (l1, int(self.p["TAKE_L1_SIZE"])),
                (l2, int(self.p["TAKE_L2_SIZE"])),
                (float(self.p["TAKE_L3_EDGE"]), int(self.p["TAKE_L3_SIZE"])),
            ]
        ...
        if mode == "calm_mm":
            buy_need = max(1.0, buy_need - 0.10)
            sell_need = max(1.0, sell_need - 0.10)
        elif mode == "toxic_clear":
            if bid_toxic:
                can_take_buy = False
            if ask_toxic:
        ...
        if mode == "calm_mm":
            ...
            if bid_toxic or pos > 0:
                buy_qe += 0.80
            if ask_toxic or pos < 0:
                sell_qe += 0.80
        ...
        if mode == "toxic_clear":
            if bid_toxic or book.imbalance < -strong or pos > soft:
                allow_bid = False
            if ask_toxic or book.imbalance > strong or pos < -soft:
                allow_ask = False
