...
DEFAULT_IPR_PARAMS = {
    ...
    "CORE_MIN_HOLD": 50.0,
    "CORE_OVERLAY_BAND": 15.0,
    "MIDDAY_PROGRESS": 0.50,
    "LATE_UNWIND_START": 0.82,
    "LATE_UNWIND_Z": 1.20,
    "LATE_UNWIND_FLOOR": 30.0,
    "BULLISH_SELL_CAP": 3.0,
    # Execution
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        core_floor = int(self.p["CORE_MIN_HOLD"])
        overlay_cap = core_floor + int(self.p["CORE_OVERLAY_BAND"])
        late_unwind = (
            progress >= float(self.p["LATE_UNWIND_START"])
            and zscore >= float(self.p["LATE_UNWIND_Z"])
        )
        active_floor = core_floor if not late_unwind else int(self.p["LATE_UNWIND_FLOOR"])
        target = max(target, float(active_floor))
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            pos = mgr.projected()
            sell_floor = active_floor
            if progress < float(self.p["MIDDAY_PROGRESS"]) and pos <= overlay_cap:
                sell_floor = overlay_cap
            qty = min(book.best_bid_vol, 16, max(0, pos - sell_floor))
            if bullish and pos < max(26, target_int - 6) and not late_unwind:
                qty = 0
            elif bullish and pos > active_floor and not late_unwind:
                qty = min(qty, int(self.p["BULLISH_SELL_CAP"]))
        ...
        if progress < float(self.p["MIDDAY_PROGRESS"]) and pos <= overlay_cap:
            allow_sell = False
        if bullish and pos <= overlay_cap and not late_unwind:
            allow_sell = False
        ...
        if allow_sell and front_sell > book.best_bid:
            if bullish and not late_unwind:
                front_sell += 1
        ...
        for side, price, size in quotes:
            ...
            if side == "buy" and pos < target_int + buy_buf:
                ...
                mgr.buy(price, qty)
            elif side == "sell" and pos > max(active_floor, target_int - sell_buf):
                qty = min(size, mgr.sell_cap, max(0, pos - max(active_floor, target_int - sell_buf)))
                if bullish and not late_unwind:
                    qty = min(qty, int(self.p["BULLISH_SELL_CAP"]))
                mgr.sell(price, qty)
