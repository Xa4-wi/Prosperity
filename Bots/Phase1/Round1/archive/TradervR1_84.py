...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        safe_take_jump = (
            ...
            and book.spread_val <= 12.0
            and abs(book.imbalance) <= 0.12
            and (book.best_bid_vol + book.best_ask_vol) >= 24
        )
        if safe_take_jump:
            buy_need = max(0.65, buy_need - 0.30)
            sell_need = max(0.65, sell_need - 0.30)
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_buy:
            ...
            for edge_thr, clip in take_levels:
                if buy_edge >= max(edge_thr, buy_need):
                    take_buy = clip
            if safe_take_jump and abs(pos) <= 8 and take_signal >= 0.65 and buy_edge >= max(0.65, buy_need - 0.15):
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_sell:
            ...
            for edge_thr, clip in take_levels:
                if sell_edge >= max(edge_thr, sell_need):
                    take_sell = clip
            if safe_take_jump and abs(pos) <= 8 and take_signal <= -0.65 and sell_edge >= max(0.65, sell_need - 0.15):
