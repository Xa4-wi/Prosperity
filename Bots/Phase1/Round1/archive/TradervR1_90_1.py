...
DEFAULT_ASH_PARAMS = {
    ...
    "VACUUM_GAP": 4,
    "VACUUM_SIZE": 2,
    "VACUUM_BID_VISIBLE_ENABLE": True,
    "VACUUM_ASK_VISIBLE_ENABLE": False,
    "VACUUM_BID_VISIBLE_EDGE": 2.2,
    "VACUUM_ASK_VISIBLE_EDGE": 99.0,
    "VACUUM_BID_VISIBLE_CAP": 18,
    "VACUUM_ASK_VISIBLE_CAP": 0,
    "VACUUM_BID_FAIR_BLEND": 0.72,
    "VACUUM_ASK_FAIR_BLEND": 0.55,
...
class AshCoatedOsmiumTrader:
    ...
    def _vacuum_fair(
        ...
        blend = float(
            self.p["VACUUM_BID_FAIR_BLEND"] if side == "bid_only" else self.p["VACUUM_ASK_FAIR_BLEND"]
        )
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if not book.valid:
            ...
            if (
                vacuum_side == "bid_only"
                and self.p.get("VACUUM_BID_VISIBLE_ENABLE", True)
                and book.best_bid > 0
                and (vacuum_fair - book.best_bid) >= float(self.p["VACUUM_BID_VISIBLE_EDGE"])
                and pos < int(self.p["VACUUM_BID_VISIBLE_CAP"])
            ):
                bid_px = int(min(book.best_bid + 1, math.floor(vacuum_fair - 1.5)))
                if bid_px > 0:
                    mgr.buy(bid_px, min(quote_size, max(0, int(self.p["VACUUM_BID_VISIBLE_CAP"]) - pos)))
            elif (
                vacuum_side == "ask_only"
                and self.p.get("VACUUM_ASK_VISIBLE_ENABLE", True)
                and book.best_ask > 0
                and (book.best_ask - vacuum_fair) >= float(self.p["VACUUM_ASK_VISIBLE_EDGE"])
                and pos > -int(self.p["VACUUM_ASK_VISIBLE_CAP"])
            ):
                ask_px = int(max(book.best_ask - 1, math.ceil(vacuum_fair + 1.5)))
                if ask_px > 0:
                    mgr.sell(ask_px, min(quote_size, max(0, pos + int(self.p["VACUUM_ASK_VISIBLE_CAP"]))))
