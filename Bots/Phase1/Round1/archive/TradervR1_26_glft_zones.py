...
# Archetype: anchored MM with zone-based nonlinear inventory pressure (GLFT-inspired)
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
...
class AshCoatedOsmiumTrader:
    """
    Anchored MM with structured nonlinear inventory control.
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        zone = float(self.p["INV_ZONE"])
        active_ratio = 0.0
        if inv_ratio > zone:
            active_ratio = inv_ratio - zone
        elif inv_ratio < -zone:
            active_ratio = inv_ratio + zone
        inv_pressure = float(self.p["INV_A1"]) * active_ratio + float(self.p["INV_A3"]) * (active_ratio ** 3)
        ...
        # Dynamic quote edges with same-side inventory widening.
        ...
        if pos >= soft:
            buy_qe += 2.0; sell_qe -= 1.0
        elif pos <= -soft:
            buy_qe -= 1.0; sell_qe += 2.0
        elif active_ratio > 0:
            buy_qe += 0.8 * active_ratio
            sell_qe -= 0.3 * active_ratio
        elif active_ratio < 0:
            buy_qe += 0.3 * active_ratio
            sell_qe -= 0.8 * active_ratio
