...
DEFAULT_ASH_PARAMS = {
    ...
    "ATTACK_SPREAD_MAX": 15.0,
    "ATTACK_DEPTH_MIN": 26.0,
    "ATTACK_IMBALANCE_MAX": 0.18,
    "ATTACK_EDGE_BONUS": 0.25,
    "ATTACK_JOIN_BONUS": 0.20,
    "ATTACK_TAKE_BONUS": 0.25,
    "ATTACK_SIZE_BONUS": 0.12,
...
class AshCoatedOsmiumTrader:
    ...
    def _attack_factor(
        ...
        score = 0.0
        if book.spread_val <= float(self.p["ATTACK_SPREAD_MAX"]):
            score += 1.0
        if (book.best_bid_vol + book.best_ask_vol) >= float(self.p["ATTACK_DEPTH_MIN"]):
            score += 1.0
        if abs(book.imbalance) <= float(self.p["ATTACK_IMBALANCE_MAX"]):
            score += 1.0
        if not bid_toxic and not ask_toxic:
            score += 1.0
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        attack = self._attack_factor(book, pos, soft, bid_toxic, ask_toxic, mode)
        ...
        if mode == "normal":
            buy_need = max(0.95, buy_need - 0.10)
            sell_need = max(0.95, sell_need - 0.10)
        elif mode == "dislocation_take":
            buy_need = max(0.80, buy_need - 0.45)
            sell_need = max(0.80, sell_need - 0.45)
        elif mode == "toxic_defense":
            if bid_toxic and pos > 6:
                can_take_buy = False
            if ask_toxic and pos < -6:
                can_take_sell = False
            buy_need += 0.20
            sell_need += 0.20
        elif mode == "inventory_clear":
        ...
        if attack > 0.0:
            take_bonus = float(self.p["ATTACK_TAKE_BONUS"]) * attack
            buy_need = max(0.70, buy_need - take_bonus)
            sell_need = max(0.70, sell_need - take_bonus)
        ...
        if attack > 0.0:
            edge_bonus = float(self.p["ATTACK_EDGE_BONUS"]) * attack
            buy_qe -= edge_bonus
            sell_qe -= edge_bonus
        ...
        if mode == "calm_mm":
            ...
            join_edge += 0.25
        if attack > 0.0:
            join_edge += float(self.p["ATTACK_JOIN_BONUS"]) * attack
        ...
        if attack > 0.0:
            size_bonus = 1.0 + float(self.p["ATTACK_SIZE_BONUS"]) * attack
            buy_mult *= size_bonus
