...
class IntarianPepperRootBot:
    TIME_FAIR_SLOPE = 0.001
    SPREAD_ALPHA = 0.08
    DEVIATION_ALPHA = 0.10
    BASE_LONG_BIAS = 10
    DEVIATION_TARGET_SCALE = 10.0
    UNDERFAIR_BONUS = 8.0
    OVERFAIR_PENALTY = 10.0
    MAX_LONG_TARGET = 56
    MAX_SHORT_TARGET = 20
    INVENTORY_SKEW = 0.11
    LATE_INVENTORY_MULTIPLIER = 1.65
    BASE_TAKE_EDGE = 2.0
    BASE_QUOTE_EDGE = 4.0
    SOFT_LIMIT = 45
    PASSIVE_SIZE = 10
    MAX_TAKE_SIZE = 14
    BULLISH_IMBALANCE = 0.06
    BULLISH_BUY_JOIN = 1
    LATE_FLATTEN_START = 0.78
    TAKE_BUFFER = 10
    PASSIVE_BUFFER = 12
    ...
    def late_factor(self) -> float:
        progress = self.progress()
        if progress <= self.LATE_FLATTEN_START:
            return 0.0
        span = 1.0 - self.LATE_FLATTEN_START
        return clamp((progress - self.LATE_FLATTEN_START) / max(0.01, span), 0.0, 1.0)
    ...
    def load_state(self) -> Dict[str, float]:
        ...
        return {
            ...
            "deviation_ema": float(raw.get("deviation_ema", 0.0)),
    ...
    def save_state(self) -> None:
        ...
        self.memory["INTARIAN_PEPPER_ROOT"] = {
            ...
            "deviation_ema": float(self.product_state["deviation_ema"]),
    ...
    def update_state(self) -> None:
        ...
        if self.product_state["initialized"] <= 0.0:
            ...
            self.product_state["deviation_ema"] = 0.0
        ...
        deviation = self.book.mid - self.trend_line()
        self.product_state["deviation_ema"] = ema(
            float(self.product_state["deviation_ema"]),
            deviation,
            self.DEVIATION_ALPHA,
        )

    def trend_line(self) -> float:
        return float(self.product_state["anchor"]) + self.TIME_FAIR_SLOPE * float(self.timestamp())

    def deviation_metrics(self) -> Tuple[float, float]:
        deviation = self.book.mid - self.trend_line()
        spread_scale = max(4.0, float(self.product_state["spread_ema"]) * 0.45)
        zscore = deviation / spread_scale
        return deviation, zscore

    def fair_value(self) -> float:
        trend = self.trend_line()
        half_spread = max(1.0, self.book.spread / 2.0)
        flow_fair = self.book.mid + self.book.imbalance * half_spread
        return 0.78 * trend + 0.12 * flow_fair + 0.10 * self.book.micro
    ...
    def target_position(self, fair: float) -> int:
        _deviation, zscore = self.deviation_metrics()
        progress = self.progress()
        late_factor = self.late_factor()
        target = self.BASE_LONG_BIAS * (1.0 - progress)
        target += self.DEVIATION_TARGET_SCALE * (fair - self.book.mid)

        if zscore < 0.0:
            target += self.UNDERFAIR_BONUS * min(1.0, abs(zscore) / 1.8)
        if zscore > 0.35:
            target -= self.OVERFAIR_PENALTY * min(1.0, (zscore - 0.35) / 1.4)
        ...
        target *= 1.0 - 0.45 * late_factor
        return int(clamp(target, -self.MAX_SHORT_TARGET, self.MAX_LONG_TARGET))

    def reservation(self, fair: float, target: int) -> float:
        inventory_skew = self.INVENTORY_SKEW * (1.0 + self.LATE_INVENTORY_MULTIPLIER * self.late_factor())
        return fair - (self.manager.projected_position() - target) * inventory_skew

    def take_edges(self, target: int, zscore: float) -> Tuple[float, float]:
        buy_edge = self.BASE_TAKE_EDGE
        sell_edge = self.BASE_TAKE_EDGE
        late_factor = self.late_factor()
        position = self.manager.projected_position()
        ...
        if late_factor > 0.0:
            if position > max(0, target):
                sell_edge -= 0.40 * late_factor
                buy_edge += 0.15 * late_factor
            elif position < min(0, target):
                buy_edge -= 0.30 * late_factor
                sell_edge += 0.15 * late_factor

        if self.bullish_state(zscore) and position > 0:
            sell_edge += 0.25

        return max(0.75, buy_edge), max(0.75, sell_edge)
    ...
    def take_orders(self, reservation: float, target: int, zscore: float) -> None:
        ...
        if buy_edge >= buy_edge_needed and self.manager.buy_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, target + self.TAKE_BUFFER - self.manager.projected_position()),
            )
        ...
        if sell_edge >= sell_edge_needed and self.manager.sell_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, self.manager.projected_position() - (target - self.TAKE_BUFFER)),
            )
    ...
    def passive_quotes(self, reservation: float, target: int, zscore: float) -> Tuple[Optional[int], Optional[int]]:
        ...
        sell_edge = self.BASE_QUOTE_EDGE + 0.40
        late_factor = self.late_factor()
        position = self.manager.projected_position()
        ...
        if late_factor > 0.0:
            if position > max(0, target):
                sell_edge -= 0.60 * late_factor
                buy_edge += 0.35 * late_factor
            elif position < min(0, target):
                buy_edge -= 0.45 * late_factor
                sell_edge += 0.25 * late_factor
        ...
        if self.bullish_state(zscore) and position < target:
            buy_quote = max(buy_quote, self.book.best_bid + self.BULLISH_BUY_JOIN)
        if self.bullish_state(zscore) and position > 0:
            sell_quote += 1
    ...
    def passive_sizes(self, target: int) -> Tuple[int, int]:
        ...
        if late_factor > 0.0 and position > max(0, target):
            sell_size += int(round(2 * late_factor))
            buy_size = max(3, buy_size - int(round(2 * late_factor)))
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        position = self.manager.projected_position()
        if buy_quote is not None and self.manager.buy_capacity > 0 and position < target + self.PASSIVE_BUFFER:
            quantity = min(buy_size, self.manager.buy_capacity, max(0, target + self.PASSIVE_BUFFER - position))
            self.manager.add_buy(buy_quote, quantity)

        position = self.manager.projected_position()
        if sell_quote is not None and self.manager.sell_capacity > 0 and position > target - self.PASSIVE_BUFFER:
            quantity = min(sell_size, self.manager.sell_capacity, max(0, position - (target - self.PASSIVE_BUFFER)))
