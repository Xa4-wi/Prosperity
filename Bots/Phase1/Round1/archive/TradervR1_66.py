...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        quote_back_bid = True
        quote_back_ask = True
        if self.p.get("FRONT_CONCENTRATION", False):
            tight_normal = (
                mode == "normal"
                and not bid_toxic
                and not ask_toxic
                and book.spread_val <= 15
                and abs(pos) < soft - 4
            )
            if tight_normal or profile in {"attack", "press"}:
                buy_front_sz += buy_back_sz
                sell_front_sz += sell_back_sz
                buy_back_sz = 0
                sell_back_sz = 0
                quote_back_bid = False
                quote_back_ask = False
        ...
        if allow_bid and front_buy > 0 and front_buy < book.best_ask:
            ...
            if quote_back_bid and back_buy > 0 and back_buy < book.best_ask:
                mgr.buy(back_buy, buy_back_sz)
        if allow_ask and front_sell > book.best_bid:
            mgr.sell(front_sell, sell_front_sz)
            if quote_back_ask and back_sell > book.best_bid:
