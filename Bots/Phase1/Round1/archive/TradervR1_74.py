...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        wide_safe_harvest = (
            ...
            and book.spread_val >= 16.0
            and abs(book.imbalance) <= 0.18
            and (book.best_bid_vol + book.best_ask_vol) >= 24
            and abs(pos) <= soft - 8
        )
        if wide_safe_harvest:
            front_buy = max(front_buy, book.best_bid + 1)
            front_sell = min(front_sell, book.best_ask - 1)
        ...
        wing_buy = min(front_buy - 4, book.best_ask - 1)
        wing_sell = max(front_sell + 4, book.best_bid + 1)
        ...
        wing_buy_sz = max(1, int(round(back_sz * 0.45)))
        wing_sell_sz = max(1, int(round(back_sz * 0.45)))
        ...
        if allow_bid and front_buy > 0 and front_buy < book.best_ask:
            ...
            if wide_safe_harvest and wing_buy > 0 and wing_buy < book.best_ask and wing_buy < back_buy:
                mgr.buy(wing_buy, wing_buy_sz)
        if allow_ask and front_sell > book.best_bid:
            ...
            if wide_safe_harvest and wing_sell > book.best_bid and wing_sell > back_sell:
                mgr.sell(wing_sell, wing_sell_sz)
