...
DEFAULT_IPR_PARAMS = {
    ...
    # Hold-longer overlay
    "HOLD_LONG_UNTIL": 0.72,
    "HOLD_LONG_SELL_Z": 1.18,
    "HOLD_LONG_TARGET_BUFFER": 8,
    "HOLD_LONG_QUOTE_LIFT": 1.10,
    "LATE_TRIM_START": 0.90,
    "LATE_TRIM_Z": 1.15,
}
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if progress > float(self.p["LATE_TRIM_START"]) and zscore > float(self.p["LATE_TRIM_Z"]):
            target -= 8.0
        ...
        if bullish:
            buy_te -= 0.10
            sell_te += 0.90
        if progress < float(self.p["HOLD_LONG_UNTIL"]) and pos < target_int + int(self.p["HOLD_LONG_TARGET_BUFFER"]):
            sell_te += 0.50
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            ...
            if bullish and pos < max(26, target_int - 6):
                ...
                qty = min(qty, 4)
            if (
                progress < float(self.p["HOLD_LONG_UNTIL"])
                and pos < target_int + int(self.p["HOLD_LONG_TARGET_BUFFER"])
                and zscore < float(self.p["HOLD_LONG_SELL_Z"])
            ):
                qty = 0
        ...
        if zscore < -0.55:
            ...
            sell_qe -= 0.15
        if progress < float(self.p["HOLD_LONG_UNTIL"]) and pos < target_int + int(self.p["HOLD_LONG_TARGET_BUFFER"]):
            sell_qe += float(self.p["HOLD_LONG_QUOTE_LIFT"])
