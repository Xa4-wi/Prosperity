...
DEFAULT_ASH_PARAMS = {
    ...
    # Lightweight state filter + fill-quality memory
    "STATE_ALPHA": 0.18,
    "MARKOUT_ALPHA": 0.28,
    "FILL_PENALTY_SCALE": 0.80,
    "TOXIC_EDGE_WIDEN": 0.95,
    "CALM_EDGE_TIGHTEN": 0.20,
    "TOXIC_SIZE_MULT": 0.62,
    "CALM_SIZE_MULT": 1.08,
    "INVENTORY_CLEAR_EDGE": 0.9,
}
...
DEFAULT_IPR_PARAMS = {
    ...
    # Small innovation gate on aggressive buys only
    "BUY_INNOVATION_NEG": -0.60,
    "BUY_INNOVATION_POS": 0.45,
    "BUY_INNOVATION_BONUS": 0.12,
    "BUY_INNOVATION_PENALTY": 0.22,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _state_probabilities(
        ...
        prev_fair = float(ash_state.get("prev_fair", fair))
        prev_micro = float(ash_state.get("prev_micro", book.micro))
        spread_ema = ema(float(ash_state.get("spread_ema", book.spread_val)), book.spread_val, float(self.p["STATE_ALPHA"]))

        p_calm = float(ash_state.get("p_calm", 0.45))
        p_normal = float(ash_state.get("p_normal", 0.40))
        p_toxic = float(ash_state.get("p_toxic", 0.15))

        innovation = book.mid - prev_fair
        micro_move = book.micro - prev_micro
        spread_shock = max(0.0, book.spread_val - spread_ema)
        stress = (
            abs(innovation)
            + 0.6 * abs(micro_move)
            + 0.35 * spread_shock
            + 0.9 * max(0.0, -buy_markout)
            + 0.9 * max(0.0, -sell_markout)
        )

        pred_calm = 0.78 * p_calm + 0.18 * p_normal + 0.06 * p_toxic
        pred_normal = 0.20 * p_calm + 0.68 * p_normal + 0.34 * p_toxic
        pred_toxic = 0.02 * p_calm + 0.14 * p_normal + 0.60 * p_toxic

        like_calm = gaussian_likelihood(stress, 0.6, 0.9)
        like_normal = gaussian_likelihood(stress, 1.8, 1.4)
        like_toxic = gaussian_likelihood(stress, 4.5, 2.1)

        p_calm = pred_calm * like_calm
        p_normal = pred_normal * like_normal
        p_toxic = pred_toxic * like_toxic
        total = max(1e-9, p_calm + p_normal + p_toxic)
        return (p_calm / total, p_normal / total, p_toxic / total), spread_ema
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_markout, sell_markout = self._infer_fill_quality(book, position, ash_state)
        (p_calm, p_normal, p_toxic), spread_ema = self._state_probabilities(
            book, fair, ash_state, buy_markout, sell_markout
        )
        ...
        bid_toxic = (book.imbalance < -adverse and book.micro < book.mid) or (p_toxic > 0.58 and book.imbalance < -0.06)
        ask_toxic = (book.imbalance > adverse and book.micro > book.mid) or (p_toxic > 0.58 and book.imbalance > 0.06)
        buy_edge_needed = (1.35 - 0.10 * p_calm) + (0.85 * p_toxic if bid_toxic else 0.25 * p_toxic)
        sell_edge_needed = (1.35 - 0.10 * p_calm) + (0.85 * p_toxic if ask_toxic else 0.25 * p_toxic)
        ...
        calm_tighten = float(self.p["CALM_EDGE_TIGHTEN"]) * p_calm
        toxic_widen = float(self.p["TOXIC_EDGE_WIDEN"]) * p_toxic
        buy_qe -= calm_tighten
        sell_qe -= calm_tighten
        buy_qe += toxic_widen
        sell_qe += toxic_widen
        if bid_toxic:
            buy_qe += 1.0
        ...
        bid_penalty = (
            float(self.p["FILL_PENALTY_SCALE"]) * max(0.0, -buy_markout)
            + 0.60 * p_toxic
            + (0.35 if bid_toxic else 0.0)
        )
        ask_penalty = (
            float(self.p["FILL_PENALTY_SCALE"]) * max(0.0, -sell_markout)
            + 0.60 * p_toxic
            + (0.35 if ask_toxic else 0.0)
        )
        ...
        allow_bid = (
            ...
            and pos < soft + 6
            and (buy_qe - bid_penalty) > 0.55
            and not (p_toxic > 0.72 and book.imbalance < 0.0)
        )
        allow_ask = (
            not (ask_toxic and pos < -8)
            and pos > -(soft + 6)
            and (sell_qe - ask_penalty) > 0.55
            and not (p_toxic > 0.72 and book.imbalance > 0.0)
        )

        inventory_stretch = abs(pos) / float(PRODUCT_LIMITS["ASH_COATED_OSMIUM"])
        size_mult = (
            (1.0 + (float(self.p["CALM_SIZE_MULT"]) - 1.0) * p_calm)
            * (1.0 - (1.0 - float(self.p["TOXIC_SIZE_MULT"])) * p_toxic)
            * (1.0 - 0.30 * inventory_stretch)
        )
        front_sz = int(clamp(round(float(self.p["FRONT_SIZE"]) * size_mult), 6, 22))
        back_sz = int(clamp(round(float(self.p["BACK_SIZE"]) * (0.95 + 0.15 * p_calm - 0.35 * p_toxic)), 2, 8))
        ...
        pos = mgr.projected()
        neutral_book = (
            abs(book.imbalance) < strong
            and abs(book.micro - book.mid) < 0.9
            and p_toxic < 0.42
        )
        near_zero_edge = abs(reservation - book.mid) <= float(self.p["INVENTORY_CLEAR_EDGE"])
        if neutral_book and near_zero_edge:
            if pos > soft - 4:
                clear_px = max(book.best_bid + 1, int(round(book.mid)))
                if clear_px > book.best_bid:
                    mgr.sell(clear_px, min(4, max(0, pos - (soft - 10))))
            elif pos < -(soft - 4):
                clear_px = min(book.best_ask - 1, int(round(book.mid)))
                if clear_px < book.best_ask:
                    mgr.buy(clear_px, min(4, max(0, (-(soft - 10)) - pos)))

        memory["ASH_STATE"] = {
            "prev_mid": book.mid,
            "prev_micro": book.micro,
            "prev_fair": fair,
            "spread_ema": spread_ema,
            "last_position": position,
            "buy_markout_ema": buy_markout,
            "sell_markout_ema": sell_markout,
            "p_calm": p_calm,
            "p_normal": p_normal,
            "p_toxic": p_toxic,
        }
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        innovation = 0.0
        if prev_mid is not None and last_ts >= 0 and timestamp > last_ts:
            expected_move = drift * (timestamp - last_ts)
            innovation = (book.mid - float(prev_mid)) - expected_move
        ...
        if pos < target_int:
            buy_te -= 0.35
        if innovation < float(self.p["BUY_INNOVATION_NEG"]):
            buy_te -= float(self.p["BUY_INNOVATION_BONUS"])
        elif innovation > float(self.p["BUY_INNOVATION_POS"]):
            buy_te += float(self.p["BUY_INNOVATION_PENALTY"])
        if pos > target_int:
            sell_te -= 0.05
        if zscore < -0.45:
            buy_te -= 0.30
