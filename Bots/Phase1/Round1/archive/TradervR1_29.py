...
# Archetype: anchored trunk with one stretch overlay and one toxic retreat overlay
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    ...
    "OU_KAPPA": 0.16,
    "STRETCH_LEVEL": 4.0,
    "STRETCH_TAKE_BONUS": 0.35,
    "STRETCH_QUOTE_BONUS": 0.45,
    "TOXIC_RETREAT": 0.65,
...
class AshCoatedOsmiumTrader:
    """
    Anchored MM with two light overlays:
    - a small OU-style stretch lean when price is displaced from the anchor
    - a small toxic retreat when imbalance/micro say passive fills are dangerous
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        stretch = abs(displacement) >= float(self.p["STRETCH_LEVEL"])
        if stretch and displacement < 0.0:
            buy_edge_needed = max(1.0, buy_edge_needed - float(self.p["STRETCH_TAKE_BONUS"]))
        elif stretch and displacement > 0.0:
            sell_edge_needed = max(1.0, sell_edge_needed - float(self.p["STRETCH_TAKE_BONUS"]))
        ...
        for edge_thr, clip in [
            ...
            if buy_edge >= max(float(edge_thr), buy_edge_needed):
                take_buy = int(clip)
        if stretch and displacement < 0.0 and book.imbalance > -strong:
            take_buy += 2
        if take_buy > 0:
            pos = mgr.projected()
        ...
        if ask_toxic:
            sell_qe += 1.0
        if stretch and displacement < 0.0:
            buy_qe -= float(self.p["STRETCH_QUOTE_BONUS"])
            sell_qe += 0.20
        elif stretch and displacement > 0.0:
            buy_qe += 0.20
            sell_qe -= float(self.p["STRETCH_QUOTE_BONUS"])
        retreat = float(self.p["TOXIC_RETREAT"])
        if bid_toxic:
            buy_qe += retreat
        if ask_toxic:
            sell_qe += retreat
        ...
        if stretch and displacement < 0.0 and book.imbalance < -strong:
            allow_ask = False
        elif stretch and displacement > 0.0 and book.imbalance > strong:
            allow_bid = False
