...
DEFAULT_ASH_PARAMS = {
    ...
    # Depth-aware fair value
    "DEPTH_IMPACT_SCALE": 95.0,
    "DEPTH_FLOOR": 8.0,
    "IMBALANCE_BIAS": 0.30,
    "MICRO_WEIGHT": 0.65,
    ...
    "FRONT_SIZE": 12,
    "BACK_SIZE": 6,
    # Aggressive taking / dislocations
    "TAKE_EDGE": 1.4,
    "DISLOCATION_EDGE": 3.8,
    "DISLOCATION_LEVELS": 2,
    "DISLOCATION_CLIP": 12,
    # State / risk controls
    "TOXIC_IMBALANCE": 0.20,
    "STRONG_IMBALANCE": 0.12,
    "SOFT_LIMIT": 52,
    "HARD_LIMIT_BUFFER": 6,
    # Fill-quality penalty model
    "PENALTY_TOXIC": 1.15,
    "SPREAD_ALPHA": 0.10,
    "VOL_ALPHA": 0.10,
    "PENALTY_ALPHA": 0.18,
...
class AshCoatedOsmiumTrader:
    """
    Osmium base engine: anchored market maker with
    - depth-aware imbalance fair value
    - nonlinear inventory pressure
    - toxicity-aware one-sided behavior
    - simple learned fill-penalty estimates from realized markout after fills
    - controlled dislocation sweeping when quotes are clearly offside
    """
    ...
    def _update_state_from_realized_fills(self, book: Book, position: int, st: dict) -> dict:
        ...
        if delta_pos > 0:
            # After net buy fills, a downward markout is adverse.
            realized = max(0.0, -move)
            st["buy_penalty_ema"] = ema(float(st["buy_penalty_ema"]), realized, penalty_alpha)
        else:
            st["buy_penalty_ema"] = ema(float(st["buy_penalty_ema"]), 0.0, 0.06)
        if delta_pos < 0:
            # After net sell fills, an upward markout is adverse.
            realized = max(0.0, move)
            st["sell_penalty_ema"] = ema(float(st["sell_penalty_ema"]), realized, penalty_alpha)
        else:
            st["sell_penalty_ema"] = ema(float(st["sell_penalty_ema"]), 0.0, 0.06)
        return st

    def _fair_value(self, book: Book, st: dict) -> float:
        ...
        return (
            ...
            + beta * book.imbalance
            + float(self.p["IMBALANCE_BIAS"]) * book.imbalance
    ...
    def _toxicity(self, book: Book, st: dict) -> tuple[bool, bool, float, float]:
        tox = float(self.p["TOXIC_IMBALANCE"])
        bid_toxic = book.imbalance < -tox and book.micro < book.mid
        ask_toxic = book.imbalance > tox and book.micro > book.mid
        base_bid_penalty = (
            float(st["buy_penalty_ema"])
            + float(self.p["PENALTY_TOXIC"]) * max(0.0, -book.imbalance)
            + max(0.0, book.mid - book.micro)
            + 0.15 * float(st["vol_ema"])
        )
        base_ask_penalty = (
        ...
        return bid_toxic, ask_toxic, base_bid_penalty, base_ask_penalty

    def _quote_half_spreads(self, book: Book, st: dict, projected_pos: int, bid_toxic: bool, ask_toxic: bool) -> tuple[float, float]:
        inv_ratio = projected_pos / float(PRODUCT_LIMITS["ASH_COATED_OSMIUM"])
        base_half = (
        ...
        # Inventory pressure should be nonlinear, especially near the limit.
        if inv_ratio > 0.0:
            buy_half += 2.2 * inv_ratio + 4.5 * (inv_ratio ** 3)
            sell_half -= 1.2 * inv_ratio
        elif inv_ratio < 0.0:
            sell_half += 2.2 * (-inv_ratio) + 4.5 * ((-inv_ratio) ** 3)
            buy_half -= 1.2 * (-inv_ratio)
    ...
    def _dislocation_take(self, mgr: Manager, book: Book, reservation: float, bid_toxic: bool, ask_toxic: bool) -> None:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        fair = self._fair_value(book, st)
        reservation = self._reservation(fair, mgr.projected())
        bid_toxic, ask_toxic, bid_penalty, ask_penalty = self._toxicity(book, st)

        # Aggressive taking is only for real dislocations, not every tiny edge.
        self._dislocation_take(mgr, book, reservation, bid_toxic, ask_toxic)
        reservation = self._reservation(fair, mgr.projected())

        buy_half, sell_half = self._quote_half_spreads(book, st, mgr.projected(), bid_toxic, ask_toxic)
        ...
        # Join only when the edge is good and the side is not too toxic.
        ...
        allow_bid = net_bid_edge >= min_net and not (bid_toxic and projected > 8)
        allow_ask = net_ask_edge >= min_net and not (ask_toxic and projected < -8)
        if projected >= soft + hard_buf:
            allow_bid = False
        if projected <= -(soft + hard_buf):
            allow_ask = False

        # Inventory recovery should still be allowed even with weak net edge.
        if projected < -soft and not ask_toxic:
            allow_bid = True
        if projected > soft and not bid_toxic:
            allow_ask = True
        ...
        if allow_bid and front_buy > 0 and front_buy < book.best_ask:
            ...
            if back_buy > 0 and back_buy < book.best_ask and net_bid_edge >= min_net + 0.45:
        ...
        if allow_ask and front_sell > book.best_bid:
            ...
            if back_sell > book.best_bid and net_ask_edge >= min_net + 0.45:
