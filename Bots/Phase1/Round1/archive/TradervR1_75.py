...
class AshCoatedOsmiumTrader:
    ...
    def _smoothed_toxicity_levels(
        ...
        if raw_bid_level == 2:
            bid_score = min(4.0, bid_score * 0.70 + 1.75)
        elif raw_bid_level == 1:
            bid_score = min(4.0, bid_score * 0.75 + 0.95)
        else:
            bid_score = max(0.0, bid_score * 0.50 - 0.20)

        if raw_ask_level == 2:
            ask_score = min(4.0, ask_score * 0.70 + 1.75)
        elif raw_ask_level == 1:
            ask_score = min(4.0, ask_score * 0.75 + 0.95)
        else:
            ask_score = max(0.0, ask_score * 0.50 - 0.20)

        bid_level = 0
        if raw_bid_level >= 1 or bid_score >= 0.85:
            bid_level = 1
        if (
            (raw_bid_level == 2 and pos > max(10, soft - 8))
            or bid_score >= 2.55
        ):
            bid_level = 2

        ask_level = 0
        if raw_ask_level >= 1 or ask_score >= 0.85:
            ask_level = 1
        if (
            (raw_ask_level == 2 and pos < -max(10, soft - 8))
            or ask_score >= 2.55
        ):
            ask_level = 2
    ...
    def _update_reload_scores(
        ...
        last_seen_ts = int(ash_state.get("last_reload_fill_ts", -1))
        if last_seen_ts >= 0 and timestamp < last_seen_ts:
            ash_state = {}
            last_seen_ts = -1

        buy_reload = float(ash_state.get("buy_reload_score", 0.0))
        sell_reload = float(ash_state.get("sell_reload_score", 0.0))
        buy_reload = max(0.0, buy_reload * 0.70 - 0.05)
        sell_reload = max(0.0, sell_reload * 0.70 - 0.05)
        ...
        for trade in state.own_trades.get("ASH_COATED_OSMIUM", []):
            ...
            step = min(1.35, 0.55 + 0.05 * qty)
            if getattr(trade, "buyer", None) == "SUBMISSION":
                buy_reload = min(2.5, buy_reload + step)
            if getattr(trade, "seller", None) == "SUBMISSION":
                sell_reload = min(2.5, sell_reload + step)

        ash_state["buy_reload_score"] = round(buy_reload, 4)
        ash_state["sell_reload_score"] = round(sell_reload, 4)
        ash_state["last_reload_fill_ts"] = int(latest_ts)
        memory["ASH_STATE"] = ash_state
        return buy_reload, sell_reload, memory
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_reload_score, sell_reload_score, memory = self._update_reload_scores(state, memory, timestamp)
        attack = self._attack_factor(book, pos, soft, bid_level, ask_level, mode)
        profile, take_bonus, edge_bonus, size_bonus, join_bonus = self._execution_profile(mode, attack)
        safe_reload = (
            mode == "normal"
            and max(bid_level, ask_level) <= 1
            and book.spread_val <= float(self.p["WIDE_SPREAD"])
            and abs(book.imbalance) <= 0.20
        )
        buy_reload = clamp(buy_reload_score, 0.0, 1.6) if safe_reload and bid_level == 0 and pos < soft - 4 else 0.0
        sell_reload = clamp(sell_reload_score, 0.0, 1.6) if safe_reload and ask_level == 0 and pos > -(soft - 4) else 0.0
        ...
        if buy_reload > 0.0:
            buy_need = max(0.70, buy_need - (0.10 + 0.18 * buy_reload))
        if sell_reload > 0.0:
            sell_need = max(0.70, sell_need - (0.10 + 0.18 * sell_reload))
        ...
        if buy_reload > 0.0:
            buy_qe -= 0.12 + 0.22 * buy_reload
        if sell_reload > 0.0:
            sell_qe -= 0.12 + 0.22 * sell_reload
        ...
        buy_join_edge = join_edge + (0.08 + 0.10 * buy_reload if buy_reload > 0.0 else 0.0)
        sell_join_edge = join_edge + (0.08 + 0.10 * sell_reload if sell_reload > 0.0 else 0.0)
        ...
        if self.p.get("ALLOW_JOIN", True):
            for price, _ in book.buy_levels[:2]:
                ...
                if mode == "inventory_clear" or bid_level >= 2:
                    break
                front_buy = price if quote_mid - price <= buy_join_edge else price + 1
                break
            for price, _ in book.sell_levels[:2]:
                ...
                if mode == "inventory_clear" or ask_level >= 2:
                    break
                front_sell = price if price - quote_mid <= sell_join_edge else price - 1
                break
        ...
        if pos > 6 and bid_level >= 1:
            ...
            buy_front_sz += 1
        if buy_reload > 0.0:
            buy_front_sz += max(1, int(round(buy_reload)))
        if sell_reload > 0.0:
            sell_front_sz += max(1, int(round(sell_reload)))
