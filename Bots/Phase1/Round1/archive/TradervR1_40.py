...
DEFAULT_ASH_PARAMS = {
    ...
    "TAKE_L3_EDGE": 8.0,
    "TAKE_L3_SIZE": 16,
    # Volume-aware nudge layer: sweep only tiny stale slices that unlock
    # a clearly better local book.
    "NUDGE_MAX_QTY": 8,
    "NUDGE_MAX_LEVELS": 3,
    "NUDGE_MIN_DIRECT_EDGE": 0.8,
    "NUDGE_MIN_GAP": 4.0,
    "NUDGE_GAP_BONUS": 0.45,
    "NUDGE_QTY_PENALTY": 0.08,
    "NUDGE_INV_PENALTY": 0.60,
    "NUDGE_STRETCH_LIMIT": 54.0,
    # Gentle capacity recycling when stretched and book edge is near zero.
    "CLEAR_STRETCH": 50.0,
    "CLEAR_CLIP": 4,
    "CLEAR_NEAR_ZERO": 0.75,
    "CLEAR_NEUTRAL_IMB": 0.08,
...
class AshCoatedOsmiumTrader:
    ...
    v40 adds a narrow "volume nudge" proxy for auction-style thinking:
    if a tiny stale slice of the book can be removed cheaply enough to reveal a
    meaningfully better next level, we allow a small controlled sweep. This is
    paired with a very gentle capacity-recycling cross when inventory is
    stretched and the current edge is basically flat.
    """
    ...
    def _find_nudge_sweep(
        ...
        max_qty = int(self.p["NUDGE_MAX_QTY"])
        max_levels = int(self.p["NUDGE_MAX_LEVELS"])
        min_direct_edge = float(self.p["NUDGE_MIN_DIRECT_EDGE"])
        min_gap = float(self.p["NUDGE_MIN_GAP"])
        gap_bonus = float(self.p["NUDGE_GAP_BONUS"])
        qty_penalty = float(self.p["NUDGE_QTY_PENALTY"])
        inv_penalty = float(self.p["NUDGE_INV_PENALTY"])
        stretch_limit = max(1.0, float(self.p["NUDGE_STRETCH_LIMIT"]))

        best_orders: List[Tuple[int, int]] = []
        best_score = -1e18
        best_qty = 10**9
        cumulative_qty = 0
        cumulative_notional = 0.0
        current_orders: List[Tuple[int, int]] = []

        for idx, (price, volume) in enumerate(levels[:max_levels]):
            remaining = max_qty - cumulative_qty
            if remaining <= 0:
                break
            ...
            if clip <= 0:
                break
            cumulative_qty += clip
            cumulative_notional += float(price) * clip
            current_orders.append((int(price), int(clip)))

            if idx + 1 >= len(levels):
                continue

            next_price = int(levels[idx + 1][0])
            gap = float(next_price - price) if buy_side else float(price - next_price)
            if gap < min_gap:
                continue

            avg_price = cumulative_notional / cumulative_qty
            direct_edge = (reservation - avg_price) if buy_side else (avg_price - reservation)
            if direct_edge < min_direct_edge:
                continue

            stretched = max(0.0, (position if buy_side else -position) / stretch_limit)
            score = direct_edge + gap_bonus * gap - qty_penalty * cumulative_qty - inv_penalty * (stretched ** 2)
            if score > best_score or (abs(score - best_score) < 1e-9 and cumulative_qty < best_qty):
                best_score = score
                best_qty = cumulative_qty
                best_orders = list(current_orders)

        return best_orders

    def _inventory_clear_clip(
        self,
        book: Book,
        reservation: float,
        position: int,
        ...
        stretch = float(self.p["CLEAR_STRETCH"])
        edge_near_zero = float(self.p["CLEAR_NEAR_ZERO"])
        neutral_imb = float(self.p["CLEAR_NEUTRAL_IMB"])
        clip = int(self.p["CLEAR_CLIP"])

        buy_qty = 0
        sell_qty = 0
        if (
            position > stretch
            and abs(book.best_bid - reservation) <= edge_near_zero
            and abs(book.imbalance) <= neutral_imb
            and not ask_toxic
        ):
            sell_qty = min(clip, max(0, position - int(stretch - 2)), book.best_bid_vol)
        elif (
            position < -stretch
            and abs(reservation - book.best_ask) <= edge_near_zero
            and abs(book.imbalance) <= neutral_imb
            and not bid_toxic
        ):
            buy_qty = min(clip, max(0, int(-position - (stretch - 2))), book.best_ask_vol)
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        # Volume-aware nudge sweep: remove only small stale slices that reveal a
        # materially better next level. This is the continuous-book proxy for
        # the advisor's "small deliberate volume move" idea.
        nudge_buy_orders: List[Tuple[int, int]] = []
        nudge_sell_orders: List[Tuple[int, int]] = []
        pos = mgr.projected()
        if pos < float(self.p["NUDGE_STRETCH_LIMIT"]) and not (book.imbalance < -strong and book.micro < book.mid):
            nudge_buy_orders = self._find_nudge_sweep(book.sell_levels, reservation, pos, buy_side=True)
        if pos > -float(self.p["NUDGE_STRETCH_LIMIT"]) and not (book.imbalance > strong and book.micro > book.mid):
            nudge_sell_orders = self._find_nudge_sweep(book.buy_levels, reservation, pos, buy_side=False)

        if nudge_buy_orders:
            for price, qty in nudge_buy_orders:
                mgr.buy(price, qty)
        if nudge_sell_orders:
            for price, qty in nudge_sell_orders:
                mgr.sell(price, qty)
        ...
        for edge_thr, clip in [
            ...
            if buy_edge >= max(float(edge_thr), buy_edge_needed):
                take_buy = int(clip)
        if take_buy > 0 and not nudge_buy_orders:
            pos = mgr.projected()
        ...
        for edge_thr, clip in [
            ...
            if sell_edge >= max(float(edge_thr), sell_edge_needed):
                take_sell = int(clip)
        if take_sell > 0 and not nudge_sell_orders:
            pos = mgr.projected()
            if pos <= -self.p["SOFT_LIMIT"]:
                take_sell = max(0, take_sell - 4)
            mgr.sell(book.best_bid, min(book.best_bid_vol, take_sell))

        # Capacity-recycling nudge: when stretched and edge is near zero, free a
        # little inventory so later better opportunities have room.
        clear_buy, clear_sell = self._inventory_clear_clip(
            book, reservation, mgr.projected(), bid_toxic, ask_toxic
        )
        if clear_buy > 0:
            mgr.buy(book.best_ask, clear_buy)
        if clear_sell > 0:
            mgr.sell(book.best_bid, clear_sell)
