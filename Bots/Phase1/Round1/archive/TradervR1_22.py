...
DEFAULT_IPR_PARAMS = {
    ...
    # Small state filter imported from the research branch
    ...
    "P_STRONG_INIT": 0.23,
    "P_NOISY_INIT": 0.12,
    "STATE_STRONG_TARGET_BONUS": 4.0,
    "LATE_NOISY_UNWIND_START": 0.90,
    "LATE_NOISY_UNWIND_Z": 1.20,
}
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        p_steady = float(ps.get("p_steady", self.p["P_STEADY_INIT"]))
        p_strong = float(ps.get("p_strong", self.p["P_STRONG_INIT"]))
        p_noisy = float(ps.get("p_noisy", self.p["P_NOISY_INIT"]))
        ...
        if last_ts >= 0 and timestamp < last_ts:
            ...
            spread_ema_val = 13.0
            p_steady = float(self.p["P_STEADY_INIT"])
            p_strong = float(self.p["P_STRONG_INIT"])
            p_noisy = float(self.p["P_NOISY_INIT"])
            prev_mid = 0.0
        ...
        # ── Smooth state filter: steady_up / strong_up / noisy_rich ─────────
        ...
        predicted = [
            p_steady * transition[0][i] + p_strong * transition[1][i] + p_noisy * transition[2][i]
            for i in range(3)
        ]
        steady_like = self._gaussian_likelihood(
            observed_return,
            base_return + float(self.p["STATE_STEADY_MEAN_BONUS"]),
            float(self.p["STATE_STEADY_SIGMA"]),
        )
        strong_like = self._gaussian_likelihood(
            observed_return,
            base_return + float(self.p["STATE_STRONG_MEAN_BONUS"]),
            float(self.p["STATE_STRONG_SIGMA"]),
        )
        noisy_like = self._gaussian_likelihood(
            observed_return,
            max(0.0, base_return * float(self.p["STATE_NOISY_MEAN_SCALE"])),
            float(self.p["STATE_NOISY_SIGMA"]),
        )
        if zscore > 0.70:
            ...
            steady_like *= 1.05
            strong_like *= 1.08
        post = [predicted[0] * steady_like, predicted[1] * strong_like, predicted[2] * noisy_like]
        total_post = sum(post)
        if total_post > 1e-12:
            p_steady = post[0] / total_post
            p_strong = post[1] / total_post
            p_noisy = post[2] / total_post
        trend_prob = p_steady + p_strong
        ...
        if bullish:
            target += 8.0
        if trend_prob >= 0.74 and p_noisy <= 0.24:
            target += float(self.p["STATE_STRONG_TARGET_BONUS"])
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            ...
            if bullish and pos < max(26, target_int - 6):
                ...
                qty = min(qty, 4)
            if trend_prob >= 0.74 and p_noisy <= 0.24 and zscore < 1.15:
                qty = 0
            late_noisy_unwind = (
                progress >= float(self.p["LATE_NOISY_UNWIND_START"])
                and p_noisy >= 0.52
                and zscore >= float(self.p["LATE_NOISY_UNWIND_Z"])
            )
            if late_noisy_unwind and pos > 0:
                qty = max(qty, min(book.best_bid_vol, 4, max(0, pos - (target_int - 8))))
        ...
        if bullish and pos < max(24, target_int - 4):
            allow_sell = False
        if trend_prob >= 0.74 and p_noisy <= 0.24 and pos < max(28, target_int):
            allow_sell = False
        if (
            progress >= float(self.p["LATE_NOISY_UNWIND_START"])
            and p_noisy >= 0.52
        ...
        if front_buy > 0 and front_buy < book.best_ask:
            quotes.append(("buy", front_buy, front_sz))
            if back_buy > 0 and back_buy < book.best_ask:
                quotes.append(("buy", back_buy, back_sz))
        ...
        for side, price, size in quotes:
            pos = mgr.projected()
            if side == "buy" and pos < target_int + buy_buf:
                qty = min(size, mgr.buy_cap, max(0, target_int + buy_buf - pos))
                mgr.buy(price, qty)
