...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_buy:
            ...
            for edge_thr, clip in take_levels:
                if buy_edge >= max(edge_thr, buy_need):
                    take_buy = clip
            if safe_take_jump and abs(pos) <= 6 and take_signal >= 0.80 and buy_edge >= max(0.70, buy_need - 0.10):
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_sell:
            ...
            for edge_thr, clip in take_levels:
                if sell_edge >= max(edge_thr, sell_need):
                    take_sell = clip
            if safe_take_jump and abs(pos) <= 6 and take_signal <= -0.80 and sell_edge >= max(0.70, sell_need - 0.10):
        ...
        if safe_take_jump:
            pos = mgr.projected()
            if pos > 10 and sell_edge >= 0.45:
                mgr.sell(book.best_bid, min(book.best_bid_vol, 3, max(0, pos - 8)))
            elif pos < -10 and buy_edge >= 0.45:
                mgr.buy(book.best_ask, min(book.best_ask_vol, 3, max(0, 8 - pos)))
