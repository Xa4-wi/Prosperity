...
# Archetype: anchored MM with passive-fill penalty gating (DeLise-inspired)
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    ...
    "FILL_PENALTY_BASE": 0.55,
    "FILL_PENALTY_IMB": 1.85,
    "FILL_PENALTY_TOXIC": 1.05,
    "FILL_PENALTY_DEPTH": 12.0,
    "MIN_NET_EDGE": 0.10,
}
...
class AshCoatedOsmiumTrader:
    """
    Anchored MM with passive quote decisions based on net edge after fill penalty.
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        penalty_bid = penalty_base + float(self.p["FILL_PENALTY_IMB"]) * abs(min(0.0, book.imbalance))
        penalty_ask = penalty_base + float(self.p["FILL_PENALTY_IMB"]) * abs(max(0.0, book.imbalance))
        if bid_toxic:
            penalty_bid += float(self.p["FILL_PENALTY_TOXIC"])
        if ask_toxic:
            penalty_ask += float(self.p["FILL_PENALTY_TOXIC"])
        penalty_depth = float(self.p["FILL_PENALTY_DEPTH"]) / depth
        net_edge_bid = reservation - front_buy - penalty_bid - penalty_depth
        net_edge_ask = front_sell - reservation - penalty_ask - penalty_depth
