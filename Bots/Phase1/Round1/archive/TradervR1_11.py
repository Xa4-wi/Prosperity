...
class AshCoatedOsmiumBot:
    ...
    IMBALANCE_FAIR_WEIGHT = 2.25
    MICRO_FAIR_WEIGHT = 0.35
    INVENTORY_SKEW = 0.108
    BASE_EDGE = 4.45
    JOIN_EDGE = 2.0
    FRONT_ORDER_SIZE = 11
    BACK_ORDER_SIZE = 5
    ...
    CALM_SPREAD = 14
    TOXIC_SPREAD = 20
    DISLOCATION_SPREAD = 18
    DISLOCATION_EDGE = 3.8
    ...
    def bid_toxic(self) -> bool:
        return self.book.imbalance < -self.ADVERSE_IMBALANCE and self.book.micro < self.book.mid

    def ask_toxic(self) -> bool:
        return self.book.imbalance > self.ADVERSE_IMBALANCE and self.book.micro > self.book.mid

    def dislocation_side(self, reservation: float) -> Optional[str]:
        buy_edge = reservation - float(self.book.best_ask)
        sell_edge = float(self.book.best_bid) - reservation
        if buy_edge >= self.DISLOCATION_EDGE and buy_edge >= sell_edge + 0.5:
            return "buy"
        if sell_edge >= self.DISLOCATION_EDGE and sell_edge >= buy_edge + 0.5:
            return "sell"
        return None

    def regime(self, reservation: float) -> str:
        dislocation_side = self.dislocation_side(reservation)
        if dislocation_side is not None or (
            self.book.spread >= self.DISLOCATION_SPREAD
            and max(
                reservation - float(self.book.best_ask),
                float(self.book.best_bid) - reservation,
            )
            >= 2.2
        ):
            return "dislocation"
        if self.bid_toxic() or self.ask_toxic() or (
            self.book.spread >= self.TOXIC_SPREAD and abs(self.book.imbalance) >= self.STRONG_IMBALANCE
        ):
            return "toxic"
        return "calm"

    def quote_edges(self, reservation: float, regime: str) -> Tuple[float, float]:
        del reservation
        buy_edge = self.BASE_EDGE
        sell_edge = self.BASE_EDGE
        ...
        if self.book.imbalance > self.STRONG_IMBALANCE:
            buy_edge -= 0.4
            sell_edge += 0.2
        elif self.book.imbalance < -self.STRONG_IMBALANCE:
            buy_edge += 0.2
            sell_edge -= 0.4
        ...
        return max(1.8, buy_edge), max(1.8, sell_edge)

    def take_orders(self, reservation: float, regime: str) -> None:
        if not self.book.valid:
            return

        position = self.manager.projected_position()
        buy_edge_needed = 1.8 if self.bid_toxic() else 1.3
        sell_edge_needed = 1.8 if self.ask_toxic() else 1.3
        size_multiplier = 1.0
        if regime == "calm":
            buy_edge_needed -= 0.15
            sell_edge_needed -= 0.15
            size_multiplier = 1.05
        elif regime == "toxic":
            buy_edge_needed += 0.45
            sell_edge_needed += 0.45
        ...
        for distance, clip in self.TAKE_LEVELS:
            if buy_edge >= max(distance, buy_edge_needed):
                buy_size = int(round(clip * size_multiplier))
        if buy_size > 0:
            if regime == "toxic" and self.bid_toxic() and position >= -8 and buy_edge < self.DISLOCATION_EDGE + 0.6:
                buy_size = 0
            if position >= self.SOFT_LIMIT:
        ...
        sell_size = 0
        for distance, clip in self.TAKE_LEVELS:
            if sell_edge >= max(distance, sell_edge_needed):
                sell_size = int(round(clip * size_multiplier))
        if sell_size > 0:
            if regime == "toxic" and self.ask_toxic() and position <= 8 and sell_edge < self.DISLOCATION_EDGE + 0.6:
                sell_size = 0
            if position <= -self.SOFT_LIMIT:
                sell_size = max(0, sell_size - 4)
            self.manager.add_sell(self.book.best_bid, min(self.book.best_bid_volume, sell_size))

    def passive_quotes(self, reservation: float, regime: str) -> List[Tuple[str, int, int]]:
        if not self.book.valid:
            return []

        buy_edge, sell_edge = self.quote_edges(reservation, regime)
        front_buy = int(round(reservation - buy_edge))
        front_sell = int(round(reservation + sell_edge))

        for price, _volume in self.book.buy_levels[:2]:
            if reservation - price >= buy_edge:
                front_buy = price if reservation - price <= self.JOIN_EDGE else price + 1
                break
        for price, _volume in self.book.sell_levels[:2]:
            if price - reservation >= sell_edge:
                front_sell = price if price - reservation <= self.JOIN_EDGE else price - 1
                break
        ...
        front_buy = min(front_buy, self.book.best_ask - 1)
        front_sell = max(front_sell, self.book.best_bid + 1)
        back_buy = min(front_buy - 2, self.book.best_ask - 1)
        back_sell = max(front_sell + 2, self.book.best_bid + 1)

        quotes: List[Tuple[str, int, int]] = []
        front_size = self.FRONT_ORDER_SIZE
        back_size = self.BACK_ORDER_SIZE
        allow_bid = True
        allow_ask = True
        ...
        if regime == "calm":
            ...
            front_size = max(4, self.FRONT_ORDER_SIZE - 5)
            back_size = 0
            if self.bid_toxic():
                allow_bid = position < -16
                allow_ask = True
            elif self.ask_toxic():
                allow_bid = True
                allow_ask = position > 16
            elif position > 10:
                allow_bid = False
            elif position < -10:
                allow_ask = False
        else:
            front_size = max(5, self.FRONT_ORDER_SIZE - 3)
            back_size = 0
            side = self.dislocation_side(reservation)
            if side == "buy":
                allow_bid = False
                allow_ask = True
            elif side == "sell":
        ...
        if position >= self.SOFT_LIMIT + 6:
            allow_bid = False
        if position <= -(self.SOFT_LIMIT + 6):
            allow_ask = False

        if allow_bid and front_buy < self.book.best_ask and front_buy > 0:
            quotes.append(("buy", front_buy, front_size))
            if back_size > 0 and back_buy > 0 and back_buy < self.book.best_ask:
                quotes.append(("buy", back_buy, back_size))
        if allow_ask and front_sell > self.book.best_bid:
            quotes.append(("sell", front_sell, front_size))
            if back_size > 0 and back_sell > self.book.best_bid:
                quotes.append(("sell", back_sell, back_size))
        return quotes
    ...
    def run(self) -> List[Order]:
        ...
        regime = self.regime(reservation)
        self.take_orders(reservation, regime)
        for side, price, size in self.passive_quotes(reservation, regime):
            if side == "buy":
...
class IntarianPepperRootBot:
    DRIFT_PER_TIMESTAMP = 0.0010186903062235366
    RESIDUAL_ALPHA = 0.10458504418927272
    SPREAD_ALPHA = 0.08
    BASE_CARRY = 12.290586011615789
    EARLY_LONG_BIAS = 31.467841611072416
    EDGE_TARGET_SCALE = 11.92823771443657
    ZSCORE_BUY_BONUS = 12.0
    ZSCORE_SELL_PENALTY = 9.052128627717313
    MAX_LONG_TARGET = 68
    MAX_SHORT_TARGET = 20
    INVENTORY_SKEW = 0.08177154288279667
    BASE_TAKE_EDGE = 1.0871313705275572
    BASE_QUOTE_EDGE = 2.4597842344947978
    SOFT_LIMIT = 52
    PASSIVE_FRONT_SIZE = 9
    PASSIVE_BACK_SIZE = 6
    MAX_TAKE_SIZE = 16
    BULLISH_IMBALANCE = 0.03566984230907246
    OVEREXTENSION_Z = 1.1854741007803764
    EARLY_ACCUM_END = 0.42
    LOOKAHEAD_BONUS = 4.086795679611541
    PASSIVE_BUY_BUFFER = 18
    PASSIVE_SELL_BUFFER = 7.810082673618135
    ...
    def take_edges(self, target: int, zscore: float) -> Tuple[float, float]:
        ...
        sell_edge = self.BASE_TAKE_EDGE + 0.55
        position = self.manager.projected_position()
        bullish = self.bullish_state(zscore)
        ...
        if self.progress() < 0.55 and position < target:
            sell_edge += 0.25
        if position < max(20, target - 10):
            sell_edge += 0.55

        return max(0.35, buy_edge), max(1.05, sell_edge)
    ...
    def take_orders(self, reservation: float, target: int, zscore: float) -> None:
        ...
        if buy_edge >= buy_edge_needed and self.manager.buy_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, target + 16 - self.manager.projected_position()),
            )
        ...
        if sell_edge >= sell_edge_needed and self.manager.sell_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, position - (target - 4)),
            )
            if bullish and position < max(26, target - 6):
                quantity = 0
            elif bullish and position > 0:
                quantity = min(quantity, 4)
            self.manager.add_sell(self.book.best_bid, quantity)

    def passive_quotes(self, reservation: float, target: int, zscore: float) -> List[Tuple[str, int, int]]:
        buy_edge = self.BASE_QUOTE_EDGE
        sell_edge = self.BASE_QUOTE_EDGE + 0.75
        position = self.manager.projected_position()
        bullish = self.bullish_state(zscore)
        ...
        front_buy = math.floor(reservation - buy_edge)
        front_sell = math.ceil(reservation + sell_edge)

        if bullish and position < target:
            front_buy = max(front_buy, self.book.best_bid + 1)
        ...
        quotes: List[Tuple[str, int, int]] = []
        allow_sell = True
        if self.progress() < self.EARLY_ACCUM_END and position < target - 6:
            allow_sell = False
        if bullish and position < max(24, target - 4):
            allow_sell = False

        if front_buy > 0 and front_buy < self.book.best_ask:
            quotes.append(("buy", front_buy, self.PASSIVE_FRONT_SIZE))
            if back_buy > 0 and back_buy < self.book.best_ask:
                quotes.append(("buy", back_buy, self.PASSIVE_BACK_SIZE))

        if allow_sell and front_sell > self.book.best_bid:
            sell_front_size = max(3, self.PASSIVE_FRONT_SIZE - 3 if bullish else self.PASSIVE_FRONT_SIZE - 1)
            sell_back_size = max(2, self.PASSIVE_BACK_SIZE - 2 if bullish else self.PASSIVE_BACK_SIZE - 1)
            quotes.append(("sell", front_sell, sell_front_size))
            if back_sell > self.book.best_bid:
                quotes.append(("sell", back_sell, sell_back_size))
        return quotes
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        for side, price, size in self.passive_quotes(reservation, target, zscore):
            position = self.manager.projected_position()
            if side == "buy" and position < target + self.PASSIVE_BUY_BUFFER:
                quantity = min(size, self.manager.buy_capacity, max(0, target + self.PASSIVE_BUY_BUFFER - position))
                self.manager.add_buy(price, quantity)
            elif side == "sell" and position > target - self.PASSIVE_SELL_BUFFER:
                quantity = min(size, self.manager.sell_capacity, max(0, position - (target - self.PASSIVE_SELL_BUFFER)))
                self.manager.add_sell(price, quantity)
