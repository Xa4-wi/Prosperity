...
DEFAULT_ASH_PARAMS = {
    ...
    "REFILL_FAIR_BLEND": 0.65,
    "REFILL_TAKE_PENALTY": 0.20,
    "REFILL_ENTRY_EDGE_PENALTY": 0.22,
    "REFILL_EXIT_EDGE_BONUS": 0.18,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        refill_side = prev_vacuum_side if prev_vacuum_ticks > 0 else None
        ...
        if not book.valid:
            ...
            vacuum_ticks = prev_vacuum_ticks + 1 if vacuum_side == prev_vacuum_side else 1
            ash_state["vacuum_side"] = vacuum_side
            ash_state["vacuum_ticks"] = vacuum_ticks
            ...
            # Fresh vacuums are treated defensively: only quote the missing side to help
            # flatten exposed inventory. Persistent vacuums may be probed lightly.
        ...
        if refill_side in ("bid_only", "ask_only"):
            blend = float(self.p["REFILL_FAIR_BLEND"])
            slow_fair = (1.0 - blend) * slow_fair + blend * prev_vacuum_fair
            if refill_side == "ask_only":
        ...
        if refill_side in ("bid_only", "ask_only"):
            refill_take_penalty = float(self.p["REFILL_TAKE_PENALTY"])
            buy_need += refill_take_penalty
            sell_need += refill_take_penalty
        ...
        if refill_side == "bid_only":
            buy_qe -= float(self.p["REFILL_EXIT_EDGE_BONUS"])
            sell_qe += float(self.p["REFILL_ENTRY_EDGE_PENALTY"])
            if pos < 0:
                buy_qe -= 0.10
        elif refill_side == "ask_only":
            buy_qe += float(self.p["REFILL_ENTRY_EDGE_PENALTY"])
            sell_qe -= float(self.p["REFILL_EXIT_EDGE_BONUS"])
            if pos > 0:
                sell_qe -= 0.10
