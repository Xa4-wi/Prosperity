...
# Archetype: anchored_mm trunk with a mild nonlinear inventory-pressure overlay
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    ...
    "INV_PRESSURE_A1": 0.60,
    "INV_PRESSURE_A3": 2.60,
...
class AshCoatedOsmiumTrader:
    """
    Anchored MM around 10000 with a narrow GLFT-style inventory overlay.
    - Keep the trunk fair / take / join logic
    - Add a mild nonlinear inventory pressure to reservation and quote asymmetry
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        inv_pressure = (
            float(self.p["INV_PRESSURE_A1"]) * inv_ratio
            + float(self.p["INV_PRESSURE_A3"]) * (inv_ratio ** 3)
        )
        reservation = fair - mgr.projected() * self.p["INVENTORY_SKEW"] - inv_pressure
        ...
        if pos >= soft:
            ...
            buy_qe += 0.35 * inv_ratio
            sell_qe -= 0.15 * inv_ratio
        elif pos < 0:
            buy_qe += 0.15 * inv_ratio
            sell_qe -= 0.35 * inv_ratio
