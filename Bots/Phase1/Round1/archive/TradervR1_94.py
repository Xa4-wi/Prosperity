...
DEFAULT_ASH_PARAMS = {
    ...
    "AGREE_SIGNAL_BONUS": 0.85,
    "MAGNET_SIGNAL_BONUS": 0.30,
    "DISAGREE_STABLE_DAMP": 0.15,
...
class AshCoatedOsmiumTrader:
    ...
    def _signal_components(
        ...
        trade_bonus = float(self.p["TRADE_CONFIRM_BONUS"])
        if trade_bonus > 0.0 and trade_confirm != 0.0 and sign(trade_confirm, 0.05) == imbalance_sign:
            fast_signal += trade_bonus * trade_confirm
            conviction += 0.15
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        attack = clamp(
            self._attack_factor(book, pos, soft, bid_level, ask_level, mode) + 0.45 * signal_conviction,
            0.0,
            1.0,
        )
        ...
        if signal_conviction > 0.0:
            if take_signal > 0.0:
                buy_need = max(0.70, buy_need - 0.25 * signal_conviction)
            elif take_signal < 0.0:
                sell_need = max(0.70, sell_need - 0.25 * signal_conviction)
        ...
        if signal_conviction > 0.0:
            if quote_signal > 0.0:
                buy_qe -= 0.20 * signal_conviction
            elif quote_signal < 0.0:
                sell_qe -= 0.20 * signal_conviction
        ...
        if signal_conviction > 0.0:
            if quote_signal > 0.0:
                buy_front_sz += int(round(signal_conviction))
            elif quote_signal < 0.0:
                sell_front_sz += int(round(signal_conviction))
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
