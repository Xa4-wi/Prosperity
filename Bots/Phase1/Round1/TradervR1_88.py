...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if not book.valid:
            ...
            # In one-sided books, trust the last healthy fair more than the visible quote.
            # Only quote the missing side, and only when it helps flatten existing inventory.
        ...
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
            slow_fair = 0.60 * slow_fair + 0.40 * frozen_fair
