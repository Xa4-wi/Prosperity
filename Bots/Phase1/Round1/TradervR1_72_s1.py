...
class AshCoatedOsmiumTrader:
    ...
    def _smoothed_toxicity_levels(
        ...
        if raw_bid_level == 2:
            bid_score = min(4.0, bid_score * 0.82 + 1.60)
        elif raw_bid_level == 1:
            bid_score = min(4.0, bid_score * 0.85 + 0.90)
        else:
            if prev_bid_level == 2:
                bid_score = max(0.0, bid_score * 0.78 - 0.02)
            elif prev_bid_level == 1:
                bid_score = max(0.0, bid_score * 0.66 - 0.08)
            else:
                bid_score = max(0.0, bid_score * 0.48 - 0.20)

        if raw_ask_level == 2:
            ask_score = min(4.0, ask_score * 0.82 + 1.60)
        elif raw_ask_level == 1:
            ask_score = min(4.0, ask_score * 0.85 + 0.90)
        else:
            if prev_ask_level == 2:
                ask_score = max(0.0, ask_score * 0.78 - 0.02)
            elif prev_ask_level == 1:
                ask_score = max(0.0, ask_score * 0.66 - 0.08)
            else:
        ...
        if raw_bid_level >= 1 or bid_score >= 0.85 or (prev_bid_level >= 1 and bid_score >= 0.45):
            bid_level = 1
        if (
            (raw_bid_level == 2 and pos > max(10, soft - 8) and bid_score >= 1.60)
            or bid_score >= 2.55
            or (prev_bid_level == 2 and bid_score >= 1.65)
        ):
            bid_level = 2
        ...
        if raw_ask_level >= 1 or ask_score >= 0.85 or (prev_ask_level >= 1 and ask_score >= 0.45):
            ask_level = 1
        if (
            (raw_ask_level == 2 and pos < -max(10, soft - 8) and ask_score >= 1.60)
            or ask_score >= 2.55
            or (prev_ask_level == 2 and ask_score >= 1.65)
        ):
            ask_level = 2
