...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "HAWKES_ALPHA_WEIGHT": 0.10,
    "HAWKES_FAIR_WEIGHT": 0.08,
    "HAWKES_EDGE_WEIGHT": 0.22,
    "HAWKES_TAKE_SHIFT": 0.14,
    "HAWKES_QUOTE_SHIFT": 0.16,
    "HAWKES_SIZE_BONUS": 1.0,
    "HAWKES_SIGNAL_ALPHA": 0.32,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.hawkes_bias_ema = float(self.memory.get("hawkes_bias_ema", 0.0))
        self.hawkes_activity_ema = float(self.memory.get("hawkes_activity_ema", 0.0))
    ...
    def trade_pressure(self) -> Tuple[float, float]:
        ...
        for trade in self.state.market_trades.get(self.product, []):
            ...
            if quantity <= 0:
                continue
            if price >= float(self.mid):
                buy_pressure += quantity
                buy_pressure += 0.4 * max(0.0, (price - float(self.mid)) / half_spread)
            else:
                sell_pressure += quantity
                sell_pressure += 0.4 * max(0.0, (float(self.mid) - price) / half_spread)
        return buy_pressure, sell_pressure

    def hawkes_burst_signal(self) -> float:
        previous = self.memory.get("prev_book", {})
        ...
        if previous:
            prev_bid = float(previous.get("best_bid", self.best_bid))
            prev_ask = float(previous.get("best_ask", self.best_ask))
            prev_bid_vol = float(previous.get("best_bid_volume", self.best_bid_volume))
            prev_ask_vol = float(previous.get("best_ask_volume", self.best_ask_volume))
            scale = max(1.0, prev_bid_vol + prev_ask_vol)

            if float(self.best_bid) > prev_bid:
                buy_impulse += 1.10
            elif float(self.best_bid) < prev_bid:
                sell_impulse += 0.75
            else:
                bid_depletion = max(0.0, prev_bid_vol - float(self.best_bid_volume)) / scale
                bid_refill = max(0.0, float(self.best_bid_volume) - prev_bid_vol) / scale
                sell_impulse += 0.90 * bid_depletion
                buy_impulse += 0.35 * bid_refill

            if float(self.best_ask) < prev_ask:
                buy_impulse += 1.10
            elif float(self.best_ask) > prev_ask:
                sell_impulse += 0.75
            else:
                ask_depletion = max(0.0, prev_ask_vol - float(self.best_ask_volume)) / scale
                ask_refill = max(0.0, float(self.best_ask_volume) - prev_ask_vol) / scale
        ...
        self.hawkes_bias_ema = (1.0 - alpha) * self.hawkes_bias_ema + alpha * raw_bias
        self.hawkes_activity_ema = (1.0 - alpha) * self.hawkes_activity_ema + alpha * min(2.0, total)
        return self.hawkes_bias_ema * min(1.6, self.hawkes_activity_ema)
    ...
    def hybrid_alpha(self, hawkes_score: float) -> Tuple[float, float]:
    ...
    def next_memory(self) -> Dict[str, float]:
        return {
            ...
            "hawkes_bias_ema": float(self.hawkes_bias_ema),
            "hawkes_activity_ema": float(self.hawkes_activity_ema),
            "prev_book": {
    ...
    def adjusted_fair_value(
        ...
        fair = self.fair_value(regime, target_position, predicted_now, predicted_next, hybrid_alpha, hawkes_score)
    ...
    def take_edge(
        ...
        if hawkes_score != 0.0:
            ...
            if aligned:
                edge -= self.HAWKES_TAKE_SHIFT * min(1.0, abs(hawkes_score))
            else:
                edge += 0.08 * min(1.0, abs(hawkes_score))

        return max(0.5, edge)

    def quote_edge(self, regime: str, volatility: float, fit_quality: float, hawkes_score: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
        ...
        if hawkes_score != 0.0 and regime != "volatile":
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, hawkes_score))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, hawkes_score))
    ...
    def passive_size(self, side: str, regime: str, volatility: float, hawkes_score: float) -> int:
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, hawkes_score)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, hawkes_score)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, hawkes_score) + required_bonus:
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        hawkes_score = self.hawkes_burst_signal()
        _hybrid_fair, hybrid_alpha = self.hybrid_alpha(hawkes_score)
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, hawkes_score), self.buy_capacity)
                if regime != "range":
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, hawkes_score), self.sell_capacity)
                if regime != "range":
