...
class TomatoesTrader(BaseProductTrader):
    ...
    def online_change_state(self, flow_metrics: Dict[str, float], volatility: float) -> Tuple[float, float]:
        half_spread = max(1.0, float(self.spread) / 2.0)
        current_mean = float(self.mid)
        current_flow = float(flow_metrics["bias"])
        current_vol = max(1.0, volatility)
        fast = float(self.CHANGE_FAST_ALPHA)
        slow = float(self.CHANGE_SLOW_ALPHA)

        mean_fast = (1.0 - fast) * float(self.product_memory.get("change_mean_fast", current_mean)) + fast * current_mean
        mean_slow = (1.0 - slow) * float(self.product_memory.get("change_mean_slow", current_mean)) + slow * current_mean
        flow_fast = (1.0 - fast) * float(self.product_memory.get("change_flow_fast", current_flow)) + fast * current_flow
        flow_slow = (1.0 - slow) * float(self.product_memory.get("change_flow_slow", current_flow)) + slow * current_flow
        vol_fast = (1.0 - fast) * float(self.product_memory.get("change_vol_fast", current_vol)) + fast * current_vol
        vol_slow = (1.0 - slow) * float(self.product_memory.get("change_vol_slow", current_vol)) + slow * current_vol

        self.product_memory["change_mean_fast"] = mean_fast
        self.product_memory["change_mean_slow"] = mean_slow
        self.product_memory["change_flow_fast"] = flow_fast
        self.product_memory["change_flow_slow"] = flow_slow
        self.product_memory["change_vol_fast"] = vol_fast
        self.product_memory["change_vol_slow"] = vol_slow

        mean_gap = abs(mean_fast - mean_slow) / half_spread
        flow_gap = abs(flow_fast - flow_slow)
        vol_gap = abs(vol_fast - vol_slow) / max(1.0, current_vol)
        raw_score = 0.55 * mean_gap + 0.30 * flow_gap + 0.15 * vol_gap
        normalized = max(0.0, min(1.0, raw_score / 1.75))
        prev_prob = float(self.product_memory.get("change_prob", 0.0))
        change_prob = (1.0 - float(self.CHANGE_PROB_ALPHA)) * prev_prob + float(self.CHANGE_PROB_ALPHA) * normalized
        self.product_memory["change_prob"] = change_prob

        direction = ((mean_fast - mean_slow) / half_spread) + 0.65 * (flow_fast - flow_slow)
        return max(0.0, min(1.0, change_prob)), max(-1.5, min(1.5, direction))
    ...
    def classify_state(
        ...
        if (
            ...
            return "volatile"
        if change_score >= self.CHANGE_VOLATILE_THRESHOLD and abs(predicted_edge) < trend_threshold:
            return "volatile"
        if (
            breakout_score >= 1.0
        ...
        if (
            ...
            and fit_quality >= max(0.25, self.FIT_THRESHOLD - (0.08 * max(0.0, breakout_score)) - 0.08 * change_score)
        ...
        if (
            ...
            and fit_quality >= max(0.25, self.FIT_THRESHOLD + (0.08 * min(0.0, breakout_score)) - 0.08 * change_score)
            and self.imbalance <= -(self.TREND_IMBALANCE_THRESHOLD - 0.04 * change_score)
            and self.momentum <= -momentum_threshold
            and float(self.micro) <= float(self.mid)
            and change_direction <= 0.15
    ...
    def take_edge(
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            ...
            if aligned:
                ...
                edge += 0.06 * min(1.0, change_score)
    ...
    def quote_edge(
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            if regime == "volatile":
                ...
                edge += 0.22 * min(1.0, change_score)
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(regime, volatility, fit_quality, breakout_score, change_score, change_direction)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, change_score: float = 0.0, change_direction: float = 0.0) -> int:
    ...
    def run(self) -> List[Order]:
        ...
        change_score, change_direction = self.online_change_state(flow_metrics, volatility)
