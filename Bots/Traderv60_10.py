...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "LOB_FILTER_ALPHA": 0.28,
    "LOB_FILTER_ALPHA_WEIGHT": 0.08,
    "LOB_FILTER_FAIR_WEIGHT": 0.08,
    "LOB_FILTER_EDGE_WEIGHT": 0.16,
    "LOB_FILTER_TAKE_SHIFT": 0.10,
    "LOB_FILTER_QUOTE_SHIFT": 0.10,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def filtered_lob_direction(self, depth_alpha: float) -> float:
        half_spread = max(1.0, float(self.spread) / 2.0)
        micro_premium = (float(self.micro) - float(self.mid)) / half_spread
        raw = (
            0.35 * self.depth_imbalance_short
            + 0.25 * self.depth_imbalance_long
            + 0.15 * self.depth_concentration
            + 0.10 * self.depth_slope
            + 0.15 * micro_premium
        )
        raw += 0.20 * (depth_alpha / half_spread)
        raw = max(-1.5, min(1.5, raw))
        prev = float(self.product_memory.get("lob_filter_ema", 0.0))
        alpha = float(self.LOB_FILTER_ALPHA)
        signal = (1.0 - alpha) * prev + alpha * raw
        self.product_memory["lob_filter_ema"] = signal
        return signal
    ...
    def hybrid_alpha(self, lob_signal: float = 0.0) -> Tuple[float, float]:
        ...
        hybrid_fair += self.LOB_FILTER_ALPHA_WEIGHT * half_spread * lob_signal
    ...
    def fair_value(
        ...
        fair += self.LOB_FILTER_FAIR_WEIGHT * half_spread * lob_signal
    ...
    def take_edge(
        ...
        if lob_signal != 0.0:
            ...
            if aligned:
                edge -= self.LOB_FILTER_TAKE_SHIFT * min(1.0, abs(lob_signal))
            else:
    ...
    def quote_edge(
        ...
        if lob_signal != 0.0 and regime != "volatile":
            edge -= self.LOB_FILTER_QUOTE_SHIFT * min(1.0, abs(lob_signal))
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(regime, volatility, fit_quality, breakout_score, lob_signal)
    ...
    def run(self) -> List[Order]:
        ...
        lob_signal = self.filtered_lob_direction(depth_alpha)
        ...
        if pressure_bias * regression_edge < 0:
            pressure_bias *= 0.60
        _hybrid_fair, hybrid_alpha = self.hybrid_alpha(lob_signal)
        provisional_edge = (
            regression_edge
            + self.LOB_FILTER_EDGE_WEIGHT * max(1.0, float(self.spread) / 2.0) * lob_signal
