...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "QUOTE_ONE_SIDE_EDGE": 1.10,
    "QUOTE_NONE_EDGE": 0.45,
    "QUOTE_ONE_SIDE_IMBALANCE": 0.16,
    "QUOTE_NONE_VOL_THRESHOLD": 3.2,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def quote_action(
        ...
        if (
            ...
            and abs(self.projected_position() - target_position) <= 6
        ):
            return "none"
        ...
        directional = (
            ...
            and abs(self.imbalance) >= self.QUOTE_ONE_SIDE_IMBALANCE
        )
        if regime == "trend_up" and directional:
            return "buy_only" if self.projected_position() < target_position else "both"
        if regime == "trend_down" and directional:
            return "sell_only" if self.projected_position() > target_position else "both"
        return "both"
    ...
    def passive_quotes(
        ...
        action = self.quote_action(regime, target_position, predicted_edge, fit_quality, volatility)
        if action == "none":
            return None, None
