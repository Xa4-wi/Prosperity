...
class TomatoesBot:
    ...
    UNCERTAINTY_QUOTE_WEIGHT = 0.42
    UNCERTAINTY_TAKE_WEIGHT = 0.14
    UNCERTAINTY_SIZE_WEIGHT = 0.16
    ...
    def uncertainty_score(
        ...
        spread_component = min(1.0, float(self.book.spread) / max(1.0, float(self.TOXIC_SPREAD)))
        volatility_component = min(1.0, volatility / max(1.0, float(self.TOXIC_VOL)))
        fit_component = max(0.0, 1.0 - fit_quality)
        conflict_component = 0.0
        if regression_edge * guarded_alpha < 0:
            conflict_component += 0.55
        if predicted_edge * self.book.imbalance < 0:
            conflict_component += 0.25
        if guarded_alpha * self.momentum() < 0:
    ...
    def take_threshold(
        ...
        if regime in {"strong_up", "strong_down"} and fit_quality < 0.65:
            threshold += 0.10
        threshold += self.UNCERTAINTY_TAKE_WEIGHT * uncertainty
    ...
    def take_orders(
        ...
        buy_threshold = self.take_threshold("BUY", regime, target, predicted_edge, fit_quality, volatility, uncertainty)
        sell_threshold = self.take_threshold("SELL", regime, target, predicted_edge, fit_quality, volatility, uncertainty)
    ...
    def quote_edge(self, side: str, regime: str, target: int, volatility: float, fit_quality: float, uncertainty: float) -> float:
    ...
    def passive_size(
        ...
        if regime in {"strong_up", "strong_down"} and fit_quality < 0.55:
            size = max(2, size - 1)
        size = max(2, int(round(size * (1.0 - self.UNCERTAINTY_SIZE_WEIGHT * uncertainty))))
        return size
    ...
    def passive_quotes(
        ...
        buy_edge = self.quote_edge("BUY", regime, target, volatility, fit_quality, uncertainty)
        sell_edge = self.quote_edge("SELL", regime, target, volatility, fit_quality, uncertainty)
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        uncertainty = self.uncertainty_score(regression_edge, guarded_alpha, predicted_edge, fit_quality, volatility)
        ...
        self.take_orders(reservation, regime, target, predicted_edge, fit_quality, volatility, uncertainty)
        self.clear_inventory(self.reservation(fair, target), regime, target)
        reservation = self.reservation(fair, target)
        buy_quote, sell_quote = self.passive_quotes(
            reservation, regime, target, predicted_edge, fit_quality, volatility, uncertainty
        )
        ...
        if (
            ...
            size = min(
                self.passive_size("BUY", regime, target, volatility, fit_quality, uncertainty),
        ...
        if (
            ...
            size = min(
                self.passive_size("SELL", regime, target, volatility, fit_quality, uncertainty),
