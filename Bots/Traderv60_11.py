...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "UNCERTAINTY_QUOTE_WEIGHT": 0.32,
    "UNCERTAINTY_TAKE_WEIGHT": 0.10,
    "UNCERTAINTY_SIZE_WEIGHT": 0.12,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def uncertainty_score(
        ...
        spread_component = min(1.0, float(self.spread) / max(1.0, float(self.TOXIC_SPREAD_THRESHOLD)))
        volatility_component = min(1.0, volatility / max(1.0, float(self.TOXIC_VOLATILITY_THRESHOLD)))
        fit_component = max(0.0, 1.0 - fit_quality)
        conflict_component = 0.0
        if regression_edge * hybrid_alpha < 0:
            conflict_component += 0.40
        if predicted_edge * flow_bias < 0:
            conflict_component += 0.25
        if breakout_score != 0.0 and breakout_score * predicted_edge < 0:
            conflict_component += 0.20
        if hybrid_alpha * self.imbalance < 0:
            conflict_component += 0.15
        conflict_component = min(1.0, conflict_component)
        score = (
            0.30 * fit_component
            + 0.28 * conflict_component
            + 0.22 * volatility_component
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(regime, volatility, fit_quality, breakout_score, uncertainty)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, uncertainty: float = 0.0) -> int:
