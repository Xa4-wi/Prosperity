...
class Trader:
    ...
    def _trade_mm(self, product: str, state: TradingState, fair: float) -> List[Order]:
        ...
        late_factor = 0.0
        if product == HYDROGEL:
            late_factor = clamp((float(state.timestamp) - 86000.0) / 14000.0, 0.0, 1.0)
        elif product == VELVET:
            late_factor = clamp((float(state.timestamp) - 90000.0) / 9000.0, 0.0, 1.0)
        min_soft_limit = 24 if product == HYDROGEL else 60
        soft_limit = cfg["soft_limit"]
        if late_factor > 0.0:
            trim_ratio = 0.35 if product == HYDROGEL else 0.22
            soft_limit = max(min_soft_limit, int(round(cfg["soft_limit"] * (1.0 - trim_ratio * late_factor))))
        clear_edge = cfg["clear_edge"] + (0.55 if product == HYDROGEL else 0.25) * late_factor
        clear_max = int(round(cfg["clear_max"] * (1.0 + (0.25 if product == HYDROGEL else 0.10) * late_factor)))
        quote_size_scale = 1.0 - (0.18 if product == HYDROGEL else 0.10) * late_factor
        ...
        # Clear toward a smaller late-session soft limit.
        proj = mgr.projected()
        ...
        reservation = fair - cfg["skew"] * (1.0 + (0.45 if product == HYDROGEL else 0.20) * late_factor) * inv_ratio
