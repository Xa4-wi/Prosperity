...
def rolling_stats(values: List[float]) -> Tuple[Optional[float], Optional[float]]:
    if not values:
        return None, None
...
class Trader:
    def _book_context(self, product: str, od: OrderDepth, memory: dict) -> Optional[dict]:
        ...
        mid_hist = list(pm.get("mid_hist", []))
        mid_hist.append(float(mid))
        if len(mid_hist) > 20:
            mid_hist = mid_hist[-20:]
        pm["mid_hist"] = mid_hist
        ...
        bb_sma, bb_std = rolling_stats(mid_hist)
        if bb_sma is None:
            bb_sma, bb_std = float(mid), 0.0
        bb_upper = float(bb_sma) + 2.0 * float(bb_std)
        bb_lower = float(bb_sma) - 2.0 * float(bb_std)
        bb_width = max(1e-9, bb_upper - bb_lower)
        bb_percent_b = clamp((float(mid) - bb_lower) / bb_width, 0.0, 1.0)
        bb_width_rel = bb_width / max(1.0, float(bb_sma))
        ...
        return {
            ...
            "bb_width_rel": float(bb_width_rel),
        }
    ...
    def _velvet_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], Optional[float], Optional[dict]]:
        ...
        target = 0.0
        if z < -1.10 and micro_gap >= -0.20 * spread:
            target += 0.60 * VELVET_ENGINE_CAP
        elif z > 1.10 and micro_gap <= 0.20 * spread:
            target -= 0.60 * VELVET_ENGINE_CAP
        ...
        orders = self._execute_underlying(
            ...
            take_edge=max(1.3, 0.23 * spread + 0.18 * move),
            quote_edge=max(1.3, 0.32 * spread + 0.13 * move),
            quote_size=10,
            take_max=14,
        )
        return orders, fair, ctx
    ...
    def _voucher_orders(self, product: str, state: TradingState, surface: dict, velvet_diag: Optional[dict] = None) -> List[Order]:
        if product in DISABLED_VOUCHERS:
            return []
        ...
        velvet_ctx = velvet_diag or {}
        bb_percent_b = float(velvet_ctx.get("bb_percent_b", 0.5))
        bb_width_rel = float(velvet_ctx.get("bb_width_rel", 0.01))
        velvet_z = float(velvet_ctx.get("z", 0.0))
        velvet_micro_gap = float(velvet_ctx.get("micro_gap", 0.0))
        velvet_ret = float(velvet_ctx.get("ret", 0.0))
        ride_up = bb_percent_b >= 0.88 and velvet_ret > 0.0 and velvet_micro_gap >= 0.0
        ride_down = bb_percent_b <= 0.12 and velvet_ret < 0.0 and velvet_micro_gap <= 0.0
        squeeze = bb_width_rel <= 0.0055
        range_bound = 0.25 <= bb_percent_b <= 0.75 and abs(velvet_z) <= 0.8

        edge = max(2.0, 1.10 * spread, 0.060 * max(10.0, fair))
        if 0.25 <= delta <= 0.75:
            edge *= 1.45
        if product in MID_VOUCHERS:
            edge *= 1.70
        if squeeze and not ride_up and not ride_down:
            edge *= 1.25

        score = residual / edge
        if product in MID_VOUCHERS:
            target = -0.45 * cap * math.tanh(0.55 * score)
            if abs(score) < 1.10:
                target = 0.0
        else:
            target = -cap * math.tanh(0.75 * score)
        trend_bias = 0.0
        if ride_up:
            trend_bias = 0.35 if product in MID_VOUCHERS else 0.18
        elif ride_down:
            trend_bias = -0.35 if product in MID_VOUCHERS else -0.18
        elif range_bound:
            trend_bias = 0.0
        target += trend_bias * cap
        if squeeze and not ride_up and not ride_down:
            target *= 0.70
        ...
        q = max(2, min(4 if product in MID_VOUCHERS else 6, int(round(3 + 2 * min(1.0, abs(score))))))
        passive_gate = 0.70 if product in MID_VOUCHERS else 0.35
        bid_gate = passive_gate * (0.82 if ride_up else 1.22 if ride_down else 1.0)
        ask_gate = passive_gate * (1.22 if ride_up else 0.82 if ride_down else 1.0)
        allow_bid = mgr.buy_cap > 0 and mgr.projected() < cap and (residual < -bid_gate * edge or mgr.projected() < -0.45 * cap)
        allow_ask = mgr.sell_cap > 0 and mgr.projected() > -cap and (residual > ask_gate * edge or mgr.projected() > 0.45 * cap)
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            ...
            if velvet_ctx is not None:
                memory["velvet_diag"] = {
                    ...
                    "ret": round(float(velvet_ctx["ret_ema"]), 3),
                    "vol": round(float(velvet_ctx["vol_ema"]), 3),
                    "bb_percent_b": round(float(velvet_ctx["bb_percent_b"]), 3),
                    "bb_width_rel": round(float(velvet_ctx["bb_width_rel"]), 5),
        ...
        if any(p in state.order_depths for p in VOUCHER_STRIKES):
            ...
            velvet_diag = memory.get("velvet_diag", {})
            for product in ACTIVE_VOUCHERS:
                if product in state.order_depths:
                    result[product] = self._voucher_orders(product, state, surface, velvet_diag)
