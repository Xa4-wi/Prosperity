...
class Trader:
    ...
    def _voucher_regime(self, product: str, fair_info: dict, outlook: dict) -> str:
        delta = float(fair_info.get("delta", 0.0))
        prob_ex = float(fair_info.get("prob_exercise", delta))
        moneyness = float(fair_info.get("moneyness", 1.0))
        squeeze = bool(outlook.get("squeeze", False))
        breakout_up = bool(outlook.get("breakout_up", False))
        breakout_down = bool(outlook.get("breakout_down", False))

        if product == "VEV_4000":
            if prob_ex > 0.84 or delta > 0.88 or moneyness < 0.91:
                return "LOW_PROXY"
        ...
        if product == "VEV_4500":
            if squeeze:
                return "WATCH"
            if prob_ex > 0.78 or delta > 0.82 or moneyness < 0.95:
                return "LOW_PROXY"
            if breakout_up and 0.42 <= delta <= 0.82:
                return "TRANSITION_LONG"
            if breakout_down and 0.42 <= delta <= 0.82:
                return "TRANSITION_SHORT"
            if 0.42 <= delta <= 0.82 or 0.35 <= prob_ex <= 0.75 or 0.93 <= moneyness <= 0.99:
                return "TRANSITION"
            return "WATCH"
        ...
        if product == "VEV_5000":
            ...
            if breakout_up or breakout_down:
                return "MID_GUARD"
            if 0.35 <= delta <= 0.72 or 0.30 <= prob_ex <= 0.70:
                return "MID_GAMMA"
            if prob_ex > 0.72:
                return "MID_DELTA"
            return "WATCH"
    ...
    def _voucher_orders(self, product: str, state: TradingState, surface: dict, outlook: Optional[dict], risk_state: dict) -> List[Order]:
        ...
        regime = self._voucher_regime(product, fair_info, outlook)
        ...
        if regime == "TRANSITION":
            short_ok = mode in ("SIDEWAYS", "DOWNTREND") and not bool(outlook.get("squeeze", False))
            long_ok = mode in ("SIDEWAYS", "UPTREND", "BREAKOUT_UP") or bool(outlook.get("near_support", False))
        elif regime == "TRANSITION_LONG":
            short_ok = False
            long_ok = True
        elif regime == "TRANSITION_SHORT":
            short_ok = True
            long_ok = False
        elif regime == "MID_GAMMA":
            short_ok = mode == "SIDEWAYS" and not bool(outlook.get("squeeze", False)) and not rv_rising and not bool(outlook.get("breakout_up", False))
            long_ok = not bool(outlook.get("breakout_down", False)) and not bool(outlook.get("squeeze", False))
        elif regime == "MID_GUARD":
            short_ok = False
            long_ok = False
        elif regime == "WATCH":
            short_ok = False
        ...
        if gamma_hot:
            ...
            eff_cap *= 0.35
        if regime == "LOW_PROXY":
            eff_cap *= 1.20 if product == "VEV_4000" else 1.05
        elif regime == "LOW_TRANSITION":
            eff_cap *= 0.95
        elif regime in ("TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT"):
            eff_cap *= 1.05
        elif regime == "MID_GAMMA":
            eff_cap *= 1.00
        elif regime == "MID_DELTA":
            eff_cap *= 0.85
        elif regime in ("MID_GUARD", "WATCH"):
            eff_cap *= 0.70
        ...
        if gamma_hot:
            edge *= 1.35
        if regime == "LOW_PROXY":
            edge *= 0.74 if product == "VEV_4000" else 0.82
        elif regime == "LOW_TRANSITION":
            edge *= 0.90
        elif regime in ("TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT"):
            edge *= 0.86
        ...
        if product in MID_VOUCHERS:
            ...
            threshold = 1.00 if gamma_hot else 0.80
            if regime == "MID_GUARD":
                threshold = 1.35
            elif regime == "MID_DELTA":
                threshold = 1.05
            ...
            if regime == "LOW_PROXY":
                threshold = 0.30 if product == "VEV_4000" else 0.38
            elif regime == "LOW_TRANSITION":
                threshold = 0.48
            elif regime in ("TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT"):
                threshold = 0.34 if product == "VEV_4500" else threshold
            elif regime == "WATCH":
                threshold = 1.50
            if abs(score) < threshold:
                target = 0.0
        ...
        if regime in ("LOW_PROXY", "LOW_TRANSITION") and underlying_like:
        ...
        if regime in ("TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT"):
            target += 0.24 * cap * low_entry_bias
            if bool(outlook.get("near_support", False)) and residual <= 0.35 * edge:
                target = max(target, 0.24 * cap)
            if bool(outlook.get("near_resistance", False)) and residual >= -0.35 * edge:
                target = min(target, -0.24 * cap)
        ...
        if regime == "LOW_PROXY":
            passive_gate = 0.18 if product == "VEV_4000" else 0.22
        elif regime == "LOW_TRANSITION":
            passive_gate = 0.24
        elif regime in ("TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT"):
            passive_gate = 0.18
        ...
        allow_bid = long_ok and mgr.buy_cap > 0 and mgr.projected() < cap and (
            ...
            or (regime in ("LOW_PROXY", "LOW_TRANSITION", "TRANSITION", "TRANSITION_LONG", "TRANSITION_SHORT") and low_entry_bias > 0.20 and mgr.projected() < (0.30 if product == "VEV_4000" else 0.22) * cap)
        )
    ...
    def run(self, state: TradingState):
        ...
        if any(p in state.order_depths for p in VOUCHER_STRIKES):
            ...
            for product in ACTIVE_VOUCHERS:
                ...
                if fair_info is not None:
                    voucher_regimes[product] = self._voucher_regime(product, fair_info, velvet_ctx or {})
            memory["surface_diag"] = {
        ...
        memory["voucher_diag"] = {
            ...
            "regimes": voucher_regimes if 'voucher_regimes' in locals() else {},
        }
