...
class Trader:
    ...
    def _trade_mm(self, product: str, state: TradingState, fair: float) -> List[Order]:
        ...
        spread = float((ba - bb) if bb is not None and ba is not None else 2.0)
        late_factor = 0.0
        if product in (HYDROGEL, VELVET):
            late_factor = clamp((float(state.timestamp) - 80000.0) / 20000.0, 0.0, 1.0)
        min_soft_limit = 16 if product == HYDROGEL else 35
        soft_limit = cfg["soft_limit"]
        if late_factor > 0.0:
            soft_limit = max(min_soft_limit, int(round(cfg["soft_limit"] * (1.0 - 0.55 * late_factor))))
        clear_edge = cfg["clear_edge"] + 0.8 * late_factor
        clear_max = int(round(cfg["clear_max"] * (1.0 + 0.35 * late_factor)))
        quote_size_scale = 1.0 - 0.35 * late_factor
        ...
        reservation = fair - cfg["skew"] * (1.0 + 0.70 * late_factor) * inv_ratio
