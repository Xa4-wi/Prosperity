...
class Trader:
    ...
    def _voucher_orders(self, product: str, state: TradingState, surface: dict, outlook: Optional[dict], total_delta: float) -> List[Order]:
        if product in DISABLED_VOUCHERS:
            return []
        ...
        if product in MID_VOUCHERS:
            edge *= 1.70
        if outlook is not None and bool(outlook.get("squeeze", False)):
            edge *= 1.20
        ...
        # Cap the combined Velvet + voucher directional exposure.
        delta_without_new = total_delta
        ...
        if outlook is not None and bool(outlook.get("squeeze", False)):
            passive_gate *= 1.15
        allow_bid = mgr.buy_cap > 0 and mgr.projected() < cap and (residual < -passive_gate * edge or mgr.projected() < -0.45 * cap)
        allow_ask = mgr.sell_cap > 0 and mgr.projected() > -cap and (residual > passive_gate * edge or mgr.projected() > 0.45 * cap)
    ...
    def run(self, state: TradingState):
        ...
        if any(p in state.order_depths for p in VOUCHER_STRIKES):
            ...
            total_delta = self._portfolio_velvet_delta(state, surface)
            memory["surface_diag"] = {
            ...
            for product in ACTIVE_VOUCHERS:
                if product in state.order_depths:
                    result[product] = self._voucher_orders(product, state, surface, velvet_ctx, total_delta)
