...
# ---------------------------------------------------------------------------
# v53 — clean final Round 4 bot
#
# Cleaned from v52. Keeps only the tested useful components:
# - Hydrogel and Velvet anchor market making.
# - Velvet voucher-delta hedge and small Olivia-style overlay.
# - Tiny Velvet Mark 67/49 sell-side filter.
# - Voucher smile engine with selected pair layer.
# - Deep-ITM VEV_4000/4500 time-value engine.
# ---------------------------------------------------------------------------
...
# Active adjacent-pair layer promoted from the v31.x matrix winner.
# The tested final only keeps symmetric 5000/5100 and 5100/5200 pairs.
...
class Trader:
    ...
    def _remaining_caps(
        ...
        projected = int(state.position.get(product, 0)) + sum(order.quantity for order in result.get(product, []))
    ...
    def _velvet_breakout_active(self, state: TradingState, velvet_fair: float) -> bool:
    ...
    def _pair_trade_vouchers(
        ...
        if self._velvet_breakout_active(state, velvet_fair):
            memory.setdefault("pair_layer", {})["skipped"] = "breakout"
            return

        pair_cfgs = [
            ("VEV_5000", "VEV_5100", 0.018, 0.8, 4, 0.024, 1.0, 3),
            ("VEV_5100", "VEV_5200", 0.024, 1.0, 2, 0.032, 1.2, 1),
        ]
        diag = {}

        for low_product, high_product, rich_iv_thresh, rich_px_thresh, rich_max_qty, cheap_iv_thresh, cheap_px_thresh, cheap_max_qty in pair_cfgs:
            ...
            if pair_edge < -cheap_iv_thresh and cheap_exec_edge > cheap_px_thresh:
