...
class Trader:
    ...
    def _velvet_overlay(self, state: TradingState, memory: dict) -> float:
        """VELVET trade tape overlay: Olivia pattern plus Mark 67 bullish follow-through."""
        ...
        for trade in sorted(state.market_trades.get(VELVET, []), key=lambda t: t.timestamp):
            ...
            if 10 <= qty <= 11 and px < float(ov["day_low"]) + 1e-6:
                ...
                saw = True
            if buyer == "Mark 67":
                ov["mark67_signal"] = min(1.8, float(ov.get("mark67_signal", 0.0)) + min(1.0, qty / 10.0))
                ov["mark67_age"] = 0
                saw_mark67 = True
            elif seller == "Mark 67":
                ov["mark67_signal"] = max(-1.2, float(ov.get("mark67_signal", 0.0)) - min(1.0, qty / 10.0))
                ov["mark67_age"] = 0
                saw_mark67 = True
        if not saw:
            ov["age"] = int(ov["age"]) + 1
        ...
        if not saw_mark67:
            ov["mark67_age"] = int(ov.get("mark67_age", 999)) + 1
            ov["mark67_signal"] = float(ov.get("mark67_signal", 0.0)) * 0.90
        olivia_bias = 0.75 * max(0.0, min(1.0, float(ov["signal"])))
        mark67_bias = 1.25 * clamp(float(ov.get("mark67_signal", 0.0)), -1.0, 1.2)
        return olivia_bias + mark67_bias
