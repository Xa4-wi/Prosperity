    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                ...
                mark67_fair_shift = 1.0 * max(-0.5, min(1.0, mark67_score))
                velvet_trade_fair = voucher_velvet_fair + mark67_fair_shift
                velvet_target = int(12 * mark67_score)
                block_velvet_shorts = mark67_score > 0.75
                if any(product in state.order_depths for product in VOUCHER_STRIKES):
                    ...
                    velvet_target = int(clamp(hedge_target + 12 * mark67_score, -100.0, 100.0))
                ...
                result[VELVET] = self._trade_mm(
                    ...
                    cover_short=False,
                    ask_edge_extra=0.5 if block_velvet_shorts else 0.0,
                )
            except Exception as exc:
