...
class HydrogelTrader(BaseEngine):
    def run(self) -> List[Order]:
        ...
        if gap > 0.60 * move:
            ...
            target = -20.0
        target += 4.0 * clamp(overlay_bias, -1.0, 1.0)
        ...
        return self.execute_underlying(
            ...
            take_edge=max(2.2, 0.50 * spread + 0.10 * move),
            quote_edge=max(1.3, 0.44 * spread + 0.06 * move),
            quote_size=5,
            take_max=4,
        )
