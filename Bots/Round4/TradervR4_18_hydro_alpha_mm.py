...
class HydrogelTrader(BaseEngine):
    def run(self) -> List[Order]:
        ...
        ema_alpha = 0.25 * (ema_mid - mid)
        micro_alpha = 0.20 * (micro - mid)
        imbalance_alpha = 0.25 * imbalance * spread
        anchor_alpha = clamp(0.10 * (10000.0 - mid), -1.2 * move, 1.2 * move)
        mark_alpha = 0.15 * overlay_bias * move
        alpha_raw = ema_alpha + micro_alpha + imbalance_alpha + anchor_alpha + mark_alpha
        alpha_z = alpha_raw / max(1.0, move)

        fair = mid + alpha_raw

        stretch = mid - ema_mid
        stretch_z = stretch / max(1.0, move)
        fade_target = 0.0
        ...
        mm_target = 10.0 * clamp(alpha_z, -1.5, 1.5)
        target = mm_target + fade_target
        ...
        self.memory["hydro_diag"] = {
            ...
            "micro_alpha": round(micro_alpha, 3),
            "imbalance_alpha": round(imbalance_alpha, 3),
            "anchor_alpha": round(anchor_alpha, 3),
            "mark_alpha": round(mark_alpha, 3),
            "stretch": round(stretch, 3),
            "stretch_z": round(stretch_z, 3),
            "mm_target": round(mm_target, 3),
            "fade_target": round(fade_target, 3),
        ...
        return self.execute_underlying(
            HYDRO,
            fair=fair,
            target=target,
