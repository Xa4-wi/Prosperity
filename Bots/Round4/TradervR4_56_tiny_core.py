...
# ---------------------------------------------------------------------------
# v56 — tiny core Round 4 bot
#
# More aggressive cleanup from v55. Keeps only products that produced PnL
# in the radical core backtest:
# - Hydrogel and Velvet anchor market making.
# - Velvet voucher-delta hedge.
# - Single-voucher smile engine for VEV_5000/5100/5200.
...
def _bs_terms(spot: float, strike: float, tte: float, sigma: float) -> Optional[Tuple[float, float]]:
    if spot <= 0.0 or strike <= 0.0 or tte <= 0.0 or sigma <= 0.0:
        return None
    vol_time = sigma * math.sqrt(tte)
    d1 = (math.log(spot / strike) + 0.5 * sigma * sigma * tte) / vol_time
    return d1, d1 - vol_time


def bs_call(spot: float, strike: float, tte: float, sigma: float) -> float:
    terms = _bs_terms(spot, strike, tte, sigma)
    if terms is None:
...
def bs_delta(spot: float, strike: float, tte: float, sigma: float) -> float:
    ...
    return _N.cdf(terms[0])


def implied_vol(price: float, spot: float, strike: float, tte: float, iters: int = 36) -> float:
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # VELVET — anchor market making + voucher-delta hedge
        voucher_velvet_fair: Optional[float] = None
        sigmas: Optional[Dict[str, float]] = None
        strip_delta = 0.0
        if VELVET in state.order_depths:
            try:
                voucher_velvet_fair = self._fair(VELVET, state.order_depths[VELVET])
