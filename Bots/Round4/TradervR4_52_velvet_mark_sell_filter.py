...
# ---------------------------------------------------------------------------
# v52 — Velvet Mark sell-side filter
#
# Starts from v47 and uses Mark 67 / Mark 49 only as a soft VELVET
# sell-side threshold modifier. No target shift, no short block, no voucher
# Mark overlay.
# ---------------------------------------------------------------------------
...
class Trader:
    ...
    def _trade_mm(
        ...
        state: TradingState,
        fair: float,
        sell_take_extra: float = 0.0,
        ask_quote_extra: float = 0.0,
        bid_quote_discount: float = 0.0,
    ) -> List[Order]:
        ...
        for bid, vol in sorted(od.buy_orders.items(), reverse=True):
            if bid - fair >= cfg["take_edge"] + sell_take_extra:
        ...
        for lvl in range(int(cfg["n_levels"])):
            ...
            size_scale = 0.70 ** lvl
            buy_px = math.floor(reservation - max(0.5, base_edge - bid_quote_discount) - offset)
            sell_px = math.ceil(reservation + base_edge + offset + ask_quote_extra)
    ...
    def _velvet_mark_sell_filter(self, state: TradingState, memory: dict) -> float:
        """Mark 67 / Mark 49 only make VELVET sells require slightly more edge."""
        signal = 0.90 * float(memory.get("velvet_mark_sell_filter", 0.0))
        for trade in state.market_trades.get(VELVET, []):
            qty = float(min(abs(int(trade.quantity)), 15))
            buyer = getattr(trade, "buyer", "")
            seller = getattr(trade, "seller", "")
            if buyer == "Mark 67" and seller == "Mark 49":
                signal += qty
            elif buyer == "Mark 67":
                signal += 0.55 * qty
            elif seller == "Mark 49":
                signal += 0.45 * qty
        signal = clamp(signal, 0.0, 35.0)
        memory["velvet_mark_sell_filter"] = signal
        return clamp(signal / 35.0, 0.0, 1.0)
    ...
    def _reset_day(self, memory: dict, timestamp: int) -> None:
        ...
        if last >= 0 and timestamp < last:
            ...
            memory.pop("velvet_mark_sell_filter", None)
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                ...
                velvet_mark_score = self._velvet_mark_sell_filter(state, memory)
                ...
                result[VELVET] = self._trade_mm(
                    ...
                    state,
                    velvet_trade_fair,
                    sell_take_extra=0.55 * velvet_mark_score,
                    ask_quote_extra=0.65 * velvet_mark_score,
                    bid_quote_discount=0.20 * velvet_mark_score,
                )
            except Exception as exc:
