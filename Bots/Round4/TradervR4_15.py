...
class Trader:
    def _update_hydro_overlay(self, state: TradingState, memory: dict) -> float:
        overlay = memory.setdefault("hydro_overlay", {})
        overlay["signal"] = HYDRO_OVERLAY_DECAY * float(overlay.get("signal", 0.0))
        seen = overlay.setdefault("seen", {})
        ...
        for trade in state.market_trades.get(HYDRO, []):
            ...
            for mark, signed_qty in ((trade.buyer, qty), (trade.seller, -qty)):
                ...
                overlay["signal"] = clamp(
                    float(overlay.get("signal", 0.0)) + weight * impact * direction,
                    -HYDRO_OVERLAY_CAP,
                    HYDRO_OVERLAY_CAP,
                )
            last_key = key
        if state.market_trades.get(HYDRO):
            seen[HYDRO] = last_key
        overlay["summary"] = round(float(overlay.get("signal", 0.0)), 3)
        return float(overlay.get("signal", 0.0))
    ...
    def _hydro_orders(self, state: TradingState, memory: dict, overlay_bias: float) -> List[Order]:
        ...
        mid = float(ctx["mid"])
        local_stretch = mid - float(ctx["ema_mid"])
        micro_gap = float(ctx["micro_gap"])
        fair = 0.60 * mid + 0.25 * float(ctx["ema_mid"]) + 0.15 * float(ctx["ema_micro"])
        fair += 0.40 * micro_gap
        fair += 0.35 * float(ctx["imbalance"]) * spread
        fair += 0.20 * overlay_bias * move

        target = 0.0
        if local_stretch > 1.55 * move and micro_gap <= 0.10 * spread:
            target = -24.0
        elif local_stretch < -1.55 * move and micro_gap >= -0.10 * spread:
            target = 24.0
        else:
            stretch_score = clamp(local_stretch / max(1.0, 1.55 * move), -1.5, 1.5)
            flow_score = clamp((0.75 * micro_gap / max(1.0, 0.5 * spread)) + 0.45 * float(ctx["imbalance"]), -1.5, 1.5)
            target = clamp(-8.0 * stretch_score + 3.0 * flow_score, -10.0, 10.0)
        target += 8.0 * clamp(overlay_bias, -1.0, 1.0)

        block_new_longs = overlay_bias <= -0.45
        block_new_shorts = overlay_bias >= 0.45
        if block_new_longs:
        ...
        return self._execute_underlying(
            ...
            take_edge=max(2.2, 0.54 * spread + 0.12 * move),
            quote_edge=max(1.35, 0.48 * spread + 0.08 * move),
            quote_size=6,
            take_max=6,
        )
    ...
    def run(self, state: TradingState):
        ...
        conversions = 0
        hydro_overlay = self._update_hydro_overlay(state, memory)

        if HYDRO in state.order_depths:
            result[HYDRO] = self._hydro_orders(state, memory, hydro_overlay)
