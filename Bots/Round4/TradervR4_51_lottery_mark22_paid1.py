...
# ---------------------------------------------------------------------------
# v51 — Mark 22 gated paid lottery test
#
# Starts from v47 and only pays 1 for VEV_6000/6500 after recent Mark 22
# supply, with tiny size and hard inventory caps.
# ---------------------------------------------------------------------------
...
class Trader:
    ...
    def _trade_mark22_lottery_paid1(self, product: str, state: TradingState, memory: dict) -> List[Order]:
        od = state.order_depths[product]
        pos = int(state.position.get(product, 0))
        limit = LIMITS[product]
        mgr = OrderManager(product, pos, limit)
        ...
        mem = memory.setdefault("lottery_mark22_signal", {})
        signal = 0.90 * float(mem.get(product, 0.0))
        for trade in state.market_trades.get(product, []):
            if getattr(trade, "seller", "") == "Mark 22":
                signal += min(abs(int(trade.quantity)), 10)
        signal = clamp(signal, 0.0, 25.0)
        mem[product] = signal

        soft_limit = 8
        if signal > 6.0 and pos < soft_limit and ba is not None and ba <= 1 and mgr.buy_cap > 0:
            mgr.buy(1, min(1, soft_limit - pos, mgr.buy_cap))
        if pos > 0 and mgr.sell_cap > 0:
            mgr.sell(1, min(pos, 4))
    ...
    def _reset_day(self, memory: dict, timestamp: int) -> None:
        ...
        if last >= 0 and timestamp < last:
            ...
            memory.pop("lottery_mark22_signal", None)
                ...
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        if product in ("VEV_6000", "VEV_6500"):
                            result[product] = self._trade_mark22_lottery_paid1(product, state, memory)
