...
HYDRO_OVERLAY_DECAY = 0.82
HYDRO_OVERLAY_CAP = 1.25
HYDRO_MARK_POLICIES = {
    "Mark 14": {"mode": "follow", "weight": 0.12},
    "Mark 38": {"mode": "fade", "weight": 0.16},
}
...
def stable_mid(od: OrderDepth, levels: int = 3) -> Optional[float]:
    ...
    ask_vol = sum(abs(min(0, vol)) for _, vol in asks)
    if bid_vol <= 0 or ask_vol <= 0:
        return mid_price(od)
    bid_px = sum(price * max(0, vol) for price, vol in bids) / bid_vol
    ask_px = sum(price * abs(min(0, vol)) for price, vol in asks) / ask_vol
    if bid_px >= ask_px:
        return mid_price(od)
...
def bs_call_theta(spot: float, strike: float, t: float, vol: float) -> float:
    ...
    if d is None:
        return 0.0
    d1, _ = d
    return -(spot * norm_pdf(d1) * vol) / (2.0 * math.sqrt(t))
...
def build_book_context(product: str, od: OrderDepth, memory: dict) -> Optional[dict]:
    mid = mid_price(od)
    if mid is None:
        mid = stable_mid(od)
    if mid is None:
...
class MarkTraderIdOverlay:
    def __init__(self, memory: dict) -> None:
        self.memory = memory
        self.store = memory.setdefault(
            "mark_overlay",
            {"hydro": {"signal": 0.0, "seen": {}}, "velvet": {}, "voucher": {}},
        )

    def update(self, state: TradingState) -> None:
        self._update_hydro(state)

    def _update_hydro(self, state: TradingState) -> None:
        hydro = self.store.setdefault("hydro", {"signal": 0.0, "seen": {}})
        hydro["signal"] = HYDRO_OVERLAY_DECAY * float(hydro.get("signal", 0.0))
        seen = hydro.setdefault("seen", {})
        last_key = seen.get(HYDRO)
        for trade in state.market_trades.get(HYDRO, []):
            key = f"{trade.timestamp}|{trade.price}|{trade.quantity}|{trade.buyer}|{trade.seller}"
            if key == last_key:
                continue
            qty = abs(int(trade.quantity))
            impact = clamp(qty / 40.0, 0.20, 1.40)
            for mark, signed_qty in ((trade.buyer, qty), (trade.seller, -qty)):
                if not mark:
                    continue
                policy = HYDRO_MARK_POLICIES.get(str(mark))
                if not policy:
                    continue
                ...
                hydro["signal"] = clamp(
                    float(hydro.get("signal", 0.0)) + weight * impact * direction,
                    -HYDRO_OVERLAY_CAP,
                    HYDRO_OVERLAY_CAP,
                )
            last_key = key
        if state.market_trades.get(HYDRO):
            seen[HYDRO] = last_key
        hydro["summary"] = round(float(hydro.get("signal", 0.0)), 3)

    def hydro_bias(self) -> float:
        return float(self.store.get("hydro", {}).get("signal", 0.0))

    def velvet_bias(self) -> float:
        return 0.0

    def voucher_bias(self, _product: str) -> float:
        return 0.0


class BaseEngine:
    def __init__(self, state: TradingState, memory: dict, overlay: MarkTraderIdOverlay) -> None:
        self.state = state
        self.memory = memory
        self.overlay = overlay

    def execute_underlying(
        self,
        product: str,
        fair: float,
        target: float,
        ...
        od = self.state.order_depths[product]
        pos = int(self.state.position.get(product, 0))
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            if edge < take_edge:
                break
            if target_gap <= 0 and edge < take_edge + 1.5:
                break
        ...
        size = max(3, int(round(quote_size * scale)))
        if mgr.buy_cap > 0 and pos < engine_cap and (ba is None or buy_px < ba):
            mgr.buy(int(buy_px), min(size, mgr.buy_cap))
        if mgr.sell_cap > 0 and pos > -engine_cap and (bb is None or sell_px > bb):
            mgr.sell(int(sell_px), min(size, mgr.sell_cap))
        return mgr.orders


class HydrogelTrader(BaseEngine):
    def run(self) -> List[Order]:
        if HYDRO not in self.state.order_depths:
            return []
        od = self.state.order_depths[HYDRO]
        ctx = build_book_context(HYDRO, od, self.memory)
        if ctx is None:
            return []
        ...
        mid = float(ctx["mid"])
        ema_mid = float(ctx["ema_mid"])
        micro = float(ctx["micro"])
        imbalance = float(ctx["imbalance"])
        micro_gap = float(ctx["micro_gap"])
        pos = int(self.state.position.get(HYDRO, 0))
        overlay_bias = self.overlay.hydro_bias()

        local_fair = 0.55 * mid + 0.25 * ema_mid + 0.20 * micro
        local_fair += 0.25 * imbalance * spread
        fair = 0.90 * local_fair + 0.10 * 10000.0
        fair += 0.15 * overlay_bias * move

        stretch = mid - ema_mid
        target = 0.0
        if stretch > 1.7 * move and micro_gap <= 0.0:
            target = -25.0
        elif stretch < -1.7 * move and micro_gap >= 0.0:
            target = 25.0
        target += float(int(round(8.0 * overlay_bias)))
        block_new_longs = overlay_bias <= -0.55
        block_new_shorts = overlay_bias >= 0.55
        if block_new_longs:
        ...
        bb = best_bid(od)
        ba = best_ask(od)
        take_edge = max(2.5, 0.70 * spread + 0.15 * move)
        quote_edge = max(1.5, 0.55 * spread + 0.10 * move)
        inventory_skew = 1.2 * quote_edge
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            if edge < take_edge:
                break
            if block_new_longs and projected >= 0:
                break
            if projected >= HYDRO_ENGINE_CAP and edge < take_edge + 2.0:
                break
            if projected >= target and edge < take_edge + 1.2:
                break
            qty = min(abs(int(volume)), 4, max(0, int(round(target - projected)) + 2))
            if qty <= 0 and edge >= take_edge + 2.0 and projected < HYDRO_ENGINE_CAP and not block_new_longs:
                qty = min(abs(int(volume)), 2)
            if qty <= 0:
                break
            mgr.buy(ask, qty)
        ...
        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            ...
            if edge < take_edge:
                break
            if block_new_shorts and projected <= 0:
                break
            if projected <= -HYDRO_ENGINE_CAP and edge < take_edge + 2.0:
                break
            if projected <= target and edge < take_edge + 1.2:
                break
            qty = min(abs(int(volume)), 4, max(0, int(round(projected - target)) + 2))
            if qty <= 0 and edge >= take_edge + 2.0 and projected > -HYDRO_ENGINE_CAP and not block_new_shorts:
                qty = min(abs(int(volume)), 2)
            if qty <= 0:
                break
            mgr.sell(bid, qty)
        ...
        projected = mgr.projected()
        relative = projected - target
        reservation = fair - inventory_skew * (relative / max(1.0, float(HYDRO_ENGINE_CAP)))
        buy_px = math.floor(reservation - quote_edge)
        sell_px = math.ceil(reservation + quote_edge)
        if bb is not None and ba is not None and bb < ba:
            if ba - bb >= 3:
        ...
        if mgr.buy_cap > 0 and not block_new_longs and projected < HYDRO_ENGINE_CAP and projected < 30:
            mgr.buy(int(buy_px), min(quote_size, mgr.buy_cap))
        if mgr.sell_cap > 0 and not block_new_shorts and projected > -HYDRO_ENGINE_CAP and projected > -30:
            mgr.sell(int(sell_px), min(quote_size, mgr.sell_cap))

        if mgr.projected() > 40 and bb is not None:
            mgr.sell(bb, min(6, mgr.sell_cap))
        elif mgr.projected() < -40 and ba is not None:
            mgr.buy(ba, min(6, mgr.buy_cap))

        gap = fair - mid
        self.memory["hydro_diag"] = {
            "mid": round(mid, 3),
            "ema_mid": round(ema_mid, 3),
            "micro": round(micro, 3),
            "fair": round(fair, 3),
            "local_fair": round(local_fair, 3),
            "gap": round(gap, 3),
            "stretch": round(stretch, 3),
            "micro_gap": round(micro_gap, 3),
            "overlay": round(overlay_bias, 3),
            "block_long": block_new_longs,
            "block_short": block_new_shorts,
...
class VelvetfruitTrader(BaseEngine):
    def _voucher_cap(self, product: str) -> int:
        return VOUCHER_LOW_CAP if product in LOW_VOUCHERS else VOUCHER_MID_CAP

    def _technical_outlook(self, ctx: dict) -> dict:
        pm = self.memory.setdefault("products", {}).setdefault(VELVET, {})
    ...
    def velvet_orders(self) -> Tuple[List[Order], Optional[float], Optional[dict]]:
        if VELVET not in self.state.order_depths:
            return [], None, None
        od = self.state.order_depths[VELVET]
        pos = int(self.state.position.get(VELVET, 0))
        ctx = build_book_context(VELVET, od, self.memory)
        if ctx is None:
            return [], None, None
        ...
        outlook = self._technical_outlook(ctx)
        ...
        orders = self.execute_underlying(
            VELVET,
            fair=fair,
            target=target,
    ...
    def build_voucher_surface(self, spot: float) -> dict:
        rows: List[Tuple[str, float, float, float, float]] = []
        product_mem = self.memory.setdefault("products", {})
        for product, strike in VOUCHER_STRIKES.items():
            if product in DISABLED_VOUCHERS or product not in self.state.order_depths:
                continue
            od = self.state.order_depths[product]
            mid = mid_price(od)
            if mid is None:
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            fairs[product] = {
                ...
                "prob_exercise": bs_call_prob_exercise(max(1.0, spot), float(strike), BS_T, fair_iv),
                "moneyness": float(strike) / max(1.0, spot),
            }
        return {"base_iv": mean_iv, "slope": slope, "fairs": fairs}

    def portfolio_voucher_risks(self, surface: dict) -> dict:
        delta_total = float(self.state.position.get(VELVET, 0))
        gamma_total = 0.0
        vega_total = 0.0
        theta_total = 0.0
        for product in ACTIVE_VOUCHERS:
            ...
            if fair_info is None:
                continue
            pos = float(self.state.position.get(product, 0))
        ...
        return {"delta": delta_total, "gamma": gamma_total, "vega": vega_total, "theta": theta_total}

    def voucher_orders(self, product: str, surface: dict, outlook: Optional[dict], risk_state: dict) -> List[Order]:
        if product in DISABLED_VOUCHERS or product not in self.state.order_depths:
            return []
        od = self.state.order_depths[product]
        pos = int(self.state.position.get(product, 0))
        ...
        gamma = float(fair_info["gamma"])
        vega = float(fair_info["vega"])
        theta = float(fair_info["theta"])
        prob_ex = float(fair_info["prob_exercise"])
        moneyness = float(fair_info["moneyness"])
        fair_iv = float(fair_info["fair_iv"])
        ...
        gamma_buffer = 0.55 * gamma * (float(outlook.get("range_width", max(1.0, spread))) * 0.10 + rv_proxy * 18.0) ** 2
        ...
        if product in MID_VOUCHERS:
            ...
            threshold = 0.32 if product == "VEV_4000" and underlying_like else (0.45 if underlying_like else 0.55)
        ...
        delta_now = float(risk_state.get("delta", 0.0))
        if target > 0.0 and delta_now > VELVET_TOTAL_DELTA_CAP:
            target = min(target, 0.0)
        if target < 0.0 and delta_now < -VELVET_TOTAL_DELTA_CAP:
            target = max(target, 0.0)
        ...
        size = max(2, min(5 if product in MID_VOUCHERS else 6, int(round(3 + 2 * min(1.0, abs(score))))))
        ...
        if allow_bid and ba is not None and bid_px < ba:
            mgr.buy(int(bid_px), min(size, mgr.buy_cap))
        if allow_ask and bb is not None and ask_px > bb:
            mgr.sell(int(ask_px), min(size, mgr.sell_cap))
        return mgr.orders
...
class Trader:
    def run(self, state: TradingState):
        ...
        overlay = MarkTraderIdOverlay(memory)
        overlay.update(state)

        hydro = HydrogelTrader(state, memory, overlay)
        velvet = VelvetfruitTrader(state, memory, overlay)

        if HYDRO in state.order_depths:
            result[HYDRO] = hydro.run()

        velvet_fair = None
        velvet_ctx = None
        if VELVET in state.order_depths:
            result[VELVET], velvet_fair, velvet_ctx = velvet.velvet_orders()
        ...
        if any(product in state.order_depths for product in ACTIVE_VOUCHERS):
            surface = velvet.build_voucher_surface(velvet_fair)
            risk_state = velvet.portfolio_voucher_risks(surface)
            memory["surface_diag"] = {
            ...
            for product in ACTIVE_VOUCHERS:
                if product in state.order_depths:
                    result[product] = velvet.voucher_orders(product, surface, velvet_ctx, risk_state)

        memory["voucher_diag"] = {
            "active": sorted(ACTIVE_VOUCHERS),
            "disabled": sorted(DISABLED_VOUCHERS),
        }
        memory["mark_overlay_diag"] = {
            "hydro_bias": round(overlay.hydro_bias(), 3),
            "velvet_bias": round(overlay.velvet_bias(), 3),
            "voucher_note": "skeleton only",
        }
