...
# ---------------------------------------------------------------------------
# v43 — small trader-ID overlays
#
# Starts from the clean v42 base, then adds three scoped public-trade overlays:
# VELVET directional bias, voucher passive-supply bias, and tiny Hydro filter.
# ---------------------------------------------------------------------------
...
class Trader:
    ...
    def _trade_mm(
        ...
        proj = mgr.projected()
        if cover_short and proj < target and ba is not None and ba <= fair + cfg["clear_edge"] + 1.0:
        ...
        for lvl in range(int(cfg["n_levels"])):
            ...
            if mgr.buy_cap > 0 and (ba is None or buy_px < ba):
                mgr.buy(buy_px, qty)
            can_sell = (not block_new_shorts) or mgr.projected() > target
            if can_sell and mgr.sell_cap > 0 and (bb is None or sell_px > bb):
    ...
    def _trade_voucher(
        ...
        high_supply_candidate = strike >= 5400 and supply_score > 0.35
        if fair < 0.5 and not high_supply_candidate:
        ...
        if strike > velvet_fair:
            buy_signal = False
            if high_supply_candidate:
                passive_supply_bid = iv_residual < -0.5 * resid_threshold
                if strike >= 6000 and ba is not None and ba <= 1:
                    passive_supply_bid = True
    ...
    def _velvet_overlay(self, state: TradingState, memory: dict) -> Tuple[float, float]:
        """VELVET tape overlay: Olivia pattern plus trader-ID rebound signal."""
        ov = memory.setdefault(
            "velvet_overlay",
            {"day_low": 1e18, "day_high": -1e18, "signal": 0.0, "age": 999, "id_signal": 0.0},
        )
        saw = False
        id_signal = 0.90 * float(ov.get("id_signal", 0.0))
        for trade in sorted(state.market_trades.get(VELVET, []), key=lambda t: t.timestamp):
            ...
            if 10 <= qty <= 11 and px < float(ov["day_low"]) + 1e-6:
                ...
                saw = True
            if buyer == "Mark 67":
                id_signal += 1.00 * min(qty, 15)
            if seller == "Mark 49":
                id_signal += 0.90 * min(qty, 15)
            if seller == "Mark 22":
                id_signal += 0.45 * min(qty, 15)
            if buyer == "Mark 49":
                id_signal -= 0.50 * min(qty, 15)
        if not saw:
            ov["age"] = int(ov["age"]) + 1
            ov["signal"] = float(ov["signal"]) * 0.96
        ov["id_signal"] = clamp(id_signal, -40.0, 40.0)
        olivia_bias = 0.75 * max(0.0, min(1.0, float(ov["signal"])))
        id_score = math.tanh(float(ov["id_signal"]) / 25.0)
        return olivia_bias, id_score

    def _voucher_supply_scores(self, state: TradingState, memory: dict) -> Dict[str, float]:
        supply = memory.setdefault("voucher_supply", {})
        scores: Dict[str, float] = {}
        for product in VOUCHER_STRIKES:
            signal = 0.90 * float(supply.get(product, 0.0))
            for trade in state.market_trades.get(product, []):
                ...
                if seller == "Mark 22":
                    signal += min(qty, 15)
                if buyer == "Mark 01":
                    signal += 0.30 * min(qty, 15)
            signal = clamp(signal, 0.0, 40.0)
            supply[product] = signal
            scores[product] = math.tanh(signal / 18.0)
        return scores
    ...
    def _reset_day(self, memory: dict, timestamp: int) -> None:
        ...
        if last >= 0 and timestamp < last:
            ...
            memory.pop("voucher_supply", None)
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                overlay_bias, velvet_id_score = self._velvet_overlay(state, memory)
                voucher_velvet_fair = self._fair(VELVET, state.order_depths[VELVET]) + overlay_bias
                id_fair_shift = 2.0 * velvet_id_score
                id_target = int(35 * velvet_id_score)
                block_velvet_shorts = velvet_id_score > 0.35
                velvet_trade_fair = voucher_velvet_fair + id_fair_shift
                velvet_target = id_target
                if any(product in state.order_depths for product in VOUCHER_STRIKES):
                    ...
                    velvet_trade_fair = voucher_velvet_fair + hedge_shift + id_fair_shift
                    velvet_target = int(clamp(hedge_target + id_target, -100.0, 100.0))
                velvet_fair = velvet_trade_fair
                result[VELVET] = self._trade_mm(
                    VELVET,
                    state,
                    velvet_trade_fair,
                    target=velvet_target,
                    block_new_shorts=block_velvet_shorts,
                    cover_short=block_velvet_shorts,
                    ask_edge_extra=0.8 if block_velvet_shorts else 0.0,
                )
            except Exception as exc:
