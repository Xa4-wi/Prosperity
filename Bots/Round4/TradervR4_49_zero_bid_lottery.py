...
# ---------------------------------------------------------------------------
# v49 — zero-bid lottery collector
#
# Starts from v47 and adds a separate VEV_6000/6500 handler:
# bid 0 for free optionality, offer only existing long inventory at 1,
# never pay 1 and never short the lottery vouchers.
# ---------------------------------------------------------------------------
...
class Trader:
    ...
    def _trade_zero_bid_lottery(self, product: str, state: TradingState) -> List[Order]:
        od = state.order_depths[product]
        pos = int(state.position.get(product, 0))
        limit = LIMITS[product]
        mgr = OrderManager(product, pos, limit)

        # Free optionality only: never cross to buy at 1, and never short.
        if mgr.buy_cap > 0:
            mgr.buy(0, min(mgr.buy_cap, 20))
        if pos > 0 and mgr.sell_cap > 0:
            mgr.sell(1, min(pos, 20))
                ...
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        if product in ("VEV_6000", "VEV_6500"):
                            result[product] = self._trade_zero_bid_lottery(product, state)
