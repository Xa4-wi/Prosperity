...
PAIR_MODE_TAG = "R4_33_BBO_5200_LITE"
...
class Trader:
    ...
    def _trade_voucher(
        ...
        allow_5200_near_otm_buy = False
        if strike >= 5400:
            resid_threshold = 0.050
        elif strike >= 5300:
            resid_threshold = 0.035
        elif strike == 5200:
            # v20-lite impulse: let 5200 behave more like a real alpha source
            # when it is close to spot, but keep it tightly leashed when it drifts
            # further out of the money.
            if strike <= velvet_fair + 50.0:
                resid_threshold = 0.017
                soft_limit = 48
                take_max = 10
                allow_5200_near_otm_buy = strip_delta < 40.0
            else:
                resid_threshold = 0.022
        sell_threshold = resid_threshold
        if strike == 5200:
            if pair_edge_5200_5300 > 0.008:
                sell_threshold = max(0.012, resid_threshold - 0.006)
                soft_limit = max(soft_limit, 52)
                take_max = max(take_max, 12)
            elif pair_edge_5200_5300 < -0.008:
                sell_threshold = resid_threshold + 0.010
        elif strike == 5300:
            if pair_edge_5200_5300 < -0.008:
        ...
        if strike > velvet_fair:
            buy_signal = (
                strike == 5200
                and allow_5200_near_otm_buy
                and iv_residual < -resid_threshold
            )
