...
class Trader:
    ...
    def _trade_mm(self, product: str, state: TradingState, fair: float) -> List[Order]:
        ...
        late_factor = 0.0
        if product == HYDROGEL:
            late_factor = clamp((float(state.timestamp) - 92000.0) / 8000.0, 0.0, 1.0)
        elif product == VELVET:
            late_factor = clamp((float(state.timestamp) - 95000.0) / 5000.0, 0.0, 1.0)
        min_soft_limit = 38 if product == HYDROGEL else 85
        soft_limit = cfg["soft_limit"]
        if late_factor > 0.0:
            trim_ratio = 0.15 if product == HYDROGEL else 0.10
            soft_limit = max(min_soft_limit, int(round(cfg["soft_limit"] * (1.0 - trim_ratio * late_factor))))
        clear_edge = cfg["clear_edge"] + (0.20 if product == HYDROGEL else 0.10) * late_factor
        clear_max = int(round(cfg["clear_max"] * (1.0 + (0.10 if product == HYDROGEL else 0.05) * late_factor)))
        quote_size_scale = 1.0 - (0.06 if product == HYDROGEL else 0.04) * late_factor
        ...
        # Clear toward a slightly smaller late-session soft limit.
        proj = mgr.projected()
        ...
        reservation = fair - cfg["skew"] * (1.0 + (0.15 if product == HYDROGEL else 0.08) * late_factor) * inv_ratio
