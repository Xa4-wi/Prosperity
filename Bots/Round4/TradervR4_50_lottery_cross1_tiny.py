...
# ---------------------------------------------------------------------------
# v50 — tiny paid lottery test
#
# Starts from v47 and tests whether paying 1 for tiny VEV_6000/6500
# inventory can recycle back out at 1. This is intentionally capped hard.
# ---------------------------------------------------------------------------
...
class Trader:
    ...
    def _trade_paid_lottery_tiny(self, product: str, state: TradingState) -> List[Order]:
        od = state.order_depths[product]
        pos = int(state.position.get(product, 0))
        limit = LIMITS[product]
        mgr = OrderManager(product, pos, limit)
        ...
        # Tiny cap: test whether paid tickets at 1 can be recycled, without
        # letting the lottery tail dominate the rest of the voucher book.
        soft_limit = 10
        if pos <= 0 and ba is not None and ba <= 1 and mgr.buy_cap > 0:
            mgr.buy(1, min(2, soft_limit - pos, mgr.buy_cap))
        elif pos > 0 and mgr.sell_cap > 0:
                ...
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        if product in ("VEV_6000", "VEV_6500"):
                            result[product] = self._trade_paid_lottery_tiny(product, state)
