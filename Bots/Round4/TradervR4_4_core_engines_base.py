...
MID_VOUCHERS = {"VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300"}
...
class Trader:
    ...
    def _voucher_orders(self, product: str, state: TradingState, surface: dict) -> List[Order]:
        ...
        # Do not churn nearly worthless wings. They are optional, not the core edge.
        if product in HIGH_VOUCHERS and market_mid <= 0.5 and fair < 0.75:
            return []

        edge = max(2.0, 1.05 * spread, 0.055 * max(10.0, fair))
        if 0.25 <= delta <= 0.75:
            edge *= 1.25  # middle strikes were the churn-loss area; require more proof.
        ...
        take_max = 8 if product in MID_VOUCHERS else 6
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            if trade_edge < edge:
                break
            if target <= mgr.projected() and trade_edge < edge + 1.0:
                break
        ...
        q = max(2, min(6, int(round(4 + 2 * min(1.0, abs(score))))))
        allow_bid = mgr.buy_cap > 0 and mgr.projected() < cap and (residual < -0.35 * edge or mgr.projected() < -0.35 * cap)
        allow_ask = mgr.sell_cap > 0 and mgr.projected() > -cap and (residual > 0.35 * edge or mgr.projected() > 0.35 * cap)
        ...
        if any(p in state.order_depths for p in VOUCHER_STRIKES):
            ...
            for product in VOUCHER_STRIKES:
                if product in state.order_depths:
                    result[product] = self._voucher_orders(product, state, surface)
