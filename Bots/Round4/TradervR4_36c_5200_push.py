...
class Trader:
    ...
    def _trade_voucher(
        ...
        # Moneyness buckets via delta: ITM calls can trade more like the underlying,
        # ATM calls need more edge, and OTM calls should be tightly constrained.
        if delta > 0.85:
        ...
        if strike == 5200:
            # Pair-edge only nudges confidence; it does not create explicit spread orders.
            if pair_edge_5100_5200 > 0.010:
                resid_threshold = max(0.013, resid_threshold - 0.003)
                sell_threshold = max(0.011, sell_threshold - 0.005)
                soft_limit = min(max(soft_limit, 48), 64)
                take_max = min(max(take_max, 11), 13)
            elif pair_edge_5100_5200 < -0.010:
                resid_threshold = resid_threshold + 0.003
                sell_threshold = sell_threshold + 0.005
                soft_limit = min(soft_limit, 42)
                take_max = min(take_max, 9)
            if pair_edge_5200_5300 > 0.008:
                sell_threshold = max(0.011, sell_threshold - 0.005)
                soft_limit = min(max(soft_limit, 48), 64)
                take_max = min(max(take_max, 11), 13)
            elif pair_edge_5200_5300 < -0.008:
                sell_threshold = max(sell_threshold, resid_threshold + 0.010)
