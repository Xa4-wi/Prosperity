...
class Trader:
    ...
    def _mark_filter(
        ...
        signal = 0.0
        for trade in sorted(state.market_trades.get(product, []), key=lambda t: t.timestamp):
            qty = abs(int(trade.quantity))
            impact = clamp(qty / scale, 0.15, 1.30)
            for mark, signed_qty in ((trade.buyer, qty), (trade.seller, -qty)):
                if not mark:
                    continue
                policy = policies.get(str(mark))
                if policy is None:
                    continue
                mode, weight = policy
                ...
                if mode == "fade":
                    direction = -direction
                signal = clamp(signal + weight * impact * direction, -cap, cap)
        return signal
    ...
    def run(self, state: TradingState):
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                hydro_fair += 0.55 * self._mark_filter(
                    state,
                    HYDROGEL,
                    {"Mark 14": ("follow", 0.10), "Mark 38": ("fade", 0.12)},
                    40.0,
                    1.25,
                )
        ...
        if VELVET in state.order_depths:
            try:
                ...
                velvet_fair += 0.85 * self._mark_filter(
                    state,
                    VELVET,
                    {
                        "Mark 67": ("follow", 0.16),
