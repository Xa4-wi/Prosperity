...
# ---------------------------------------------------------------------------
# v48 — simple separated Mark tracks
#
# Starts from v47 and adds only small product-specific Mark overlays:
# - Velvet: Mark 67 buyer / Mark 49 seller bullish fair-and-block filter.
# - Hydrogel: tiny Mark 38 fade / Mark 14 confirmation fair shift.
# - Vouchers: Mark 22 seller as passive-supply confidence only.
# ---------------------------------------------------------------------------
...
_TTE_TOTAL_DAYS = 5  # assumed total trading days in the round

# Pair-trading settings promoted from the v31.x matrix winner.
# OFF     = no pair layer for that adjacent strike pair
# ONEWAY  = only rich-lower / cheap-higher hedge trades
# BOTH    = allow the reverse pair as well
PAIR_MODE_5000_5100 = "BOTH"
PAIR_MODE_5100_5200 = "BOTH"
PAIR_MODE_5200_5300 = "OFF"
PAIR_MODE_TAG = "R4_32_BBO"
...
class Trader:
    ...
    def _projected_position(
        ...
        return int(state.position.get(product, 0)) + sum(order.quantity for order in result.get(product, []))
    ...
    def _remaining_caps(
        ...
        projected = self._projected_position(state, result, product)
        limit = LIMITS[product]
        buy_cap = max(0, limit - projected)
        sell_cap = max(0, limit + projected)
    ...
    def _trade_mm(
        ...
        cover_short: bool = False,
        ask_edge_extra: float = 0.0,
    ) -> List[Order]:
        ...
        for bid, vol in sorted(od.buy_orders.items(), reverse=True):
            if block_new_shorts and mgr.projected() <= target:
                break
            if bid - fair >= cfg["take_edge"] + ask_edge_extra:
        ...
        if cover_short:
            proj = mgr.projected()
            if proj < target and ba is not None and ba <= fair + cfg["clear_edge"] + 0.8:
                mgr.buy(ba, min(target - proj, cfg["clear_max"]))

        # Clear toward soft_limit (not toward 0)
        proj = mgr.projected()
        ...
        proj = mgr.projected()
        inv_ratio = (proj - target) / float(limit)
        ...
        for lvl in range(int(cfg["n_levels"])):
            ...
            sell_px = math.ceil(reservation + base_edge + offset + ask_edge_extra)
            ...
            if mgr.buy_cap > 0 and (ba is None or buy_px < ba):
                mgr.buy(buy_px, qty)
            can_sell_quote = not block_new_shorts or mgr.projected() > target
            if can_sell_quote and mgr.sell_cap > 0 and (bb is None or sell_px > bb):
    ...
    def _trade_voucher(
        ...
        voucher_supply_signal: float = 0.0,
    ) -> List[Order]:
        ...
        # Moneyness buckets via delta: ITM calls can trade more like underlying,
        # ATM calls need more edge, and OTM calls remain tightly constrained.
        if delta > 0.85:
            resid_threshold = 0.014
            soft_limit = max(soft_limit, 80)
            take_max = max(take_max, 14)
        elif delta > 0.65:
            resid_threshold = 0.016
            soft_limit = max(soft_limit, 70)
            take_max = max(take_max, 12)
        elif delta > 0.35:
        ...
        sell_signal = iv_residual > sell_threshold    # market is RICH vs smile
        passive_supply_bid = False

        # Keep OTM buying fully off.
        if strike > velvet_fair:
            buy_signal = False
            if strike >= 5400 and voucher_supply_signal > 12.0:
                # Mark 22 supply only makes us more willing to leave a passive
                # cheap bid; it never enables aggressive OTM taking.
                passive_supply_bid = iv_residual < -0.50 * resid_threshold
        ...
        if passive_supply_bid:
            qty = min(qty, 2)
            if ba is not None:
                buy_px = min(buy_px, int(ba) - 1)
            buy_px = max(0, buy_px)

        can_bid = mgr.buy_cap > 0 and pos < soft_limit and (buy_signal or passive_supply_bid or pos < 0)
    ...
    def _deep_itm_bias(self, product: str, state: TradingState, memory: dict) -> Tuple[float, float]:
        bias_mem = memory.setdefault("deep_itm_mark_bias", {})
        saved = bias_mem.setdefault(product, {"rich": 0.0, "cheap": 0.0})
        rich = 0.88 * float(saved.get("rich", 0.0))
        cheap = 0.88 * float(saved.get("cheap", 0.0))
        for trade in state.market_trades.get(product, []):
            ...
            if seller == "Mark 14":
                rich += 0.10 * qty
        rich = clamp(rich, 0.0, 12.0)
        cheap = clamp(cheap, 0.0, 12.0)
        saved["rich"] = rich
        saved["cheap"] = cheap
        return rich, cheap

    def _trade_deep_itm_voucher(
        self,
        product: str,
        state: TradingState,
        velvet_fair: float,
        ...
        ba = best_ask(od)
        mid = raw_mid(od)
        if mid is None or bb is None or ba is None or bb >= ba:
            return mgr.flush()

        intrinsic = max(0.0, velvet_fair - strike)
        tv = float(mid) - intrinsic
        tv_mem = memory.setdefault("deep_itm_tv", {})
        saved = tv_mem.setdefault(product, {"tv_ema": tv})
        tv_ema = 0.05 * tv + 0.95 * float(saved.get("tv_ema", tv))
        saved["tv_ema"] = tv_ema
        tv_edge = tv - tv_ema

        spread = float(ba - bb)
        rich_bias, cheap_bias = self._deep_itm_bias(product, state, memory)
        rich_threshold = max(0.8, 0.30 * spread) - 0.035 * rich_bias
        cheap_threshold = max(1.0, 0.35 * spread) - 0.025 * cheap_bias
        rich_threshold = max(0.45, rich_threshold)
        cheap_threshold = max(0.55, cheap_threshold)

        soft_limit = 90 if product == "VEV_4000" else 80
        take_max = 14 if product == "VEV_4000" else 12
        quote_edge = max(0.9, 0.45 * spread)
        clear_edge = max(0.4, 0.15 * spread)

        delta_pressure = clamp(strip_delta / 120.0, -1.5, 1.5)
        buy_signal = tv_edge < -cheap_threshold and delta_pressure <= 0.9
        sell_signal = tv_edge > rich_threshold and delta_pressure >= -0.9

        fair = intrinsic + tv_ema
        for ask, vol in sorted(od.sell_orders.items()):
            if buy_signal and fair - ask >= max(0.6, 0.25 * spread) and pos < soft_limit:
                mgr.buy(ask, min(-vol, take_max))
            else:
                break
        for bid, vol in sorted(od.buy_orders.items(), reverse=True):
            if sell_signal and bid - fair >= max(0.6, 0.25 * spread) and pos > -soft_limit:
                mgr.sell(bid, min(vol, take_max))
            else:
                break

        proj = mgr.projected()
        if proj > soft_limit and bb >= fair - clear_edge:
            mgr.sell(bb, min(proj - soft_limit, 30))
        elif proj < -soft_limit and ba <= fair + clear_edge:
            mgr.buy(ba, min(-soft_limit - proj, 30))

        proj = mgr.projected()
        inv_ratio = proj / float(limit)
        reservation = fair - (0.012 * max(20.0, fair) + 1.0) * inv_ratio
        buy_px = math.floor(reservation - quote_edge)
        sell_px = math.ceil(reservation + quote_edge)
        buy_px = min(max(buy_px, bb + 1), ba - 1)
        sell_px = max(min(sell_px, ba - 1), bb + 1)
        qty = max(3, int(round(7 * max(0.25, 1.0 - abs(inv_ratio)))))

        if mgr.buy_cap > 0 and pos < soft_limit and (buy_signal or pos < 0) and buy_px < ba:
            mgr.buy(buy_px, qty)
        if mgr.sell_cap > 0 and pos > -soft_limit and (sell_signal or pos > 0) and sell_px > bb:
            mgr.sell(sell_px, qty)
    ...
    def _velvet_mark_score(self, state: TradingState, memory: dict) -> float:
        signal = 0.90 * float(memory.get("velvet_mark_signal", 0.0))
        for trade in state.market_trades.get(VELVET, []):
            qty = min(abs(int(trade.quantity)), 15)
            buyer = getattr(trade, "buyer", "")
            seller = getattr(trade, "seller", "")
            if buyer == "Mark 67":
                signal += float(qty)
            if seller == "Mark 49":
                signal += 0.80 * float(qty)
        signal = clamp(signal, 0.0, 40.0)
        memory["velvet_mark_signal"] = signal
        return clamp(signal / 40.0, 0.0, 1.0)

    def _hydro_mark_shift(self, state: TradingState, memory: dict) -> float:
        signal = 0.90 * float(memory.get("hydro_mark_signal", 0.0))
        for trade in state.market_trades.get(HYDROGEL, []):
            buyer = getattr(trade, "buyer", "")
            seller = getattr(trade, "seller", "")
            if buyer == "Mark 38":
                signal -= 1.0
            ...
            if seller == "Mark 14":
                signal -= 0.3
        signal = clamp(signal, -6.0, 6.0)
        memory["hydro_mark_signal"] = signal
        return clamp(signal, -1.0, 1.0)

    def _voucher_supply_scores(self, state: TradingState, memory: dict) -> Dict[str, float]:
        supply_mem = memory.setdefault("voucher_supply_signal", {})
        scores: Dict[str, float] = {}
        for product in VOUCHER_STRIKES:
            signal = 0.90 * float(supply_mem.get(product, 0.0))
            for trade in state.market_trades.get(product, []):
                if getattr(trade, "seller", "") == "Mark 22":
                    signal += min(abs(int(trade.quantity)), 20)
            signal = clamp(signal, 0.0, 50.0)
            supply_mem[product] = signal
            scores[product] = signal
        return scores
    ...
    def _velvet_breakout_down(self, state: TradingState, velvet_fair: float) -> bool:
        od = state.order_depths.get(VELVET)
        if od is None:
            return False
        mid = raw_mid(od)
    ...
    def _pair_trade_vouchers(
        ...
        if self._velvet_breakout_up(state, velvet_fair) or self._velvet_breakout_down(state, velvet_fair):
            memory.setdefault("pair_layer", {})["skipped"] = "breakout"
            return

        pair_cfgs = [
            ("VEV_5000", "VEV_5100", PAIR_MODE_5000_5100, 0.018, 0.8, 4, 0.024, 1.0, 3),
            ("VEV_5100", "VEV_5200", PAIR_MODE_5100_5200, 0.024, 1.0, 2, 0.032, 1.2, 1),
            ("VEV_5200", "VEV_5300", PAIR_MODE_5200_5300, 0.014, 0.6, 3, 0.020, 0.8, 2),
        ]
        diag = {}

        for low_product, high_product, pair_mode, rich_iv_thresh, rich_px_thresh, rich_max_qty, cheap_iv_thresh, cheap_px_thresh, cheap_max_qty in pair_cfgs:
            if pair_mode == "OFF":
                continue
            if low_product not in state.order_depths or high_product not in state.order_depths:
                continue

            low_od = state.order_depths[low_product]
            high_od = state.order_depths[high_product]
            low_mid = raw_mid(low_od)
            high_mid = raw_mid(high_od)
            low_bb = best_bid(low_od)
            low_ba = best_ask(low_od)
            high_bb = best_bid(high_od)
            high_ba = best_ask(high_od)
            if None in (low_mid, high_mid, low_bb, low_ba, high_bb, high_ba):
                continue

            low_strike = VOUCHER_STRIKES[low_product]
            high_strike = VOUCHER_STRIKES[high_product]
            low_sigma = sigmas[low_product]
            high_sigma = sigmas[high_product]
            low_fair = bs_call(velvet_fair, low_strike, tte, low_sigma)
            high_fair = bs_call(velvet_fair, high_strike, tte, high_sigma)
            if low_fair < 0.5 or high_fair < 0.5:
                continue

            low_guarded = max(float(low_mid), max(velvet_fair - low_strike, 0.0) + 1e-3)
            high_guarded = max(float(high_mid), max(velvet_fair - high_strike, 0.0) + 1e-3)
            low_iv = implied_vol(low_guarded, velvet_fair, low_strike, tte)
            high_iv = implied_vol(high_guarded, velvet_fair, high_strike, tte)
            low_resid = low_iv - low_sigma
            high_resid = high_iv - high_sigma
            pair_edge = low_resid - high_resid
            rich_exec_edge = (float(low_bb) - low_fair) + (high_fair - float(high_ba))
            cheap_exec_edge = (low_fair - float(low_ba)) + (float(high_bb) - high_fair)

            diag[f"{low_product}/{high_product}"] = {
                "mode": pair_mode,
                "pair_edge": round(pair_edge, 4),
                "rich_exec": round(rich_exec_edge, 3),
                "cheap_exec": round(cheap_exec_edge, 3),
            }

            low_buy_cap, low_sell_cap = self._remaining_caps(state, result, low_product)
            high_buy_cap, high_sell_cap = self._remaining_caps(state, result, high_product)
            low_top_bid = max(0, int(low_od.buy_orders.get(low_bb, 0)))
            high_top_ask = max(0, int(abs(high_od.sell_orders.get(high_ba, 0))))
            low_top_ask = max(0, int(abs(low_od.sell_orders.get(low_ba, 0))))
            high_top_bid = max(0, int(high_od.buy_orders.get(high_bb, 0)))

            # Rich-lower / cheap-higher: sell lower strike, buy higher strike.
            if pair_edge > rich_iv_thresh and rich_exec_edge > rich_px_thresh:
                qty = min(rich_max_qty, low_sell_cap, high_buy_cap, low_top_bid, high_top_ask)
                if qty > 0:
                    result.setdefault(low_product, []).append(Order(low_product, int(low_bb), -int(qty)))
                    result.setdefault(high_product, []).append(Order(high_product, int(high_ba), int(qty)))
                    continue

            # Only the symmetric mode lets the higher strike influence back.
            if pair_mode == "BOTH" and pair_edge < -cheap_iv_thresh and cheap_exec_edge > cheap_px_thresh:
                qty = min(cheap_max_qty, low_buy_cap, high_sell_cap, low_top_ask, high_top_bid)
                if qty > 0:
                    result.setdefault(low_product, []).append(Order(low_product, int(low_ba), int(qty)))
                    result.setdefault(high_product, []).append(Order(high_product, int(high_bb), -int(qty)))

        memory["pair_layer"] = diag

    def _reset_day(self, memory: dict, timestamp: int) -> None:
        last = memory.get("last_timestamp", -1)
        if last >= 0 and timestamp < last:
            # Timestamp rolled back → new competition day
            memory["day_count"] = int(memory.get("day_count", 0)) + 1
            memory.pop("velvet_overlay", None)
            memory.pop("deep_itm_tv", None)
            memory.pop("deep_itm_mark_bias", None)
            memory.pop("velvet_mark_signal", None)
            memory.pop("hydro_mark_signal", None)
            memory.pop("voucher_supply_signal", None)
    ...
    def run(self, state: TradingState):
        ...
        # Continuously-decaying TTE: counts completed days + intra-day fraction.
        # Persists across days via memory — same approach as top reference bots.
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                hydro_fair += self._hydro_mark_shift(state, memory)
                result[HYDROGEL] = self._trade_mm(HYDROGEL, state, hydro_fair)
            except Exception as exc:
                memory.setdefault("errors", {})["hydrogel"] = type(exc).__name__

        # VELVET — anchor market making + Olivia detector overlay + voucher-delta hedge
        ...
        if VELVET in state.order_depths:
            try:
                ...
                velvet_mark_score = self._velvet_mark_score(state, memory)
                voucher_velvet_fair = self._fair(VELVET, state.order_depths[VELVET]) + overlay_bias
                velvet_trade_fair = voucher_velvet_fair + 2.0 * velvet_mark_score
                if any(product in state.order_depths for product in VOUCHER_STRIKES):
                    ...
                    velvet_trade_fair = voucher_velvet_fair + hedge_shift + 2.0 * velvet_mark_score
                velvet_fair = velvet_trade_fair
                block_shorts = velvet_mark_score > 0.35
                result[VELVET] = self._trade_mm(
                    VELVET,
                    state,
                    velvet_trade_fair,
                    block_new_shorts=block_shorts,
                    cover_short=block_shorts,
                    ask_edge_extra=0.5 if block_shorts else 0.0,
                )
            except Exception as exc:
        ...
        if voucher_velvet_fair is not None:
            try:
                ...
                voucher_supply_scores = self._voucher_supply_scores(state, memory)
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        if product in ("VEV_4000", "VEV_4500"):
                            result[product] = self._trade_deep_itm_voucher(
                                product,
                                state,
                                voucher_velvet_fair,
                                sigmas[product],
                            ...
                            result[product] = self._trade_voucher(
                                ...
                                pair_edge_5100_5200,
                                pair_edge_5200_5300,
                                voucher_supply_scores.get(product, 0.0),
                            )
                self._pair_trade_vouchers(
                    state,
                    result,
                    voucher_velvet_fair,
                    sigmas,
