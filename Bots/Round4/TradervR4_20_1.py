...
class Trader:
    ...
    def _trade_voucher(
        ...
        # Never buy OTM calls — symmetric smile always overestimates OTM call IV,
        # creating false cheap signals as spot falls away from strike.
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            try:
                sigmas = self._build_vol_surface(state, velvet_fair, tte)
                # Compute strip delta for delta-pressure guard
                strip_delta = sum(
                    int(state.position.get(p, 0)) * bs_delta(velvet_fair, k, tte, sigmas.get(p, 0.18))
                    for p, k in VOUCHER_STRIKES.items()
                )
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        result[product] = self._trade_voucher(
                            product, state, velvet_fair, sigmas[product], strip_delta, tte
                        )
            except Exception as exc:
