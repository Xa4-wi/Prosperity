...
class Trader:
    ...
    def _trade_voucher(
        ...
        # Tiny OTM window: only near OTM, only with real breakout confirmation,
        # and only when the option still has enough delta to matter.
        if strike > velvet_fair:
            buy_signal = (
                iv_residual < -0.035
                and velvet_breakout_up
                and strip_delta < 15.0
                and strike <= velvet_fair + 100.0
                and delta >= 0.12
            )
    ...
    def run(self, state: TradingState):
        ...
        if voucher_velvet_fair is not None:
            try:
                velvet_breakout_up = self._velvet_breakout_up(state, voucher_velvet_fair)
                if sigmas is None:
                    sigmas = self._build_vol_surface(state, voucher_velvet_fair, tte)
                    strip_delta = sum(
                        int(state.position.get(p, 0)) * bs_delta(voucher_velvet_fair, k, tte, sigmas.get(p, 0.18))
