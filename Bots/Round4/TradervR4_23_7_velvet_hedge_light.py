        ...
        if VELVET in state.order_depths:
            try:
                ...
                if any(product in state.order_depths for product in VOUCHER_STRIKES):
                    sigmas = self._build_vol_surface(state, voucher_velvet_fair)
                    strip_delta = sum(
                        int(state.position.get(p, 0)) * bs_delta(voucher_velvet_fair, k, TTE_YEARS, sigmas.get(p, 0.18))
                        for p, k in VOUCHER_STRIKES.items()
                    )
                    velvet_pos = int(state.position.get(VELVET, 0))
                    hedge_target = clamp(-0.25 * strip_delta, -40.0, 40.0)
                    hedge_gap = hedge_target - velvet_pos
                    hedge_shift = clamp(hedge_gap / 35.0, -1.25, 1.25)
                ...
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        result[product] = self._trade_voucher(
                            product, state, voucher_velvet_fair, sigmas[product], strip_delta
                        )
            except Exception as exc:
