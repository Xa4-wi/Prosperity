...
class Trader:
    ...
    def _trade_voucher(
        ...
        if strike >= 5400:
            ...
            resid_threshold = 0.018
        sell_threshold = resid_threshold
        if strike == 5200:
            # Restore some v20-style impulse, but only through local thresholds/sizing.
            if pair_edge_5100_5200 > 0.010:
                resid_threshold = min(resid_threshold, 0.016)
                sell_threshold = min(sell_threshold, 0.016)
                soft_limit = max(soft_limit, 56)
                take_max = max(take_max, 12)
            elif pair_edge_5100_5200 < -0.010:
                resid_threshold = max(resid_threshold, 0.020)
                sell_threshold = max(sell_threshold, 0.020)
                soft_limit = min(soft_limit, 44)
                take_max = min(take_max, 9)
            if pair_edge_5200_5300 > 0.008:
                sell_threshold = max(0.012, sell_threshold - 0.006)
                soft_limit = max(soft_limit, 60)
                take_max = max(take_max, 12)
