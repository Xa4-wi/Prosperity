...
class Trader:
    ...
    def _voucher_orders(self, product: str, state: TradingState, surface: dict, outlook: Optional[dict], risk_state: dict) -> List[Order]:
        ...
        score = residual / edge
        if product in MID_VOUCHERS:
            target = -0.38 * cap * math.tanh(0.70 * score)
            if abs(score) < (1.20 if gamma_hot else 0.95):
                target = 0.0
        else:
            target = -0.78 * cap * math.tanh(0.85 * score)
            if abs(score) < (0.75 if underlying_like else 0.55):
                target = 0.0
        ...
        if bool(outlook.get("squeeze", False)):
            passive_gate *= 1.15
        allow_bid = long_ok and mgr.buy_cap > 0 and mgr.projected() < cap and (residual < -passive_gate * edge or mgr.projected() < -0.45 * cap)
        allow_ask = short_ok and mgr.sell_cap > 0 and mgr.projected() > -cap and (residual > passive_gate * edge or mgr.projected() > 0.45 * cap)
