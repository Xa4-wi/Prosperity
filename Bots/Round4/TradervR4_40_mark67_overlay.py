...
class Trader:
    ...
    def _trade_mm(
        ...
        proj = mgr.projected()
        if cover_short and proj < target and ba is not None and ba <= fair + cfg["clear_edge"] + 1.2:
    ...
    def _velvet_overlay(self, state: TradingState, memory: dict) -> Tuple[float, float]:
        """Detect VELVET tape signals.

        Returns (olivia_bias, mark67_score). Mark 67 score is used only by
        VELVET execution, not as a standalone market-taking trigger.
        """
        ov = memory.setdefault(
            "velvet_overlay",
            {"day_low": 1e18, "day_high": -1e18, "signal": 0.0, "age": 999, "m67_signal": 0.0},
        )
        saw = False
        m67_signal = 0.90 * float(ov.get("m67_signal", 0.0))
        for trade in sorted(state.market_trades.get(VELVET, []), key=lambda t: t.timestamp):
            ...
            if buyer == "Mark 67":
                if seller == "Mark 49":
                    ...
                    weight = 0.60
                m67_signal += weight * min(qty, 15)
            elif seller == "Mark 67":
                m67_signal -= 0.60 * min(qty, 15)
        if not saw:
            ov["age"] = int(ov["age"]) + 1
            ov["signal"] = float(ov["signal"]) * 0.96
        ov["m67_signal"] = clamp(m67_signal, -25.0, 45.0)
        olivia_bias = 0.75 * max(0.0, min(1.0, float(ov["signal"])))
        mark67_score = math.tanh(float(ov["m67_signal"]) / 25.0)
        return olivia_bias, mark67_score
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                olivia_bias, mark67_score = self._velvet_overlay(state, memory)
                voucher_velvet_fair = self._fair(VELVET, state.order_depths[VELVET]) + olivia_bias
                mark67_fair_shift = 2.0 * max(-0.5, min(1.0, mark67_score))
                velvet_trade_fair = voucher_velvet_fair + mark67_fair_shift
                velvet_target = int(35 * mark67_score)
                block_velvet_shorts = mark67_score > 0.35
                if any(product in state.order_depths for product in VOUCHER_STRIKES):
                    ...
                    velvet_target = int(clamp(hedge_target + 35 * mark67_score, -100.0, 100.0))
                    velvet_trade_fair += mark67_fair_shift
                ...
                result[VELVET] = self._trade_mm(
                    ...
                    ask_edge_extra=1.0 if block_velvet_shorts else 0.0,
                )
            except Exception as exc:
