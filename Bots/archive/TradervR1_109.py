...
class AshCoatedOsmiumTrader:
    ...
    def _queue_priority_price(
        ...
        if mode == "inventory_clear" or toxic_level >= 2:
            return raw_price

        if side == "buy":
            best_px = book.best_bid
            opp_px = book.best_ask
            best_vol = book.best_bid_vol
            second_vol = book.buy_levels[1][1] if len(book.buy_levels) > 1 else 0
            touch_edge = quote_mid - best_px
            queue_load = max(best_vol, second_vol)
            if opp_px - best_px <= 1:
                return min(raw_price, best_px)
            if (
                take_signal > 0.55
                and conviction >= 0.55
                and touch_edge >= edge_need + 0.35
                and queue_load <= 18
            ):
                return min(best_px + 1, opp_px - 1)
            if queue_load >= 28 and take_signal < 0.20 and conviction < 0.80:
                return min(raw_price, max(1, best_px - 1))
            if touch_edge >= edge_need and queue_load <= 28:
                return max(raw_price, best_px)
            return raw_price

        best_px = book.best_ask
        opp_px = book.best_bid
        best_vol = book.best_ask_vol
        second_vol = book.sell_levels[1][1] if len(book.sell_levels) > 1 else 0
        touch_edge = best_px - quote_mid
        queue_load = max(best_vol, second_vol)
        if best_px - opp_px <= 1:
            return max(raw_price, best_px)
        if (
            ...
            return max(best_px - 1, opp_px + 1)
        if queue_load >= 28 and take_signal > -0.20 and conviction < 0.80:
            return max(raw_price, best_px + 1)
        if touch_edge >= edge_need and queue_load <= 28:
            return min(raw_price, best_px)
        return raw_price

    def _catalyst_state(self, book: Book, take_signal: float) -> Tuple[int, float]:
        stable_mid = self._stable_mid(book)
        stable_gap = stable_mid - book.mid
        micro_gap = book.micro - book.mid
        half_spread = max(1.0, book.spread_val / 2.0)
        imbalance_signal = half_spread * book.imbalance

        micro_sign = sign(micro_gap, 0.20)
        imbalance_sign = sign(imbalance_signal, 0.18)
        stable_sign = sign(stable_gap, 0.40)

        direction = 0
        strength = 0.0

        if book.spread_val <= 16 and micro_sign != 0 and micro_sign == imbalance_sign:
            direction = micro_sign
            strength += 0.35
            if abs(micro_gap) >= 0.40:
                strength += 0.20
            if abs(imbalance_signal) >= 0.50:
                strength += 0.20
            if stable_sign == direction and abs(stable_gap) >= 0.50:
                strength += 0.20
            elif stable_sign == -direction and abs(stable_gap) >= 0.75:
                strength -= 0.15
        elif stable_sign != 0 and stable_sign == imbalance_sign and abs(stable_gap) >= 1.25:
            direction = stable_sign
            strength += 0.45

        if direction == 0:
            return 0, 0.0
        if direction > 0 and take_signal < -0.15:
            strength -= 0.20
        elif direction < 0 and take_signal > 0.15:
            strength -= 0.20
        return direction, clamp(strength, 0.0, 1.0)
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        bid_level, ask_level, memory = self._smoothed_toxicity_levels(
            raw_bid_level, raw_ask_level, pos, soft, memory, timestamp
        )
        attack = clamp(
        ...
        catalyst_dir, catalyst_strength = self._catalyst_state(book, take_signal)
        catalyst_active = mode == "normal" and catalyst_strength >= 0.45
        ...
        if catalyst_active:
            if catalyst_dir > 0 and bid_level == 0:
                buy_need = max(0.70, buy_need - 0.30 * catalyst_strength)
            elif catalyst_dir < 0 and ask_level == 0:
                sell_need = max(0.70, sell_need - 0.30 * catalyst_strength)
        ...
        if catalyst_active:
            if catalyst_dir > 0 and bid_level == 0:
                buy_qe -= 0.40 * catalyst_strength
                sell_qe += 0.10 * catalyst_strength
            elif catalyst_dir < 0 and ask_level == 0:
                buy_qe += 0.10 * catalyst_strength
                sell_qe -= 0.40 * catalyst_strength
        ...
        if catalyst_active:
            if catalyst_dir > 0 and bid_level == 0:
                front_buy = max(front_buy, book.best_bid + (1 if catalyst_strength >= 0.75 else 0))
            elif catalyst_dir < 0 and ask_level == 0:
                front_sell = min(front_sell, book.best_ask - (1 if catalyst_strength >= 0.75 else 0))

        front_buy = self._queue_priority_price(
            "buy",
            book,
            front_buy,
            quote_mid,
            buy_qe,
            take_signal,
            signal_conviction + 0.25 * catalyst_strength,
            bid_level,
            mode,
        )
        front_sell = self._queue_priority_price(
            "sell",
            book,
            front_sell,
            quote_mid,
        ...
        if catalyst_active:
            if catalyst_dir > 0 and bid_level == 0:
                buy_front_sz += 1 + int(catalyst_strength >= 0.80)
            elif catalyst_dir < 0 and ask_level == 0:
                sell_front_sz += 1 + int(catalyst_strength >= 0.80)
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
