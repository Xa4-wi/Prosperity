...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "ANCHOR_PHI": 0.52,
    "ANCHOR_BETA": 0.16,
    "ANCHOR_CAP_RATIO": 0.28,
    "ANCHOR_TARGET_BLEND": 0.18,
    "ANCHOR_RESERVATION_WEIGHT": 0.10,
    "WRONG_WAY_RESERVATION_MULT": 1.55,
    "ALIGNED_RESERVATION_MULT": 0.68,
    "ANCHOR_QUOTE_EDGE_WEIGHT": 0.10,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def signal_confidence(self, predicted_edge: float, fit_quality: float) -> float:
        ...
        normalized_edge = min(1.0, abs(predicted_edge) / (1.75 * scale))
        queue_bias, _queue_activity = self.queue_reactive_signal()
        lob_signal = self.filtered_lob_signal()
        edge_sign = 0.0
        ...
        agreement = 0.0
        if edge_sign != 0.0:
            for signal, scale_value in (
                (self.imbalance, 1.0),
                (queue_bias, 1.0),
                (lob_signal, 1.0),
                (self.momentum, max(1.0, scale)),
            ):
                if signal * edge_sign > 0:
                    agreement += min(1.0, abs(signal) / scale_value)
                elif signal * edge_sign < 0:
                    agreement -= min(1.0, abs(signal) / scale_value)
        confidence = 0.45 * fit_quality + 0.30 * normalized_edge + 0.25 * ((agreement + 4.0) / 8.0)
    ...
    def anchor_target(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        scale = max(1.0, float(self.spread) / 2.0)
        signal = max(-1.0, min(1.0, predicted_edge / (2.0 * scale))) * max(0.25, confidence)
        cap = float(self.soft_limit) * self.ANCHOR_CAP_RATIO
        desired = cap * signal
        if regime == "range":
            desired *= 0.45
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float, target_position: int) -> float:
        ...
        edge += self.ANCHOR_QUOTE_EDGE_WEIGHT * abs(position - self.current_anchor_target()) / max(1.0, float(self.soft_limit))
