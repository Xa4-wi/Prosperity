...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "CHAOS_LOW_THRESHOLD": 0.28,
    "CHAOS_HIGH_THRESHOLD": 0.68,
    "CHAOS_QUOTE_WIDEN": 0.42,
    "CHAOS_QUOTE_TIGHTEN": 0.10,
    "CHAOS_TAKE_EDGE_PENALTY": 0.26,
    "CHAOS_PASSIVE_SIZE_PENALTY": 2.0,
    ...
    def run(self) -> List[Order]:
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                quantity = min(
                    self.passive_size("BUY", regime, volatility, chaos_metrics["score"], chaos_metrics["state"]),
                    self.buy_capacity,
                )
                if regime != "range":
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                quantity = min(
                    self.passive_size("SELL", regime, volatility, chaos_metrics["score"], chaos_metrics["state"]),
                    self.sell_capacity,
                )
                if regime != "range":
