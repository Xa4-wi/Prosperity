...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.18,
    "ANCHOR_CAP_RATIO": 0.35,
    "ANCHOR_TARGET_BLEND": 0.18,
    "ANCHOR_RESERVATION_WEIGHT": 0.06,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def signal_confidence(self, predicted_edge: float, fit_quality: float) -> float:
        ...
        confidence = 0.60 * fit_quality + 0.25 * normalized_edge + 0.15 * max(0.0, self.imbalance * (1.0 if predicted_edge >= 0 else -1.0))
    ...
    def anchor_target(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        scale = max(1.0, float(self.spread) / 2.0)
        signal = max(-1.0, min(1.0, predicted_edge / (2.0 * scale))) * max(0.30, confidence)
        cap = float(self.soft_limit) * self.ANCHOR_CAP_RATIO
        desired = cap * signal
        if regime == "range":
            desired *= 0.40
