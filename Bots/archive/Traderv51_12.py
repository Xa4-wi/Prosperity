...
class TomatoesBot:
    ...
    EXEC_AGGRESSIVE_GAP = 12
    EXEC_PASSIVE_GAP = 5
    EXEC_REDUCE_GAP = 8
    EXEC_TAKE_BONUS = 0.16
    EXEC_SIZE_BONUS = 2
    ...
    def execution_plan(
        ...
        if position - target >= self.EXEC_REDUCE_GAP:
            plan["sell_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["buy_take_bias"] = 0.08
            plan["allow_buy_passive"] = False
            plan["sell_size_bonus"] = self.EXEC_SIZE_BONUS
            return plan
        if target - position >= self.EXEC_REDUCE_GAP and position < 0:
            plan["buy_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["sell_take_bias"] = 0.08
            plan["allow_sell_passive"] = False
            plan["buy_size_bonus"] = self.EXEC_SIZE_BONUS
            return plan

        if regime in {"trend_up", "strong_up"} and gap >= self.EXEC_AGGRESSIVE_GAP and predicted_edge > 0 and fit_quality >= self.FIT_THRESHOLD:
            plan["buy_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["sell_take_bias"] = 0.08
            plan["buy_size_bonus"] = self.EXEC_SIZE_BONUS
        elif regime in {"trend_down", "strong_down"} and -gap >= self.EXEC_AGGRESSIVE_GAP and predicted_edge < 0 and fit_quality >= self.FIT_THRESHOLD:
            plan["sell_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["buy_take_bias"] = 0.08
            plan["sell_size_bonus"] = self.EXEC_SIZE_BONUS
    ...
    def take_threshold(
        ...
        if regime in {"strong_up", "strong_down"} and fit_quality < 0.65:
            threshold += 0.10
        if side == "BUY":
            threshold += float(execution_plan.get("buy_take_bias", 0.0))
        else:
            threshold += float(execution_plan.get("sell_take_bias", 0.0))
    ...
    def take_orders(
        ...
        buy_threshold = self.take_threshold("BUY", regime, target, predicted_edge, fit_quality, volatility, execution_plan)
        sell_threshold = self.take_threshold("SELL", regime, target, predicted_edge, fit_quality, volatility, execution_plan)
    ...
    def allow_passive(self, side: str, regime: str, target: int, execution_plan: Dict[str, object]) -> bool:
        position = self.manager.projected_position()
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        execution_plan = self.execution_plan(regime, target, predicted_edge, fit_quality, volatility)
        fair = self.fair_value(regime, target, predicted_now, predicted_next, guarded_alpha)
        reservation = self.reservation(fair, target)

        self.take_orders(reservation, regime, target, predicted_edge, fit_quality, volatility, execution_plan)
        ...
        if (
            ...
            and self.allow_passive("BUY", regime, target, execution_plan)
        ):
            size = min(
                self.passive_size("BUY", regime, target, volatility, fit_quality, execution_plan),
        ...
        if (
            ...
            and self.allow_passive("SELL", regime, target, execution_plan)
        ):
            size = min(
                self.passive_size("SELL", regime, target, volatility, fit_quality, execution_plan),
