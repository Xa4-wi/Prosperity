...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "FORCE_RANGE_SPRING_WEIGHT": 0.18,
    "FORCE_RESONANCE_WEIGHT": 0.16,
    "FORCE_RESTORING_WEIGHT": 0.12,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def physics_force(
        ...
        if driving * velocity > 0:
            resonance += math.copysign(
                min(abs(driving), abs(velocity) + 0.25 * abs(acceleration)),
                driving,
            )
        if driving * flow_metrics["bias"] > 0:
            resonance += math.copysign(
                0.35 * min(abs(driving), abs(flow_metrics["bias"])),
                driving,
            )
        restoring = 0.0
        if spring * velocity > 0:
            restoring = math.copysign(
                min(abs(spring), abs(velocity) + 0.20 * abs(acceleration)),
                spring,
            )
        return {
    ...
    def fair_value(
        ...
        fair += self.FORCE_FAIR_WEIGHT * physics_force.get("contrib", physics_force["net"]) * half_spread
    ...
    def quote_edge(
        ...
        if regime == "range" and volatility <= 1.65 and abs(breakout_score) <= 0.30:
            edge -= 0.12
    ...
    def run(self) -> List[Order]:
        ...
        quiet_range = (
            abs(regression_edge) < self.TREND_EDGE_THRESHOLD
            and abs(breakout_score) < 0.55
            and abs(flow_metrics["bias"]) < 0.16
        )
        force_contrib = self.FORCE_BLEND_WEIGHT * physics_force["net"]
        if quiet_range:
            force_contrib += self.FORCE_RANGE_SPRING_WEIGHT * physics_force["spring"]
            force_contrib += self.FORCE_RESTORING_WEIGHT * physics_force["restoring"]
        if physics_force["resonance"] * regression_edge > 0 or physics_force["resonance"] * breakout_score > 0:
            force_contrib += self.FORCE_RESONANCE_WEIGHT * physics_force["resonance"]
        if physics_force["net"] * regression_edge < 0 and physics_force["net"] * breakout_score < 0:
            force_contrib *= 0.75
        elif physics_force["net"] * regression_edge < 0:
            force_contrib *= 0.88
        physics_force["contrib"] = max(-self.FORCE_CAP, min(self.FORCE_CAP, force_contrib))
        _hybrid_fair, hybrid_alpha = self.hybrid_alpha()
        provisional_edge = (
            regression_edge
            + (self.BREAKOUT_FOLLOW_SCALE * breakout_score)
            + (0.20 * pressure_bias)
            + physics_force["contrib"]
        )
        ...
        self.diagnostic_context = {
            ...
            "physics_force": physics_force["contrib"],
