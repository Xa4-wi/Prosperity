...
DEFAULT_ASH_PARAMS = {
    ...
    "MICRO_DOMINANCE_WEIGHT": 1.25,
    "MICRO_DOMINANCE_IMB_WEIGHT": 0.95,
    "AGREE_BASE_EDGE": 0.50,
    "AGREE_MEDIUM_EDGE": 1.00,
    "STABLE_MAGNET_EDGE": 1.0,
    "STABLE_STRONG_EDGE": 1.25,
    "AGREE_SIGNAL_BONUS": 0.60,
    "MAGNET_SIGNAL_BONUS": 0.20,
    "DISAGREE_STABLE_DAMP": 0.00,
    "DISAGREE_IMBALANCE_BOOST": 0.20,
    "TRADE_CONFIRM_BONUS": 0.00,
...
class AshCoatedOsmiumTrader:
    ...
    def _signal_components(
        ...
        if tight_book and abs(micro_gap) >= float(self.p["MICRO_IMBALANCE_FLOOR"]):
            ...
            base_signal = (
                float(self.p["MICRO_DOMINANCE_WEIGHT"]) * micro_gap
                + float(self.p["MICRO_DOMINANCE_IMB_WEIGHT"]) * imbalance_signal
            )
        ...
        agree = (
            ...
            and abs(stable_gap) >= float(self.p["AGREE_BASE_EDGE"])
        )
        disagree = (
            stable_sign != 0
        ...
        conviction = 0.0
        if agree:
            gap_abs = abs(stable_gap)
            if gap_abs >= float(self.p["STABLE_STRONG_EDGE"]):
                fast_signal += (
                    float(self.p["AGREE_SIGNAL_BONUS"]) + float(self.p["MAGNET_SIGNAL_BONUS"])
                ) * stable_gap
                conviction += 0.55
            elif gap_abs >= float(self.p["AGREE_MEDIUM_EDGE"]):
                fast_signal += float(self.p["AGREE_SIGNAL_BONUS"]) * stable_gap
                conviction += 0.35
            else:
                fast_signal += 0.30 * stable_gap
                conviction += 0.18
            if gap_abs >= float(self.p["STABLE_MAGNET_EDGE"]):
                fast_signal += float(self.p["MAGNET_SIGNAL_BONUS"]) * stable_gap
                conviction += 0.10
        elif disagree:
            # In disagreement states the queue wins. Push harder toward the
            # imbalance/micro signal instead of preserving stable-book pull.
            fast_signal += float(self.p["DISAGREE_IMBALANCE_BOOST"]) * imbalance_signal
            conviction += 0.14
        ...
        if (
            float(self.p["TRADE_CONFIRM_BONUS"]) > 0.0
            and trade_confirm != 0.0
            and sign(trade_confirm, 0.05) == imbalance_sign
        ):
