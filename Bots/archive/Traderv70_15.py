...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "GRAVITY_CAP": 1.40,
    "HOLD_TARGET_FRACTION": 0.45,
    "ADVERSE_INVENTORY_EDGE": 1.15,
    "ADVERSE_INVENTORY_BREAKOUT": 0.85,
    "ADVERSE_INVENTORY_POSITION": 12,
    "ADVERSE_RISK_REDUCE_BONUS": 0.18,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def adjusted_fair_value(
        ...
        position = self.projected_position()
        inventory_anchor = float(position)
        if regime == "trend_up" and target_position > 0 and position > 0:
            inventory_anchor = max(0.0, inventory_anchor - (self.HOLD_TARGET_FRACTION * float(target_position)))
        elif regime == "trend_down" and target_position < 0 and position < 0:
            inventory_anchor = min(0.0, inventory_anchor - (self.HOLD_TARGET_FRACTION * float(target_position)))
        return fair - (inventory_anchor * self.INVENTORY_SKEW)
    ...
    def take_orders(
        ...
        if (
            self.projected_position() < 0
            and (
                predicted_edge >= self.ADVERSE_INVENTORY_EDGE
                or breakout_score >= self.ADVERSE_INVENTORY_BREAKOUT
            )
        ):
            buy_threshold += self.ADVERSE_RISK_REDUCE_BONUS
        ...
        if (
            ...
            sell_threshold -= self.ADVERSE_RISK_REDUCE_BONUS
    ...
    def run(self) -> List[Order]:
        ...
        position = self.projected_position()
        adverse_short = (
            position <= -int(self.ADVERSE_INVENTORY_POSITION)
            and (
                predicted_edge >= self.ADVERSE_INVENTORY_EDGE
                or breakout_score >= self.ADVERSE_INVENTORY_BREAKOUT
            )
        ...
        if adverse_short:
            sell_quote = None
            if buy_quote is not None:
                buy_quote = min(int(self.best_ask) - 1, buy_quote + 1)
        if adverse_long:
            buy_quote = None
            if sell_quote is not None:
                sell_quote = max(int(self.best_bid) + 1, sell_quote - 1)
