...
DEFAULT_ASH_PARAMS = {
    ...
    "PLATEAU_WAIT_TICKS": 900,
    "PLATEAU_FLAT_POS_MAX": 12,
    "PLATEAU_SPREAD_MAX": 17.0,
    "PLATEAU_DEPTH_MIN": 20.0,
    "PLATEAU_EDGE_BONUS": 0.18,
    "PLATEAU_JOIN_BONUS": 0.22,
    "PLATEAU_FRONT_SIZE_BONUS": 1,
    "PLATEAU_TAKE_RELIEF": 0.08,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        plateau_mode = (
            ...
            and book.spread_val <= float(self.p["PLATEAU_SPREAD_MAX"])
        ...
        if plateau_mode:
            attack = clamp(attack + 0.10, 0.0, 1.0)
        ...
        if plateau_mode:
            buy_need = max(0.70, buy_need - float(self.p["PLATEAU_TAKE_RELIEF"]))
            sell_need = max(0.70, sell_need - float(self.p["PLATEAU_TAKE_RELIEF"]))
        ...
        if plateau_mode:
            buy_qe -= float(self.p["PLATEAU_EDGE_BONUS"])
            sell_qe -= float(self.p["PLATEAU_EDGE_BONUS"])
        ...
        if plateau_mode:
            join_edge += float(self.p["PLATEAU_JOIN_BONUS"])
