...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.apply_parameter_overrides(self.PARAMETER_DEFAULTS, params)
        self.soft_limit = int(position_limit * self.SOFT_LIMIT_RATIO)
        self.current_timestamp = self.get_timestamp_marker()
    ...
    def get_timestamp_marker(self) -> Optional[int]:
    ...
    def handle_day_reset(self) -> None:
        previous_timestamp = self.memory.get("last_timestamp")
        if self.current_timestamp is None or previous_timestamp is None:
            return
        try:
            previous_timestamp_int = int(previous_timestamp)
        except (TypeError, ValueError):
            return
        if self.current_timestamp < previous_timestamp_int:
            self.memory = {}
    ...
    def next_memory(self) -> Dict[str, object]:
        ...
        if self.current_timestamp is not None:
            data["last_timestamp"] = int(self.current_timestamp)
        return data
