...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MICRO_WEIGHT": 0.30,
    "HISTORY_WEIGHT": 0.25,
    "REGRESSION_WEIGHT": 0.08,
    "BLACK_SCHOLES_WEIGHT": 0.12,
    ...
    "BS_STRIKE_HISTORY_WEIGHT": 0.55,
    "BS_STRIKE_REGRESSION_WEIGHT": 0.45,
    "BS_SIGMA_MULTIPLIER": 10.0,
    "BS_SIGMA_FLOOR": 0.0015,
    "BS_SIGMA_CEILING": 0.0140,
    "BS_BIAS_SCALE": 2.2,
    "BS_BIAS_CAP": 1.6,
    "BS_TIME_FLOOR": 0.06,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def fair_value(
        ...
        bs_fair = self.black_scholes_fair(predicted_next)
        ...
        fair = (
            ...
            + self.BLACK_SCHOLES_WEIGHT * bs_fair
    ...
    def black_scholes_fair(self, predicted_next: float) -> float:
        spot = max(1.0, float(self.mid))
        strike = (
            self.BS_STRIKE_HISTORY_WEIGHT * float(self.recent_average)
            + self.BS_STRIKE_REGRESSION_WEIGHT * predicted_next
        )
        strike = max(1.0, strike)
        sigma_points = max(1.0, float(getattr(self, "volatility", 1.0)))
        sigma = min(
            self.BS_SIGMA_CEILING,
            max(self.BS_SIGMA_FLOOR, (sigma_points / spot) * self.BS_SIGMA_MULTIPLIER),
        )
        tau = max(self.BS_TIME_FLOOR, min(1.0, self.time_fraction_remaining()))
        denom = max(1e-9, sigma * math.sqrt(tau))
        d1 = (math.log(spot / strike) + 0.5 * sigma * sigma * tau) / denom
        directional_probability = (2.0 * normal_cdf(d1)) - 1.0
        bias_points = directional_probability * sigma_points * self.BS_BIAS_SCALE
        bias_points = max(-self.BS_BIAS_CAP, min(self.BS_BIAS_CAP, bias_points))
