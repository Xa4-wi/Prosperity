...
DEFAULT_ASH_PARAMS = {
    ...
    "PLATEAU_WAIT_TICKS": 300,
    "PLATEAU_FLAT_POS_MAX": 12,
    "PLATEAU_SPREAD_MIN": 15.0,
    "PLATEAU_SPREAD_MAX": 18.0,
    "PLATEAU_DEPTH_MIN": 20.0,
    "PLATEAU_SIGNAL_MIN": 0.35,
    "PLATEAU_TAKER_SIZE": 2,
    "PLATEAU_FRONT_SIZE_BONUS": 1,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        drought_age = timestamp - last_fill_ts if last_fill_ts >= 0 else timestamp
        plateau_mode = (
            prev_vacuum_side not in ("bid_only", "ask_only")
            and abs(pos) <= int(self.p["PLATEAU_FLAT_POS_MAX"])
            and drought_age >= int(self.p["PLATEAU_WAIT_TICKS"])
            and float(self.p["PLATEAU_SPREAD_MIN"]) <= book.spread_val <= float(self.p["PLATEAU_SPREAD_MAX"])
            and (book.best_bid_vol + book.best_ask_vol) >= float(self.p["PLATEAU_DEPTH_MIN"])
        ...
        if plateau_mode and can_take_buy and take_signal > float(self.p["PLATEAU_SIGNAL_MIN"]):
            mgr.buy(book.best_ask, min(int(self.p["PLATEAU_TAKER_SIZE"]), book.best_ask_vol))
        elif plateau_mode and can_take_sell and take_signal < -float(self.p["PLATEAU_SIGNAL_MIN"]):
            mgr.sell(book.best_bid, min(int(self.p["PLATEAU_TAKER_SIZE"]), book.best_bid_vol))
        ...
        if signal_conviction >= 0.95:
            if quote_signal > 0.0:
                ...
                sell_front_sz += 1
        if plateau_mode:
            buy_front_sz += int(self.p["PLATEAU_FRONT_SIZE_BONUS"])
            sell_front_sz += int(self.p["PLATEAU_FRONT_SIZE_BONUS"])
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
        ...
        if allow_bid and front_buy > 0 and front_buy < book.best_ask:
            ...
            if not plateau_mode and back_buy > 0 and back_buy < book.best_ask:
                mgr.buy(back_buy, buy_back_sz)
        if allow_ask and front_sell > book.best_bid:
            mgr.sell(front_sell, sell_front_sz)
            if not plateau_mode and back_sell > book.best_bid:
        ...
        ash_state["plateau_mode"] = bool(plateau_mode)
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
