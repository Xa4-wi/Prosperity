...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "HAWKES_BURST_WEIGHT": 0.22,
    "HAWKES_ALPHA_WEIGHT": 0.08,
    "HAWKES_FAIR_WEIGHT": 0.08,
    "HAWKES_EDGE_SHIFT": 0.12,
    "HAWKES_SIGNAL_ALPHA": 0.26,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def hawkes_burst_signal(self, flow_metrics: Dict[str, float]) -> float:
        previous = self.previous_book_state()
        buy_trades, sell_trades = self.trade_pressure()
        buy_impulse = 0.50 * buy_trades
        sell_impulse = 0.50 * sell_trades
        ...
        if previous:
            ...
            if float(self.best_bid) > float(previous["best_bid"]):
                ...
                bid_depletion = max(0.0, float(previous["best_bid_volume"]) - float(self.best_bid_volume)) / scale
                bid_rebuild = max(0.0, float(self.best_bid_volume) - float(previous["best_bid_volume"])) / scale
                sell_impulse += 0.90 * bid_depletion
                buy_impulse += 0.30 * bid_rebuild
            ...
            if float(self.best_ask) < float(previous["best_ask"]):
                ...
                ask_depletion = max(0.0, float(previous["best_ask_volume"]) - float(self.best_ask_volume)) / scale
                ask_rebuild = max(0.0, float(self.best_ask_volume) - float(previous["best_ask_volume"])) / scale
                buy_impulse += 0.90 * ask_depletion
                sell_impulse += 0.30 * ask_rebuild

        buy_impulse += 0.25 * max(0.0, flow_metrics["signed_flow"])
        sell_impulse += 0.25 * max(0.0, -flow_metrics["signed_flow"])
        total = buy_impulse + sell_impulse
        raw_bias = 0.0 if total <= 1e-9 else (buy_impulse - sell_impulse) / total
        alpha = float(self.HAWKES_SIGNAL_ALPHA)
        prev_bias = float(self.product_memory.get("hawkes_bias_ema", 0.0))
        prev_activity = float(self.product_memory.get("hawkes_activity_ema", 0.0))
        bias_ema = (1.0 - alpha) * prev_bias + alpha * raw_bias
        activity_ema = (1.0 - alpha) * prev_activity + alpha * min(2.0, total)
        self.product_memory["hawkes_bias_ema"] = bias_ema
        self.product_memory["hawkes_activity_ema"] = activity_ema
        return bias_ema * min(1.8, activity_ema)
    ...
    def hybrid_alpha(self, hawkes_score: float = 0.0) -> Tuple[float, float]:
    ...
    def take_edge(
        ...
        if hawkes_score != 0.0:
            ...
            if aligned:
                edge -= self.HAWKES_EDGE_SHIFT * min(1.0, abs(hawkes_score))
            else:
                edge += 0.05 * min(1.0, abs(hawkes_score))
    ...
    def quote_edge(
        ...
        if hawkes_score != 0.0 and regime != "volatile":
            edge -= 0.10 * min(1.0, abs(hawkes_score))
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(regime, volatility, fit_quality, breakout_score, hawkes_score)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, hawkes_score: float = 0.0) -> int:
    ...
    def run(self) -> List[Order]:
        ...
        hawkes_score = self.hawkes_burst_signal(flow_metrics)
        ...
        breakout_score += self.HAWKES_BURST_WEIGHT * hawkes_score
