...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "QUEUE_LAYER_MIN_SPREAD": 6.0,
    "QUEUE_LAYER_VOL_MAX": 1.9,
    "QUEUE_LAYER_FIT_MIN": 0.30,
    "QUEUE_LAYER_FRACTION": 0.35,
    "PROTECTED_MARK_PREDICTED_WEIGHT": 0.45,
    "PROTECTED_MARK_HISTORY_WEIGHT": 0.35,
    "PROTECTED_MARK_MID_WEIGHT": 0.20,
    "PROTECTION_BAND": 1.35,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def allow_queue_layer(
        ...
        return (
            regime == "range"
            and float(self.spread) >= self.QUEUE_LAYER_MIN_SPREAD
            and volatility <= self.QUEUE_LAYER_VOL_MAX
            and fit_quality >= self.QUEUE_LAYER_FIT_MIN
            and abs(breakout_score) <= 0.45
        )

    def protected_mark(self, predicted_now: float) -> float:
        return (
            self.PROTECTED_MARK_PREDICTED_WEIGHT * predicted_now
            + self.PROTECTED_MARK_HISTORY_WEIGHT * float(self.recent_average)
            + self.PROTECTED_MARK_MID_WEIGHT * float(self.mid)
        )
    ...
    def run(self) -> List[Order]:
        ...
        protected_mark = self.protected_mark(predicted_now)
        half_spread = max(1.0, float(self.spread) / 2.0)
        protection_band = (self.PROTECTION_BAND + 0.15 * volatility + 0.10 * abs(breakout_score)) * half_spread
        adjusted_fair = max(protected_mark - protection_band, min(protected_mark + protection_band, adjusted_fair))
        self.diagnostic_context = {
            "regime": regime,
            "target_position": target_position,
            "adjusted_fair": adjusted_fair,
            "protected_mark": protected_mark,
        ...
        layered_range = self.allow_queue_layer(regime, fit_quality, volatility, breakout_score)
        position = self.projected_position()
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                ...
                if layered_range and quantity >= 3:
                    back = max(1, int(quantity * self.QUEUE_LAYER_FRACTION))
                    front = max(1, quantity - back)
                    self.add_buy(buy_quote, front)
                    back_quote = max(1, int(buy_quote) - 1)
                    self.add_buy(back_quote, back)
                else:
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                ...
                if layered_range and quantity >= 3:
                    back = max(1, int(quantity * self.QUEUE_LAYER_FRACTION))
                    front = max(1, quantity - back)
                    self.add_sell(sell_quote, front)
                    self.add_sell(int(sell_quote) + 1, back)
                else:
