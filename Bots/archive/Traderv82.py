...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "CONFIDENCE_TARGET_SCALE": 0.45,
    "EXIT_URGENCY_QUOTE_SHIFT": 0.35,
    "EXIT_URGENCY_TAKE_SHIFT": 0.20,
    "EXIT_URGENCY_SIZE_BONUS": 2.0,
    "WRONGWAY_GAP_MULTIPLIER": 1.20,
    "ALIGNED_GAP_MULTIPLIER": 0.82,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def gap_pressure(
        ...
        queue_bias, _queue_activity = self.queue_reactive_signal()
        lob_signal = self.filtered_lob_signal()
        signal_bias = 0.55 * lob_signal + 0.45 * queue_bias
        if gap * signal_bias > 0:
            curved *= self.WRONGWAY_GAP_MULTIPLIER
        elif gap * signal_bias < 0:
            curved *= self.ALIGNED_GAP_MULTIPLIER
        if regime == "range":
            curved *= 1.12
    ...
    def signal_confidence(self, predicted_edge: float, fit_quality: float) -> float:
        ...
        agreement = 0.0
        if edge_sign != 0.0:
            for sig in (self.imbalance, queue_bias, lob_signal):
                if sig * edge_sign > 0:
                    agreement += min(1.0, abs(sig))
                elif sig * edge_sign < 0:
                    agreement -= min(1.0, abs(sig))
        normalized_edge = min(1.0, abs(predicted_edge) / (1.25 * scale))
        confidence = 0.45 * fit_quality + 0.30 * normalized_edge + 0.25 * ((agreement + 3.0) / 6.0)
    ...
    def exit_urgency(
        ...
        position = self.projected_position()
        excess = 0.0
        if side == "SELL" and position > target_position:
            excess = (position - target_position) / max(1.0, float(self.soft_limit))
        elif side == "BUY" and position < target_position:
            excess = (target_position - position) / max(1.0, float(self.soft_limit))
        if excess <= 0.0:
            return 0.0

        confidence = self.signal_confidence(predicted_edge, fit_quality)
        directional_penalty = 0.0
        if side == "SELL" and predicted_edge > 0:
            directional_penalty = min(1.0, abs(predicted_edge) / max(1.0, float(self.spread)))
        elif side == "BUY" and predicted_edge < 0:
            directional_penalty = min(1.0, abs(predicted_edge) / max(1.0, float(self.spread)))

        urgency = excess * (1.0 - 0.65 * confidence) * (1.0 - 0.55 * directional_penalty)
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        confidence = self.signal_confidence(predicted_edge, fit_quality)
        if regime != "volatile":
            compression = 1.0 - self.CONFIDENCE_TARGET_SCALE * max(0.0, 0.60 - confidence) / 0.60
            lower = int(round(lower * compression))
            upper = int(round(upper * compression))
        if position < lower:
            return lower
        if position > upper:
            return upper
    ...
    def take_edge(
        ...
        edge -= self.EXIT_URGENCY_TAKE_SHIFT * self.exit_urgency(side, self.target_position(regime, predicted_edge, fit_quality), predicted_edge, fit_quality)

        return max(0.5, edge)

    def quote_edge(self, regime: str, volatility: float, fit_quality: float, target_position: int) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
        ...
        sell_urgency = self.exit_urgency("SELL", target_position, (1.0 if regime == "trend_up" else -1.0 if regime == "trend_down" else 0.0) * max(0.0, fit_quality), fit_quality)
        buy_urgency = self.exit_urgency("BUY", target_position, (1.0 if regime == "trend_up" else -1.0 if regime == "trend_down" else 0.0) * max(0.0, fit_quality), fit_quality)
        edge -= 0.5 * self.EXIT_URGENCY_QUOTE_SHIFT * max(buy_urgency, sell_urgency)
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, target_position))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, target_position))
        buy_quote, sell_quote = self.clamp_inside_spread(buy_quote, sell_quote)

        position = self.projected_position()
        buy_urgency = self.exit_urgency("BUY", target_position, predicted_edge, fit_quality)
        sell_urgency = self.exit_urgency("SELL", target_position, predicted_edge, fit_quality)
        if regime == "trend_up":
            ...
            if sell_quote is not None and position > 0:
                ...
                sell_quote += lift
            if sell_quote is not None and sell_urgency > 0.0:
                sell_quote -= int(round(1.0 + sell_urgency))
        elif regime == "trend_down":
            ...
            if buy_quote is not None and position < 0:
                ...
                if predicted_edge <= -self.STRONG_TREND_EDGE:
                    drop += 1
                buy_quote -= drop
            if buy_quote is not None and buy_urgency > 0.0:
                buy_quote += int(round(1.0 + buy_urgency))
        elif regime == "volatile" and abs(position) <= 6:
            buy_quote = None
            sell_quote = None
        else:
            if buy_quote is not None and buy_urgency > 0.45:
                buy_quote += 1
            if sell_quote is not None and sell_urgency > 0.45:
                sell_quote -= 1
    ...
    def take_orders(
        ...
        if (
            ...
            take_limit += int(round(self.EXIT_URGENCY_SIZE_BONUS * buy_urgency))
        ...
        if (
            ...
            take_limit += int(round(self.EXIT_URGENCY_SIZE_BONUS * sell_urgency))
            required_bonus = 0.0
