...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "UNCERTAINTY_QUOTE_WEIGHT": 0.55,
    "UNCERTAINTY_TAKE_WEIGHT": 0.18,
    "UNCERTAINTY_SIZE_WEIGHT": 0.18,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def uncertainty_score(
        ...
        spread_component = min(1.0, float(self.spread) / max(1.0, self.TOXIC_SPREAD_THRESHOLD))
        volatility_component = min(1.0, volatility / max(1.0, self.TOXIC_VOLATILITY_THRESHOLD))
        fit_component = max(0.0, 1.0 - fit_quality)
        conflict_component = 0.0
        if regression_edge * hybrid_alpha < 0:
            conflict_component += 0.55
        if predicted_edge * self.imbalance < 0:
            conflict_component += 0.25
        if hybrid_alpha * self.momentum < 0:
            conflict_component += 0.20
        conflict_component = min(1.0, conflict_component)
        score = (
            0.32 * fit_component
            + 0.30 * conflict_component
    ...
    def take_edge(
        ...
        edge += self.UNCERTAINTY_TAKE_WEIGHT * uncertainty
        return max(0.5, edge)

    def quote_edge(self, regime: str, volatility: float, fit_quality: float, uncertainty: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
        ...
        edge += self.UNCERTAINTY_QUOTE_WEIGHT * uncertainty
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, uncertainty))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, uncertainty))
    ...
    def passive_size(self, side: str, regime: str, volatility: float, uncertainty: float) -> int:
        ...
        size = max(1, int(round(size * (1.0 - self.UNCERTAINTY_SIZE_WEIGHT * uncertainty))))
        return size
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, uncertainty)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, uncertainty)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, uncertainty) + required_bonus:
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        uncertainty = self.uncertainty_score(regression_edge, hybrid_alpha, predicted_edge, fit_quality, volatility)
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, uncertainty), self.buy_capacity)
                if regime != "range":
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, uncertainty), self.sell_capacity)
                if regime != "range":
