...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "FORCE_CAP": 2.20,
    "FORCE_BLEND_WEIGHT": 0.24,
    "FORCE_FAIR_WEIGHT": 0.06,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def run(self) -> List[Order]:
        ...
        if physics_force["net"] * regression_edge < 0:
            physics_force["net"] *= 0.45
        if physics_force["net"] * breakout_score < 0:
            physics_force["net"] *= 0.70
        if abs(flow_metrics["bias"]) < 0.08 and abs(breakout_score) < 0.50 and abs(regression_edge) < self.TREND_EDGE_THRESHOLD:
            physics_force["net"] *= 0.60
