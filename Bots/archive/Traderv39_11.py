...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "EXEC_AGGRESSIVE_GAP": 12.0,
    "EXEC_PASSIVE_GAP": 5.0,
    "EXEC_REDUCE_GAP": 8.0,
    "EXEC_TAKE_BONUS": 0.18,
    "EXEC_QUOTE_SHIFT": 1.0,
    "EXEC_SIZE_BONUS": 2.0,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def execution_plan(
        ...
        plan = {
            ...
            "buy_size_bonus": 0,
            "sell_size_bonus": 0,
            "allow_buy_passive": True,
            "allow_sell_passive": True,
        }

        if position - target_position >= self.EXEC_REDUCE_GAP:
            plan["mode"] = "reduce_long"
            plan["sell_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["buy_take_bias"] = 0.10
            plan["allow_buy_passive"] = False
            plan["sell_quote_shift"] = -1
            plan["sell_size_bonus"] = int(self.EXEC_SIZE_BONUS)
            return plan
        if target_position - position >= self.EXEC_REDUCE_GAP and position < 0:
            plan["mode"] = "reduce_short"
            plan["buy_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["sell_take_bias"] = 0.10
            plan["allow_sell_passive"] = False
            plan["buy_quote_shift"] = 1
            plan["buy_size_bonus"] = int(self.EXEC_SIZE_BONUS)
            return plan

        if regime == "trend_up" and gap >= self.EXEC_AGGRESSIVE_GAP and predicted_edge > 0 and fit_quality >= self.FIT_THRESHOLD:
            plan["mode"] = "aggressive_buy"
            plan["buy_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["sell_take_bias"] = 0.10
            plan["buy_quote_shift"] = 1
            plan["sell_quote_shift"] = 1
            plan["buy_size_bonus"] = int(self.EXEC_SIZE_BONUS)
        elif regime == "trend_down" and -gap >= self.EXEC_AGGRESSIVE_GAP and predicted_edge < 0 and fit_quality >= self.FIT_THRESHOLD:
            plan["mode"] = "aggressive_sell"
            plan["sell_take_bias"] = -self.EXEC_TAKE_BONUS
            plan["buy_take_bias"] = 0.10
            plan["buy_quote_shift"] = -1
            plan["sell_quote_shift"] = -1
            plan["sell_size_bonus"] = int(self.EXEC_SIZE_BONUS)
        elif gap >= self.EXEC_PASSIVE_GAP:
            plan["mode"] = "passive_buy"
            plan["buy_quote_shift"] = 1
            plan["buy_size_bonus"] = 1
        elif -gap >= self.EXEC_PASSIVE_GAP:
            plan["mode"] = "passive_sell"
            plan["sell_quote_shift"] = -1
            plan["sell_size_bonus"] = 1

        if volatility >= self.TOXIC_VOLATILITY_THRESHOLD and abs(gap) <= 4:
            plan["mode"] = "neutral"
            plan["allow_buy_passive"] = False
            plan["allow_sell_passive"] = False
    ...
    def take_edge(
        ...
        if side == "BUY":
            edge += float(execution_plan.get("buy_take_bias", 0.0))
        else:
            edge += float(execution_plan.get("sell_take_bias", 0.0))
    ...
    def passive_quotes(
        ...
        if buy_quote is not None:
            buy_quote += int(execution_plan.get("buy_quote_shift", 0))
        if sell_quote is not None:
            sell_quote += int(execution_plan.get("sell_quote_shift", 0))

        return self.clamp_inside_spread(buy_quote, sell_quote)

    def passive_size(self, side: str, regime: str, volatility: float, execution_plan: Dict[str, object]) -> int:
        ...
        if side == "BUY":
            size += int(execution_plan.get("buy_size_bonus", 0))
        else:
            size += int(execution_plan.get("sell_size_bonus", 0))

        return size

    def allow_passive(self, side: str, regime: str, execution_plan: Dict[str, object]) -> bool:
        position = self.projected_position()
        ...
        if regime == "volatile" and abs(position) <= 6:
            return False
        if side == "BUY" and not bool(execution_plan.get("allow_buy_passive", True)):
            return False
        if side == "SELL" and not bool(execution_plan.get("allow_sell_passive", True)):
            return False
        return True
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, execution_plan)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, execution_plan)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, execution_plan) + required_bonus:
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        execution_plan = self.execution_plan(regime, target_position, predicted_edge, fit_quality, volatility)
        adjusted_fair = self.adjusted_fair_value(
            regime,
            target_position,
            predicted_now,
        ...
        if (
            ...
            and buy_quote is not None
            and self.buy_capacity > 0
            and self.allow_passive("BUY", regime, execution_plan)
        ):
            if regime == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, execution_plan), self.buy_capacity)
                if regime != "range":
        ...
        if (
            ...
            and sell_quote is not None
            and self.sell_capacity > 0
            and self.allow_passive("SELL", regime, execution_plan)
        ):
            if regime == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, execution_plan), self.sell_capacity)
                if regime != "range":
