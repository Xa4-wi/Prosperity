...
class TomatoesBot:
    ...
    MID_EMA_ALPHA = 0.18
    VOL_EMA_ALPHA = 0.22
    TREND_EMA_ALPHA = 0.34

    FAIR_WALL_WEIGHT = 0.48
    FAIR_MID_WEIGHT = 0.14
    FAIR_MICRO_WEIGHT = 0.24
    FAIR_FLOW_WEIGHT = 0.14

    INVENTORY_SKEW = 0.026
    BASE_QUOTE_EDGE = 1.55
    BASE_TAKE_EDGE = 0.58
    MAX_TAKE_SIZE = 18
    PASSIVE_SIZE = 12
    SOFT_LIMIT = 38
    ...
    def update_state(self) -> None:
        ...
        trend_raw = (
            0.65 * (self.product_state["wall_fair_ema"] - self.product_state["mid_ema"])
            + 0.95 * (self.book.micro - self.book.mid)
            + 1.05 * current_flow
        )
    ...
    def fair_value(self) -> float:
        ...
        if trend > self.TREND_SCORE:
            fair += 0.34 * min(2.4, trend)
        elif trend < -self.TREND_SCORE:
            fair -= 0.34 * min(2.4, abs(trend))
        return fair
    ...
    def target_position(self, regime: str, fair: float) -> int:
        ...
        if regime == "strong_up":
            return int(round((self.SOFT_LIMIT + 22) * max(0.45, conviction)))
        if regime == "strong_down":
            return -int(round((self.SOFT_LIMIT + 22) * max(0.45, conviction)))
        if regime == "trend_up":
            return int(round((self.SOFT_LIMIT + 10) * max(0.35, conviction)))
        ...
        return int(round(-0.18 * self.SOFT_LIMIT * clamp(normalized, -1.0, 1.0)))
    ...
    def take_threshold(self, side: str, regime: str, target: int) -> float:
        ...
        if regime == "stable":
            ...
            threshold += -0.48 if side == "BUY" else 0.62
        elif regime == "trend_down":
            threshold += -0.48 if side == "SELL" else 0.62
        elif regime == "strong_up":
            threshold += -0.72 if side == "BUY" else 0.88
        elif regime == "strong_down":
            threshold += -0.72 if side == "SELL" else 0.88
        else:
            threshold += 0.65
        ...
        if side == "SELL" and position > target:
            threshold -= 0.18
        if side == "BUY" and position < 0 and regime in {"trend_up", "strong_up"}:
            threshold -= 0.22
        if side == "SELL" and position > 0 and regime in {"trend_down", "strong_down"}:
            threshold -= 0.22
    ...
    def take_size(self, side: str, regime: str, target: int) -> int:
        ...
        size = min(self.MAX_TAKE_SIZE, max(3, desired))
    ...
    def clear_inventory(self, reservation: float, regime: str, target: int) -> None:
        ...
        if regime in {"trend_up", "strong_up"} and position > 0 and position <= max(target, 1):
            long_clear += 1.2 if regime == "strong_up" else 0.7
        if regime in {"trend_down", "strong_down"} and position < 0 and position >= min(target, -1):
            short_clear -= 1.2 if regime == "strong_down" else 0.7
    ...
    def take_orders(self, reservation: float, regime: str, target: int) -> None:
        ...
        for price, volume in self.book.sell_levels[:2]:
            if self.manager.buy_capacity <= 0:
                break
            if regime in {"trend_down", "strong_down"} and self.manager.projected_position() >= 0:
                break
        ...
        for price, volume in self.book.buy_levels[:2]:
            if self.manager.sell_capacity <= 0:
                break
            if regime in {"trend_up", "strong_up"} and self.manager.projected_position() <= 0:
                break
    ...
    def allow_passive(self, side: str, regime: str, target: int) -> bool:
        ...
        if regime == "toxic" and abs(position) <= 4:
            return False
        if regime == "strong_up" and side == "SELL" and position <= max(10, target // 3):
            return False
        if regime == "trend_up" and side == "SELL" and position <= 8:
            return False
        if regime == "strong_down" and side == "BUY" and position >= min(-10, target // 3):
            return False
        if regime == "trend_down" and side == "BUY" and position >= -8:
            return False
        return True
    ...
    def passive_quotes(self, reservation: float, regime: str, target: int) -> Tuple[Optional[int], Optional[int]]:
        ...
        if regime in {"trend_up", "strong_up"} and self.manager.projected_position() < target:
            buy_quote = max(self.book.best_bid, buy_quote + 1)
        if regime in {"trend_down", "strong_down"} and self.manager.projected_position() > target:
            sell_quote = min(self.book.best_ask, sell_quote - 1)
