...
class TomatoesTrader(BaseProductTrader):
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float, lob_signal: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, lob_signal))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, lob_signal))
