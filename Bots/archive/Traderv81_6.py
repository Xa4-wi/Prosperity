...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "GAP_ALIGN_RELIEF": 0.30,
    "GAP_WRONGWAY_PENALTY": 0.18,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def gap_pressure(
        ...
        if gap * signal_bias < 0:
            curved *= 1.0 - self.GAP_ALIGN_RELIEF * min(1.0, abs(signal_bias))
        elif gap * signal_bias > 0:
            curved *= 1.0 + self.GAP_WRONGWAY_PENALTY * min(1.0, abs(signal_bias))
        if regime == "range":
            curved *= 1.12
