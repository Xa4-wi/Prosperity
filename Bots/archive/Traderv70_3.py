...
class TomatoesTrader(BaseProductTrader):
    ...
    def run(self) -> List[Order]:
        ...
        if provisional_regime == "range":
            physics_contrib = 0.42 * physics_force["spring"] + 0.12 * physics_force["damping"]
        else:
            drive = physics_force["driving"]
            if drive * regression_edge < 0 and drive * breakout_score < 0:
                drive = 0.0
            elif drive * regression_edge < 0 or drive * breakout_score < 0:
                drive *= 0.45
            physics_contrib = self.FORCE_BLEND_WEIGHT * drive
        physics_force["contrib"] = physics_contrib
