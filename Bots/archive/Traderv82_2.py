...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "INVENTORY_BUFFER_RATIO": 0.22,
    "REORDER_POINT_RATIO": 0.55,
    "EXCESS_TIER_MULTIPLIER": 1.30,
    "BUFFER_RELIEF_MULTIPLIER": 0.55,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def gap_pressure(
        ...
        if normalized <= self.INVENTORY_BUFFER_RATIO:
            return 0.0
        normalized = (normalized - self.INVENTORY_BUFFER_RATIO) / max(1e-9, 1.0 - self.INVENTORY_BUFFER_RATIO)
        curved = normalized ** self.GAP_CURVE_POWER
        if normalized >= self.REORDER_POINT_RATIO:
            curved *= self.EXCESS_TIER_MULTIPLIER
        else:
            curved *= self.BUFFER_RELIEF_MULTIPLIER + (1.0 - self.BUFFER_RELIEF_MULTIPLIER) * (normalized / max(1e-9, self.REORDER_POINT_RATIO))
        if regime == "range":
            curved *= 1.12
    ...
    def excess_inventory_ratio(self, target_position: int) -> float:
        gap = abs(self.projected_position() - target_position) / max(1.0, float(self.soft_limit))
        if gap <= self.REORDER_POINT_RATIO:
            return 0.0
        return max(0.0, min(1.0, (gap - self.REORDER_POINT_RATIO) / max(1e-9, 1.0 - self.REORDER_POINT_RATIO)))
    ...
    def passive_quotes(
        ...
        position = self.projected_position()
        excess_ratio = self.excess_inventory_ratio(target_position)
        if regime == "trend_up":
            ...
            if sell_quote is not None and position > 0:
                ...
                sell_quote += lift
            if sell_quote is not None and position > target_position and excess_ratio > 0.0:
                sell_quote -= int(round(1.0 + excess_ratio))
        elif regime == "trend_down":
            ...
            if buy_quote is not None and position < 0:
                ...
                if predicted_edge <= -self.STRONG_TREND_EDGE:
                    drop += 1
                buy_quote -= drop
            if buy_quote is not None and position < target_position and excess_ratio > 0.0:
                buy_quote += int(round(1.0 + excess_ratio))
        elif regime == "volatile" and abs(position) <= 6:
            buy_quote = None
            sell_quote = None
        else:
            if sell_quote is not None and position > target_position and excess_ratio > 0.35:
                sell_quote -= 1
            if buy_quote is not None and position < target_position and excess_ratio > 0.35:
                buy_quote += 1
    ...
    def take_orders(
        ...
        if (
            ...
            if self.projected_position() < target_position:
                take_limit += int(round(2.0 * excess_ratio))
