...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MR_TREND_DAMP": 0.82,
    "MR_RANGE_MIN_STRENGTH": 0.14,
    "MR_RANGE_MIN_SIGNAL": 0.20,
    "MR_RANGE_TARGET_DEADZONE": 1.0,
    "MR_RANGE_CONTINUATION_VETO": 0.26,
    "MR_RANGE_OPPOSING_DAMP": 0.68,
    "MR_RANGE_POSITION_BUFFER": 4.0,
    "RANGE_LONG_HOLD_EDGE_MIN": 0.45,
    "RANGE_LONG_HOLD_MOMENTUM_MIN": 0.75,
    "RANGE_LONG_HOLD_QUEUE_MIN": 0.06,
    "RANGE_LONG_HOLD_LOB_MIN": 0.08,
    "RANGE_LONG_HOLD_TARGET_BONUS": 3.0,
    "RANGE_LONG_HOLD_EXIT_BONUS": 0.28,
    "RANGE_LONG_HOLD_QUOTE_LIFT": 1.0,
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.14,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def gate_range_mean_reversion(
        ...
        if regime != "range":
            return mr_signal, mr_strength

        if abs(mr_signal) < self.MR_RANGE_MIN_SIGNAL or mr_strength < self.MR_RANGE_MIN_STRENGTH:
            return 0.0, 0.0

        veto = self.MR_RANGE_CONTINUATION_VETO
        long_fade_blocked = (
            mr_signal > 0
            and predicted_edge < -veto
            and queue_bias < -veto
            and lob_signal < -veto
        ...
        if long_fade_blocked or short_fade_blocked:
            return mr_signal * 0.40, mr_strength * 0.40

        opposing_votes = 0
        if mr_signal * predicted_edge < -veto:
            opposing_votes += 1
        if mr_signal * queue_bias < -veto:
            opposing_votes += 1
        if mr_signal * lob_signal < -veto:
            opposing_votes += 1
        if opposing_votes >= 2:
            return mr_signal * self.MR_RANGE_OPPOSING_DAMP, mr_strength * self.MR_RANGE_OPPOSING_DAMP
        return mr_signal, mr_strength
    ...
    def range_long_hold_strength(self, predicted_edge: float, queue_bias: float, lob_signal: float) -> float:
        if self.projected_position() <= 0:
            return 0.0
        edge_score = max(0.0, predicted_edge - self.RANGE_LONG_HOLD_EDGE_MIN)
        momentum_score = max(0.0, self.momentum - self.RANGE_LONG_HOLD_MOMENTUM_MIN)
        queue_score = max(0.0, queue_bias - self.RANGE_LONG_HOLD_QUEUE_MIN)
        lob_score = max(0.0, lob_signal - self.RANGE_LONG_HOLD_LOB_MIN)
        micro_score = 0.15 if float(self.micro) >= float(self.mid) else 0.0
        raw = (
            0.38 * min(1.0, edge_score / max(0.25, self.TREND_EDGE_THRESHOLD))
            + 0.28 * min(1.0, momentum_score / 1.5)
            + 0.18 * min(1.0, queue_score / 0.25)
            + 0.16 * min(1.0, lob_score / 0.30)
            + micro_score
        )
    ...
    def target_position(
        ...
        if regime == "range":
            hold_strength = self.range_long_hold_strength(predicted_edge, queue_bias, lob_signal)
            if hold_strength > 0.0:
                base_target = max(base_target, int(round(self.RANGE_LONG_HOLD_TARGET_BONUS * hold_strength)))
            if abs(base_target) <= self.MR_RANGE_TARGET_DEADZONE:
                return 0
    ...
    def passive_quotes(
        ...
        position = self.projected_position()
        long_hold_strength = self.range_long_hold_strength(predicted_edge, queue_bias, lob_signal)
        if regime == "trend_up":
            ...
            if buy_quote is not None and position < 0:
                ...
                if predicted_edge <= -self.STRONG_TREND_EDGE:
                    drop += 1
                buy_quote -= drop
        elif regime == "range" and sell_quote is not None and position > 0 and long_hold_strength > 0.0:
            sell_quote += int(round(self.RANGE_LONG_HOLD_QUOTE_LIFT * long_hold_strength))
    ...
    def passive_size(self, side: str, regime: str, volatility: float, queue_bias: float) -> int:
    ...
    def take_orders(
        ...
        if (
            ...
            if regime != "range" and self.projected_position() >= target_position:
                ...
                if regime == "range":
                    limit_position = target_position + int(self.MR_RANGE_POSITION_BUFFER)
                    quantity = min(quantity, max(0, limit_position - self.projected_position()))
                else:
        ...
        if (
            ...
            if regime == "range" and self.projected_position() > max(0, target_position) and long_hold_strength > 0.0:
                required_bonus += self.RANGE_LONG_HOLD_EXIT_BONUS * long_hold_strength
            ...
            if regime != "range" and self.projected_position() <= target_position:
                ...
                if regime == "range":
                    limit_position = target_position - int(self.MR_RANGE_POSITION_BUFFER)
                    quantity = min(quantity, max(0, self.projected_position() - limit_position))
                else:
    ...
    def run(self) -> List[Order]:
        ...
        mr_signal, mr_strength = self.gate_range_mean_reversion(
            mr_signal,
            mr_strength,
            provisional_edge,
            provisional_regime,
        ...
        if (
            ...
            range_limit = target_position + int(self.MR_RANGE_POSITION_BUFFER)
            if (regime == "range" and position < range_limit) or (regime != "range" and position < target_position):
                quantity = min(self.passive_size("BUY", regime, volatility, queue_bias), self.buy_capacity)
                if regime == "range":
                    quantity = min(quantity, max(0, range_limit - position))
                else:
        ...
        if (
            ...
            range_limit = target_position - int(self.MR_RANGE_POSITION_BUFFER)
            if (regime == "range" and position > range_limit) or (regime != "range" and position > target_position):
                quantity = min(self.passive_size("SELL", regime, volatility, queue_bias), self.sell_capacity)
                if regime == "range":
                    quantity = min(quantity, max(0, position - range_limit))
                else:
