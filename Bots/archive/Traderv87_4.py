...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MR_RSI_WINDOW": 12,
    "MR_SIGNAL_CAP": 2.4,
    "MR_RANGE_TARGET_SCALE": 8.5,
    "MR_FAIR_WEIGHT": 0.18,
    "MR_TAKE_SHIFT": 0.08,
    "MR_QUOTE_SHIFT": 0.02,
    "MR_ZSCORE_ACTIVATION": 0.95,
    "MR_RSI_ACTIVATION": 8.0,
    "MR_CONFLICT_DAMP": 0.62,
    "MR_TREND_DAMP": 0.58,
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.14,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def refine_mean_reversion_signal(
        ...
        z_activation = clamp((abs(zscore) - self.MR_ZSCORE_ACTIVATION) / 0.90, 0.0, 1.0)
        rsi_activation = clamp((abs(rsi - 50.0) - self.MR_RSI_ACTIVATION) / 18.0, 0.0, 1.0)
        activation = 0.65 * z_activation + 0.35 * rsi_activation
        ...
        for signal in (provisional_edge, self.imbalance, queue_bias, lob_signal):
            if mr_signal * signal < -0.04:
                activation *= self.MR_CONFLICT_DAMP

        refined_strength = mr_strength * activation
        if refined_strength < 0.10:
            return 0.0, 0.0
