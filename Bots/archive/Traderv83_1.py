...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "ANCHOR_PHI": 0.42,
    "ANCHOR_BETA": 0.34,
    "ANCHOR_CAP_RATIO": 0.62,
    "ANCHOR_RANGE_SCALE": 0.42,
    "ANCHOR_RESERVATION_WEIGHT": 0.15,
    "ANCHOR_QUOTE_WEIGHT": 0.95,
    "ANCHOR_PASSIVE_SIZE_WEIGHT": 0.32,
    "ANCHOR_TAKE_EDGE_SHIFT": 0.22,
    "ANCHOR_FAIR_WEIGHT": 0.12,
    "TURNOVER_ALPHA": 0.35,
    "TURNOVER_TARGET_DAMP": 0.35,
    "TURNOVER_QUOTE_PENALTY": 0.35,
    "TURNOVER_TAKE_PENALTY": 0.16,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.turnover_ema = float(self.memory.get("turnover_ema", 0.0))
        self.prev_position_snapshot = float(self.memory.get("prev_position_snapshot", float(self.position)))
    ...
    def anchor_signal(self, predicted_edge: float, fit_quality: float) -> float:
        scale = max(1.0, float(self.spread) / 2.0)
        queue_bias, _queue_activity = self.queue_reactive_signal()
        lob_signal = self.filtered_lob_signal()
        normalized_edge = max(-1.0, min(1.0, predicted_edge / (1.5 * scale)))
        signal = (
        ...
        return max(-1.0, min(1.0, signal * max(0.30, fit_quality)))

    def anchor_target(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        signal = self.anchor_signal(predicted_edge, fit_quality)
        cap = float(self.soft_limit) * self.ANCHOR_CAP_RATIO
        desired = cap * signal
        if regime == "range":
            desired *= self.ANCHOR_RANGE_SCALE
    ...
    def update_turnover(self) -> None:
        change = abs(float(self.position) - self.prev_position_snapshot) / max(1.0, float(self.soft_limit))
        self.turnover_ema = (1.0 - self.TURNOVER_ALPHA) * self.turnover_ema + self.TURNOVER_ALPHA * min(1.5, change)
        self.prev_position_snapshot = float(self.position)

    def anchor_gap(self, target_position: int) -> float:
        return float(self.projected_position() - target_position)
    ...
    def reservation_adjustment(
        ...
        reservation_shift += self.ANCHOR_RESERVATION_WEIGHT * self.anchor_gap(target_position)
    ...
    def target_band(self, regime: str, predicted_edge: float, fit_quality: float) -> Tuple[int, int]:
        ...
        if regime == "trend_up":
            return (8, 34) if conviction >= self.STRONG_TREND_EDGE else (4, 22)
        if regime == "trend_down":
            return (-34, -8) if conviction >= self.STRONG_TREND_EDGE else (-22, -4)
        if regime == "volatile":
            return -6, 6
        return -12, 12
    ...
    def target_position(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        damp = max(0.45, 1.0 - self.TURNOVER_TARGET_DAMP * min(1.0, self.turnover_ema))
        anchor_target = int(round(anchor_target * damp))
        return max(lower, min(upper, anchor_target))
    ...
    def fair_value(
        ...
        fair -= self.ANCHOR_FAIR_WEIGHT * (self.anchor_gap(target_position) / max(1.0, float(self.soft_limit))) * half_spread
    ...
    def next_memory(self) -> Dict[str, float]:
        return {
            ...
            "turnover_ema": float(self.turnover_ema),
            "prev_position_snapshot": float(self.prev_position_snapshot),
            "prev_book": {
    ...
    def take_edge(
        ...
        anchor_gap = self.anchor_gap(self.target_position(regime, predicted_edge, fit_quality))
        if side == "BUY" and anchor_gap < 0:
            edge -= self.ANCHOR_TAKE_EDGE_SHIFT * min(1.0, abs(anchor_gap) / max(1.0, float(self.soft_limit)))
        elif side == "SELL" and anchor_gap > 0:
            edge -= self.ANCHOR_TAKE_EDGE_SHIFT * min(1.0, abs(anchor_gap) / max(1.0, float(self.soft_limit)))

        edge += self.TURNOVER_TAKE_PENALTY * min(1.0, self.turnover_ema)
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float, target_position: int) -> float:
        ...
        queue_bias, _queue_activity = self.queue_reactive_signal()
        lob_signal = self.filtered_lob_signal()
        anchor_pressure = min(1.0, abs(self.anchor_gap(target_position)) / max(1.0, float(self.soft_limit)))
        ...
        edge += self.ANCHOR_QUOTE_WEIGHT * anchor_pressure
        edge += self.TURNOVER_QUOTE_PENALTY * min(1.0, self.turnover_ema)
        if regime == "trend_up":
    ...
    def passive_size(self, side: str, regime: str, volatility: float) -> int:
        ...
        size = max(1, int(round(size * (1.0 - self.ANCHOR_PASSIVE_SIZE_WEIGHT * gap_norm))))
