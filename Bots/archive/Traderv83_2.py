...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "ANCHOR_FAST_DECAY": 0.72,
    "ANCHOR_SLOW_DECAY": 0.42,
    "ANCHOR_WEAK_SIGNAL_SCALE": 0.72,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def anchor_target(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        if regime == "range":
            ...
            desired *= 0.20
        decay = self.ANCHOR_SLOW_DECAY
        if regime == "range" or fit_quality < self.FIT_THRESHOLD or abs(signal) < 0.18:
            decay = self.ANCHOR_FAST_DECAY
            desired *= self.ANCHOR_WEAK_SIGNAL_SCALE
        self.inventory_anchor = decay * self.inventory_anchor + self.ANCHOR_BETA * desired
