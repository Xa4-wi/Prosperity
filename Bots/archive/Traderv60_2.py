...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "CHAOS_MICRO_WEIGHT": 0.26,
    "CHAOS_IMBALANCE_WEIGHT": 0.20,
    "CHAOS_FLOW_WEIGHT": 0.22,
    "CHAOS_SPREAD_WEIGHT": 0.12,
    "CHAOS_ENTROPY_WEIGHT": 0.20,
    "CHAOS_LOW_THRESHOLD": 0.24,
    "CHAOS_HIGH_THRESHOLD": 0.56,
    "CHAOS_QUOTE_WIDEN": 0.72,
    "CHAOS_QUOTE_TIGHTEN": 0.14,
    "CHAOS_TAKE_EDGE_PENALTY": 0.40,
    "CHAOS_PASSIVE_SIZE_PENALTY": 3.0,
    "CHAOS_PASSIVE_SIZE_BONUS": 1.0,
    "CHAOS_WINDOW": 10,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def market_flow_metrics(self) -> Dict[str, float]:
        ...
        spread_instability = abs(float(self.spread) - float(previous["spread"])) / max(
            1.0,
            float(previous["spread"]),
        )
    ...
    def normalized_sign_entropy(self, values: List[float], neutral_band: float) -> float:
        if len(values) < 4:
            return 0.0
    ...
    def chaos_metrics(self, flow_metrics: Dict[str, float]) -> Dict[str, float]:
        window = max(6, int(self.CHAOS_WINDOW))

        mid_window = self.mid_history.get(self.product, [])[-window:]
        returns = [
            float(mid_window[index] - mid_window[index - 1])
            for index in range(1, len(mid_window))
        ]
        micro_window = self.memory_list("micro_premium_history")[-window:]
        imbalance_window = self.memory_list("imbalance_history")[-window:]
        flow_window = self.memory_list("signed_flow_history")[-window:]

        return_entropy = self.normalized_sign_entropy(returns, 0.08)
        micro_entropy = self.normalized_sign_entropy(micro_window, 0.04)
        imbalance_entropy = self.normalized_sign_entropy(imbalance_window, 0.03)
        flow_entropy = self.normalized_sign_entropy(flow_window, 0.12)
        entropy_proxy = (
            0.30 * return_entropy
            + 0.25 * micro_entropy
            + 0.25 * imbalance_entropy
            + 0.20 * flow_entropy
        )

        micro_component = min(1.5, abs(flow_metrics["micro_drift"]))
        imbalance_component = min(1.5, 1.8 * abs(flow_metrics["imbalance_drift"]))
        flow_component = min(1.5, abs(flow_metrics["flow_acceleration"]))
        spread_component = min(1.5, 2.5 * flow_metrics["spread_instability"])

        raw_score = (
            self.CHAOS_MICRO_WEIGHT * micro_component
            + self.CHAOS_IMBALANCE_WEIGHT * imbalance_component
            + self.CHAOS_FLOW_WEIGHT * flow_component
            + self.CHAOS_SPREAD_WEIGHT * spread_component
            + self.CHAOS_ENTROPY_WEIGHT * entropy_proxy
        )
        total_weight = (
            self.CHAOS_MICRO_WEIGHT
            + self.CHAOS_IMBALANCE_WEIGHT
            + self.CHAOS_FLOW_WEIGHT
            + self.CHAOS_SPREAD_WEIGHT
            + self.CHAOS_ENTROPY_WEIGHT
        )
        score = raw_score / max(1e-9, total_weight)

        if score >= self.CHAOS_HIGH_THRESHOLD:
            state = "chaotic"
        elif score <= self.CHAOS_LOW_THRESHOLD:
            state = "orderly"
        else:
            state = "tradeable"
    ...
    def take_edge(
        ...
        if chaos_state == "chaotic":
            edge += self.CHAOS_TAKE_EDGE_PENALTY * min(1.5, chaos_score)
        elif chaos_state == "orderly" and regime != "volatile":
            edge -= self.CHAOS_QUOTE_TIGHTEN * 0.6
    ...
    def quote_edge(
        ...
        if chaos_state == "chaotic":
            edge += self.CHAOS_QUOTE_WIDEN * min(1.5, chaos_score)
        elif chaos_state == "orderly" and regime != "volatile":
            edge -= self.CHAOS_QUOTE_TIGHTEN * max(0.2, 1.0 - chaos_score)
    ...
    def passive_quotes(
        ...
        quote_edge = self.quote_edge(regime, volatility, fit_quality, breakout_score, chaos_score, chaos_state)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, chaos_score: float = 0.0, chaos_state: str = "tradeable") -> int:
        ...
        if chaos_state == "chaotic":
            size = max(1, size - int(round(self.CHAOS_PASSIVE_SIZE_PENALTY * min(1.5, chaos_score))))
        elif chaos_state == "orderly" and regime != "volatile":
            size += int(self.CHAOS_PASSIVE_SIZE_BONUS)
    ...
    def run(self) -> List[Order]:
        ...
        chaos_metrics = self.chaos_metrics(flow_metrics)
        ...
        execution_regime = "volatile" if chaos_metrics["state"] == "chaotic" else regime
        ...
        self.diagnostic_context = {
            ...
            "chaos_score": chaos_metrics["score"],
            "chaos_state": chaos_metrics["state"],
        }
        ...
        if (
            ...
            and buy_quote is not None
            and self.buy_capacity > 0
            and self.allow_passive("BUY", execution_regime)
        ):
            if execution_regime == "range" or position < target_position:
                quantity = min(
                    self.passive_size("BUY", execution_regime, volatility, chaos_metrics["score"], chaos_metrics["state"]),
                    self.buy_capacity,
                )
                if execution_regime != "range":
        ...
        if (
            ...
            and sell_quote is not None
            and self.sell_capacity > 0
            and self.allow_passive("SELL", execution_regime)
        ):
            if execution_regime == "range" or position > target_position:
                quantity = min(
                    self.passive_size("SELL", execution_regime, volatility, chaos_metrics["score"], chaos_metrics["state"]),
                    self.sell_capacity,
                )
                if execution_regime != "range":
