...
class TomatoesTrader(BaseProductTrader):
    ...
    def market_temperature(
        ...
        return min(
            3.0,
            0.55 * flow_metrics["activity"]
            + 0.35 * abs(flow_metrics["flow_acceleration"])
            + 0.25 * abs(breakout_score)
            + 0.20 * max(0.0, volatility - 1.5),
        )
    ...
    def take_edge(
        ...
        temperature = getattr(self, "thermal_state", 1.2)
        ...
        if regime == "trend_up":
            ...
            if side == "SELL" and predicted_edge >= self.TREND_EDGE_THRESHOLD:
                edge += 0.35
            if temperature <= 0.95 and volatility <= 1.9:
                edge -= 0.08
    ...
    def quote_edge(
        ...
        if regime == "range":
            if temperature <= 0.95 and volatility <= 1.9 and fit_quality >= 0.30:
                edge -= 0.18
            elif temperature >= 1.8:
                edge += 0.12
        elif regime == "volatile" and temperature >= 2.0:
            edge += 0.20
    ...
    def run(self) -> List[Order]:
        ...
        if pressure_bias * regression_edge < 0:
            pressure_bias *= 0.60
        self.thermal_state = self.market_temperature(flow_metrics, volatility, breakout_score)
