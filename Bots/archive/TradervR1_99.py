...
DEFAULT_ASH_PARAMS = {
    ...
    "REFILL_STRONG_SPREAD_MAX": 16.0,
    "REFILL_STRONG_DEPTH_MIN": 24.0,
    "REFILL_STRONG_FAIR_GAP": 1.5,
    "REFILL_UNSTABLE_SPREAD_MIN": 18.0,
    "REFILL_UNSTABLE_FAIR_GAP": 3.5,
    "REFILL_EXIT_EDGE_BONUS": 0.15,
...
class AshCoatedOsmiumTrader:
    ...
    def _refill_state(
        self,
        book: Book,
        prev_vacuum_side: Optional[str],
        prev_vacuum_fair: float,
    ) -> str:
        if prev_vacuum_side not in ("bid_only", "ask_only"):
            return "none"

        depth = float(book.best_bid_vol + book.best_ask_vol)
        fair_gap = abs(book.mid - prev_vacuum_fair)
        micro_gap = book.micro - book.mid

        if prev_vacuum_side == "bid_only":
            directional_ok = micro_gap >= -0.10 and book.imbalance >= -0.08
            opposite_pressure = micro_gap <= -0.20 and book.imbalance <= -0.18
        else:
            directional_ok = micro_gap <= 0.10 and book.imbalance <= 0.08
            opposite_pressure = micro_gap >= 0.20 and book.imbalance >= 0.18

        if (
            book.spread_val <= float(self.p["REFILL_STRONG_SPREAD_MAX"])
            and depth >= float(self.p["REFILL_STRONG_DEPTH_MIN"])
            and fair_gap <= float(self.p["REFILL_STRONG_FAIR_GAP"])
            and directional_ok
        ):
            return "strong"

        if (
            book.spread_val >= float(self.p["REFILL_UNSTABLE_SPREAD_MIN"])
            or fair_gap >= float(self.p["REFILL_UNSTABLE_FAIR_GAP"])
            or opposite_pressure
        ):
            return "unstable"
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
            refill_state = self._refill_state(book, prev_vacuum_side, prev_vacuum_fair)
            if refill_state == "strong":
                slow_fair = 0.80 * slow_fair + 0.20 * prev_vacuum_fair
            elif refill_state == "weak":
                ...
                slow_fair = 0.35 * slow_fair + 0.65 * prev_vacuum_fair
                if prev_vacuum_side == "ask_only":
                    quote_signal = min(quote_signal, -0.05)
                    take_signal = min(take_signal, 0.0)
                else:
                    quote_signal = max(quote_signal, 0.05)
                    take_signal = max(take_signal, 0.0)
        ...
        if refill_state == "strong":
            attack = clamp(attack + 0.08, 0.0, 1.0)
        elif refill_state == "unstable":
            attack = clamp(attack - 0.18, 0.0, 1.0)
        ...
        if refill_state == "weak":
            exit_bonus = float(self.p["REFILL_EXIT_EDGE_BONUS"])
            if pos > 0:
                buy_qe += 0.10
                sell_qe -= exit_bonus
            elif pos < 0:
        ...
        ash_state["last_refill_state"] = refill_state
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
