...
class Trader:
    ...
    - Step 12: Hydrogel refinement from the R3_28 family:
      add a light emergency target, explicit fade score, and conditional
      absolute-danger clearing without changing the overall regime structure
    """
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if current_pos > 120 and long_fade_score > 0.95:
            emergency_target = min(emergency_target, 40 if abs(current_pos) >= 150 else 60)
        if current_pos > 140 and long_fade_score > 1.45:
            emergency_target = min(emergency_target, 0 if progress > 0.90 else 20)
        if current_pos < -120 and short_fade_score > 0.95:
            emergency_target = max(emergency_target, -40 if abs(current_pos) >= 150 else -60)
        if current_pos < -140 and short_fade_score > 1.45:
            emergency_target = max(emergency_target, 0 if progress > 0.90 else -20)
        if abs(current_pos) >= 150 and (
            (current_pos > 0 and long_fade_score > 0.95)
            or (current_pos < 0 and short_fade_score > 0.95)
        ):
        ...
        absolute_danger_long = current_pos >= 150 and (long_fade_score > 0.95 or progress > 0.92)
        absolute_danger_short = current_pos <= -150 and (short_fade_score > 0.95 or progress > 0.92)
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        if unwind_long or unwind_short:
            ...
            clear_edge += 0.7
            hard_clear_edge += 1.4
            soft_clear_max = max(cfg["clear_max"], 46)
