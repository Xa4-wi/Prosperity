...
class TomatoesBot:
    ...
    CHANGE_EDGE_WEIGHT = 0.16
    CHANGE_QUOTE_WIDEN = 0.45
    CHANGE_TAKE_TIGHTEN = 0.10
    ...
    def load_product_state(self) -> Dict[str, object]:
        ...
        return {
            ...
            "change_mean_fast": float(raw.get("change_mean_fast", 0.0)),
            "change_mean_slow": float(raw.get("change_mean_slow", 0.0)),
            "change_flow_fast": float(raw.get("change_flow_fast", 0.0)),
            "change_flow_slow": float(raw.get("change_flow_slow", 0.0)),
            "change_vol_fast": float(raw.get("change_vol_fast", 1.0)),
            "change_vol_slow": float(raw.get("change_vol_slow", 1.0)),
            "change_prob": float(raw.get("change_prob", 0.0)),
    ...
    def save_product_state(self) -> None:
        self.memory["tomatoes"] = {
            ...
            "change_mean_fast": float(self.product_state["change_mean_fast"]),
            "change_mean_slow": float(self.product_state["change_mean_slow"]),
            "change_flow_fast": float(self.product_state["change_flow_fast"]),
            "change_flow_slow": float(self.product_state["change_flow_slow"]),
            "change_vol_fast": float(self.product_state["change_vol_fast"]),
            "change_vol_slow": float(self.product_state["change_vol_slow"]),
            "change_prob": float(self.product_state["change_prob"]),
    ...
    def online_change_state(self, volatility: float) -> Tuple[float, float]:
        ...
        current_mean = float(self.book.mid)
        current_flow = float(self.book.imbalance)
        current_vol = max(1.0, volatility)
        fast = self.CHANGE_FAST_ALPHA
        slow = self.CHANGE_SLOW_ALPHA

        self.product_state["change_mean_fast"] = ema(float(self.product_state["change_mean_fast"]), current_mean, fast)
        self.product_state["change_mean_slow"] = ema(float(self.product_state["change_mean_slow"]), current_mean, slow)
        self.product_state["change_flow_fast"] = ema(float(self.product_state["change_flow_fast"]), current_flow, fast)
        self.product_state["change_flow_slow"] = ema(float(self.product_state["change_flow_slow"]), current_flow, slow)
        self.product_state["change_vol_fast"] = ema(float(self.product_state["change_vol_fast"]), current_vol, fast)
        self.product_state["change_vol_slow"] = ema(float(self.product_state["change_vol_slow"]), current_vol, slow)

        mean_gap = abs(float(self.product_state["change_mean_fast"]) - float(self.product_state["change_mean_slow"])) / half_spread
        flow_gap = abs(float(self.product_state["change_flow_fast"]) - float(self.product_state["change_flow_slow"]))
        vol_gap = abs(float(self.product_state["change_vol_fast"]) - float(self.product_state["change_vol_slow"])) / max(1.0, current_vol)
        raw_score = 0.55 * mean_gap + 0.30 * flow_gap + 0.15 * vol_gap
        normalized = clamp(raw_score / 1.75, 0.0, 1.0)
        self.product_state["change_prob"] = ema(float(self.product_state["change_prob"]), normalized, self.CHANGE_PROB_ALPHA)
        direction = (
            (float(self.product_state["change_mean_fast"]) - float(self.product_state["change_mean_slow"])) / half_spread
            + 0.65 * (float(self.product_state["change_flow_fast"]) - float(self.product_state["change_flow_slow"]))
        )
        return clamp(float(self.product_state["change_prob"]), 0.0, 1.0), clamp(direction, -1.5, 1.5)
    ...
    def update_state(self) -> None:
        ...
        if self.product_state["initialized"] <= 0.0:
            ...
            self.product_state["change_mean_fast"] = current_mid
            self.product_state["change_mean_slow"] = current_mid
            self.product_state["change_flow_fast"] = current_flow
            self.product_state["change_flow_slow"] = current_flow
            self.product_state["change_vol_fast"] = max(1.0, self.book.spread / 2.0)
            self.product_state["change_vol_slow"] = max(1.0, self.book.spread / 2.0)
            self.product_state["change_prob"] = 0.0
    ...
    def classify_regime(self, predicted_edge: float, fit_quality: float, volatility: float) -> str:
        change_score = float(self.product_state["change_prob"])
        change_direction = (
            float(self.product_state["change_mean_fast"]) - float(self.product_state["change_mean_slow"])
        ) / max(1.0, float(self.book.spread) / 2.0) + 0.65 * (
            float(self.product_state["change_flow_fast"]) - float(self.product_state["change_flow_slow"])
        )
        if self.book.spread >= self.TOXIC_SPREAD and volatility >= self.TOXIC_VOL:
            return "toxic"
        if change_score >= self.CHANGE_VOLATILE_THRESHOLD and abs(predicted_edge) < self.TREND_EDGE_THRESHOLD:
            return "toxic"
        if (
        ...
        if (
            ...
            and fit_quality >= max(0.30, self.FIT_THRESHOLD - 0.10 * change_score)
            and self.book.imbalance >= 0.01 - 0.04 * change_score
            and change_direction >= -0.15
        ):
            return "trend_up"
        if (
    ...
    def take_threshold(
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            ...
            if aligned:
                threshold -= self.CHANGE_TAKE_TIGHTEN * min(1.0, change_score)
            else:
                threshold += 0.06 * min(1.0, change_score)
    ...
    def quote_edge(self, side: str, regime: str, target: int, volatility: float, fit_quality: float) -> float:
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            ...
            elif (side == "BUY" and change_direction > 0) or (side == "SELL" and change_direction < 0):
                edge += 0.10 * min(1.0, change_score)
            else:
                edge += 0.25 * min(1.0, change_score)
        return max(1.2, edge)
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        self.update_state()
        change_score, change_direction = self.online_change_state(volatility=max(1.0, float(self.product_state["vol_ema"])))
