...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "ANCHOR_PHI": 0.55,
    "ANCHOR_BETA": 0.22,
    "ANCHOR_CAP_RATIO": 0.45,
    "ANCHOR_TARGET_BLEND": 0.28,
    "ANCHOR_RESERVATION_WEIGHT": 0.10,
    "EXIT_URGENCY_TAKE_SHIFT": 0.14,
    "EXIT_URGENCY_QUOTE_SHIFT": 0.30,
    "EXIT_URGENCY_SIZE_BONUS": 1.0,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def signal_confidence(self, predicted_edge: float, fit_quality: float) -> float:
        ...
        agreement = 0.0
        if edge_sign != 0.0:
            if self.imbalance * edge_sign > 0:
                agreement += min(1.0, abs(self.imbalance))
            elif self.imbalance * edge_sign < 0:
                agreement -= min(1.0, abs(self.imbalance))
            if self.momentum * edge_sign > 0:
                agreement += min(1.0, abs(self.momentum) / max(1.0, scale))
            elif self.momentum * edge_sign < 0:
                agreement -= min(1.0, abs(self.momentum) / max(1.0, scale))
        normalized_edge = min(1.0, abs(predicted_edge) / (1.5 * scale))
        confidence = 0.50 * fit_quality + 0.30 * normalized_edge + 0.20 * ((agreement + 2.0) / 4.0)
    ...
    def anchor_target(self, regime: str, predicted_edge: float, fit_quality: float) -> int:
        ...
        scale = max(1.0, float(self.spread) / 2.0)
        signal = max(-1.0, min(1.0, predicted_edge / (1.75 * scale))) * max(0.30, confidence)
        cap = float(self.soft_limit) * self.ANCHOR_CAP_RATIO
        desired = cap * signal
        if regime == "range":
            desired *= 0.45
    ...
    def exit_urgency(
        ...
        urgency = excess * (1.0 - 0.65 * confidence) * (1.0 - 0.50 * directional_penalty)
    ...
    def reservation_adjustment(
        ...
        reservation_shift += self.ANCHOR_RESERVATION_WEIGHT * (position - self.current_anchor_target())
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            ...
            if sell_quote is not None and sell_urgency > 0.0:
                sell_quote -= int(round(self.EXIT_URGENCY_QUOTE_SHIFT * (1.0 + sell_urgency)))
        elif regime == "trend_down":
            ...
            if buy_quote is not None and buy_urgency > 0.0:
                buy_quote += int(round(self.EXIT_URGENCY_QUOTE_SHIFT * (1.0 + buy_urgency)))
    ...
    def take_orders(
        ...
        if (
            ...
            take_limit = self.MAX_TAKE_SIZE + (int(self.TREND_ENTRY_TAKE_BONUS) if regime == "trend_up" else 0) + int(round(self.EXIT_URGENCY_SIZE_BONUS * buy_urgency))
        ...
        if (
            ...
            take_limit = self.MAX_TAKE_SIZE + (int(self.TREND_ENTRY_TAKE_BONUS) if regime == "trend_down" else 0) + int(round(self.EXIT_URGENCY_SIZE_BONUS * sell_urgency))
            required_bonus = 0.0
