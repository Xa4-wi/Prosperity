...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "LOB_ALPHA_WEIGHT": 0.10,
    "LOB_FAIR_WEIGHT": 0.08,
    "LOB_EDGE_WEIGHT": 0.18,
    "LOB_TAKE_SHIFT": 0.12,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def hybrid_alpha(self, lob_signal: float) -> Tuple[float, float]:
    ...
    def adjusted_fair_value(
        ...
        fair = self.fair_value(regime, target_position, predicted_now, predicted_next, hybrid_alpha, lob_signal)
    ...
    def quote_edge(self, regime: str, volatility: float, fit_quality: float, target_position: int, lob_signal: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, target_position, lob_signal))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, target_position, lob_signal))
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, lob_signal)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, lob_signal)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, lob_signal) + required_bonus:
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        predicted_edge += self.LOB_EDGE_WEIGHT * max(1.0, float(self.spread) / 2.0) * lob_signal
