...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "FUSED_DEPTH_FAST_WEIGHT": 0.58,
    "FUSED_DEPTH_SLOW_WEIGHT": 0.48,
    "FUSED_DEPTH_MEMORY_WEIGHT": 0.22,
    "FUSED_DEPTH_BLEND_WEIGHT": 0.18,
    "FUSED_DEPTH_FAIR_WEIGHT": 0.08,
    "CHANNEL_WINDOW": 18,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def fused_depth_signal(self, slow_depth_alpha: float) -> Dict[str, float]:
        half_spread = max(1.0, float(self.spread) / 2.0)
        top_alpha = (float(self.micro) - float(self.mid)) + (0.70 * half_spread * float(self.imbalance))
        ...
        if top_alpha * slow_depth_alpha < 0:
            ...
            fused += math.copysign(0.14 * agreement * abs(memory_component), fused if fused != 0 else top_alpha)
        if agreement < 0.40:
            fused *= 0.65
    ...
    def run(self) -> List[Order]:
        ...
        if fused_depth["agreement"] >= 0.60:
            predicted_edge += 0.08 * fused_depth["net"]
