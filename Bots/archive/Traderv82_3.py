...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "SIGNAL_INVENTORY_BETA": 0.28,
    "SIGNAL_INVENTORY_PHI": 0.34,
    "SIGNAL_TARGET_BLEND": 0.45,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.signal_inventory_anchor = float(self.memory.get("signal_inventory_anchor", 0.0))
    ...
    def signal_inventory_target(
        ...
        queue_bias, _queue_activity = self.queue_reactive_signal()
        lob_signal = self.filtered_lob_signal()
        signal_mix = 0.50 * (predicted_edge / scale) + 0.30 * queue_bias + 0.20 * lob_signal
        signal_mix = max(-1.0, min(1.0, signal_mix))
        confidence = max(0.25, fit_quality)
        decay = max(0.0, min(1.0, 1.0 - self.SIGNAL_INVENTORY_PHI))
        target_scale = float(self.soft_limit) * self.SIGNAL_INVENTORY_BETA * confidence
        self.signal_inventory_anchor = decay * self.signal_inventory_anchor + target_scale * signal_mix

        lower, upper = self.target_band(regime, predicted_edge, fit_quality)
        forecast_target = int(round(max(lower, min(upper, self.signal_inventory_anchor))))
        blended = int(round(
            (1.0 - self.SIGNAL_TARGET_BLEND) * base_target
            + self.SIGNAL_TARGET_BLEND * forecast_target
        ))
    ...
    def next_memory(self) -> Dict[str, float]:
        return {
            ...
            "signal_inventory_anchor": float(self.signal_inventory_anchor),
            "prev_book": {
    ...
    def run(self) -> List[Order]:
        ...
        target_position = self.signal_inventory_target(regime, predicted_edge, fit_quality, target_position)
        adjusted_fair = self.adjusted_fair_value(
            regime,
            target_position,
            predicted_now,
