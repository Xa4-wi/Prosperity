...
class TomatoesBot:
    ...
    TREND_QUOTE_EDGE = 2.55
    STRONG_QUOTE_EDGE = 3.10
    BASE_TAKE_EDGE = 0.95
    HYBRID_TAKE_EDGE = 0.60
    STRONG_TAKE_EDGE = 0.30

    BASE_PASSIVE_SIZE = 8
    TAKE_SIZE = 10
    SOFT_LIMIT = 28
    TARGET_CAP = 34
    INVENTORY_SKEW = 0.060
    TARGET_BIAS_DIVISOR = 15.0

    RANGE_THRESHOLD = 0.60
    TREND_THRESHOLD = 1.10
    STRONG_THRESHOLD = 1.90
    ...
    def update_state(self) -> None:
        ...
        raw_trend = (
            1.15 * (self.fast_fair_ema - self.slow_fair_ema)
    ...
    def fair_value(self, target: int) -> float:
        fair = 0.40 * self.slow_fair_ema + 0.34 * self.fast_fair_ema + 0.16 * self.book.micro + 0.10 * (self.book.mid + self.flow_ema)
    ...
    def execution_mode(self, regime: str, target: int) -> str:
        ...
        if regime == "chaotic":
            return "defensive"
        if regime in {"strong_up", "strong_down"} and abs(gap) >= 6:
            return "aggressive"
    ...
    def passive_quotes(self, reservation: float, regime: str, mode: str, target: int) -> Tuple[Optional[int], Optional[int], int, int]:
        ...
        if mode == "market_make":
            ...
            if regime == "trend_up":
                ...
                if position > 0:
                    sell_quote = math.ceil(reservation + edge + 1.0)
                ...
                if position < 0:
                    buy_quote = math.floor(reservation - edge - 1.0)
            ...
            if regime == "strong_up":
                ...
                sell_size = max(2, self.BASE_PASSIVE_SIZE - 3)
            elif regime == "strong_down":
                sell_quote = self.book.best_ask - 1 if position > target else None
                if position < 0:
                    buy_quote = math.floor(reservation - edge - 1.0)
                buy_size = max(2, self.BASE_PASSIVE_SIZE - 3)
