...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "CHANGE_FAST_ALPHA": 0.34,
    "CHANGE_SLOW_ALPHA": 0.08,
    "CHANGE_PROB_ALPHA": 0.24,
    "CHANGE_REGIME_THRESHOLD": 0.42,
    "CHANGE_VOLATILE_THRESHOLD": 0.72,
    "CHANGE_EDGE_WEIGHT": 0.18,
    "CHANGE_QUOTE_WIDEN": 0.55,
    "CHANGE_TAKE_TIGHTEN": 0.12,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def __init__(
        ...
        self.change_mean_fast = float(self.memory.get("change_mean_fast", self.mid or 0.0))
        self.change_mean_slow = float(self.memory.get("change_mean_slow", self.mid or 0.0))
        self.change_flow_fast = float(self.memory.get("change_flow_fast", self.imbalance if self.has_book() else 0.0))
        self.change_flow_slow = float(self.memory.get("change_flow_slow", self.imbalance if self.has_book() else 0.0))
        self.change_vol_fast = float(self.memory.get("change_vol_fast", 1.0))
        self.change_vol_slow = float(self.memory.get("change_vol_slow", 1.0))
        self.change_prob = float(self.memory.get("change_prob", 0.0))
    ...
    def online_change_state(self, volatility: float) -> Tuple[float, float]:
        half_spread = max(1.0, float(self.spread) / 2.0)
        fast = self.CHANGE_FAST_ALPHA
        slow = self.CHANGE_SLOW_ALPHA
        current_mean = float(self.mid)
        current_flow = float(self.imbalance)
        current_vol = max(1.0, volatility)

        self.change_mean_fast = (1.0 - fast) * self.change_mean_fast + fast * current_mean
        self.change_mean_slow = (1.0 - slow) * self.change_mean_slow + slow * current_mean
        self.change_flow_fast = (1.0 - fast) * self.change_flow_fast + fast * current_flow
        self.change_flow_slow = (1.0 - slow) * self.change_flow_slow + slow * current_flow
        self.change_vol_fast = (1.0 - fast) * self.change_vol_fast + fast * current_vol
        self.change_vol_slow = (1.0 - slow) * self.change_vol_slow + slow * current_vol

        mean_gap = abs(self.change_mean_fast - self.change_mean_slow) / half_spread
        flow_gap = abs(self.change_flow_fast - self.change_flow_slow)
        vol_gap = abs(self.change_vol_fast - self.change_vol_slow) / max(1.0, current_vol)
        raw_score = 0.55 * mean_gap + 0.30 * flow_gap + 0.15 * vol_gap
        normalized = max(0.0, min(1.5, raw_score / 1.75))
        self.change_prob = (1.0 - self.CHANGE_PROB_ALPHA) * self.change_prob + self.CHANGE_PROB_ALPHA * normalized

        direction = (
            (self.change_mean_fast - self.change_mean_slow) / half_spread
            + 0.65 * (self.change_flow_fast - self.change_flow_slow)
        )
        return max(0.0, min(1.0, self.change_prob)), max(-1.5, min(1.5, direction))
    ...
    def classify_state(
        ...
        if change_score >= self.CHANGE_VOLATILE_THRESHOLD and abs(predicted_edge) < 0.8 * trend_threshold:
            return "volatile"
        ...
        if (
            ...
            and fit_quality >= max(0.30, self.FIT_THRESHOLD - 0.10 * change_score)
            and self.imbalance >= self.TREND_IMBALANCE_THRESHOLD - 0.04 * change_score
            and self.momentum >= 0.75
            and float(self.micro) >= float(self.mid)
            and change_direction >= -0.15
        ):
            return "trend_up"
        if (
            predicted_edge <= -trend_threshold
    ...
    def next_memory(self) -> Dict[str, float]:
        return {
            ...
            "change_mean_fast": float(self.change_mean_fast),
            "change_mean_slow": float(self.change_mean_slow),
            "change_flow_fast": float(self.change_flow_fast),
            "change_flow_slow": float(self.change_flow_slow),
            "change_vol_fast": float(self.change_vol_fast),
            "change_vol_slow": float(self.change_vol_slow),
    ...
    def take_edge(
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            aligned = (side == "BUY" and change_direction > 0) or (side == "SELL" and change_direction < 0)
            if aligned:
                edge -= self.CHANGE_TAKE_TIGHTEN * min(1.0, change_score)
            else:
                edge += 0.10 * min(1.0, change_score)

        return max(0.5, edge)

    def quote_edge(self, regime: str, volatility: float, fit_quality: float, change_score: float, change_direction: float) -> float:
        tau = self.time_fraction_remaining()
        gamma = self.control_gamma(regime)
        ...
        if change_score >= self.CHANGE_REGIME_THRESHOLD:
            if regime == "volatile":
                edge += self.CHANGE_QUOTE_WIDEN * min(1.0, change_score)
            elif (regime == "trend_up" and change_direction > 0) or (regime == "trend_down" and change_direction < 0):
                edge += 0.15 * min(1.0, change_score)
            else:
                edge += 0.35 * min(1.0, change_score)
    ...
    def passive_quotes(
        ...
        buy_quote = math.floor(adjusted_fair - self.quote_edge(regime, volatility, fit_quality, change_score, change_direction))
        sell_quote = math.ceil(adjusted_fair + self.quote_edge(regime, volatility, fit_quality, change_score, change_direction))
    ...
    def passive_size(self, side: str, regime: str, volatility: float, change_score: float, change_direction: float) -> int:
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, change_score, change_direction)
            and self.buy_capacity > 0
        ):
        ...
        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, change_score, change_direction)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, change_score, change_direction) + required_bonus:
                pass
            else:
    ...
    def run(self) -> List[Order]:
        ...
        change_score, change_direction = self.online_change_state(volatility)
        ...
        provisional_regime = self.classify_state(provisional_edge, fit_quality, volatility, change_score, change_direction)
        ...
        predicted_edge += self.CHANGE_EDGE_WEIGHT * change_score * change_direction
        predicted_next = float(self.mid) + predicted_edge
        regime = self.classify_state(predicted_edge, fit_quality, volatility, change_score, change_direction)
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, change_score, change_direction), self.buy_capacity)
                if regime != "range":
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, change_score, change_direction), self.sell_capacity)
                if regime != "range":
