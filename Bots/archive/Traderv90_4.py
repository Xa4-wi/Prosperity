...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MR_TAKE_SHIFT": 0.10,
    "MR_QUOTE_SHIFT": 0.06,
    "MR_RANGE_EXIT_TAKE_BONUS": 0.24,
    "MR_RANGE_EXIT_PASSIVE_SHIFT": 1.0,
    "MR_RANGE_EXIT_STRENGTH_MIN": 0.38,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def range_exit_bonus(
        ...
        if regime != "range" or mr_strength < self.MR_RANGE_EXIT_STRENGTH_MIN:
            return 0.0

        position = self.projected_position()
        if side == "SELL":
            if position > max(0, target_position) and mr_signal > 0:
                return self.MR_RANGE_EXIT_TAKE_BONUS * mr_strength
        else:
            if position < min(0, target_position) and mr_signal < 0:
                return self.MR_RANGE_EXIT_TAKE_BONUS * mr_strength
        return 0.0
    ...
    def passive_quotes(
        ...
        if regime == "trend_up":
            ...
            shift = int(round(self.MR_RANGE_EXIT_PASSIVE_SHIFT * mr_strength))
            if shift > 0:
                if sell_quote is not None and position > max(0, target_position) and mr_signal > 0:
                    sell_quote += shift
                if buy_quote is not None and position < min(0, target_position) and mr_signal < 0:
                    buy_quote -= shift
    ...
    def take_orders(
        ...
        took_buy = False
        took_sell = False
        buy_exit_bonus = self.range_exit_bonus("BUY", regime, target_position, mr_signal, mr_strength)
        sell_exit_bonus = self.range_exit_bonus("SELL", regime, target_position, mr_signal, mr_strength)

        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, mr_signal, mr_strength, queue_bias, lob_signal) - buy_exit_bonus
            and self.buy_capacity > 0
        ):
