...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "MR_TREND_DAMP": 0.82,
    "MR_EARLY_TARGET_SCALE": 0.40,
    "MR_EARLY_TAKE_SCALE": 0.35,
    "MR_EARLY_PASSIVE_SCALE": 0.55,
    "MR_EARLY_QUOTE_WIDEN": 0.40,
    "MR_RISK_ON_START": 0.72,
    "MR_RISK_ON_TARGET_BONUS": 1.25,
    "MR_RISK_ON_TAKE_BONUS": 1.20,
    "MR_RISK_ON_PASSIVE_BONUS": 1.12,
    "MR_RANGE_BRAKE_QUEUE": 0.22,
    "MR_RANGE_BRAKE_LOB": 0.18,
    "MR_RANGE_BRAKE_EDGE": 0.15,
    "MR_RANGE_BRAKE_FLATTEN": 0.45,
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.14,
...
class TomatoesTrader(BaseProductTrader):
    ...
    def readiness_scale(self) -> float:
        ...
        return clamp(ready / max(1.0, float(required)), 0.0, 1.0)

    def risk_on_scale(self, readiness: float, regime: str, mr_strength: float) -> float:
        if regime != "range":
            return 1.0
        if readiness < self.MR_RISK_ON_START:
            return 1.0
        excess = (readiness - self.MR_RISK_ON_START) / max(1e-9, 1.0 - self.MR_RISK_ON_START)
        return 1.0 + excess * mr_strength
    ...
    def adverse_range_brake(
        ...
        long_adverse = (
            position > 0
            and queue_bias < -self.MR_RANGE_BRAKE_QUEUE
            and lob_signal < -self.MR_RANGE_BRAKE_LOB
            and float(self.micro) <= float(self.mid)
            and predicted_edge < -self.MR_RANGE_BRAKE_EDGE
            and mr_signal <= 0.0
        )
        short_adverse = (
            position < 0
        ...
        if long_adverse:
            ...
            block_buy = True
            flatten_bias = self.MR_RANGE_BRAKE_FLATTEN * half_spread * max(0.35, mr_strength)
        elif short_adverse:
    ...
    def target_position(
        ...
        if position < lower:
            ...
            early_scale = self.MR_EARLY_TARGET_SCALE + (1.0 - self.MR_EARLY_TARGET_SCALE) * readiness
            base_target = int(round(base_target * early_scale))
            base_target = int(round(base_target * self.risk_on_scale(readiness, regime, mr_strength) * self.MR_RISK_ON_TARGET_BONUS))
        if regime == "range":
    ...
    def quote_edge(
        ...
        if regime == "trend_up":
            ...
            edge += self.MR_EARLY_QUOTE_WIDEN * (1.0 - readiness)
    ...
    def passive_quotes(
        ...
        edge = self.quote_edge(regime, volatility, fit_quality, target_position, mr_strength, queue_bias, lob_signal, readiness)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, queue_bias: float, readiness: float, mr_strength: float) -> int:
        ...
        if side == "BUY" and queue_bias > 0:
            ...
            size += int(self.QUEUE_SIZE_BONUS)
        if regime == "range":
            early_scale = self.MR_EARLY_PASSIVE_SCALE + (1.0 - self.MR_EARLY_PASSIVE_SCALE) * readiness
            size = max(1, int(round(size * early_scale)))
            size = max(1, int(round(size * self.risk_on_scale(readiness, regime, mr_strength) * self.MR_RISK_ON_PASSIVE_BONUS)))
        return size
    ...
    def take_orders(
        ...
        if (
            ...
            if regime == "range":
                early_scale = self.MR_EARLY_TAKE_SCALE + (1.0 - self.MR_EARLY_TAKE_SCALE) * readiness
                take_limit = max(1, int(round(take_limit * early_scale)))
                take_limit = max(1, int(round(take_limit * self.risk_on_scale(readiness, regime, mr_strength) * self.MR_RISK_ON_TAKE_BONUS)))
    ...
    def run(self) -> List[Order]:
        ...
        target_position = self.target_position(regime, predicted_edge, fit_quality, mr_signal, mr_strength, queue_bias, lob_signal, readiness)
        target_position, block_buy, block_sell, flatten_bias = self.adverse_range_brake(
            regime,
            target_position,
            predicted_edge,
            queue_bias,
        ...
        if (
            ...
            if regime == "range" or position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, queue_bias, readiness, mr_strength), self.buy_capacity)
                if regime != "range":
        ...
        if (
            ...
            if regime == "range" or position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, queue_bias, readiness, mr_strength), self.sell_capacity)
                if regime != "range":
