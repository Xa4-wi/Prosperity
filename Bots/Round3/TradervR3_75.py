...
try:
    from r3_bs_fair_module import build_r3_bs_fair_surface
except ModuleNotFoundError:
    from Bots.Round3.r3_bs_fair_module import build_r3_bs_fair_surface
...
class Trader:
    def _reset_if_new_day(self, memory: dict, timestamp: int) -> None:
        ...
        memory.setdefault("bs_fair_module", {})
    ...
    def _build_strip_context(self, state: TradingState, surface: Dict[str, dict], meta: dict) -> dict:
        ...
        shock_mode = bool(
            meta["broad_dislocation"]
            or (meta["avg_abs_iv_residual"] > 0.0108 and meta["pair_agreement_count"] >= 1)
            or (meta["avg_abs_iv_residual"] > 0.0098 and meta["pair_agreement_count"] >= 2)
        )
        hedge_ratio = 0.75 if shock_mode else (0.50 if meta["avg_abs_iv_residual"] > 0.0105 else 0.25)
    ...
    def _select_pairs_and_targets(
        ...
        ordered = sorted(surface, key=lambda p: VOUCHERS[p])
        entry_threshold = max(0.0090, 0.72 * meta["avg_abs_iv_residual"])
        shock_entry_threshold = max(0.0068, 0.72 * entry_threshold)
        outright_threshold = max(0.033, 2.10 * entry_threshold)
    ...
    def _trade_voucher(self, product: str, state: TradingState, row: dict, target: int, source: str, meta: dict, strip: dict) -> List[Order]:
        ...
        iv_band = max(0.0085, 0.45 * max(0.011, 0.92 * meta["avg_abs_iv_residual"]))
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            spot_fair, surface, meta = build_r3_bs_fair_surface(
                state.order_depths, memory["bs_fair_module"], VELVET, VOUCHERS, TTE_YEARS
            )
