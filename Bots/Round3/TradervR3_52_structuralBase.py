...
class Trader:
    ...
    - Step 16: blend structural smile IV with a short rolling local IV fair,
      use a buffered dynamic Velvet hedge, and strengthen pair-first voucher execution
    """
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float, memory: dict) -> Dict[str, dict]:
        ...
        local_iv_ema = dict(vev_state.get("local_iv_ema", {}))
        alpha_local_iv = 0.18
        structural_weight = 0.75
        if fit_diag.get("model") != "svi" or int(repair_diag.get("repair_count", 0)) >= 3:
            structural_weight = 0.55
        surface: Dict[str, dict] = {}
        provisional_prices: List[Tuple[str, int, float]] = []
        for product, strike in VOUCHER_STRIKES.items():
            row = strike_rows.get(product, {})
            struct_sigma = fitted_sigmas[product]
            if row.get("market_iv") is not None:
                prev_local = float(local_iv_ema.get(product, row["market_iv"]))
                local_sigma = (1.0 - alpha_local_iv) * prev_local + alpha_local_iv * float(row["market_iv"])
            else:
                local_sigma = float(local_iv_ema.get(product, struct_sigma))
            local_iv_ema[product] = round(float(local_sigma), 8)

            blend_weight = structural_weight
            if row.get("repaired_flag", False):
                blend_weight = min(blend_weight, 0.65)
            if float(row.get("spread", 4.0)) > 6.0:
                blend_weight = min(blend_weight, 0.65)
            sigma = clamp(blend_weight * struct_sigma + (1.0 - blend_weight) * local_sigma, 1e-4, 3.0)
            fair_price = black_scholes_call(velvet_fair, strike, TTE_YEARS, sigma)
            surface[product] = {
                "sigma": sigma,
                "struct_sigma": struct_sigma,
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        ...
        hedge_ratio = 0.50
        if abs(strip_delta) < 40.0:
            hedge_ratio = 0.25
        if broad_dislocation:
            hedge_ratio = 0.75
        if resid_compression:
            hedge_ratio = max(0.15, hedge_ratio - 0.20)
        effective_strip_delta = 0.0 if abs(strip_delta) < 20.0 else strip_delta
        ...
        return {
            ...
            "effective_strip_delta": effective_strip_delta,
            "strip_vega_proxy": strip_vega,
            "delta_pressure": clamp(strip_delta / 140.0, -2.0, 2.0),
            "hedge_ratio": hedge_ratio,
            "target_velvet_pos": int(round(clamp(-hedge_ratio * effective_strip_delta, -100.0, 100.0))),
    ...
    def _build_velvet_context(self, state: TradingState, velvet_fair: float, risk_ctx: dict) -> dict:
        ...
        alpha_cap = 0
        if abs(strip_delta) < 40.0 and book_good:
            alpha_cap = 40
        elif abs(strip_delta) < 80.0 and book_good:
            alpha_cap = 20
        if bool(risk_ctx["broad_dislocation"]):
            alpha_cap = min(alpha_cap, 10)
        if bool(risk_ctx["resid_compression"]) or abs(strip_delta) >= 80.0:
            alpha_cap = 0
        alpha_target = int(round(clamp(30.0 * math.tanh(0.75 * alpha_signal), -float(alpha_cap), float(alpha_cap))))
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        take_edge = max(0.75, 0.35 * spread) + (0.30 if middle_band else 0.0) + (0.22 if not confirmed else 0.0)
        if broad_dislocation:
            take_edge -= 0.15
        ...
        take_max = 9 if middle_band else 18
        if broad_dislocation:
            take_max += 2
        if confirmed:
            take_max += 2
            ...
            try:
                ...
                memory["voucher_targets"] = {
                    product: {
                        ...
                        "sigma": round(float(voucher_surface[product]["sigma"]), 5),
                        "struct_sigma": round(float(voucher_surface[product].get("struct_sigma", voucher_surface[product]["sigma"])), 5),
                        "local_sigma": round(float(voucher_surface[product].get("local_sigma", voucher_surface[product]["sigma"])), 5),
