...
class Trader:
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        unwind_long_hard = current_pos > 170 and (
            (long_drawdown >= 28.0 and ret_ema < -0.18)
        ...
        unwind_short_hard = current_pos < -170 and (
            (short_drawup >= 28.0 and ret_ema > 0.18)
        ...
        return {
            ...
            "buy_take_extra": 3.0 if exit_mode.startswith("long") else 2.0 if stretch_long and not exceptional_buy else 0.0,
            "sell_take_extra": 3.0 if exit_mode.startswith("short") else 2.0 if stretch_short and not exceptional_sell else 0.0,
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 56 if unwind_long else 56)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 40 if unwind_long else cfg["clear_max"])))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 56 if unwind_short else 56)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 40 if unwind_short else cfg["clear_max"])))
