...
class Trader:
    ...
    - Step 12: Hydrogel research branch:
      add entry/hold/emergency targets, fade-score exits, adaptive Hydrogel fair,
      and a small time-of-day prior
    """
    ...
    def _underlying_fair(self, product: str, od: OrderDepth, overlay_bias: float = 0.0) -> float:
        ...
        if product == HYDROGEL:
            raw_component = raw_mid(od)
            if raw_component is None:
                raw_component = stable_component
            top_depth = 0.0
            if bb is not None:
            ...
            if top_depth >= 18.0:
                health += 1.0
            if abs(float(stable_component) - float(raw_component)) <= 1.2:
                health += 1.0
            book_health = health / 4.0
            if book_health < 0.50:
                anchor_w, stable_w, micro_w = 0.68, 0.24, 0.08
            elif book_health < 0.75:
                anchor_w, stable_w, micro_w = 0.58, 0.27, 0.15
            else:
                anchor_w, stable_w, micro_w = cfg["anchor_w"], cfg["stable_w"], cfg["micro_w"]
            fair = (
                anchor_w * anchor
                + stable_w * stable_component
                + micro_w * micro_component
            )
            fair += (0.75 + 0.35 * book_health) * cfg["imbalance_w"] * book_imbalance(od, levels=2) * max(1.0, 0.5 * spread)
            fair += overlay_bias
            return fair
        fair = (
            cfg["anchor_w"] * anchor
    ...
    def _hydrogel_time_bias(self, progress: float) -> float:
        if progress < 0.16:
            return -0.18
        if progress < 0.32:
            return -0.06
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if top_depth >= 20.0:
            health += 1.0
        if abs(float(stable) - float(mid)) <= 1.1:
            health += 1.0
        book_health = health / 4.0
        ...
        time_bias = self._hydrogel_time_bias(progress)
        regime_score = 0.54 * trend_score + 0.24 * anchor_score + 0.12 * flow_score + 0.10 * time_bias
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 64 if (unwind_long or absolute_danger_long) else 56)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 44 if (unwind_long or absolute_danger_long) else cfg["clear_max"])))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 64 if (unwind_short or absolute_danger_short) else 56)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 44 if (unwind_short or absolute_danger_short) else cfg["clear_max"])))
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                memory["hydro_book"] = {
                    ...
                    "book_health": round(float(hydro_ctx["book_health"]), 3),
                    "time_bias": round(float(hydro_ctx["time_bias"]), 3),
