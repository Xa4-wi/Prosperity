...
class Trader:
    ...
    - Step 4: only add targeted voucher risk control where the first three bots bled:
      near-ATM / middle strikes
    """
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float) -> Dict[str, dict]:
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            fair = black_scholes_call(velvet_fair, strike, TTE_YEARS, sigma)
            delta = black_scholes_delta_call(velvet_fair, strike, TTE_YEARS, sigma)
            market_iv = implied_vol_call(max(float(market_mid or fair), max(velvet_fair - strike, 0.0) + 1e-3), velvet_fair, strike, TTE_YEARS)
            surface[product] = {
                "fair": fair,
                "sigma": sigma,
                "delta": delta,
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict) -> List[Order]:
        od = state.order_depths[product]
        limit = LIMITS[product]
        mgr = OrderManager(product, state.position.get(product, 0), limit)
        ...
        rv_score = clamp(-iv_residual / 0.06, -1.0, 1.0)

        take_edge = max(0.75, 0.35 * spread) + 0.18 * atm_risk
        clear_edge = max(0.35, 0.15 * spread)
        soft_limit = max(100, int(round(180 - 70 * atm_risk)))
        take_max = 16 if atm_risk > 0.55 else 24

        # Step 1: Take
        for ask, volume in sorted(od.sell_orders.items()):
            if fair - ask >= take_edge - max(0.0, 0.08 * rv_score):
                mgr.buy(ask, min(-volume, take_max))
            else:
                break

        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            if bid - fair >= take_edge - max(0.0, -0.08 * rv_score):
        ...
        reservation = fair - inv_penalty + 0.12 * rv_score
        quote_edge = max(1.0, 0.45 * spread, 0.015 * max(20.0, fair)) + 0.35 * atm_risk
        ...
        size_scale *= max(0.50, 1.0 - 0.45 * atm_risk)
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                ...
                memory["engine_errors"]["velvet"] = type(exc).__name__
            ...
            try:
                ...
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        result[product] = self._trade_voucher(product, state, voucher_surface[product])
            except Exception as exc:
                memory["engine_errors"]["voucher"] = type(exc).__name__
