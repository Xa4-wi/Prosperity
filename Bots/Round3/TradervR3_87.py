...
LOW_AGGR_VOUCHERS = ("VEV_4000", "VEV_4500")
MID_V47_VOUCHERS = ("VEV_5000",)
DISABLED_VOUCHERS = ("VEV_5100",)
LOW_AGGR_LIMITS: Dict[str, int] = {
    "VEV_4000": 72,
    "VEV_4500": 108,
}
...
class Trader:
    ...
    def _trade_targeted(
        ...
        if relative > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative - soft_limit)), take_max + 8))
        elif relative < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative)), take_max + 8))
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        if product == "VEV_5000":
            soft_limit = 105
            per_strike_cap = 120 if (buy_confirmed or pair_target > 0 or cheap_signal >= 0.60 * resid_threshold) else 95
            take_max += 2
            take_edge = max(0.60, take_edge - 0.08)
            clear_edge = max(clear_edge, 0.45)
        elif product == "VEV_5100":
            soft_limit = 68
            per_strike_cap = 60
            take_max = max(6, take_max - 2)
            take_edge += 0.12
            if pair_target <= 0 and cheap_signal < 1.10 * low_strike_threshold:
                buy_confirmed = False
            if pair_target >= 0 and rich_signal < 0.95 * low_strike_threshold:
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            try:
                ...
                for product in DISABLED_VOUCHERS:
                    if product in risk_ctx["pair_targets"]:
                        risk_ctx["pair_targets"][product] = 0
                    if product in risk_ctx["pair_bias"]:
                        risk_ctx["pair_bias"][product] = 0
                    if product in risk_ctx["residual_rank"]:
                        risk_ctx["residual_rank"][product] = 0
                ...
                low_voucher_diag: Dict[str, dict] = {}
                alpha_signal = float(velvet_ctx["alpha_signal"])
                bull_peak = alpha_signal < -0.90
                bear_trough = alpha_signal > 0.90
                for product in VOUCHER_STRIKES:
                    ...
                    if product in DISABLED_VOUCHERS:
                        result[product] = []
                        continue
                    if product in LOW_AGGR_VOUCHERS:
                        ctx = voucher_surface[product]
                        od = state.order_depths[product]
                        opt_mid = raw_mid(od)
                        if opt_mid is None:
                        ...
                        limit = LOW_AGGR_LIMITS[product]
                        edge_scale = 8.0 if product == "VEV_4000" else 6.5
                        dir_weight = 0.45 + 0.35 * delta
                        dir_target = 0.0
                        if product == "VEV_4000":
                            bs_target = 0.52 * limit * math.tanh((fair - opt_mid) / edge_scale)
                            if bull_peak:
                                dir_target = -0.32 * limit * dir_weight
                            elif bear_trough:
                                dir_target = 0.42 * limit * dir_weight
                            target = clamp(dir_target + bs_target, -float(limit), float(limit))
                            take_edge = max(1.8, 0.98 * edge_scale)
                            clear_edge = max(1.2, 0.82 * edge_scale)
                            quote_edge = max(1.3, 0.80 * edge_scale)
                            quote_size = 4
                            take_max = 8
                            soft_limit = max(10, int(0.52 * limit))
                            inv_skew = 3.3
                            take_bias = 0.10 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale + 0.08 * iv_residual
                            quote_bias = 0.05 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale
                        else:
                            bs_target = 0.95 * limit * math.tanh((fair - opt_mid) / edge_scale)
                            if bull_peak:
                                dir_target = -0.22 * limit * dir_weight
                            elif bear_trough:
                                dir_target = 0.84 * limit * dir_weight
                            target = clamp(dir_target + bs_target, -float(limit), float(limit))
                            take_edge = max(1.0, 0.78 * edge_scale)
                            clear_edge = max(0.9, 0.58 * edge_scale)
                            quote_edge = max(1.0, 0.62 * edge_scale)
                            quote_size = 6
                            take_max = 14
                            soft_limit = max(14, int(0.72 * limit))
                            inv_skew = 2.5
                            take_bias = 0.18 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale + 0.12 * iv_residual
                            quote_bias = 0.10 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale
                        ...
                        low_voucher_diag[product] = {
                            "lane": "low_aggressive_bs",
                            "fair": round(fair, 3),
                            "mid": round(float(opt_mid), 3),
                            "delta": round(delta, 4),
                            "iv_residual": round(iv_residual, 4),
                            "target": int(round(target)),
                            "limit": limit,
                        }
                    elif product in MID_V47_VOUCHERS:
                        result[product] = self._trade_voucher(product, state, voucher_surface[product], risk_ctx)
                    else:
                        result[product] = []
                memory["voucher_split"] = {
                    "low_aggressive": low_voucher_diag,
                    "mid_v47": {product: int(risk_ctx["pair_targets"].get(product, 0)) for product in MID_V47_VOUCHERS},
                    "disabled": list(DISABLED_VOUCHERS),
                }
            except Exception as exc:
