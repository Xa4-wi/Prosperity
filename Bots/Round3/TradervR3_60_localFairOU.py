...
class Trader:
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        ret_ema = 0.22 * ret + 0.78 * prev_ret_ema
        ...
        same_side_cooldown = max(0, int(hydro_state.get("same_side_cooldown", 0)) - 1)
        ...
        good_book = (
            ...
            and spread <= 8.0
            and top_depth >= 10.0
            and abs(float(stable) - float(mid)) <= 1.8
        )

        anchor_gap = float(slow_fair_ema) - float(mid)
        trend_gap = float(local_fair_ema) - float(slow_fair_ema)
        micro_gap = float(local_fair_ema) - float(mid)
        imbalance = book_imbalance(od, levels=2)

        anchor_score = clamp(anchor_gap / max(3.0, 0.80 * spread), -3.0, 3.0)
        trend_score = clamp(trend_gap / max(2.0, 0.90 * spread), -2.5, 2.5)
        flow_score = clamp(0.75 * micro_gap / max(1.0, 0.5 * spread) + 0.55 * imbalance - 0.25 * clamp(ret_ema / 2.5, -2.0, 2.0), -2.0, 2.0)
        deviation_score = clamp((local_fair_ema - float(mid)) / max(1.0, 0.55 * spread), -3.0, 3.0)
        regime_score = 0.65 * deviation_score + 0.20 * anchor_score - 0.15 * trend_score
        ...
        if not good_book:
            confidence = "guarded"
            entry_cap = 40
        elif abs(regime_score) < 0.55:
            confidence = "neutral"
            entry_cap = 0
        elif abs(regime_score) < 1.10:
            confidence = "light"
            entry_cap = 60
        elif abs(regime_score) < 1.75:
            confidence = "normal"
            entry_cap = 90
        else:
        ...
        if progress > 0.90:
            entry_cap = int(round(entry_cap * 0.80))
        if progress > 0.97:
            entry_cap = min(entry_cap, 40)

        hold_cap = min(entry_cap, 90 if abs(current_pos) > 90 else entry_cap)
        if abs(current_pos) > 120:
            hold_cap = min(hold_cap, 70)
        if abs(current_pos) > 150:
            hold_cap = min(hold_cap, 50)

        entry_target = int(round(clamp(110.0 * math.tanh(0.85 * regime_score), -float(entry_cap), float(entry_cap))))
        hold_target = int(round(clamp(95.0 * math.tanh(0.80 * regime_score), -float(hold_cap), float(hold_cap))))
        target = entry_target if abs(current_pos) < 80 else hold_target

        if abs(current_pos) > 100 and abs(regime_score) < 0.90:
            target = int(round(target * 0.70))
        if abs(current_pos) > 140:
        ...
        if current_pos * target < 0 and abs(current_pos) > 60:
            ...
            same_side_cooldown = 5

        if abs(current_pos) > 120 and abs(regime_score) < 0.75:
            same_side_cooldown = max(same_side_cooldown, 3)
        hydro_state["same_side_cooldown"] = same_side_cooldown

        same_side_bid_block = same_side_cooldown > 0 and current_pos > 0
        same_side_ask_block = same_side_cooldown > 0 and current_pos < 0
        absolute_danger_long = current_pos >= 140
        absolute_danger_short = current_pos <= -140
        if absolute_danger_long:
            same_side_bid_block = True
        if absolute_danger_short:
            same_side_ask_block = True

        quote_bias = 0.0 if abs(regime_score) < 0.60 else -0.05 * deviation_score
        fair_shift = clamp(8.0 * deviation_score + 4.0 * anchor_score, -20.0, 20.0)
        size_mult = 0.90 if good_book else 0.65
        ...
        return {
            ...
            "good_book": good_book,
            "progress": progress,
            "extreme_hold_bars": int(max(0, abs(current_pos) - 80) / 20),
            "flatten_before_flip": flatten_before_flip,
            "exit_mode": "recycler" if same_side_cooldown > 0 else "",
            "exit_age": int(same_side_cooldown),
            "long_peak_score": float(deviation_score if current_pos > 0 else 0.0),
            "short_peak_score": float(deviation_score if current_pos < 0 else 0.0),
            "long_peak_trend": float(trend_score if current_pos > 0 else 0.0),
            "short_peak_trend": float(trend_score if current_pos < 0 else 0.0),
            "long_drawdown": float(max(0.0, current_pos - target)),
            "short_drawup": float(max(0.0, target - current_pos)),
            "stretch_long": current_pos > 100,
            "stretch_short": current_pos < -100,
            ...
            "buy_take_extra": 1.8 if absolute_danger_long else 0.7 if same_side_bid_block else 0.0,
            "sell_take_extra": 1.8 if absolute_danger_short else 0.7 if same_side_ask_block else 0.0,
            ...
            "same_side_cooldown": int(same_side_cooldown),
        }
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        current_pos = int(state.position.get(HYDROGEL, 0))
        unwind_long = bool(hydro_ctx.get("same_side_bid_block", False) and current_pos > 0)
        unwind_short = bool(hydro_ctx.get("same_side_ask_block", False) and current_pos < 0)
        ...
        buy_take_allowed = not (same_side_bid_block or absolute_danger_long or current_pos >= 130)
        sell_take_allowed = not (same_side_ask_block or absolute_danger_short or current_pos <= -130)
        ...
        soft_limit = 65
        clear_edge = cfg["clear_edge"] + 0.10 * max(0.0, spread - 2.0)
        clear_max = max(cfg["clear_max"], 44)
        if unwind_long or unwind_short:
            soft_limit = 30
            clear_edge += 0.35
            clear_max = max(clear_max, 60)
        elif absolute_danger_long or absolute_danger_short:
            soft_limit = 25
            clear_edge += 0.45
            clear_max = max(clear_max, 72)
        ...
        reservation = fair + float(hydro_ctx["quote_bias"]) - (cfg["inv_skew"] + 1.5) * inv_ratio
        ...
        size_scale = max(0.25, 1.0 - abs(inv_ratio)) * max(0.35, float(hydro_ctx["size_mult"]))
        quote_size = max(6, int(round((0.70 * cfg["quote_size"]) * size_scale)))

        can_bid = mgr.buy_cap > 0 and pos < target + max(18, quote_size)
        can_ask = mgr.sell_cap > 0 and pos > target - max(18, quote_size)
