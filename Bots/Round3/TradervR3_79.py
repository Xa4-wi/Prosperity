...
class Trader:
    ...
    def _build_velvet_context(self, state: TradingState, memory: dict, overlay_bias: float) -> dict:
        ...
        mode = "NEUTRAL"
        if good_book:
            if directional_signal > 1.05 and trend_score > 0.35 and momentum_score > 0.20 and micro_gap > 0.0:
                mode = "TREND_LONG"
            elif directional_signal < -1.05 and trend_score < -0.35 and momentum_score < -0.20 and micro_gap < 0.0:
                mode = "TREND_SHORT"
            elif local_stretch <= -2.2 and micro_gap >= 0.0:
                mode = "FADE_LONG"
            elif local_stretch >= 2.2 and micro_gap <= 0.0:
                mode = "FADE_SHORT"
            elif signal > 0.65:
        ...
        if mode == "MARKET_MAKE":
            entry_cap = int(round(35 * vol_scale))
        elif mode in ("LOCAL_LONG", "LOCAL_SHORT"):
            entry_cap = int(round((65 if abs(conviction) < 1.8 else 85) * vol_scale))
        elif mode in ("FADE_LONG", "FADE_SHORT"):
            entry_cap = int(round((72 if abs(conviction) < 2.0 else 92) * vol_scale))
        elif mode in ("TREND_LONG", "TREND_SHORT"):
            entry_cap = int(round((85 if abs(conviction) < 2.1 else 110) * vol_scale))
        if not good_book:
            entry_cap = 20
        if progress > 0.80:
        ...
        if mode == "MARKET_MAKE":
            ...
            target = int(round(clamp(82.0 * math.tanh(max(0.0, -0.60 * local_stretch + 0.30 * micro_gap)), 0.0, float(entry_cap))))
        elif mode == "FADE_SHORT":
            target = int(round(clamp(82.0 * math.tanh(max(0.0, 0.60 * local_stretch - 0.30 * micro_gap)), 0.0, float(entry_cap)))) * -1
        ...
        exit_mode = ""
        if pos > 70 and ((directional_signal < 0.10 and signal < 0.20) or long_drawdown >= max(4.0, 2.1 * realized_vol) or (progress > 0.88 and long_drawdown >= 3.5)):
            exit_mode = "long"
        elif pos < -70 and ((directional_signal > -0.10 and signal > -0.20) or short_drawup >= max(4.0, 2.1 * realized_vol) or (progress > 0.88 and short_drawup >= 3.5)):
            exit_mode = "short"
    ...
    def _trade_velvet(self, state: TradingState, fair: float, velvet_ctx: dict) -> List[Order]:
        ...
        if mode in ("TREND_LONG", "LOCAL_LONG", "FADE_LONG"):
            buy_take_edge -= 0.18
        elif mode in ("TREND_SHORT", "LOCAL_SHORT", "FADE_SHORT"):
            sell_take_edge -= 0.18
        if mode == "MARKET_MAKE":
            buy_take_edge += 0.12
            sell_take_edge += 0.12
