...
LOW_STRIKE_PAIR_PRODUCTS = ("VEV_4000", "VEV_4500", "VEV_5000")
...
class Trader:
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        ...
        low_lane_liquid = [product for product in LOW_STRIKE_PAIR_PRODUCTS if product in liquid_products]
        low_pair_bias: Dict[str, int] = {}
        low_pair_targets: Dict[str, int] = {product: 0 for product in VOUCHER_STRIKES}
        low_pair_gap = 0.0
        low_pair_active = False
        low_pair_cheapest: Optional[str] = None
        low_pair_richest: Optional[str] = None
        if len(low_lane_liquid) >= 2:
            low_pair_cheapest = min(low_lane_liquid, key=lambda product: float(surface[product]["iv_residual"]))
            low_pair_richest = max(low_lane_liquid, key=lambda product: float(surface[product]["iv_residual"]))
            low_pair_gap = float(surface[low_pair_richest]["iv_residual"]) - float(surface[low_pair_cheapest]["iv_residual"])
            low_pair_active = low_pair_gap >= max(0.045, 0.95 * resid_scale)
            if low_pair_active and low_pair_cheapest != low_pair_richest:
                low_pair_bias[low_pair_cheapest] = 1
                low_pair_bias[low_pair_richest] = -1
                low_pair_targets[low_pair_cheapest] = 18
                low_pair_targets[low_pair_richest] = -18
        ...
        return {
            ...
            "low_pair_bias": low_pair_bias,
            "low_pair_targets": low_pair_targets,
            "low_pair_active": low_pair_active,
            "low_pair_gap": low_pair_gap,
            "low_pair_cheapest": low_pair_cheapest,
            "low_pair_richest": low_pair_richest,
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        low_pair_lane = product in LOW_STRIKE_PAIR_PRODUCTS
        ...
        low_pair_bias = int(risk_ctx["low_pair_bias"].get(product, 0))
        low_pair_target = int(risk_ctx["low_pair_targets"].get(product, 0))
        low_pair_active = bool(risk_ctx["low_pair_active"]) and low_pair_lane
        pair_bias += low_pair_bias
        pair_target += low_pair_target
        ...
        if low_strike_live and (confirmed or abs(bs_gap) >= 0.75):
            take_edge -= 0.10
        if low_pair_active and pair_target != 0:
            take_edge -= 0.12
        ...
        if low_pair_active and pair_target != 0:
            quote_edge = max(0.75, quote_edge - 0.10)
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "low_pair_active": bool(risk_ctx["low_pair_active"]),
                    "low_pair_gap": round(float(risk_ctx["low_pair_gap"]), 4),
                    "low_pair_cheapest": risk_ctx["low_pair_cheapest"],
                    "low_pair_richest": risk_ctx["low_pair_richest"],
                }
