...
class Trader:
    ...
    - Step 4: explicitly gate risk in the dangerous middle voucher band instead of
      letting generic portfolio logic touch the whole strip
    """
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict) -> List[Order]:
        ...
        middle_band = 1.0 if 0.20 <= delta <= 0.75 else 0.0

        take_edge = max(0.75, 0.35 * spread) + 0.30 * atm_risk + 0.30 * middle_band
        clear_edge = max(0.35, 0.15 * spread)
        soft_limit = max(70, int(round(180 - 110 * atm_risk - 20 * middle_band)))
        take_max = 8 if middle_band > 0.5 else 24
        ...
        quote_edge = max(1.0, 0.45 * spread, 0.015 * max(20.0, fair)) + 0.45 * atm_risk + 0.25 * middle_band
        ...
        size_scale *= max(0.30, 1.0 - 0.60 * atm_risk - 0.10 * middle_band)
