...
class Trader:
    ...
    Round 3 v22:
    - R3_16 baseline
    - Version B: voucher vanna hedge overlay
      - add strip vanna
      - hedge to strip_delta + lambda * strip_vanna * expected_vol_change
      - keep lambda small
    """
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        strip_vega = 0.0
        strip_vanna = 0.0
        near_atm_signed_resids: List[float] = []
        middle_abs = 0
        middle_net = 0
        low_wing_net = 0
        high_wing_net = 0
        ...
        for product, ctx in surface.items():
            ...
            strip_vanna += pos * float(ctx["vanna_proxy"])
            if 0.30 <= delta <= 0.70:
                near_atm_signed_resids.append(float(ctx["iv_residual"]))
            if 0.20 <= delta <= 0.80:
                middle_abs += abs(pos)
                middle_net += pos
        ...
        expected_vol_change = clamp(
            -(sum(near_atm_signed_resids) / len(near_atm_signed_resids) if near_atm_signed_resids else 0.0),
            -0.10,
            0.10,
        )
        vanna_lambda = 0.35
        hedge_delta = strip_delta + vanna_lambda * strip_vanna * expected_vol_change
        ...
        return {
            ...
            "delta_pressure": clamp(hedge_delta / 140.0, -2.0, 2.0),
            "target_velvet_pos": int(round(clamp(-0.50 * hedge_delta, -100.0, 100.0))),
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "expected_vol_change": round(float(risk_ctx["expected_vol_change"]), 4),
                    "hedge_delta": round(float(risk_ctx["hedge_delta"]), 3),
