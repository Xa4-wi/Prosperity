...
class Trader:
    ...
    - Step 8: lighter Hydrogel Phase B/C retune with late-session and stretched-book focus
    """
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if extreme_hold_bars > 36:
            target = int(round(target * 0.72))
        elif extreme_hold_bars > 20:
        ...
        stretch_long = current_pos > 125
        stretch_short = current_pos < -125
        exceptional_buy = signal >= 3.1 and progress < 0.80
        exceptional_sell = signal <= -3.1 and progress < 0.80
        ...
        return {
            ...
            "buy_take_extra": 2.5 if stretch_long and not exceptional_buy else 0.0,
            "sell_take_extra": 2.5 if stretch_short and not exceptional_sell else 0.0,
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        soft_limit = 70
        hard_zone = 115
        clear_edge = cfg["clear_edge"] + (0.15 if hydro_ctx["progress"] > 0.80 else 0.0)
        hard_clear_edge = clear_edge + 2.0

        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 64)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 40)))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 64)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 40)))

        pos = mgr.projected()
        relative_pos = pos - target
        inv_ratio = relative_pos / LIMITS[HYDROGEL]
        dynamic_skew = cfg["inv_skew"] + 4.0
        if abs(relative_pos) > 125:
            dynamic_skew += 2.0
        if hydro_ctx["progress"] > 0.88:
            dynamic_skew += 1.5
