...
class Trader:
    ...
    def _build_velvet_context(self, state: TradingState, memory: dict, overlay_bias: float) -> dict:
        ...
        flow_score = clamp(0.55 * micro_gap + 0.85 * imbalance + 0.45 * momentum_score, -3.0, 3.0)
        directional_signal = 0.45 * trend_score + 0.25 * momentum_score + 0.20 * imbalance + 0.10 * micro_gap
        conviction = 0.45 * signal + 0.25 * directional_signal + 0.20 * fair_ema_gap + 0.10 * flow_score
        ...
        mode = "NEUTRAL"
        if good_book:
            if directional_signal > 0.90 and trend_score > 0.20:
                mode = "TREND_LONG"
            elif directional_signal < -0.90 and trend_score < -0.20:
                mode = "TREND_SHORT"
            elif signal > 0.65:
                mode = "LOCAL_LONG"
            elif signal < -0.65:
        ...
        if mode == "MARKET_MAKE":
            entry_cap = int(round(26 * vol_scale))
        elif mode in ("LOCAL_LONG", "LOCAL_SHORT"):
            entry_cap = int(round((52 if abs(conviction) < 1.8 else 70) * vol_scale))
        elif mode in ("TREND_LONG", "TREND_SHORT"):
            entry_cap = int(round((72 if abs(conviction) < 2.1 else 96) * vol_scale))
        if not good_book:
            entry_cap = 20
        if progress > 0.80:
        ...
        target = 0
        if mode == "MARKET_MAKE":
            target = int(round(clamp(22.0 * math.tanh(0.95 * signal + 0.35 * micro_gap), -float(entry_cap), float(entry_cap))))
        ...
        exit_mode = ""
        if pos > 70 and ((directional_signal < 0.10 and signal < 0.20) or long_drawdown >= max(5.0, 2.4 * realized_vol) or (progress > 0.88 and long_drawdown >= 4.0)):
            exit_mode = "long"
        elif pos < -70 and ((directional_signal > -0.10 and signal > -0.20) or short_drawup >= max(5.0, 2.4 * realized_vol) or (progress > 0.88 and short_drawup >= 4.0)):
            exit_mode = "short"
        ...
        take_bias = 0.16 * directional_signal * move_unit + 0.08 * signal * move_unit
        if exit_mode == "long":
    ...
    def _trade_velvet(self, state: TradingState, fair: float, velvet_ctx: dict) -> List[Order]:
        ...
        if mode in ("TREND_LONG", "LOCAL_LONG"):
            buy_take_edge -= 0.18
        elif mode in ("TREND_SHORT", "LOCAL_SHORT"):
            sell_take_edge -= 0.18
        if mode == "MARKET_MAKE":
            buy_take_edge += 0.12
            sell_take_edge += 0.12
