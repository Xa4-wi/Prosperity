...
# Explicit scalp layers so we can separate fast relative-value flow from the
# normal inventory / market-making book in traderData.
SCALP_NONE = 0
...
SCALP_TAGS = {
    SCALP_NONE: "normal_mm",
    SCALP_VOUCHER_PAIR: "voucher_pair",
    SCALP_VOUCHER_SHOCK: "voucher_shock",
    SCALP_VEV_HEDGE: "vev_hedge_rebalance",
    SCALP_HYDRO_LOCAL: "hydro_local_fair",
}
...
class Trader:
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float, memory: dict) -> Dict[str, dict]:
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            # New: clean the bid-side and ask-side option slices separately so
            # the trading engine gets execution-aware fair values instead of a
            # single mid-only smile.
            if bb is not None:
                bid_quote_rows.append(
                    {
                        "product": product,
                        "strike": strike,
                        "raw_mid": float(bb),
        ...
        for product, row in repaired_rows.items():
            ...
            if 1e-6 < market_iv < 3.0:
                strike_rows[product] = {
                    ...
                    "bid_repair_amount": 0.0 if bid_side_row is None else float(bid_side_row["repair_amount"]),
                    "ask_repair_amount": 0.0 if ask_side_row is None else float(ask_side_row["repair_amount"]),
                    "market_iv": market_iv,
        ...
        smile_stability_score = clamp(
            0.45 * clamp(len(points_iv) / 8.0, 0.0, 1.0)
            + 0.35 * clamp(1.0 - float(fit_diag.get("sse") or 0.0) / 0.02, 0.0, 1.0)
            + 0.20 * clamp(1.0 - int(repair_diag.get("repair_count", 0)) / max(1.0, float(len(points_iv) or 1)), 0.0, 1.0),
            0.0,
            1.0,
        )
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            if dislocation > 0.09:
                ...
                rolling_w += 0.12
                previous_w -= 0.04
            # New: use more local IV when the smile fit itself looks unstable.
            if smile_stability_score < 0.70:
                shift = clamp(0.70 - smile_stability_score, 0.0, 0.25)
                structural_w -= 0.45 * shift
                rolling_w += 0.35 * shift
                previous_w += 0.10 * shift
            if market_iv is None:
                structural_w = 0.80
                rolling_w = 0.0
                previous_w = 0.20
            total_w = max(1e-6, structural_w + rolling_w + previous_w)
            sigma = clamp(
                (structural_w * structural_sigma + rolling_w * rolling_iv + previous_w * prev_fair_iv) / total_w,
                1e-4,
                3.0,
            )
            ...
            observed_iv_width = max(
                0.01 + 0.004 * spread,
                0.0 if observed_bid_iv is None or observed_ask_iv is None else float(observed_ask_iv) - float(observed_bid_iv),
            )
            ...
            if bid_coeffs is not None:
                fitted_bid_iv = clamp(float(polyval(bid_coeffs, m)), 1e-4, 3.0)
            if ask_coeffs is not None:
                fitted_ask_iv = clamp(float(polyval(ask_coeffs, m)), 1e-4, 3.0)
            fair_bid_iv = 0.60 * fitted_bid_iv + 0.40 * (sigma - side_half)
            fair_ask_iv = 0.60 * fitted_ask_iv + 0.40 * (sigma + side_half)
            if fair_bid_iv >= fair_ask_iv:
                fair_bid_iv = max(1e-4, sigma - max(0.006, side_half))
            ...
            surface[product] = {
                ...
                "market_iv": market_iv,
                "market_bid_iv": observed_bid_iv,
                "market_ask_iv": observed_ask_iv,
                "fair_bid_iv": fair_bid_iv,
                "fair_ask_iv": fair_ask_iv,
                "fair_bid": black_scholes_call(velvet_fair, strike, TTE_YEARS, fair_bid_iv),
                "fair_ask": black_scholes_call(velvet_fair, strike, TTE_YEARS, fair_ask_iv),
                "local_dislocation": dislocation,
                "spread": float(row.get("spread", 4.0)),
                "liquidity": float(row.get("liquidity", 0.0)),
            }
        ...
        surface["_repair_diag"] = {
            **repair_diag,
            "bid_repair_count": int(bid_repair_diag.get("repair_count", 0)),
            "ask_repair_count": int(ask_repair_diag.get("repair_count", 0)),
        }
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        ...
        # New: build an explicit pair-scalp layer. This keeps fast voucher pair
        # trades auditable instead of letting them disappear inside the normal
        # strip book. Active pairs stay on until their spread normalizes.
        vev_state = memory["vev_voucher_state"]
        ...
        for idx, left in enumerate(sorted_products):
            ...
            for right in sorted_products[idx + 1 : min(len(sorted_products), idx + 4)]:
                ...
                both_executable = (
                    cheap_ctx.get("market_ask_iv") is not None
                    and rich_ctx.get("market_bid_iv") is not None
                )
                ...
                if VOUCHER_STRIKES[cheap_product] <= 5000 and VOUCHER_STRIKES[rich_product] <= 5100:
                    candidate_qty = 24
                elif 0.20 <= float(cheap_ctx["delta"]) <= 0.80 or 0.20 <= float(rich_ctx["delta"]) <= 0.80:
                    candidate_qty = 16
                candidate_score = spread - 0.20 * pair_delta_balance
                ...
                pair_candidates[pair_key] = {
                    ...
                    "opposite_signed": opposite_signed,
                    "qty": candidate_qty,
                }
        ...
        pair_exit_threshold = max(0.030, 0.55 * pair_entry_threshold)
        ...
        if abs(strip_delta) < 0.95 * total_delta_cap and middle_abs < middle_cap:
            for pair_key, active_payload in active_pair_scalps.items():
                ...
                if float(candidate["spread"]) <= pair_exit_threshold:
                    continue
                next_active_pairs[pair_key] = {
                    "cheap": cheap_product,
                    "rich": rich_product,
                    "spread": float(candidate["spread"]),
                    "qty": int(active_payload.get("qty", candidate["qty"])),
                }
        ...
        # New: use a buffered hedge ladder so Velvet acts as a hedge book first
        # and only scales up when the strip dislocation is broad and confirmed.
        hedge_ratio = 0.25
        hedge_deadband = 20.0
        ...
        if smile_stability_score < 0.55:
            hedge_ratio = max(hedge_ratio, 0.40 if not broad_dislocation else hedge_ratio)
        ...
        for product in VOUCHER_STRIKES:
            ...
            pair_confirmed = (
                abs(int(pair_targets.get(product, 0))) > 0
                or abs(int(pair_bias.get(product, 0))) > 0
                or abs(int(pair_scalp_targets.get(product, 0))) > 0
            )
            if strike <= 5000:
                base_cap = 90
            elif strike <= 5100:
            ...
            if broad_dislocation and strike <= 5000:
                hedge_cap *= 1.08
            # New: pair-backed middle strikes can carry a little more size than
            # unpaired middle strikes, but outright flow stays capped tighter.
            if 0.20 <= float(ctx["delta"]) <= 0.80:
                hedge_cap *= 1.10 if pair_confirmed else 0.85
        ...
        hedge_feasible_size_score = clamp(
            0.55 * clamp(hedge_headroom / 150.0, 0.0, 1.0)
            + 0.25 * clamp(1.0 - abs(strip_delta) / max(1.0, total_delta_cap), 0.0, 1.0)
            + 0.20 * clamp(1.0 - middle_abs / max(1.0, middle_cap), 0.0, 1.0),
            0.0,
            1.0,
        )
        ...
        return {
            ...
            "active_pair_scalps": next_active_pairs,
            "pair_entry_threshold": pair_entry_threshold,
            "pair_exit_threshold": pair_exit_threshold,
            "pair_scalp_targets": pair_scalp_targets,
            "scalp_mode_by_product": scalp_mode_by_product,
            "scalp_tag_by_product": scalp_tag_by_product,
            "scalp_pair_spread": scalp_pair_spread,
            "scalp_pair_key": scalp_pair_key,
            "residual_rank": residual_rank,
            "smile_stability_score": smile_stability_score,
    ...
    def _build_velvet_context(self, state: TradingState, velvet_fair: float, risk_ctx: dict) -> dict:
        ...
        # New: keep Velvet explicitly split into hedge and capped alpha books.
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        # New: pair-backed trades are the default path; outright trades need
        # stronger confirmation and stay smaller unless the strip is extreme.
        ...
        outright_threshold = 1.20 * low_strike_threshold if middle_band else 1.10 * low_strike_threshold
        buy_confirmed = (
            pair_bias > 0
            or pair_target > 0
            or cheap_signal >= outright_threshold
            or iv_band_buy_gap >= 1.10 * band_threshold
        )
        sell_confirmed = (
            pair_bias < 0
            or pair_target < 0
            or rich_signal >= outright_threshold
            or iv_band_sell_gap >= 1.10 * band_threshold
        )
        ...
        if scalp_mode == SCALP_VOUCHER_PAIR:
            take_max += 2
            take_edge = max(0.55, take_edge - 0.20)
        elif scalp_mode == SCALP_VOUCHER_SHOCK:
            take_edge = max(0.60, take_edge - 0.10)

        # New: explicit scalp layer. Voucher pair scalps are handled before the
        # normal book so we can later separate fast RV flow from ordinary MM.
        pos = mgr.projected()
        ...
        reservation = fair - inv_penalty + 0.30 * signal_shift + 0.12 * pair_bias + 0.004 * pair_target + 0.005 * scalp_target
    ...
    def run(self, state: TradingState):
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                if result[HYDROGEL]:
                    order_tags[HYDROGEL] = [
                        {"price": int(order.price), "qty": int(order.quantity), "tag": SCALP_TAGS[SCALP_HYDRO_LOCAL]}
                        for order in result[HYDROGEL]
                    ]
            except Exception as exc:
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "hedge_feasible_size_score": round(float(risk_ctx["hedge_feasible_size_score"]), 4),
                    "pair_agreement_count": int(risk_ctx["pair_agreement_count"]),
                    "pair_entry_threshold": round(float(risk_ctx["pair_entry_threshold"]), 4),
                    "pair_exit_threshold": round(float(risk_ctx["pair_exit_threshold"]), 4),
                    "broad_dislocation": bool(risk_ctx["broad_dislocation"]),
                    ...
                    "vev_final_target": int(velvet_ctx["target"]),
                    "vev_hedge_target": int(risk_ctx["target_velvet_pos"]),
                    "velvet_alpha_target": int(velvet_ctx["alpha_target"]),
                    "surface_model": str(voucher_surface.get("_fit_diag", {}).get("model", "quadratic")),
                    ...
                    "active_pair_scalps": dict(risk_ctx["active_pair_scalps"]),
                }
                memory["voucher_targets"] = {
                    product: {
                        "scalp_mode": int(risk_ctx["scalp_mode_by_product"].get(product, SCALP_NONE)),
                        "trade_tag": str(risk_ctx["scalp_tag_by_product"].get(product, SCALP_TAGS[SCALP_NONE])),
                        "scalp_pair_key": str(risk_ctx["scalp_pair_key"].get(product, "")),
                        "scalp_pair_spread": round(float(risk_ctx["scalp_pair_spread"].get(product, 0.0)), 4),
                        "scalp_target": int(risk_ctx["pair_scalp_targets"].get(product, 0)),
                        "pair_target": int(risk_ctx["pair_targets"].get(product, 0)),
                        "pair_bias": int(risk_ctx["pair_bias"].get(product, 0)),
                        "iv_residual": round(float(voucher_surface[product]["iv_residual"]), 4),
                        "fair_mid_iv": round(float(voucher_surface[product]["sigma"]), 4),
                        "iv_band_buy_gap": round(float(voucher_surface[product].get("iv_band_buy_gap", 0.0)), 4),
                        "iv_band_sell_gap": round(float(voucher_surface[product].get("iv_band_sell_gap", 0.0)), 4),
                        "fair_bid_iv": round(float(voucher_surface[product].get("fair_bid_iv", voucher_surface[product]["sigma"])), 4),
                        "fair_ask_iv": round(float(voucher_surface[product].get("fair_ask_iv", voucher_surface[product]["sigma"])), 4),
                        "rolling_iv": round(float(voucher_surface[product].get("rolling_iv", voucher_surface[product]["sigma"])), 4),
                        "delta": round(float(voucher_surface[product]["delta"]), 4),
                        "feasible_cap": int(risk_ctx["feasible_caps"].get(product, LIMITS[product])),
                ...
                if result[VELVET]:
                    ...
                    order_tags[VELVET] = [
                        {"price": int(order.price), "qty": int(order.quantity), "tag": vev_tag}
                        for order in result[VELVET]
                    ]
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        ...
                        if result[product]:
                            trade_tag = str(risk_ctx["scalp_tag_by_product"].get(product, SCALP_TAGS[SCALP_NONE]))
                            order_tags[product] = [
                                {"price": int(order.price), "qty": int(order.quantity), "tag": trade_tag}
