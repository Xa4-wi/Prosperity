...
class Trader:
    ...
    Round 3 v23:
    - R3_16 baseline
    - Version C: voucher vanna event mode
      - detect high near-ATM vanna concentration + large near-ATM residual
      - relax pair-entry thresholds slightly during those events
    """
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        for product, ctx in surface.items():
            ...
            if 0.30 <= delta <= 0.70:
                near_atm_vanna_abs += abs(pos * float(ctx["vanna_proxy"]))
        ...
        vanna_event = near_atm_vanna_abs > 18.0 and near_atm_resid_abs_mean > max(0.045, 1.10 * resid_scale)
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        vanna_event = bool(risk_ctx.get("vanna_event", False))
        if vanna_event and middle_band:
            resid_threshold *= 0.92
            extreme_resid_threshold *= 0.95
        ...
        if vanna_event and middle_band:
            take_edge = max(0.60, take_edge - 0.15)
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "vanna_event": bool(risk_ctx["vanna_event"]),
