...
class Trader:
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        ...
        activation_allowed: Dict[str, bool] = {}
        ...
        for product in ordered_products:
            ...
            allow_trade = True
            # Upper strikes stay mostly dormant unless they have real IV edge or pair backing.
            if strike == 5100:
                allow_trade = (
                    active_score >= 0.72
                    or abs(pair_target_bias[product]) >= 0.35
                    or abs(float(ctx["iv_residual"])) >= 0.75 * extreme_resid_threshold
                )
                if not allow_trade:
                    active_cap = max(16, int(round(0.45 * active_cap)))
            elif strike == 5200:
                allow_trade = (
                    active_score >= 0.92
                    or abs(pair_target_bias[product]) >= 0.55
                    or abs(float(ctx["iv_residual"])) >= 1.00 * extreme_resid_threshold
                )
                if not allow_trade:
                    active_cap = 0
            elif strike >= 5300:
                allow_trade = (
                    ...
                    or (
                        provisional_broad
                        and abs(float(ctx["iv_residual"])) >= 1.15 * extreme_resid_threshold
                        and float(ctx["liquidity"]) >= 12.0
                    )
                )
                if not allow_trade:
                    active_cap = 0
            activation_allowed[product] = allow_trade
            if allow_trade and (active_score >= 0.95 or abs(pair_target_bias[product]) >= PAIR_PRIMARY_BIAS_THRESHOLD):
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        harvest_mode = bool(risk_ctx.get("harvest_mode", False))
        strip_mode = str(risk_ctx.get("strip_mode", "DISCOVERY"))
        activation_allowed = bool(risk_ctx.get("activation_allowed", {}).get(product, True))
        bb = best_bid(od)
        ba = best_ask(od)
        spread = (ba - bb) if bb is not None and ba is not None else 2.0
        ...
        if not activation_allowed and mgr.projected() == 0:
            target = 0
        ...
        if fair >= 0.5:
            ...
            if not activation_allowed and pos == 0:
                can_bid = False
                can_ask = False
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "upper_strike_active": [
                        product
                        for product in sorted(VOUCHER_STRIKES, key=VOUCHER_STRIKES.get)
                        if VOUCHER_STRIKES[product] > 5000 and bool(risk_ctx["activation_allowed"].get(product, False))
                    ][:5],
                    "selected_pairs": [
