...
VOUCHER_STRIKES: Dict[str, int] = {'VEV_4000': 4000, 'VEV_4500': 4500, 'VEV_5000': 5000, 'VEV_5100': 5100, 'VEV_5200': 5200, 'VEV_5300': 5300, 'VEV_5400': 5400, 'VEV_5500': 5500, 'VEV_6000': 6000, 'VEV_6500': 6500}
SCALP_NONE = 0
SCALP_VOUCHER_PAIR = 1
SCALP_VOUCHER_SHOCK = 2
SCALP_VEV_HEDGE = 3
SCALP_HYDRO_LOCAL = 4
SCALP_TAGS = {SCALP_NONE: 'normal_mm', SCALP_VOUCHER_PAIR: 'voucher_pair', SCALP_VOUCHER_SHOCK: 'voucher_shock', SCALP_VEV_HEDGE: 'vev_hedge_rebalance', SCALP_HYDRO_LOCAL: 'hydro_local_fair'}
LIMITS: Dict[str, int] = {HYDROGEL: 200, VELVET: 200, **{product: 300 for product in VOUCHER_STRIKES}}
TTE_YEARS = 5.0 / 365.0
UNDERLYING_CFG = {HYDROGEL: {'anchor': 10000.0, 'anchor_w': 0.54, 'stable_w': 0.29, 'micro_w': 0.17, 'imbalance_w': 1.05, 'take_edge': 2.2, 'quote_edge': 3.0, 'clear_edge': 1.0, 'soft_limit': 100, 'take_max': 22, 'clear_max': 36, 'quote_size': 24, 'inv_skew': 8.0}, VELVET: {'anchor': 5250.0, 'anchor_w': 0.46, 'stable_w': 0.34, 'micro_w': 0.2, 'imbalance_w': 0.85, 'take_edge': 1.2, 'quote_edge': 1.5, 'clear_edge': 0.6, 'soft_limit': 120, 'take_max': 34, 'clear_max': 50, 'quote_size': 36, 'inv_skew': 5.0}}
...
def fit_quadratic(xs: Sequence[float], ys: Sequence[float]) -> Tuple[float, float, float]:
    ...
    sol = solve_3x3([[s4, s3, s2], [s3, s2, s1], [s2, s1, s0]], [t2, t1, t0])
    if sol is None:
        median = sorted(ys)[len(ys) // 2]
        return (0.0, 0.0, float(median))
    return (float(sol[0]), float(sol[1]), float(sol[2]))

def fit_quadratic_weighted(xs: Sequence[float], ys: Sequence[float], ws: Sequence[float]) -> Tuple[float, float, float]:
    if len(xs) != len(ys) or len(xs) != len(ws) or len(xs) < 3:
        return fit_quadratic(xs, ys)
    safe_ws = [max(1e-06, float(w)) for w in ws]
    s0 = sum(safe_ws)
...
def fit_svi_slice_weighted(ks: Sequence[float], total_vars: Sequence[float], ws: Sequence[float]) -> Optional[dict]:
    if len(ks) != len(total_vars) or len(ks) != len(ws) or len(ks) < 4:
        return None
    k_min = min(ks)
    k_max = max(ks)
    k_span = max(0.08, k_max - k_min)
    rho_grid = [-0.85, -0.6, -0.35, -0.1, 0.15, 0.4, 0.65, 0.85]
    m_grid = [k_min - 0.15 * k_span + 0.18 * k_span * i for i in range(9)]
    sigma_grid = [0.01, 0.03, 0.06, 0.1, 0.16, 0.24]
    ...
    def evaluate(rho: float, m: float, sigma: float) -> Optional[dict]:
        ...
        if min(fitted) <= 1e-08:
            return None
        sse = sum((max(1e-06, float(w)) * (fit - target) * (fit - target) for (fit, target, w) in zip(fitted, total_vars, ws)))
    ...
    refine_m = [best_m + delta * k_span for delta in (-0.1, -0.05, 0.0, 0.05, 0.1)]
    refine_sigma = [max(0.005, best_sigma * scale) for scale in (0.65, 0.85, 1.0, 1.2, 1.45)]
...
def repair_call_prices(quote_rows: List[dict], spot: float) -> Tuple[Dict[str, dict], dict]:
    if not quote_rows:
        return ({}, {'repair_count': 0, 'total_repair': 0.0, 'max_repair': 0.0, 'repaired_products': []})
    rows = [dict(row) for row in sorted(quote_rows, key=lambda item: int(item['strike']))]
    for row in rows:
        strike = float(row['strike'])
        intrinsic = max(spot - strike, 0.0)
        raw_mid = float(row['raw_mid'])
        guarded_price = max(raw_mid, intrinsic + 0.001)
    ...
    for row in rows:
        ...
        floored = max(float(row['intrinsic']) + 0.001, capped)
        if floored + 1e-09 < float(row['repaired_price']):
    ...
    for _ in range(3):
        ...
        for idx in range(1, len(rows) - 1):
            ...
            mid = rows[idx]
            right = rows[idx + 1]
            upper = linear_interp(float(left['strike']), float(left['repaired_price']), float(right['strike']), float(right['repaired_price']), float(mid['strike']))
            upper = min(upper, float(left['repaired_price']))
            lower = max(float(mid['intrinsic']) + 0.001, float(right['repaired_price']))
            if float(mid['repaired_price']) > upper + 1e-09:
    ...
    for row in rows:
        ...
        row['repaired_flag'] = abs(repair_amount) > 1e-09
    ...
    diagnostics = {'repair_count': len(repaired_products), 'total_repair': round(total_repair, 6), 'max_repair': round(max_repair, 6), 'repaired_products': repaired_products}
...
class Trader:
    ...
    - Step 16: add execution-aware voucher improvements:
      side-specific smile bands, hybrid local IV fair, dynamic buffered
      Velvet hedge, and pair-first strip trading with shock mode
    - Step 17: add an explicit scalp layer so voucher pair scalps are
      auditable separately from the normal strip market-making book
    """
    ...
    def _reset_day_if_needed(self, memory: dict, timestamp: int) -> None:
        ...
        if last_ts is not None and timestamp < last_ts:
            memory['velvet_overlay'] = {'day_low': 10 ** 18, 'day_high': -10 ** 18, 'signal': 0.0, 'age': 999}
            memory['hydro_state'] = {'extreme_side': 0, 'extreme_hold_bars': 0, 'ema_fast': None, 'ema_slow': None, 'ret_ema': 0.0, 'last_mid': None, 'long_peak_score': 0.0, 'short_peak_score': 0.0, 'long_peak_mid': None, 'short_peak_mid': None, 'long_peak_trend': 0.0, 'short_peak_trend': 0.0, 'exit_mode': '', 'exit_age': 0}
        memory['last_timestamp'] = timestamp
        memory.setdefault('velvet_overlay', {'day_low': 10 ** 18, 'day_high': -10 ** 18, 'signal': 0.0, 'age': 999})
        memory.setdefault('hydro_state', {'extreme_side': 0, 'extreme_hold_bars': 0, 'ema_fast': None, 'ema_slow': None, 'ret_ema': 0.0, 'last_mid': None, 'long_peak_score': 0.0, 'short_peak_score': 0.0, 'long_peak_mid': None, 'short_peak_mid': None, 'long_peak_trend': 0.0, 'short_peak_trend': 0.0, 'exit_mode': '', 'exit_age': 0})
        memory.setdefault('vev_voucher_state', {'last_avg_abs_resid': 0.0, 'rolling_iv': {}, 'rolling_fair_iv': {}, 'last_surface_model': 'quadratic', 'active_pair_scalps': {}})
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        good_book = bb is not None and ba is not None and (bb < ba) and (spread <= 18.0) and (top_depth >= 20.0) and (abs(float(stable) - float(mid)) <= 1.1)
        ...
        regime_score = 0.6 * trend_score + 0.3 * anchor_score + 0.1 * flow_score
        ...
        if not good_book:
            ...
            confidence = 'strong'
            target_cap = 200
        progress = clamp(state.timestamp / 3000000.0, 0.0, 1.0)
        ...
        trend_fade_long = long_peak_trend > 0.0 and trend_score < 0.7 * long_peak_trend
        trend_fade_short = short_peak_trend < 0.0 and trend_score > 0.7 * short_peak_trend
        fair_gap = abs(fair - float(mid))
        signal_fade = fair_gap < max(4.0, 0.55 * spread)
        unwind_long = current_pos > 150 and (long_peak_score >= 1.35 and long_drawdown >= 12.0 and (regime_score < max(0.7, 0.65 * long_peak_score)) and (trend_score < 1.25) or (trend_fade_long and long_drawdown >= 8.0) or (ret_ema < 0.0 and long_drawdown >= 8.0) or (signal_fade and long_drawdown >= 10.0 and (current_pos > 140)) or (long_drawdown >= 18.0 and ret_ema < -0.12) or (progress > 0.9 and long_drawdown >= 10.0 and (trend_score < 0.95)))
        unwind_long_hard = current_pos > 170 and (trend_fade_long and long_drawdown >= 16.0 and (ret_ema < -0.05) or (long_drawdown >= 28.0 and ret_ema < -0.18) or (progress > 0.94 and long_drawdown >= 12.0))
        unwind_short = current_pos < -150 and (short_peak_score <= -1.35 and short_drawup >= 12.0 and (regime_score > min(-0.7, 0.65 * short_peak_score)) and (trend_score > -1.25) or (trend_fade_short and short_drawup >= 8.0) or (ret_ema > 0.0 and short_drawup >= 8.0) or (signal_fade and short_drawup >= 10.0 and (current_pos < -140)) or (short_drawup >= 18.0 and ret_ema > 0.12) or (progress > 0.9 and short_drawup >= 10.0 and (trend_score > -0.95)))
        unwind_short_hard = current_pos < -170 and (trend_fade_short and short_drawup >= 16.0 and (ret_ema > 0.05) or (short_drawup >= 28.0 and ret_ema > 0.18) or (progress > 0.94 and short_drawup >= 12.0))
        prev_exit_mode = str(hydro_state.get('exit_mode', ''))
        prev_exit_age = int(hydro_state.get('exit_age', 0))
        strong_long_reconfirm = current_pos > 60 and long_peak_trend > 0.0 and (trend_score > max(1.9, 0.94 * long_peak_trend)) and (regime_score > max(1.65, 0.85 * long_peak_score)) and (ret_ema > 0.1) and (long_drawdown < 6.0) and (fair_gap > max(4.0, 0.55 * spread))
        strong_short_reconfirm = current_pos < -60 and short_peak_trend < 0.0 and (trend_score < min(-1.9, 0.94 * short_peak_trend)) and (regime_score < min(-1.65, 0.85 * short_peak_score)) and (ret_ema < -0.1) and (short_drawup < 6.0) and (fair_gap > max(4.0, 0.55 * spread))
        exit_mode = ''
        ...
        if exit_mode == 'long':
            if exit_age <= 3:
                ...
                target = min(target, 0 if progress > 0.9 or long_drawdown >= 16.0 else 20)
            ...
            if exit_age <= 3:
                ...
                target = max(target, 0 if progress > 0.9 or short_drawup >= 16.0 else -20)
        ...
        if confidence == 'neutral':
            size_mult *= 0.45
        if progress > 0.9 and abs(regime_score) < 1.6:
            size_mult *= 0.8
        ...
        if absolute_danger_long or absolute_danger_short:
            size_mult *= 0.75
        return {'mid': float(mid), 'signal': float(signal), 'regime_score': float(regime_score), 'trend_score': float(trend_score), 'anchor_score': float(anchor_score), 'flow_score': float(flow_score), 'confidence': confidence, 'entry_cap': int(entry_cap), 'hold_cap': int(hold_cap), 'entry_target': int(entry_target), 'hold_target': int(hold_target), 'target': int(clamp(float(target), -200.0, 200.0)), 'good_book': good_book, 'progress': progress, 'extreme_hold_bars': extreme_hold_bars, 'flatten_before_flip': flatten_before_flip, 'exit_mode': exit_mode, 'exit_age': exit_age, 'long_peak_score': long_peak_score, 'short_peak_score': short_peak_score, 'long_peak_trend': long_peak_trend, 'short_peak_trend': short_peak_trend, 'long_drawdown': long_drawdown, 'short_drawup': short_drawup, 'stretch_long': stretch_long, 'stretch_short': stretch_short, 'absolute_danger_long': absolute_danger_long, 'absolute_danger_short': absolute_danger_short, 'same_side_bid_block': exit_mode.startswith('long') or absolute_danger_long or (stretch_long and (not exceptional_buy)), 'same_side_ask_block': exit_mode.startswith('short') or absolute_danger_short or (stretch_short and (not exceptional_sell)), 'buy_take_extra': 3.0 if exit_mode.startswith('long') else 2.5 if absolute_danger_long else 2.0 if stretch_long and (not exceptional_buy) else 0.0, 'sell_take_extra': 3.0 if exit_mode.startswith('short') else 2.5 if absolute_danger_short else 2.0 if stretch_short and (not exceptional_sell) else 0.0, 'quote_bias': quote_bias, 'fair_shift': fair_shift, 'size_mult': size_mult}
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        size_scale = max(0.2, 1.0 - abs(inv_ratio)) * max(0.35, float(hydro_ctx['size_mult']))
    ...
    def _trade_underlying(self, product: str, state: TradingState, fair: float, position_target: float=0.0, take_bias: float=0.0, quote_bias: float=0.0, size_mult: float=1.0) -> List[Order]:
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float, memory: dict) -> Dict[str, dict]:
        ...
        rolling_fair_state = vev_state.setdefault('rolling_fair_iv', {})
        points_m: List[float] = []
        points_iv: List[float] = []
        points_w: List[float] = []
        points_k: List[float] = []
        bid_points_m: List[float] = []
        bid_points_iv: List[float] = []
        bid_points_w: List[float] = []
        ask_points_m: List[float] = []
        ask_points_iv: List[float] = []
        ask_points_w: List[float] = []
        strike_rows: Dict[str, dict] = {}
        raw_quote_rows: List[dict] = []
        bid_quote_rows: List[dict] = []
        ask_quote_rows: List[dict] = []
        for (product, strike) in VOUCHER_STRIKES.items():
            ...
            if market_mid is None:
                continue
            raw_quote_rows.append({'product': product, 'strike': strike, 'raw_mid': float(market_mid), 'spread': spread, 'liquidity': liquidity, 'best_bid': float(bb) if bb is not None else None, 'best_ask': float(ba) if ba is not None else None})
            if bb is not None:
                bid_quote_rows.append({'product': product, 'strike': strike, 'raw_mid': float(bb), 'spread': spread, 'liquidity': liquidity, 'best_bid': float(bb), 'best_ask': float(ba) if ba is not None else None})
            if ba is not None:
                ask_quote_rows.append({'product': product, 'strike': strike, 'raw_mid': float(ba), 'spread': spread, 'liquidity': liquidity, 'best_bid': float(bb) if bb is not None else None, 'best_ask': float(ba)})
        (repaired_rows, repair_diag) = repair_call_prices(raw_quote_rows, velvet_fair)
        (bid_repaired_rows, bid_repair_diag) = repair_call_prices(bid_quote_rows, velvet_fair)
        (ask_repaired_rows, ask_repair_diag) = repair_call_prices(ask_quote_rows, velvet_fair)
        for (product, row) in repaired_rows.items():
            ...
            intrinsic = max(velvet_fair - float(row['strike']), 0.0)
            bid_iv = None
            ask_iv = None
            bid_side_row = bid_repaired_rows.get(product)
            ask_side_row = ask_repaired_rows.get(product)
            if bid_side_row is not None:
                bid_price = max(float(bid_side_row['repaired_price']), intrinsic + 0.001)
                bid_iv_val = implied_vol_call(bid_price, velvet_fair, float(row['strike']), TTE_YEARS)
                if 1e-06 < bid_iv_val < 3.0:
                    bid_iv = bid_iv_val
            if ask_side_row is not None:
                ask_price = max(float(ask_side_row['repaired_price']), intrinsic + 0.001)
                ask_iv_val = implied_vol_call(ask_price, velvet_fair, float(row['strike']), TTE_YEARS)
                if 1e-06 < ask_iv_val < 3.0:
                    ask_iv = ask_iv_val
            if 1e-06 < market_iv < 3.0:
                strike_rows[product] = {'strike': int(row['strike']), 'log_moneyness': k, 'moneyness': m, 'market_mid': float(row['raw_mid']), 'raw_guarded_price': float(row['raw_guarded_price']), 'guarded_price': repaired_price, 'repaired_price': repaired_price, 'repair_amount': float(row['repair_amount']), 'repaired_flag': bool(row['repaired_flag']), 'repair_reasons': list(row['repair_reasons']), 'bid_repair_amount': 0.0 if bid_side_row is None else float(bid_side_row['repair_amount']), 'ask_repair_amount': 0.0 if ask_side_row is None else float(ask_side_row['repair_amount']), 'market_iv': market_iv, 'market_bid_iv': bid_iv, 'market_ask_iv': ask_iv, 'spread': float(row['spread']), 'liquidity': float(row['liquidity'])}
                weight = clamp((math.log1p(float(row['liquidity'])) + 0.5) / float(row['spread']), 0.4, 4.0)
                weight *= clamp(math.exp(-0.55 * abs(m)), 0.45, 1.0)
                weight *= 1.0 if not row['repaired_flag'] else 0.9
                points_k.append(k)
                points_m.append(m)
                points_iv.append(market_iv)
                points_w.append(weight)
                if bid_iv is not None:
                    bid_points_m.append(m)
                    bid_points_iv.append(bid_iv)
                    bid_points_w.append(weight)
                if ask_iv is not None:
                    ask_points_m.append(m)
                    ask_points_iv.append(ask_iv)
                    ask_points_w.append(weight)
        if points_iv:
        ...
        if clipped_ivs:
            ...
            if svi_fit is not None:
                ...
                fitted_sigmas_svi = [math.sqrt(max(1e-08, svi_total_variance(k, params) / TTE_YEARS)) for k in points_k]
                svi_sse_sigma = sum((max(1e-06, float(w)) * (fit - actual) * (fit - actual) for (fit, actual, w) in zip(fitted_sigmas_svi, clipped_ivs, points_w)))
                fit_diag = {'model': 'svi', 'sse': round(float(svi_sse_sigma), 8), 'params': {'a': round(float(params[0]), 8), 'b': round(float(params[1]), 8), 'rho': round(float(params[2]), 6), 'm': round(float(params[3]), 6), 'sigma': round(float(params[4]), 6)}}
        else:
            coeffs = (0.0, 0.0, 0.18)

        def fit_side_coeffs(side_points_m: List[float], side_points_iv: List[float], side_points_w: List[float]):
            if not side_points_iv:
                return None
            if len(side_points_iv) >= 3:
                side_sorted = sorted(side_points_iv)
                side_median = side_sorted[len(side_sorted) // 2]
                side_clipped = [clamp(iv, side_median - 0.25, side_median + 0.25) for iv in side_points_iv]
                return fit_quadratic_weighted(side_points_m, side_clipped, side_points_w)
            side_sorted = sorted(side_points_iv)
            return (0.0, 0.0, float(side_sorted[len(side_sorted) // 2]))
        bid_coeffs = fit_side_coeffs(bid_points_m, bid_points_iv, bid_points_w)
        ask_coeffs = fit_side_coeffs(ask_points_m, ask_points_iv, ask_points_w)
        fitted_sigmas: Dict[str, float] = {}
        for (product, strike) in VOUCHER_STRIKES.items():
            k = math.log(strike / velvet_fair)
            m = k / math.sqrt(TTE_YEARS)
            if svi_fit is not None:
                sigma = math.sqrt(max(1e-08, svi_total_variance(k, svi_fit['params']) / TTE_YEARS))
            else:
                sigma = float(polyval(coeffs, m))
            fitted_sigmas[product] = clamp(sigma, 0.0001, 3.0)
        ...
        for (idx, product) in enumerate(ordered_products):
            ...
            if row is not None and row['liquidity'] >= 20.0 and (row['spread'] <= 6.0):
                sigma = 0.85 * sigma + 0.15 * clamp(float(row['market_iv']), sigma - 0.1, sigma + 0.1)
            fitted_sigmas[product] = clamp(sigma, 0.0001, 3.0)
        smile_stability_score = clamp(0.45 * clamp(len(points_iv) / 8.0, 0.0, 1.0) + 0.35 * clamp(1.0 - float(fit_diag.get('sse') or 0.0) / 0.02, 0.0, 1.0) + 0.2 * clamp(1.0 - int(repair_diag.get('repair_count', 0)) / max(1.0, float(len(points_iv) or 1)), 0.0, 1.0), 0.0, 1.0)
        surface: Dict[str, dict] = {}
        provisional_prices: List[Tuple[str, int, float]] = []
        for (product, strike) in VOUCHER_STRIKES.items():
            row = strike_rows.get(product, {})
            structural_sigma = fitted_sigmas[product]
            market_iv = row.get('market_iv')
            prev_roll_iv = float(rolling_iv_state.get(product, market_iv if market_iv is not None else structural_sigma))
            rolling_iv = prev_roll_iv if market_iv is None else 0.82 * prev_roll_iv + 0.18 * float(market_iv)
            rolling_iv_state[product] = rolling_iv
            prev_fair_iv = float(rolling_fair_state.get(product, structural_sigma))
            liquidity = float(row.get('liquidity', 0.0))
            spread = float(row.get('spread', 4.0))
            dislocation = 0.0 if market_iv is None else abs(float(market_iv) - structural_sigma)
            structural_w = 0.66
            rolling_w = 0.24
            previous_w = 0.1
            if fit_diag.get('model') != 'svi':
                structural_w -= 0.05
                rolling_w += 0.03
                previous_w += 0.02
            if spread > 6.0 or liquidity < 14.0:
                structural_w += 0.1
                rolling_w -= 0.06
                previous_w -= 0.04
            if dislocation > 0.09:
                ...
                rolling_w += 0.12
                previous_w -= 0.04
            if smile_stability_score < 0.7:
                shift = clamp(0.7 - smile_stability_score, 0.0, 0.25)
                structural_w -= 0.45 * shift
                rolling_w += 0.35 * shift
                previous_w += 0.1 * shift
            if market_iv is None:
                structural_w = 0.8
                rolling_w = 0.0
                previous_w = 0.2
            total_w = max(1e-06, structural_w + rolling_w + previous_w)
            sigma = clamp((structural_w * structural_sigma + rolling_w * rolling_iv + previous_w * prev_fair_iv) / total_w, 0.0001, 3.0)
            rolling_fair_state[product] = 0.75 * prev_fair_iv + 0.25 * sigma
            m = math.log(strike / velvet_fair) / math.sqrt(TTE_YEARS)
            observed_bid_iv = row.get('market_bid_iv')
            observed_ask_iv = row.get('market_ask_iv')
            observed_iv_width = max(0.01 + 0.004 * spread, 0.0 if observed_bid_iv is None or observed_ask_iv is None else float(observed_ask_iv) - float(observed_bid_iv))
            side_half = max(0.008, 0.5 * observed_iv_width)
            fitted_bid_iv = sigma - side_half
            fitted_ask_iv = sigma + side_half
            if bid_coeffs is not None:
                fitted_bid_iv = clamp(float(polyval(bid_coeffs, m)), 0.0001, 3.0)
            if ask_coeffs is not None:
                fitted_ask_iv = clamp(float(polyval(ask_coeffs, m)), 0.0001, 3.0)
            fair_bid_iv = 0.6 * fitted_bid_iv + 0.4 * (sigma - side_half)
            fair_ask_iv = 0.6 * fitted_ask_iv + 0.4 * (sigma + side_half)
            if fair_bid_iv >= fair_ask_iv:
                fair_bid_iv = max(0.0001, sigma - max(0.006, side_half))
                fair_ask_iv = sigma + max(0.006, side_half)
            fair_price = black_scholes_call(velvet_fair, strike, TTE_YEARS, sigma)
            surface[product] = {'sigma': sigma, 'structural_sigma': structural_sigma, 'rolling_iv': rolling_iv, 'delta': black_scholes_delta_call(velvet_fair, strike, TTE_YEARS, sigma), 'vega_proxy': black_scholes_vega_proxy(velvet_fair, strike, TTE_YEARS, sigma), 'moneyness': math.log(strike / velvet_fair) / math.sqrt(TTE_YEARS), 'market_mid': row.get('market_mid'), 'raw_guarded_price': row.get('raw_guarded_price'), 'guarded_price': row.get('guarded_price'), 'repaired_price': row.get('repaired_price'), 'repair_amount': float(row.get('repair_amount', 0.0)), 'repaired_flag': bool(row.get('repaired_flag', False)), 'repair_reasons': list(row.get('repair_reasons', [])), 'market_iv': market_iv, 'market_bid_iv': observed_bid_iv, 'market_ask_iv': observed_ask_iv, 'fair_bid_iv': fair_bid_iv, 'fair_ask_iv': fair_ask_iv, 'fair_bid': black_scholes_call(velvet_fair, strike, TTE_YEARS, fair_bid_iv), 'fair_ask': black_scholes_call(velvet_fair, strike, TTE_YEARS, fair_ask_iv), 'local_dislocation': dislocation, 'spread': float(row.get('spread', 4.0)), 'liquidity': float(row.get('liquidity', 0.0))}
        ...
        for (product, _, fair_price) in sorted(provisional_prices, key=lambda item: item[1]):
            ...
            market_price = max(float(market_price), max(velvet_fair - VOUCHER_STRIKES[product], 0.0) + 0.001)
            ...
            market_bid_iv = surface[product]['market_bid_iv']
            market_ask_iv = surface[product]['market_ask_iv']
            surface[product]['iv_band_buy_gap'] = 0.0 if market_ask_iv is None else float(surface[product]['fair_bid_iv']) - float(market_ask_iv)
            surface[product]['iv_band_sell_gap'] = 0.0 if market_bid_iv is None else float(market_bid_iv) - float(surface[product]['fair_ask_iv'])
            surface[product]['bs_gap'] = market_price - fair_price
        surface['_repair_diag'] = {**repair_diag, 'bid_repair_count': int(bid_repair_diag.get('repair_count', 0)), 'ask_repair_count': int(ask_repair_diag.get('repair_count', 0))}
        surface['_fit_diag'] = fit_diag
        surface['_smile_stability_score'] = smile_stability_score
        vev_state['last_surface_model'] = str(fit_diag.get('model', 'quadratic'))
        return surface

    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        smile_stability_score = float(surface.get('_smile_stability_score', 0.5))
        liquid_products = [product for product in VOUCHER_STRIKES if product in surface and surface[product].get('market_mid') is not None and (float(surface[product]['market_mid']) > 0.5) and (float(surface[product]['liquidity']) >= 10.0)]
        ...
        for (left, right) in zip(sorted_products, sorted_products[1:]):
            ...
            if gap >= 0.07:
                ...
                pair_targets[left] += 14 if 0.2 <= float(left_ctx['delta']) <= 0.8 else 18
                pair_targets[right] -= 14 if 0.2 <= float(right_ctx['delta']) <= 0.8 else 18
            elif gap <= -0.07:
                pair_agreement_count += 1
                pair_bias[left] = pair_bias.get(left, 0) - 1
                pair_bias[right] = pair_bias.get(right, 0) + 1
                ...
                pair_targets[right] += 14 if 0.2 <= float(right_ctx['delta']) <= 0.8 else 18
        vev_state = memory['vev_voucher_state']
        active_pair_scalps = vev_state.setdefault('active_pair_scalps', {})
        pair_candidates: Dict[str, dict] = {}
        pair_scalp_targets: Dict[str, int] = {product: 0 for product in VOUCHER_STRIKES}
        scalp_mode_by_product: Dict[str, int] = {product: SCALP_NONE for product in VOUCHER_STRIKES}
        scalp_tag_by_product: Dict[str, str] = {product: SCALP_TAGS[SCALP_NONE] for product in VOUCHER_STRIKES}
        scalp_pair_spread: Dict[str, float] = {product: 0.0 for product in VOUCHER_STRIKES}
        scalp_pair_key: Dict[str, str] = {product: '' for product in VOUCHER_STRIKES}
        for (idx, left) in enumerate(sorted_products):
            if left not in liquid_products:
                continue
            left_ctx = surface[left]
            left_resid = float(left_ctx['iv_residual'])
            for right in sorted_products[idx + 1:min(len(sorted_products), idx + 4)]:
                if right not in liquid_products:
                    continue
                right_ctx = surface[right]
                right_resid = float(right_ctx['iv_residual'])
                (cheap_product, cheap_resid, cheap_ctx) = (left, left_resid, left_ctx)
                (rich_product, rich_resid, rich_ctx) = (right, right_resid, right_ctx)
                if left_resid > right_resid:
                    (cheap_product, cheap_resid, cheap_ctx) = (right, right_resid, right_ctx)
                    (rich_product, rich_resid, rich_ctx) = (left, left_resid, left_ctx)
                spread = rich_resid - cheap_resid
                if spread <= 0.0:
                    continue
                opposite_signed = cheap_resid <= 0.0 <= rich_resid
                both_executable = cheap_ctx.get('market_ask_iv') is not None and rich_ctx.get('market_bid_iv') is not None
                if not both_executable:
                    continue
                pair_delta_balance = abs(float(cheap_ctx['delta']) - float(rich_ctx['delta']))
                candidate_qty = 20
                if VOUCHER_STRIKES[cheap_product] <= 5000 and VOUCHER_STRIKES[rich_product] <= 5100:
                    candidate_qty = 24
                elif 0.2 <= float(cheap_ctx['delta']) <= 0.8 or 0.2 <= float(rich_ctx['delta']) <= 0.8:
                    candidate_qty = 16
                candidate_score = spread - 0.2 * pair_delta_balance
                pair_key = f'{cheap_product}|{rich_product}'
                pair_candidates[pair_key] = {'cheap': cheap_product, 'rich': rich_product, 'spread': spread, 'score': candidate_score, 'opposite_signed': opposite_signed, 'qty': candidate_qty}
        strip_delta = 0.0
        strip_vega = 0.0
        middle_abs = 0
        middle_net = 0
        ...
        broad_dislocation = avg_abs_resid > max(0.07, 1.05 * resid_scale) and pair_agreement_count >= 4 and (abs(strip_delta) < 0.8 * total_delta_cap) and (middle_abs < 0.8 * middle_cap)
        last_avg_abs_resid = float(vev_state.get('last_avg_abs_resid', 0.0))
        resid_compression = last_avg_abs_resid > 1e-06 and avg_abs_resid < 0.7 * last_avg_abs_resid
        vev_state['last_avg_abs_resid'] = avg_abs_resid
        cheapest = list(sorted_residuals[:3])
        richest = list(sorted_residuals[-3:][::-1])
        pair_entry_threshold = max(0.075, 1.35 * resid_scale)
        pair_exit_threshold = max(0.03, 0.55 * pair_entry_threshold)
        used_products: set[str] = set()
        next_active_pairs: Dict[str, dict] = {}
        if abs(strip_delta) < 0.95 * total_delta_cap and middle_abs < middle_cap:
            for (pair_key, active_payload) in active_pair_scalps.items():
                candidate = pair_candidates.get(pair_key)
                if candidate is None:
                    continue
                cheap_product = str(candidate['cheap'])
                rich_product = str(candidate['rich'])
                if cheap_product in used_products or rich_product in used_products:
                    continue
                if float(candidate['spread']) <= pair_exit_threshold:
                    continue
                next_active_pairs[pair_key] = {'cheap': cheap_product, 'rich': rich_product, 'spread': float(candidate['spread']), 'qty': int(active_payload.get('qty', candidate['qty']))}
                used_products.add(cheap_product)
                used_products.add(rich_product)
            for (pair_key, candidate) in sorted(pair_candidates.items(), key=lambda item: float(item[1]['score']), reverse=True):
                cheap_product = str(candidate['cheap'])
                rich_product = str(candidate['rich'])
                if cheap_product in used_products or rich_product in used_products:
                    continue
                if float(candidate['spread']) <= pair_entry_threshold:
                    continue
                if not bool(candidate['opposite_signed']) and float(candidate['spread']) < pair_entry_threshold + 0.02:
                    continue
                if len(next_active_pairs) >= 2:
                    break
                next_active_pairs[pair_key] = {'cheap': cheap_product, 'rich': rich_product, 'spread': float(candidate['spread']), 'qty': int(candidate['qty'])}
                used_products.add(cheap_product)
                used_products.add(rich_product)
        vev_state['active_pair_scalps'] = next_active_pairs
        for (pair_key, payload) in next_active_pairs.items():
            cheap_product = str(payload['cheap'])
            rich_product = str(payload['rich'])
            qty = int(payload['qty'])
            pair_scalp_targets[cheap_product] += qty
            pair_scalp_targets[rich_product] -= qty
            scalp_mode_by_product[cheap_product] = SCALP_VOUCHER_PAIR
            scalp_mode_by_product[rich_product] = SCALP_VOUCHER_PAIR
            scalp_tag_by_product[cheap_product] = SCALP_TAGS[SCALP_VOUCHER_PAIR]
            scalp_tag_by_product[rich_product] = SCALP_TAGS[SCALP_VOUCHER_PAIR]
            scalp_pair_spread[cheap_product] = float(payload['spread'])
            scalp_pair_spread[rich_product] = float(payload['spread'])
            scalp_pair_key[cheap_product] = pair_key
            scalp_pair_key[rich_product] = pair_key
        hedge_ratio = 0.25
        hedge_deadband = 20.0
        if pair_agreement_count >= 2 or avg_abs_resid > max(0.055, 0.95 * resid_scale):
            hedge_ratio = 0.5
            hedge_deadband = 18.0
        ...
        if smile_stability_score < 0.55:
            hedge_ratio = max(hedge_ratio, 0.4 if not broad_dislocation else hedge_ratio)
        target_velvet_pos = 0 if abs(strip_delta) < hedge_deadband else int(round(clamp(-hedge_ratio * strip_delta, -100.0, 100.0)))
        ...
        for product in VOUCHER_STRIKES:
            ...
            pair_confirmed = abs(int(pair_targets.get(product, 0))) > 0 or abs(int(pair_bias.get(product, 0))) > 0 or abs(int(pair_scalp_targets.get(product, 0))) > 0
            if strike <= 5000:
                base_cap = 90
            elif strike <= 5100:
                base_cap = 78
            elif 0.2 <= float(ctx['delta']) <= 0.8:
                base_cap = 68
            else:
                base_cap = 86
            hedge_cap = 35.0 + 0.4 * hedge_headroom / delta_abs
            if abs(strip_delta) > 0.85 * total_delta_cap:
                hedge_cap *= 0.88
            if broad_dislocation and strike <= 5000:
                hedge_cap *= 1.08
            if 0.2 <= float(ctx['delta']) <= 0.8:
                hedge_cap *= 1.1 if pair_confirmed else 0.85
            if not pair_confirmed and strike >= 5100:
                hedge_cap *= 0.9
            feasible_caps[product] = int(clamp(hedge_cap, 35.0, float(base_cap)))
        hedge_feasible_size_score = clamp(0.55 * clamp(hedge_headroom / 150.0, 0.0, 1.0) + 0.25 * clamp(1.0 - abs(strip_delta) / max(1.0, total_delta_cap), 0.0, 1.0) + 0.2 * clamp(1.0 - middle_abs / max(1.0, middle_cap), 0.0, 1.0), 0.0, 1.0)
        for product in VOUCHER_STRIKES:
            if scalp_mode_by_product[product] == SCALP_NONE and broad_dislocation and (abs(float(surface[product]['iv_residual'])) >= max(0.8 * pair_entry_threshold, 0.055)):
                scalp_mode_by_product[product] = SCALP_VOUCHER_SHOCK
                scalp_tag_by_product[product] = SCALP_TAGS[SCALP_VOUCHER_SHOCK]
        return {'resid_scale': resid_scale, 'avg_abs_resid': avg_abs_resid, 'resid_threshold': max(0.035, 0.85 * resid_scale), 'extreme_resid_threshold': max(0.06, 1.5 * resid_scale), 'pair_bias': pair_bias, 'pair_targets': pair_targets, 'pair_agreement_count': pair_agreement_count, 'active_pair_scalps': next_active_pairs, 'pair_entry_threshold': pair_entry_threshold, 'pair_exit_threshold': pair_exit_threshold, 'pair_scalp_targets': pair_scalp_targets, 'scalp_mode_by_product': scalp_mode_by_product, 'scalp_tag_by_product': scalp_tag_by_product, 'scalp_pair_spread': scalp_pair_spread, 'scalp_pair_key': scalp_pair_key, 'residual_rank': residual_rank, 'smile_stability_score': smile_stability_score, 'strip_delta': strip_delta, 'strip_vega_proxy': strip_vega, 'delta_pressure': clamp(strip_delta / 140.0, -2.0, 2.0), 'target_velvet_pos': target_velvet_pos, 'hedge_ratio': hedge_ratio, 'hedge_deadband': hedge_deadband, 'hedge_headroom': hedge_headroom, 'middle_abs': middle_abs, 'middle_net': middle_net, 'middle_long_gross': middle_long_gross, 'middle_short_gross': middle_short_gross, 'middle_same_side_gross': max(middle_long_gross, middle_short_gross), 'middle_cap': middle_cap, 'adjacent_same_side_max': adjacent_same_side_max, 'adjacent_same_side_products': adjacent_same_side_products, 'adjacent_cap': 170, 'low_wing_net': low_wing_net, 'high_wing_net': high_wing_net, 'total_delta_cap': total_delta_cap, 'unconfirmed_outright_cap': 85, 'feasible_caps': feasible_caps, 'hedge_feasible_size_score': hedge_feasible_size_score, 'broad_dislocation': broad_dislocation, 'resid_compression': resid_compression, 'cheapest_strikes': cheapest, 'richest_strikes': richest}
    ...
    def _build_velvet_context(self, state: TradingState, velvet_fair: float, risk_ctx: dict) -> dict:
        ...
        if ba is not None:
            top_depth += float(abs(od.sell_orders.get(ba, 0)))
        book_good = bb is not None and ba is not None and (bb < ba) and (spread <= 6.0) and (top_depth >= 12.0) and (abs(float(stable) - float(mid)) <= 1.2)
        ...
        if abs(strip_delta) >= 95.0:
            alpha_cap = 0
        alpha_target = int(round(clamp(35.0 * math.tanh(0.75 * alpha_signal), -float(alpha_cap), float(alpha_cap))))
        final_target = int(round(clamp(float(hedge_target + alpha_target), -100.0, 100.0)))
        ...
        if abs(strip_delta) < float(risk_ctx['hedge_deadband']):
            size_mult *= 1.05
        return {'hedge_target': hedge_target, 'alpha_target': alpha_target, 'target': final_target, 'alpha_signal': float(alpha_signal), 'quote_bias': quote_bias, 'take_bias': take_bias, 'size_mult': size_mult, 'book_good': book_good}
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        iv_band_buy_gap = float(voucher_ctx.get('iv_band_buy_gap', 0.0))
        iv_band_sell_gap = float(voucher_ctx.get('iv_band_sell_gap', 0.0))
        ...
        middle_band = 0.2 <= delta <= 0.8
        pair_bias = int(clamp(float(risk_ctx['pair_bias'].get(product, 0)), -1.0, 1.0))
        pair_target = int(risk_ctx['pair_targets'].get(product, 0))
        scalp_target = int(risk_ctx['pair_scalp_targets'].get(product, 0))
        scalp_mode = int(risk_ctx['scalp_mode_by_product'].get(product, SCALP_NONE))
        pair_confirmed = pair_bias != 0 or pair_target != 0 or scalp_target != 0
        ...
        low_strike_live = low_strike and abs(float(risk_ctx['strip_delta'])) < 1.1 * float(risk_ctx['total_delta_cap'])
        low_strike_threshold = 0.82 * resid_threshold if low_strike_live else resid_threshold
        band_threshold = 0.4 * low_strike_threshold
        outright_threshold = 1.2 * low_strike_threshold if middle_band else 1.1 * low_strike_threshold
        buy_confirmed = pair_bias > 0 or pair_target > 0 or cheap_signal >= outright_threshold or (iv_band_buy_gap >= 1.1 * band_threshold)
        sell_confirmed = pair_bias < 0 or pair_target < 0 or rich_signal >= outright_threshold or (iv_band_sell_gap >= 1.1 * band_threshold)
        ...
        adjacent_block = product in risk_ctx['adjacent_same_side_products'] and int(risk_ctx['adjacent_same_side_max']) > int(risk_ctx['adjacent_cap'])
        broad_dislocation = bool(risk_ctx['broad_dislocation'])
        take_edge = max(0.75, 0.35 * spread) + (0.3 if middle_band else 0.0) + (0.2 if not confirmed else 0.0)
        if broad_dislocation:
            take_edge -= 0.15
        if low_strike_live and (confirmed or abs(bs_gap) >= 0.75):
            take_edge -= 0.1
        if iv_band_buy_gap >= band_threshold or iv_band_sell_gap >= band_threshold:
            take_edge -= 0.1
        if not pair_confirmed and middle_band:
            take_edge += 0.15
        ...
        if scalp_mode == SCALP_VOUCHER_PAIR:
            take_max += 2
            take_edge = max(0.55, take_edge - 0.2)
        elif scalp_mode == SCALP_VOUCHER_SHOCK:
            take_edge = max(0.6, take_edge - 0.1)
        pos = mgr.projected()
        scalp_gap = scalp_target - pos
        if scalp_mode == SCALP_VOUCHER_PAIR and scalp_gap > 0 and (not delta_block_buy):
            for (ask, volume) in sorted(od.sell_orders.items()):
                scalp_edge = fair_bid - ask + 0.12 * max(0, pair_bias)
                if scalp_edge >= max(0.55, take_edge - 0.15):
                    mgr.buy(ask, min(-volume, min(take_max, scalp_gap)))
                    pos = mgr.projected()
                    scalp_gap = scalp_target - pos
                    if scalp_gap <= 0:
                        break
                    ...
                    break
        elif scalp_mode == SCALP_VOUCHER_PAIR and scalp_gap < 0 and (not delta_block_sell):
            for (bid, volume) in sorted(od.buy_orders.items(), reverse=True):
                scalp_edge = bid - fair_ask + 0.12 * max(0, -pair_bias)
                if scalp_edge >= max(0.55, take_edge - 0.15):
                    mgr.sell(bid, min(volume, min(take_max, -scalp_gap)))
                    pos = mgr.projected()
                    scalp_gap = scalp_target - pos
                    if scalp_gap >= 0:
                        break
        ...
        elif pos > 0 and abs(iv_residual) < (0.4 if low_strike_live else 0.55) * resid_threshold and (bb is not None) and (bb >= fair - (clear_edge + 0.2)):
        ...
        elif pos < 0 and abs(iv_residual) < (0.4 if low_strike_live else 0.55) * resid_threshold and (ba is not None) and (ba <= fair + (clear_edge + 0.2)):
        ...
        signal_shift = clamp((cheap_signal - rich_signal) / max(resid_threshold, 1e-06), -1.5, 1.5)
        reservation = fair - inv_penalty + 0.3 * signal_shift + 0.12 * pair_bias + 0.004 * pair_target + 0.005 * scalp_target
        ...
        if broad_dislocation and pair_target != 0:
            quote_edge = max(0.85, quote_edge - 0.1)
        if low_strike_live and (confirmed or abs(bs_gap) >= 0.75):
            quote_edge = max(0.8, quote_edge - 0.08)
        if iv_band_buy_gap >= band_threshold or iv_band_sell_gap >= band_threshold:
            quote_edge = max(0.75, quote_edge - 0.08)
        buy_px = round_down(reservation - quote_edge)
        sell_px = round_up(reservation + quote_edge)
        if bb is not None and ba is not None and (bb < ba):
            if spread >= 3:
            ...
            buy_px = min(buy_px, ba - 1)
            sell_px = max(sell_px, bb + 1)
        size_scale = max(0.2, 1.0 - abs(inv_ratio))
        if middle_band:
            size_scale *= 0.45
        if not confirmed:
            size_scale *= 0.75
        ...
        if abs(residual_rank) >= 3:
            size_scale *= 0.9
        if pair_target != 0 or scalp_target != 0:
            size_scale *= 1.08
        if broad_dislocation:
            size_scale *= 1.06
        if low_strike_live and (confirmed or abs(bs_gap) >= 0.75):
        ...
        if fair >= 0.5:
            ...
            if adjacent_block:
                ...
                if pos <= -int(0.7 * per_strike_cap) and pair_bias <= 0:
                    can_ask = False
            if scalp_mode == SCALP_VOUCHER_PAIR and scalp_target > pos and (not delta_block_buy) and (not adjacent_block):
                can_bid = can_bid or mgr.buy_cap > 0
            if scalp_mode == SCALP_VOUCHER_PAIR and scalp_target < pos and (not delta_block_sell) and (not adjacent_block):
                can_ask = can_ask or mgr.sell_cap > 0
            if can_bid and (ba is None or buy_px < ba):
                mgr.buy(buy_px, quote_size)
            if can_ask and (bb is None or sell_px > bb):
                mgr.sell(sell_px, quote_size)
    ...
    def run(self, state: TradingState):
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                memory['hydro_book'] = {'mid': round(float(hydro_ctx['mid']), 3), 'signal': round(float(hydro_ctx['signal']), 3), 'regime_score': round(float(hydro_ctx['regime_score']), 3), 'trend_score': round(float(hydro_ctx['trend_score']), 3), 'anchor_score': round(float(hydro_ctx['anchor_score']), 3), 'flow_score': round(float(hydro_ctx['flow_score']), 3), 'confidence': str(hydro_ctx['confidence']), 'entry_cap': int(hydro_ctx['entry_cap']), 'hold_cap': int(hydro_ctx['hold_cap']), 'entry_target': int(hydro_ctx['entry_target']), 'hold_target': int(hydro_ctx['hold_target']), 'target': int(hydro_ctx['target']), 'good_book': bool(hydro_ctx['good_book']), 'progress': round(float(hydro_ctx['progress']), 3), 'extreme_hold_bars': int(hydro_ctx['extreme_hold_bars']), 'flatten_before_flip': bool(hydro_ctx['flatten_before_flip']), 'exit_mode': str(hydro_ctx['exit_mode']), 'exit_age': int(hydro_ctx['exit_age']), 'long_peak_score': round(float(hydro_ctx['long_peak_score']), 3), 'short_peak_score': round(float(hydro_ctx['short_peak_score']), 3), 'long_peak_trend': round(float(hydro_ctx['long_peak_trend']), 3), 'short_peak_trend': round(float(hydro_ctx['short_peak_trend']), 3), 'long_drawdown': round(float(hydro_ctx['long_drawdown']), 3), 'short_drawup': round(float(hydro_ctx['short_drawup']), 3), 'absolute_danger_long': bool(hydro_ctx['absolute_danger_long']), 'absolute_danger_short': bool(hydro_ctx['absolute_danger_short']), 'same_side_bid_block': bool(hydro_ctx['same_side_bid_block']), 'same_side_ask_block': bool(hydro_ctx['same_side_ask_block'])}
                result[HYDROGEL] = self._trade_hydrogel(state, hydro_fair, hydro_ctx)
                if result[HYDROGEL]:
                    order_tags[HYDROGEL] = [{'price': int(order.price), 'qty': int(order.quantity), 'tag': SCALP_TAGS[SCALP_HYDRO_LOCAL]} for order in result[HYDROGEL]]
            except Exception as exc:
        ...
        if velvet_fair is not None:
            try:
                ...
                memory['strip_monitor'] = {'strip_delta': round(float(risk_ctx['strip_delta']), 3), 'strip_vega_proxy': round(float(risk_ctx['strip_vega_proxy']), 3), 'avg_abs_resid': round(float(risk_ctx['avg_abs_resid']), 4), 'smile_stability_score': round(float(risk_ctx['smile_stability_score']), 4), 'hedge_feasible_size_score': round(float(risk_ctx['hedge_feasible_size_score']), 4), 'pair_agreement_count': int(risk_ctx['pair_agreement_count']), 'pair_entry_threshold': round(float(risk_ctx['pair_entry_threshold']), 4), 'pair_exit_threshold': round(float(risk_ctx['pair_exit_threshold']), 4), 'broad_dislocation': bool(risk_ctx['broad_dislocation']), 'resid_compression': bool(risk_ctx['resid_compression']), 'hedge_ratio': round(float(risk_ctx['hedge_ratio']), 3), 'hedge_deadband': round(float(risk_ctx['hedge_deadband']), 3), 'vev_final_target': int(velvet_ctx['target']), 'vev_hedge_target': int(risk_ctx['target_velvet_pos']), 'velvet_alpha_target': int(velvet_ctx['alpha_target']), 'surface_model': str(voucher_surface.get('_fit_diag', {}).get('model', 'quadratic')), 'middle_abs': int(risk_ctx['middle_abs']), 'middle_net': int(risk_ctx['middle_net']), 'middle_same_side_gross': int(risk_ctx['middle_same_side_gross']), 'middle_cap': int(risk_ctx['middle_cap']), 'adjacent_same_side_max': int(risk_ctx['adjacent_same_side_max']), 'adjacent_cap': int(risk_ctx['adjacent_cap']), 'adjacent_same_side_products': list(risk_ctx['adjacent_same_side_products']), 'low_wing_net': int(risk_ctx['low_wing_net']), 'high_wing_net': int(risk_ctx['high_wing_net']), 'cheapest_strikes': list(risk_ctx['cheapest_strikes']), 'richest_strikes': list(risk_ctx['richest_strikes']), 'active_pair_scalps': dict(risk_ctx['active_pair_scalps'])}
                memory['voucher_targets'] = {product: {'scalp_mode': int(risk_ctx['scalp_mode_by_product'].get(product, SCALP_NONE)), 'trade_tag': str(risk_ctx['scalp_tag_by_product'].get(product, SCALP_TAGS[SCALP_NONE])), 'scalp_pair_key': str(risk_ctx['scalp_pair_key'].get(product, '')), 'scalp_pair_spread': round(float(risk_ctx['scalp_pair_spread'].get(product, 0.0)), 4), 'scalp_target': int(risk_ctx['pair_scalp_targets'].get(product, 0)), 'pair_target': int(risk_ctx['pair_targets'].get(product, 0)), 'pair_bias': int(risk_ctx['pair_bias'].get(product, 0)), 'iv_residual': round(float(voucher_surface[product]['iv_residual']), 4), 'fair_mid_iv': round(float(voucher_surface[product]['sigma']), 4), 'iv_band_buy_gap': round(float(voucher_surface[product].get('iv_band_buy_gap', 0.0)), 4), 'iv_band_sell_gap': round(float(voucher_surface[product].get('iv_band_sell_gap', 0.0)), 4), 'fair_bid_iv': round(float(voucher_surface[product].get('fair_bid_iv', voucher_surface[product]['sigma'])), 4), 'fair_ask_iv': round(float(voucher_surface[product].get('fair_ask_iv', voucher_surface[product]['sigma'])), 4), 'rolling_iv': round(float(voucher_surface[product].get('rolling_iv', voucher_surface[product]['sigma'])), 4), 'delta': round(float(voucher_surface[product]['delta']), 4), 'feasible_cap': int(risk_ctx['feasible_caps'].get(product, LIMITS[product])), 'raw_guarded_price': round(float(voucher_surface[product].get('raw_guarded_price', 0.0)), 4), 'repaired_price': round(float(voucher_surface[product].get('repaired_price', 0.0)), 4), 'repair_amount': round(float(voucher_surface[product].get('repair_amount', 0.0)), 6), 'repaired_flag': bool(voucher_surface[product].get('repaired_flag', False)), 'repair_reasons': list(voucher_surface[product].get('repair_reasons', []))} for product in VOUCHER_STRIKES}
                memory['voucher_quote_repairs'] = dict(voucher_surface.get('_repair_diag', {}))
                memory['voucher_surface_fit'] = dict(voucher_surface.get('_fit_diag', {}))
                result[VELVET] = self._trade_underlying(VELVET, state, velvet_fair, position_target=float(velvet_ctx['target']), take_bias=float(velvet_ctx['take_bias']), quote_bias=float(velvet_ctx['quote_bias']), size_mult=float(velvet_ctx['size_mult']))
                if result[VELVET]:
                    vev_tag = SCALP_TAGS[SCALP_VEV_HEDGE] if abs(int(velvet_ctx['hedge_target'])) >= max(12, abs(int(velvet_ctx['alpha_target']))) else SCALP_TAGS[SCALP_NONE]
                    order_tags[VELVET] = [{'price': int(order.price), 'qty': int(order.quantity), 'tag': vev_tag} for order in result[VELVET]]
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        result[product] = self._trade_voucher(product, state, voucher_surface[product], risk_ctx)
                        if result[product]:
                            trade_tag = str(risk_ctx['scalp_tag_by_product'].get(product, SCALP_TAGS[SCALP_NONE]))
                            order_tags[product] = [{'price': int(order.price), 'qty': int(order.quantity), 'tag': trade_tag} for order in result[product]]
            except Exception as exc:
                memory['engine_errors']['voucher_strip'] = type(exc).__name__
        memory['order_tags'] = order_tags
