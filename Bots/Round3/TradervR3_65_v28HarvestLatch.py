...
class Trader:
    ...
    def _build_velvet_plan(self, state: TradingState, velvet_fair: float, risk_ctx: dict) -> dict:
        ...
        # Velvet stays hedge-first; strip-aware alpha is allowed, but it stays small.
        alpha_signal = (velvet_fair - float(mid)) / max(1.0, 0.5 * spread)
        local_alpha = 18.0 * math.tanh(0.65 * alpha_signal)
        strip_alpha = 9.0 * math.tanh(0.8 * low_lane_bias) + 3.0 * clamp(float(cheap_rich_balance), -2.0, 2.0)
        alpha_cap = 28.0 if broad_dislocation else 22.0
        if abs(strip_delta) >= 60.0:
            alpha_cap = min(alpha_cap, 10.0)
        if resid_compression > 0.012:
            strip_alpha *= 0.60
            local_alpha *= 0.80
        ...
        quote_bias = -0.12 * float(risk_ctx["delta_pressure"]) - 0.08 * clamp(hedge_gap / 35.0, -1.0, 1.0)
        quote_bias += 0.04 * math.tanh(0.8 * low_lane_bias)
        if resid_compression > 0.012:
            quote_bias *= 0.60
        if harvest_mode:
            quote_bias *= 0.75

        size_mult = 0.90 + 0.20 * hedge_ratio
        if resid_compression > 0.012:
            size_mult *= 0.88
        if harvest_mode:
            size_mult *= 0.72
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        pair_confirmed = abs(pair_support) >= 0.35 or pair_bias != 0
        ...
        score = (-iv_residual / max(resid_threshold, 1e-6)) + 0.75 * pair_support + 0.18 * pair_bias
        if broad_dislocation:
            score += 0.15 * sign(score)
        buy_confirmed = cheap_signal >= resid_threshold or pair_confirmed or pair_bias > 0
        sell_confirmed = rich_signal >= resid_threshold or pair_confirmed or pair_bias < 0
        ...
        if broad_dislocation and pair_confirmed:
            take_iv_edge *= 0.88
        take_edge = max(0.75, 0.35 * spread) + (0.30 if middle_band else 0.0) + (0.20 if not pair_confirmed else 0.0)
        clear_edge = max(0.35, 0.15 * spread)
        soft_limit = 85 if middle_band else 125
        per_strike_cap = MIDDLE_STRIKE_CAP if middle_band else BASE_WORKING_VOUCHER_CAP
        if middle_band and not pair_confirmed:
            per_strike_cap = min(per_strike_cap, UNCONFIRMED_OUTRIGHT_CAP)
        per_strike_cap = max(30, int(round(per_strike_cap * hedge_feasible_size_score)))
        take_max = 12 if pair_confirmed else (8 if middle_band else 14)
        if broad_dislocation and pair_confirmed:
            take_max += 3

        target_scale = 1.0 + (0.18 if pair_confirmed else -0.05)
        if broad_dislocation and pair_confirmed:
            target_scale += 0.08
        raw_target = int(round(per_strike_cap * clamp(math.tanh(0.9 * score), -1.0, 1.0) * target_scale))
        ...
        reservation = fair - inv_penalty + 0.28 * signal_shift + 0.14 * pair_bias + 0.10 * pair_support
