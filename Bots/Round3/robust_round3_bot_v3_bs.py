...
class Trader:
    """
    Round 3 robust v3: Black-Scholes voucher model.
    ...
    The voucher model is now Black-Scholes based:
        fair = BS_call(S=VELVETFRUIT_EXTRACT fair, K=strike, T=TTE/365, sigma=robust_sigma, r=0)
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "VELVETFRUIT_EXTRACT":
                ...
                orders = self._trade_voucher(product, depth, pos, fair, extract_fair, delta)
    ...
    def _voucher_fair(self, product: str, underlying_fair: float, timestamp: int, data: Optional[dict] = None) -> float:
        """Black-Scholes call fair value for VEV vouchers.

        The voucher is treated as a European-style call on VELVETFRUIT_EXTRACT.
        We use r=0 because no interest/carry signal is given, TTE=5 days at the
        start of final simulation, and a bounded volatility estimate.
        """
        K = self.VOUCHER_STRIKES[product]
        T = self._tte_years(timestamp)
        sigma = self._sigma_for_product(data or {}, product)
    ...
    def _sigma_for_product(self, data: dict, product: str) -> float:
        # Conservative prior. The live implied-vol update can move it, but only slowly.
        global_sigma = data.get("bs_sigma", 0.20)
        product_sigma = data.get("bs_sigma_by_product", {}).get(product, global_sigma)
        return self._clip(0.70 * global_sigma + 0.30 * product_sigma, 0.11, 0.36)
    ...
    def _update_voucher_sigma(self, data: dict, order_depths: Dict[str, OrderDepth], underlying_fair: float, timestamp: int) -> None:
        ...
        for product, K in self.VOUCHER_STRIKES.items():
            ...
            moneyness = abs(underlying_fair - K)

            # Far ITM/OTM vouchers carry little clean vol information.
            if extrinsic < 0.5 or moneyness > 420:
                continue

            iv = self._implied_vol_call(mid, underlying_fair, K, T)
            if iv is None:
                continue
            iv = self._clip(iv, 0.11, 0.36)

            old_p = data["bs_sigma_by_product"].get(product, data.get("bs_sigma", 0.20))
            data["bs_sigma_by_product"][product] = 0.94 * old_p + 0.06 * iv
            implieds.append(iv)

        if implieds:
            implieds.sort()
            med = implieds[len(implieds) // 2]
            old = data.get("bs_sigma", 0.20)
            data["bs_sigma"] = self._clip(0.97 * old + 0.03 * med, 0.11, 0.36)
        elif "bs_sigma" not in data:
            data["bs_sigma"] = 0.20
    ...
    def _trade_voucher(self, product: str, depth: OrderDepth, pos: int, fair: float, underlying_fair: float, delta: float) -> List[Order]:
        orders: List[Order] = []
        limit = self.POSITION_LIMITS[product]
        best_bid, bid_vol = self._best_bid(depth)
        best_ask, ask_vol = self._best_ask(depth)
    ...
    def _load_data(self, trader_data: str) -> dict:
        if not trader_data:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.20, "bs_sigma_by_product": {}}
        try:
