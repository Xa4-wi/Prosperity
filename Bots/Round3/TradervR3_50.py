...
class Trader:
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float, memory: dict) -> Dict[str, dict]:
        ...
        for product, row in repaired_rows.items():
            ...
            if row["best_bid"] is not None:
                bid_price = max(float(row["best_bid"]), intrinsic + 1e-3)
            ...
            if row["best_ask"] is not None:
                ask_price = max(float(row["best_ask"]), intrinsic + 1e-3)
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        buy_confirmed = cheap_signal >= low_strike_threshold or iv_band_buy_gap >= band_threshold or pair_bias > 0
        sell_confirmed = rich_signal >= low_strike_threshold or iv_band_sell_gap >= band_threshold or pair_bias < 0
        ...
        if fair >= 0.5:
            ...
            if mode == "HARVEST":
                if pos >= 0:
                    can_bid = can_bid and buy_extreme and abs(pos) < int(0.25 * per_strike_cap)
                if pos <= 0:
                    can_ask = can_ask and sell_extreme and abs(pos) < int(0.25 * per_strike_cap)
