...
class Trader:
    ...
    def _build_velvet_context(self, state: TradingState, fair: float, memory: dict, overlay_bias: float) -> dict:
        ...
        state_mem["ema_fair"] = float(fair) if state_mem.get("ema_fair") is None else 0.14 * float(fair) + 0.86 * float(state_mem["ema_fair"])
        trend_gap = float(state_mem["ema_fast"]) - float(state_mem["ema_slow"])
        trend_score = clamp(trend_gap / 4.8, -3.0, 3.0)
        signal = (float(fair) - float(mid)) / max(1.0, 0.5 * spread)
        fair_ema_gap = (float(state_mem["ema_fair"]) - float(mid)) / max(1.0, 0.5 * spread)
        micro_gap = (float(micro) - float(mid)) / max(1.0, 0.5 * spread)
        imbalance = book_imbalance(od, levels=2)
        flow_score = clamp(micro_gap + 0.70 * imbalance + 0.30 * clamp(float(state_mem["ret_ema"]) / 1.8, -2.0, 2.0), -2.5, 2.5)
        conviction = 0.52 * signal + 0.20 * fair_ema_gap + 0.16 * trend_score + 0.12 * flow_score
        ...
        mode = "NEUTRAL"
        if good_book:
            if conviction > 1.35 and trend_score > 0.25:
                mode = "TREND_LONG"
            elif conviction < -1.35 and trend_score < -0.25:
                mode = "TREND_SHORT"
            elif signal > 0.95:
                mode = "LOCAL_LONG"
            elif signal < -0.95:
                mode = "LOCAL_SHORT"
        entry_cap = 0
        if mode in ("LOCAL_LONG", "LOCAL_SHORT"):
            entry_cap = 55 if abs(conviction) < 1.8 else 75
        elif mode in ("TREND_LONG", "TREND_SHORT"):
            entry_cap = 80 if abs(conviction) < 2.1 else 105
        if not good_book:
            entry_cap = 20
        if progress > 0.80:
            entry_cap = int(round(entry_cap * 0.85))
        if progress > 0.92:
            entry_cap = int(round(entry_cap * 0.60))
        if progress > 0.97:
            entry_cap = min(entry_cap, 25)
        target = 0
        if mode == "LOCAL_LONG":
            target = int(round(clamp(70.0 * math.tanh(0.80 * conviction), 0.0, float(entry_cap))))
        elif mode == "LOCAL_SHORT":
            target = int(round(clamp(70.0 * math.tanh(-0.80 * conviction), 0.0, float(entry_cap)))) * -1
        elif mode == "TREND_LONG":
            target = int(round(clamp(95.0 * math.tanh(0.72 * conviction), 0.0, float(entry_cap))))
        elif mode == "TREND_SHORT":
            target = int(round(clamp(95.0 * math.tanh(-0.72 * conviction), 0.0, float(entry_cap)))) * -1
        ...
        exit_mode = ""
        if pos > 70 and ((signal < 0.25 and trend_score < 0.15) or long_drawdown >= 7.0 or (progress > 0.88 and long_drawdown >= 4.0)):
            exit_mode = "long"
        elif pos < -70 and ((signal > -0.25 and trend_score > -0.15) or short_drawup >= 7.0 or (progress > 0.88 and short_drawup >= 4.0)):
            exit_mode = "short"
        ...
        if pos * target < 0 and abs(pos) > 35:
            target = 0
        quote_bias = 0.0
        if mode in ("LOCAL_LONG", "TREND_LONG"):
            quote_bias += 0.12 * clamp(signal, -1.0, 1.8)
        elif mode in ("LOCAL_SHORT", "TREND_SHORT"):
            quote_bias += 0.12 * clamp(signal, -1.8, 1.0)
        quote_bias += 0.10 * clamp(overlay_bias, 0.0, 1.0)
        take_bias = 0.12 * clamp(conviction, -1.5, 1.5)
        if exit_mode == "long":
            take_bias -= 0.18
        elif exit_mode == "short":
            take_bias += 0.18
        ...
        if exit_mode:
            size_mult *= 0.85
        same_side_bid_block = exit_mode == "long" or (mode == "TREND_SHORT" and pos < -80)
        same_side_ask_block = exit_mode == "short" or (mode == "TREND_LONG" and pos > 80)
        return {
            "mid": float(mid),
            "fair": float(fair),
            "signal": float(signal),
    ...
    def _trade_velvet(self, state: TradingState, fair: float, velvet_ctx: dict) -> List[Order]:
        ...
        buy_take_edge = cfg["take_edge"] - min(0.25, max(0.0, float(velvet_ctx["take_bias"])))
        sell_take_edge = cfg["take_edge"] + min(0.25, min(0.0, float(velvet_ctx["take_bias"])))
        for ask, volume in sorted(od.sell_orders.items()):
        ...
        reservation = fair + float(velvet_ctx["quote_bias"]) - cfg["inv_skew"] * inv_ratio
        quote_edge = cfg["quote_edge"] + max(0.0, 0.10 * (spread - 3.0))
        buy_px = round_down(reservation - quote_edge)
        sell_px = round_up(reservation + quote_edge)
        if bb is not None and ba is not None and bb < ba:
            if spread >= 3:
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            ...
            velvet_fair = self._underlying_fair(VELVET, state.order_depths[VELVET], overlay_bias)
            velvet_ctx = self._build_velvet_context(state, velvet_fair, memory, overlay_bias)
            result[VELVET] = self._trade_velvet(state, velvet_fair, velvet_ctx)
            memory["velvet_book"] = {
