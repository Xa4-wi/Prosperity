...
def black_scholes_vanna_proxy(spot: float, strike: float, tte_years: float, sigma: float) -> float:
    if spot <= 0.0 or strike <= 0.0 or tte_years <= 0.0 or sigma <= 0.0:
        return 0.0
    sqrt_t = math.sqrt(tte_years)
    d1 = (math.log(spot / strike) + 0.5 * sigma * sigma * tte_years) / (sigma * sqrt_t)
    d2 = d1 - sigma * sqrt_t
    return -norm_pdf(d1) * d2 / max(sigma, 1e-6)
...
class Trader:
    ...
    Round 3 v21:
    - R3_16 baseline
    - Version A: voucher vanna risk overlay
      - track strip vanna
      - track near-ATM vanna concentration
      - reduce middle-strike size/caps when vanna concentration is high
    """
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if progress > 0.70:
            cap = int(round(cap * 0.85))
        if progress > 0.85:
            cap = int(round(cap * 0.65))
        if progress > 0.95:
            cap = min(cap, 40)

        raw_target = 160.0 * math.tanh(0.9 * signal)
        target = int(round(clamp(raw_target, -float(cap), float(cap))))
        ...
        if extreme_hold_bars > 30:
            target = int(round(target * 0.75))
        elif extreme_hold_bars > 15:
        ...
        exceptional_buy = signal >= 2.5
        exceptional_sell = signal <= -2.5

        quote_bias = 0.0
        if strength < 0.8:
            quote_bias = -0.08 * signal

        size_mult = 0.90 if confidence == "guarded" else 1.0
        if extreme_hold_bars > 20:
            size_mult *= 0.85
        ...
        return {
            ...
            "same_side_bid_block": stretch_long and not exceptional_buy,
            "same_side_ask_block": stretch_short and not exceptional_sell,
            "buy_take_extra": 2.0 if stretch_long and not exceptional_buy else 0.0,
            "sell_take_extra": 2.0 if stretch_short and not exceptional_sell else 0.0,
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            edge = fair - ask
            if edge >= buy_take_edge and not (hydro_ctx["same_side_bid_block"] and current_pos >= 140):
        ...
        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            edge = bid - fair
            if edge >= sell_take_edge and not (hydro_ctx["same_side_ask_block"] and current_pos <= -140):
        ...
        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 56)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), cfg["clear_max"]))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 56)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), cfg["clear_max"]))
        ...
        if hydro_ctx["same_side_bid_block"] or hydro_ctx["same_side_ask_block"]:
            size_scale *= 0.85
        quote_size = max(6, int(round(cfg["quote_size"] * size_scale)))

        can_bid = mgr.buy_cap > 0
        can_ask = mgr.sell_cap > 0
        if hydro_ctx["same_side_bid_block"]:
            can_bid = False
        if hydro_ctx["same_side_ask_block"]:
            can_ask = False
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float) -> Dict[str, dict]:
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            surface[product] = {
                ...
                "vanna_proxy": black_scholes_vanna_proxy(velvet_fair, strike, TTE_YEARS, sigma),
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        strip_vega = 0.0
        strip_vanna = 0.0
        near_atm_vanna_abs = 0.0
        total_abs_vanna = 0.0
        near_atm_residuals: List[float] = []
        middle_abs = 0
        middle_net = 0
        low_wing_net = 0
        high_wing_net = 0
        ...
        for product, ctx in surface.items():
            ...
            vanna_pos = pos * float(ctx["vanna_proxy"])
            strip_vanna += vanna_pos
            total_abs_vanna += abs(vanna_pos)
            if 0.30 <= delta <= 0.70:
                near_atm_vanna_abs += abs(vanna_pos)
                near_atm_residuals.append(abs(float(ctx["iv_residual"])))
            if 0.20 <= delta <= 0.80:
                middle_abs += abs(pos)
                middle_net += pos
        ...
        near_atm_vanna_share = near_atm_vanna_abs / max(1.0, total_abs_vanna)
        near_atm_resid_abs_mean = (
            sum(near_atm_residuals) / len(near_atm_residuals) if near_atm_residuals else 0.0
        )
        vanna_pressure = clamp(near_atm_vanna_abs / 18.0, 0.0, 1.0) * clamp(near_atm_vanna_share / 0.60, 0.0, 1.0)
        ...
        return {
            ...
            "strip_vanna_proxy": strip_vanna,
            "near_atm_vanna_abs": near_atm_vanna_abs,
            "near_atm_vanna_share": near_atm_vanna_share,
            "near_atm_resid_abs_mean": near_atm_resid_abs_mean,
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        vanna_pressure = float(risk_ctx.get("vanna_pressure", 0.0))
        ...
        if middle_band:
            per_strike_cap = max(45, int(round(per_strike_cap * (1.0 - 0.30 * vanna_pressure))))
            take_max = max(6, int(round(take_max * (1.0 - 0.20 * vanna_pressure))))
        ...
        if middle_band:
            size_scale *= 0.45
            size_scale *= 1.0 - 0.25 * vanna_pressure
        if not confirmed:
            size_scale *= 0.75
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "strip_vanna_proxy": round(float(risk_ctx["strip_vanna_proxy"]), 3),
                    ...
                    "near_atm_vanna_abs": round(float(risk_ctx["near_atm_vanna_abs"]), 3),
                    "near_atm_vanna_share": round(float(risk_ctx["near_atm_vanna_share"]), 3),
                    "near_atm_resid_abs_mean": round(float(risk_ctx["near_atm_resid_abs_mean"]), 4),
                    "vanna_pressure": round(float(risk_ctx["vanna_pressure"]), 3),
