...
class Trader:
    ...
    def _repair_call_slice(
        ...
        # Enforce decreasing call prices by strike on each side separately.
        ...
        # Lightweight convexity repair keeps the smile inversion stable.
        for _ in range(2):
    ...
    def _build_voucher_surface(self, state: TradingState, velvet_fair: float, memory: dict) -> Dict[str, dict]:
        ...
        # Neighbor smoothing keeps the side surfaces execution-aware without making them noisy.
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict], memory: dict) -> dict:
        ...
        for product in ordered_products:
            ...
            base_cap = MIDDLE_STRIKE_CAP if VOUCHER_STRIKES[product] in MIDDLE_STRIKE_SET else BASE_WORKING_VOUCHER_CAP
            cap_scale = clamp(0.36 + 0.28 * active_score, 0.30, 1.00)
            active_cap = max(24, int(round(base_cap * cap_scale)))
            if active_score >= 0.95 or abs(pair_target_bias[product]) >= PAIR_PRIMARY_BIAS_THRESHOLD:
        ...
        if harvest_mode:
            strip_mode = "HARVEST"
        elif broad_dislocation and pair_intensity >= 1.45:
            strip_mode = "STRIP_SHOCK"
        elif pair_intensity >= 1.0 or avg_abs_resid >= resid_threshold:
            strip_mode = "RV_ACTIVE"
        else:
            strip_mode = "DISCOVERY"
        if abs(strip_delta) < 35.0:
            hedge_ratio = 0.0
        elif strip_mode == "STRIP_SHOCK":
            hedge_ratio = 0.75
        elif strip_mode == "RV_ACTIVE":
            hedge_ratio = 0.50
        else:
            hedge_ratio = 0.25
        if resid_compression > 0.012:
            hedge_ratio *= 0.70
        if harvest_mode:
            hedge_ratio *= 0.55

        raw_velvet_target = float(clamp(-hedge_ratio * strip_delta, -100.0, 100.0))
        prev_buffered_target = float(strip_state.get("buffered_velvet_target", raw_velvet_target))
        if strip_mode == "STRIP_SHOCK":
            target_step = 18.0
        elif strip_mode == "RV_ACTIVE":
            target_step = 12.0
        else:
            target_step = 8.0
        if harvest_mode:
            target_step = max(target_step, 16.0)
        if abs(raw_velvet_target - prev_buffered_target) < 10.0 and abs(strip_delta) < 55.0:
            buffered_velvet_target = prev_buffered_target
        else:
            buffered_velvet_target = prev_buffered_target + clamp(raw_velvet_target - prev_buffered_target, -target_step, target_step)
        if harvest_mode or compression_from_peak >= 0.25:
            buffered_velvet_target *= 0.55
        strip_state["buffered_velvet_target"] = buffered_velvet_target
        target_velvet_pos = int(round(clamp(buffered_velvet_target, -100.0, 100.0)))
        ...
        return {
            ...
            "raw_target_velvet_pos": int(round(raw_velvet_target)),
    ...
    def _build_velvet_plan(self, state: TradingState, velvet_fair: float, risk_ctx: dict) -> dict:
        ...
        strip_mode = str(risk_ctx.get("strip_mode", "DISCOVERY"))
        broad_dislocation = bool(risk_ctx["broad_dislocation"])
        smile_stability = float(risk_ctx["smile_stability_score"])

        # Velvet stays hedge-first; strip-aware alpha is secondary to strip execution.
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        if strip_mode == "DISCOVERY" and not pair_confirmed:
            target_scale *= 0.75
        if broad_dislocation and pair_confirmed:
            target_scale += 0.10
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            try:
                ...
                strip_ledger = memory.setdefault(
                    "strip_ledger",
                    {
                        "pair_trade_qty": 0,
                        "pair_trade_orders": 0,
                        "outright_trade_qty": 0,
                        "outright_trade_orders": 0,
                        "shock_trade_qty": 0,
                        "vev_hedge_qty": 0,
                        "deep_itm_qty": 0,
                        "near_atm_qty": 0,
                ...
                memory["strip_monitor"] = {
                    ...
                    "raw_target_velvet_pos": int(risk_ctx["raw_target_velvet_pos"]),
                    ...
                    "selected_pairs": [
                        f"{row['cheap']}->{row['rich']}:{row['spread']:.3f}"
                        for row in risk_ctx["selected_pairs"]
                    ],
                ...
                strip_ledger["vev_hedge_qty"] += sum(abs(order.quantity) for order in result[VELVET])
                for product in VOUCHER_STRIKES:
                    if product in state.order_depths:
                        ...
                        traded_qty = sum(abs(order.quantity) for order in voucher_orders)
                        if traded_qty <= 0:
                            continue
                        pair_backed = (
                            abs(float(risk_ctx["pair_target_bias"].get(product, 0.0))) >= PAIR_PRIMARY_BIAS_THRESHOLD
                            or abs(float(risk_ctx["pair_support"].get(product, 0.0))) >= 0.35
                        )
                        if pair_backed:
                            strip_ledger["pair_trade_qty"] += traded_qty
                            strip_ledger["pair_trade_orders"] += len(voucher_orders)
                        else:
                            strip_ledger["outright_trade_qty"] += traded_qty
                            strip_ledger["outright_trade_orders"] += len(voucher_orders)
                        if bool(risk_ctx["broad_dislocation"]):
                            strip_ledger["shock_trade_qty"] += traded_qty
                        strike = VOUCHER_STRIKES[product]
                        if strike <= 4500:
                            strip_ledger["deep_itm_qty"] += traded_qty
                        elif strike <= 5300:
                            strip_ledger["near_atm_qty"] += traded_qty
                        else:
                            strip_ledger["otm_qty"] += traded_qty
            except Exception as exc:
