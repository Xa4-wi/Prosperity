...
class Trader:
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if progress > 0.78:
            target_cap = int(round(target_cap * 0.88))
        if progress > 0.90:
            target_cap = int(round(target_cap * 0.65))
        if progress > 0.97:
            target_cap = min(target_cap, 50)

        target = int(round(clamp(200.0 * math.tanh(0.95 * regime_score), -float(target_cap), float(target_cap))))
        ...
        unwind_long = current_pos > 120 and (
            (
                long_peak_score >= 1.2
                and regime_score < max(0.45, 0.52 * long_peak_score)
                and trend_score < 1.0
            )
            or (long_drawdown >= 14.0 and ret_ema < -0.18)
            or (progress > 0.78 and trend_score < 0.85 and regime_score < 1.1)
        )
        unwind_long_hard = current_pos > 150 and (
            (long_drawdown >= 24.0 and ret_ema < -0.20)
            or (progress > 0.86 and trend_score < 0.65)
            or (progress > 0.92)
        )
        unwind_short = current_pos < -120 and (
            (
                short_peak_score <= -1.2
                and regime_score > min(-0.45, 0.52 * short_peak_score)
                and trend_score > -1.0
            )
            or (short_drawup >= 14.0 and ret_ema > 0.18)
            or (progress > 0.78 and trend_score > -0.85 and regime_score > -1.1)
        )
        unwind_short_hard = current_pos < -150 and (
            (short_drawup >= 24.0 and ret_ema > 0.20)
            or (progress > 0.86 and trend_score > -0.65)
            or (progress > 0.92)
        )
        ...
        if exit_mode == "long":
            target = min(target, 80 if progress < 0.86 else 40)
        elif exit_mode == "long_hard":
            target = min(target, 0 if progress > 0.80 else 20)
        elif exit_mode == "short":
            target = max(target, -80 if progress < 0.86 else -40)
        elif exit_mode == "short_hard":
            target = max(target, 0 if progress > 0.80 else -20)
        ...
        return {
            ...
            "same_side_bid_block": exit_mode.startswith("long") or (stretch_long and not exceptional_buy),
            "same_side_ask_block": exit_mode.startswith("short") or (stretch_short and not exceptional_sell),
            "buy_take_extra": 4.0 if exit_mode.startswith("long") else 2.0 if stretch_long and not exceptional_buy else 0.0,
            "sell_take_extra": 4.0 if exit_mode.startswith("short") else 2.0 if stretch_short and not exceptional_sell else 0.0,
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            elif edge >= cfg["take_edge"] + 2.5 and not unwind_long:
                mgr.buy(ask, min(-volume, cfg["take_max"]))
            else:
                break
        ...
        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            ...
            elif edge >= cfg["take_edge"] + 2.5 and not unwind_short:
                mgr.sell(bid, min(volume, cfg["take_max"]))
            else:
                break
        ...
        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 80 if unwind_long else 56)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 44 if unwind_long else cfg["clear_max"])))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 80 if unwind_short else 56)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 44 if unwind_short else cfg["clear_max"])))
