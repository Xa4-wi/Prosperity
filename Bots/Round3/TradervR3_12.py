...
class Trader:
    ...
    - Step 6: add conservative lower-strip pair-first residual-value targets
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        for left, right in zip(sorted_products, sorted_products[1:]):
            ...
            if VOUCHER_STRIKES[right] <= 5100 and abs(gap) >= 0.11:
                strength = 1 + int(abs(gap) >= 0.18)
                unit = min(16, 6 * strength)
                if gap > 0:
        ...
        for product in VOUCHER_STRIKES:
            pair_target[product] = int(clamp(float(pair_target.get(product, 0)), -16.0, 16.0))
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        pair_confirmed = abs(pair_target) >= 6
        ...
        reservation += 0.06 * clamp(pair_target / 8.0, -1.0, 1.0)
        ...
        if adjacent_block:
            quote_edge += 0.15
        if pair_confirmed and pair_gap_signal >= 0.11:
            quote_edge -= 0.05
