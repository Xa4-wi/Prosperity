...
class Trader:
    ...
    def _build_velvet_spot_context(self, state: TradingState, memory: dict) -> dict:
        ...
        velvet["ema_mid"] = float(mid) if velvet.get("ema_mid") is None else 0.14 * float(mid) + 0.86 * float(velvet["ema_mid"])
        velvet["ema_micro"] = float(micro) if velvet.get("ema_micro") is None else 0.18 * float(micro) + 0.82 * float(velvet["ema_micro"])
        velvet["ret_ema"] = 0.18 * ret + 0.82 * float(velvet.get("ret_ema", 0.0))
        velvet["vol_ema"] = 0.20 * abs(ret) + 0.80 * float(velvet.get("vol_ema", 0.0))
        move_unit = max(1.0, 0.45 * spread + 0.35 * float(velvet["vol_ema"]))
        fair = 0.55 * float(mid) + 0.25 * float(velvet["ema_mid"]) + 0.20 * float(velvet["ema_micro"])
        fair += 0.35 * imbalance * move_unit
        fair += 0.18 * clamp(float(velvet["ret_ema"]) / max(1.0, move_unit), -1.5, 1.5) * move_unit
        signal = (fair - float(mid)) / move_unit
        return {
            "fair": float(fair),
            "mid": float(mid),
            "spread": float(spread),
            ...
            "signal": float(signal),
            "realized_vol": float(velvet["vol_ema"]),
        }
    ...
    def _trade_velvet(self, state: TradingState, spot_ctx: dict, strip_delta: float, avg_abs_resid: float) -> List[Order]:
        od = state.order_depths[VELVET]
        hedge_ratio = 0.30 if avg_abs_resid < 0.015 else 0.45 if avg_abs_resid < 0.030 else 0.60
        alpha_target = 26.0 * math.tanh(0.90 * float(spot_ctx["signal"]))
        target = clamp(-hedge_ratio * strip_delta + alpha_target, -140.0, 140.0)
        fair = float(spot_ctx["fair"])
        move_unit = float(spot_ctx["move_unit"])
        cfg = {
            "soft_limit": 18 if abs(strip_delta) > 60.0 else 14,
            "take_max": 22,
            "clear_max": 34,
            "quote_size": 16,
            "take_edge": max(1.0, 0.42 * move_unit),
            "clear_edge": max(1.0, 0.28 * move_unit),
            "quote_edge": max(1.5, 0.58 * move_unit),
            "inv_skew": 5.0,
        }
        return trade_book(
            VELVET,
            ...
            LIMITS[VELVET],
            cfg,
            take_bias=0.10 * float(spot_ctx["signal"]) * move_unit,
            quote_bias=0.14 * float(spot_ctx["signal"]) * move_unit,
            size_mult=0.90,
        )
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            spot_ctx = self._build_velvet_spot_context(state, memory)
            ...
            if surface:
                for product, row in surface.items():
                    if product.startswith("_"):
                        continue
                    strip_delta += float(state.position.get(product, 0)) * float(row["delta"])
                result[VELVET] = self._trade_velvet(
                    state,
                    spot_ctx,
                    strip_delta,
                    float(surface["_diag"]["avg_abs_resid"]),
                )
                for product, row in surface.items():
                ...
                memory["diag"]["surface"] = {
                    ...
                    "strip_delta": round(float(strip_delta), 3),
                }
            else:
                result[VELVET] = self._trade_velvet(state, spot_ctx, 0.0, 0.0)
                memory["diag"]["surface"] = {
                    "fit_conf": 0.0,
                    "usable_strikes": 0,
                    "avg_abs_resid": 0.0,
            ...
            memory["diag"]["velvet"] = {
                ...
                "signal": round(float(spot_ctx["signal"]), 3),
                "realized_vol": round(float(spot_ctx["realized_vol"]), 3),
            }
