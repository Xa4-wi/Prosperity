            ...
            try:
                ...
                for product in VOUCHER_STRIKES:
                    ...
                    if product in LOW_AGGR_VOUCHERS:
                        ...
                        bs_target = 0.70 * limit * math.tanh((fair - opt_mid) / edge_scale)
                        dir_weight = 0.45 + 0.35 * delta
                        dir_target = 0.0
                        if bull_peak:
                            dir_target = -0.35 * limit * dir_weight
                        elif bear_trough:
                            dir_target = 0.60 * limit * dir_weight
                        ...
                        result[product] = self._trade_targeted(
                            ...
                            fair,
                            target,
                            take_edge=max(1.2, 0.90 * edge_scale),
                            clear_edge=max(1.0, 0.68 * edge_scale),
                            quote_edge=max(1.1, 0.72 * edge_scale),
                            quote_size=5 if product == "VEV_4000" else 4,
                            take_max=12 if product == "VEV_4000" else 10,
                            soft_limit=max(10, int(0.65 * limit)),
                            inv_skew=2.8 if product == "VEV_4000" else 3.0,
                            take_bias=0.14 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale + 0.10 * iv_residual,
                            quote_bias=0.08 * math.tanh((fair - opt_mid) / edge_scale) * edge_scale,
                            bid_block=False,
                            ask_block=False,
