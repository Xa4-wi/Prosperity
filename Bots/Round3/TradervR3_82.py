...
class Trader:
    ...
    def _build_velvet_z_context(self, state: TradingState, memory: dict) -> dict:
        ...
        velvet["ema_mid"] = float(mid) if velvet.get("ema_mid") is None else 0.12 * float(mid) + 0.88 * float(velvet["ema_mid"])
        velvet["ema_micro"] = float(micro) if velvet.get("ema_micro") is None else 0.16 * float(micro) + 0.84 * float(velvet["ema_micro"])
        fair = 0.58 * float(velvet["ema_mid"]) + 0.24 * float(stable) + 0.18 * float(velvet["ema_micro"])
        fair += 0.30 * imbalance
        dev = abs(float(mid) - float(fair))
        velvet["dev_ema"] = dev if velvet.get("dev_ema") is None else 0.14 * dev + 0.86 * float(velvet["dev_ema"])
        sigma = max(1.2, 1.35 * float(velvet["dev_ema"]), 0.80 * spread)
        z = (float(mid) - float(fair)) / sigma
        target = -68.0 * math.tanh(0.78 * z)
        take_bias = -0.12 * z
        quote_bias = -0.26 * z
        return {
            "fair": float(fair),
    ...
    def _trade_velvet(self, state: TradingState, spot_ctx: dict) -> List[Order]:
        ...
        cfg = {
            ...
            "clear_max": 28,
            "quote_size": 14,
            "take_edge": max(1.0, 0.40 * sigma),
            "clear_edge": max(1.0, 0.28 * sigma),
            "quote_edge": max(1.4, 0.54 * sigma),
            "inv_skew": 4.8,
        }
        return trade_book(
            VELVET,
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            spot_ctx = self._build_velvet_z_context(state, memory)
