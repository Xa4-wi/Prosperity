...
RESEARCH_FLAGS = {
    "hydrogel_simple_anchor_mm": False,
    "hydrogel_state_machine": True,
...
class HydrogelEngine:
    def _strong_up(self, trend_score: float, ret_ema: float, fair_gap: float, book_good: bool) -> bool:
        return trend_score > 1.6 and ret_ema > 0.0 and fair_gap > 0.35 and book_good

    def _strong_down(self, trend_score: float, ret_ema: float, fair_gap: float, book_good: bool) -> bool:
        return trend_score < -1.6 and ret_ema < 0.0 and fair_gap < -0.35 and book_good
    ...
    def build(self, state: TradingState, memory: dict) -> Tuple[float, dict]:
        ...
        stable = stable_mid(od) or mid
        micro = micro_price(od) or stable
        spread = float((ba - bb) if bb is not None and ba is not None else 3.0)
        depth = 0.0
        if bb is not None:
            depth += float(max(0, od.buy_orders.get(bb, 0)))
        if ba is not None:
            depth += float(abs(od.sell_orders.get(ba, 0)))
        book_good = bb is not None and ba is not None and bb < ba and spread <= 5.0 and depth >= 12.0
        progress = float(state.timestamp % 100000) / 100000.0
        imbalance = book_imbalance(od)

        hydro_state = memory.setdefault(
            "hydro_state",
            {
                "state": NEUTRAL,
                "fast_ema": mid,
        ...
        prev_mid = float(hydro_state.get("prev_mid", mid))
        fast_ema = 0.62 * float(hydro_state.get("fast_ema", mid)) + 0.38 * mid
        slow_ema = 0.90 * float(hydro_state.get("slow_ema", mid)) + 0.10 * mid
        ret = mid - prev_mid
        ret_ema = 0.80 * float(hydro_state.get("ret_ema", 0.0)) + 0.20 * ret
        hydro_state["prev_mid"] = mid
        hydro_state["fast_ema"] = fast_ema
        hydro_state["slow_ema"] = slow_ema
        hydro_state["ret_ema"] = ret_ema

        fair_gap = (fair - mid) / max(1.0, 0.5 * spread)
        trend_score = (fast_ema - slow_ema) / max(1.0, 0.55 * spread)
        anchor_score = (HYDRO_CFG["anchor"] - mid) / max(2.0, 0.80 * spread)
        flow_score = (micro - mid) / max(1.0, 0.5 * spread) + 0.90 * imbalance
        ...
        up_peak_mid = float(hydro_state.get("up_peak_mid", mid))
        down_peak_mid = float(hydro_state.get("down_peak_mid", mid))

        if state_name == UP_TREND:
            up_peak_trend = max(up_peak_trend, trend_score)
            up_peak_mid = max(up_peak_mid, mid)
        elif state_name == DOWN_TREND:
            down_peak_trend = min(down_peak_trend if down_peak_trend != 0.0 else trend_score, trend_score)
            down_peak_mid = min(down_peak_mid, mid)

        long_drawdown = max(0.0, up_peak_mid - mid)
        short_drawup = max(0.0, mid - down_peak_mid)
        ...
        strong_up = self._strong_up(trend_score, ret_ema, fair_gap, book_good)
        strong_down = self._strong_down(trend_score, ret_ema, fair_gap, book_good)
        strong_reconfirm_up = trend_score > 2.2 and ret_ema > 0.05 and fair_gap > 0.55 and book_good
        strong_reconfirm_down = trend_score < -2.2 and ret_ema < -0.05 and fair_gap < -0.55 and book_good
        neutral_zone = abs(trend_score) < 0.45 and abs(fair_gap) < 0.40

        # State transitions come first; size is decided only after regime is known.
        if state_name == UP_TREND:
            if (
            ...
            if strong_reconfirm_up and unwind_cooldown == 0 and mid >= up_peak_mid - 0.5:
                state_name = UP_TREND
                up_peak_trend = max(up_peak_trend, trend_score)
                up_peak_mid = max(up_peak_mid, mid)
            ...
            if strong_reconfirm_down and unwind_cooldown == 0 and mid <= down_peak_mid + 0.5:
                state_name = DOWN_TREND
                down_peak_trend = min(down_peak_trend if down_peak_trend != 0.0 else trend_score, trend_score)
                down_peak_mid = min(down_peak_mid, mid)
        ...
        if state_name == UP_TREND:
            ...
            if trend_score > 2.0 and fair_gap > 0.45:
                cap = 120
            if trend_score > 2.5 and fair_gap > 0.70 and book_good:
                cap = 140
            if trend_score > 3.1 and fair_gap > 0.95 and book_good and progress < 0.72:
                cap = 170
            ...
            if fade_score > 1.25 or progress > 0.94:
                target = 0
            elif fade_score > 0.80 or progress > 0.88:
                target = 30
            else:
                target = 60
            state_bias = -0.10
        ...
        flatten_before_flip = pos * target < 0 and abs(pos) > 60
        if flatten_before_flip:
            target = 0
        ...
        return fair, {
            ...
            "unwind_cooldown": unwind_cooldown,
            "fair_gap": fair_gap,
            "trend_score": trend_score,
            "anchor_score": anchor_score,
            "flow_score": flow_score,
            "ret_ema": ret_ema,
            "up_peak_trend": up_peak_trend,
            "down_peak_trend": down_peak_trend,
            "up_peak_mid": up_peak_mid,
            "down_peak_mid": down_peak_mid,
    ...
    def trade(self, state: TradingState, fair: float, ctx: dict) -> List[Order]:
        ...
        take_edge = HYDRO_CFG["take_edge"] + 0.10 * max(0.0, spread - 2.0)
        ...
        if ba is not None and not ctx["same_side_bid_block"] and fair - ba >= buy_take_edge and pos < LIMITS[HYDROGEL]:
            mgr.buy(ba, min(-od.sell_orders[ba], HYDRO_CFG["take_max"]))
        if bb is not None and not ctx["same_side_ask_block"] and bb - fair >= sell_take_edge and pos > -LIMITS[HYDROGEL]:
            mgr.sell(bb, min(od.buy_orders[bb], HYDRO_CFG["take_max"]))

        pos = mgr.projected()
        clear_edge = HYDRO_CFG["clear_edge"] + 0.05 * max(0.0, spread - 2.0)
        soft_limit = HYDRO_CFG["soft_limit"]
        if state_name == UP_UNWIND and pos > 0 and bb is not None and bb >= fair - (clear_edge + 0.25):
            mgr.sell(bb, min(max(0, pos - target), HYDRO_CFG["clear_max"]))
        elif state_name == DOWN_UNWIND and pos < 0 and ba is not None and ba <= fair + (clear_edge + 0.25):
            mgr.buy(ba, min(max(0, target - pos), HYDRO_CFG["clear_max"]))
        elif pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(pos - soft_limit, HYDRO_CFG["clear_max"]))
        elif pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min((-soft_limit) - pos, HYDRO_CFG["clear_max"]))

        pos = mgr.projected()
        inv_ratio = pos / float(LIMITS[HYDROGEL])
        reservation = fair + float(ctx["state_bias"]) - HYDRO_CFG["inv_skew"] * inv_ratio
        quote_edge = HYDRO_CFG["quote_edge"] + 0.10 * max(0.0, spread - 3.0)
...
class VoucherStripEngine:
    ...
    def trade_product(self, product: str, state: TradingState, ctx: dict, risk: dict) -> List[Order]:
        ...
        pair_confirmed = pair_target != 0 or pair_bias != 0
...
class Trader:
    ...
    def _reset_day_if_needed(self, memory: dict, timestamp: int) -> None:
        ...
        if memory.get("day") != current_day:
            ...
            memory["voucher"] = {"rolling_iv": {}, "last_avg_abs_resid": 0.0}
            memory["diag"] = {}
        ...
        if HYDROGEL in state.order_depths:
            try:
                ...
                memory["diag"]["hydrogel"] = {
                    ...
                    "stable": round(float(hydro_ctx["stable"]), 3),
                    "micro": round(float(hydro_ctx["micro"]), 3),
                    "spread": round(float(hydro_ctx["spread"]), 3),
                    "depth": round(float(hydro_ctx["depth"]), 3),
                    "book_good": bool(hydro_ctx["book_good"]),
                    "state": str(hydro_ctx["state"]),
                    ...
                    "unwind_cooldown": int(hydro_ctx["unwind_cooldown"]),
                    "fair_gap": round(float(hydro_ctx["fair_gap"]), 3),
                    ...
                    "ret_ema": round(float(hydro_ctx["ret_ema"]), 3),
                    "up_peak_trend": round(float(hydro_ctx["up_peak_trend"]), 3),
                    "down_peak_trend": round(float(hydro_ctx["down_peak_trend"]), 3),
                    "up_peak_mid": round(float(hydro_ctx["up_peak_mid"]), 3),
                    "down_peak_mid": round(float(hydro_ctx["down_peak_mid"]), 3),
