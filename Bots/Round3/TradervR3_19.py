...
class Trader:
    ...
    - Step 8: Hydrogel Phase B/C retune:
      earlier same-side blocking, stronger late shrink, and more proactive clear
    """
    ...
    def _build_hydrogel_targets(self, state: TradingState, fair: float, memory: dict) -> dict:
        ...
        if progress > 0.60:
            cap = int(round(cap * 0.82))
        if progress > 0.78:
            cap = int(round(cap * 0.58))
        if progress > 0.90:
            cap = min(cap, 45)
        if progress > 0.96:
            cap = min(cap, 20)
        ...
        side = 1 if current_pos > 100 else -1 if current_pos < -100 else 0
        if side == 0:
        ...
        if extreme_hold_bars > 36:
            target = int(round(target * 0.50))
        elif extreme_hold_bars > 24:
            target = int(round(target * 0.70))
        elif extreme_hold_bars > 12:
            target = int(round(target * 0.88))

        flatten_before_flip = False
        if current_pos * target < 0 and abs(current_pos) > 40:
            target = 0
            flatten_before_flip = True

        stretch_long = current_pos > 120
        stretch_short = current_pos < -120
        exceptional_buy = signal >= 3.0 and progress < 0.72
        exceptional_sell = signal <= -3.0 and progress < 0.72

        quote_bias = 0.0
        if strength < 1.0:
            quote_bias = -0.10 * signal

        size_mult = 0.85 if confidence == "guarded" else 1.0
        if abs(current_pos) > 100:
            size_mult *= 0.82
        ...
        return {
            ...
            "buy_take_extra": 3.0 if stretch_long and not exceptional_buy else 0.8 if current_pos > 100 else 0.0,
            "sell_take_extra": 3.0 if stretch_short and not exceptional_sell else 0.8 if current_pos < -100 else 0.0,
    ...
    def _trade_hydrogel(self, state: TradingState, fair: float, hydro_ctx: dict) -> List[Order]:
        ...
        soft_limit = 65
        hard_zone = 105
        clear_edge = cfg["clear_edge"] + (0.25 if hydro_ctx["progress"] > 0.72 else 0.0)
        hard_clear_edge = clear_edge + 2.5

        if relative_pos > hard_zone and bb is not None and bb >= fair - hard_clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 72)))
        elif relative_pos > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative_pos - soft_limit)), max(cfg["clear_max"], 42)))

        pos = mgr.projected()
        relative_pos = pos - target
        if relative_pos < -hard_zone and ba is not None and ba <= fair + hard_clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 72)))
        elif relative_pos < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative_pos)), max(cfg["clear_max"], 42)))

        pos = mgr.projected()
        relative_pos = pos - target
        inv_ratio = relative_pos / LIMITS[HYDROGEL]
        dynamic_skew = cfg["inv_skew"] + 5.5
        if abs(relative_pos) > 100:
            dynamic_skew += 3.0
        if hydro_ctx["progress"] > 0.85:
            dynamic_skew += 2.0
        reservation = fair + float(hydro_ctx["quote_bias"]) - dynamic_skew * inv_ratio
        ...
        size_scale = max(0.12, 1.0 - 1.25 * abs(inv_ratio)) * max(0.30, float(hydro_ctx["size_mult"]))
        if abs(relative_pos) > 100:
            size_scale *= 0.70
        if hydro_ctx["progress"] > 0.85:
            size_scale *= 0.70
