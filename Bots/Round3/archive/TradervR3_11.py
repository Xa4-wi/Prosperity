...
class Trader:
    ...
    - Step 6: add small pair-first residual-value targets across neighboring strikes
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        pair_target: Dict[str, int] = {}
        pair_gap_signal: Dict[str, float] = {}
        ...
        for left, right in zip(sorted_products, sorted_products[1:]):
            ...
            pair_gap_signal[left] = max(abs(gap), abs(pair_gap_signal.get(left, 0.0)))
            pair_gap_signal[right] = max(abs(gap), abs(pair_gap_signal.get(right, 0.0)))
            if gap >= 0.07:
            ...
            if abs(gap) >= 0.10:
                strength = 1 + int(abs(gap) >= 0.16)
                base_unit = 10
                if 0.20 <= float(left_ctx["delta"]) <= 0.80 or 0.20 <= float(right_ctx["delta"]) <= 0.80:
                    base_unit = 8
                unit = min(20, base_unit * strength)
                if gap > 0:
                    pair_target[left] = pair_target.get(left, 0) + unit
                    pair_target[right] = pair_target.get(right, 0) - unit
                else:
                    pair_target[left] = pair_target.get(left, 0) - unit
                    pair_target[right] = pair_target.get(right, 0) + unit

        for product in VOUCHER_STRIKES:
            pair_target[product] = int(clamp(float(pair_target.get(product, 0)), -24.0, 24.0))
        ...
        return {
            ...
            "pair_gap_signal": pair_gap_signal,
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        pair_target = int(risk_ctx["pair_target"].get(product, 0))
        pair_gap_signal = float(risk_ctx["pair_gap_signal"].get(product, 0.0))
        bb = best_bid(od)
        ba = best_ask(od)
        spread = (ba - bb) if bb is not None and ba is not None else 2.0
        ...
        pair_confirmed = abs(pair_target) >= 8
        ...
        reservation += 0.10 * clamp(pair_target / 12.0, -1.0, 1.0)
        ...
        if adjacent_block:
            quote_edge += 0.15
        if pair_confirmed and pair_gap_signal >= 0.10:
            quote_edge -= 0.08
        ...
        if fair >= 0.5:
            long_cap = per_strike_cap + max(0, pair_target)
            short_cap = per_strike_cap + max(0, -pair_target)
            can_bid = mgr.buy_cap > 0 and pos < long_cap
            can_ask = mgr.sell_cap > 0 and pos > -short_cap
            if middle_band and risk_ctx["middle_abs"] > risk_ctx["middle_cap"]:
                if risk_ctx["middle_net"] >= 0:
                    can_bid = False
                if risk_ctx["middle_net"] <= 0:
                    can_ask = False
            if not confirmed and not pair_confirmed and pos >= risk_ctx["unconfirmed_outright_cap"]:
                can_bid = False
            if not confirmed and not pair_confirmed and pos <= -risk_ctx["unconfirmed_outright_cap"]:
                can_ask = False
            if delta_block_buy:
            ...
            if delta_block_sell:
                can_ask = False
            if adjacent_block:
                if pos >= max(int(0.7 * per_strike_cap), pair_target + int(0.4 * per_strike_cap)) and pair_bias >= 0:
                    can_bid = False
                if pos <= min(-int(0.7 * per_strike_cap), pair_target - int(0.4 * per_strike_cap)) and pair_bias <= 0:
                    can_ask = False
        ...
        if velvet_fair is not None:
            try:
                ...
                memory["strip_monitor"] = {
                    ...
                    "pair_target": {k: int(v) for k, v in risk_ctx["pair_target"].items() if int(v) != 0},
                }
