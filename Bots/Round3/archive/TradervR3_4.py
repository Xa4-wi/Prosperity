...
class VoucherEngine:
    ...
    def build_risk_context(self, state: TradingState, surface: Dict[str, dict], velvet_ctx: dict) -> dict:
        ...
        long_candidates = {product for product, _ in sorted(residual_pairs, key=lambda item: item[1])[:2]}
        short_candidates = {product for product, _ in sorted(residual_pairs, key=lambda item: item[1], reverse=True)[:2]}
    ...
    def trade_product(self, product: str, state: TradingState, voucher_ctx: dict, velvet_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        linked_shift = 0.22 * delta * spot_signal - 0.08 * delta * delta_pressure + 0.18 * rv_score

        buy_take_edge = max(
            0.60,
            max(0.85, 0.38 * spread)
            - max(0.0, 0.10 * rv_score)
            + max(0.0, 0.10 * delta * delta_pressure)
            + 0.10 * atm_risk,
        )
        sell_take_edge = max(
            0.60,
            max(0.85, 0.38 * spread)
            - max(0.0, -0.10 * rv_score)
            - min(0.0, 0.10 * delta * delta_pressure)
            + 0.10 * atm_risk,
        )
        clear_edge = max(0.35, 0.15 * spread)
        soft_limit = max(90, int(round(140 - 50 * atm_risk)))
        ...
        quote_edge += 0.25 * atm_risk + 0.18 * abs(delta_pressure) * delta
        if product not in risk_ctx["long_candidates"] and product not in risk_ctx["short_candidates"]:
            quote_edge += 0.25
        else:
            quote_edge = max(0.9, quote_edge - 0.12 * abs(rv_score))
        ...
        size_scale *= max(0.55, 1.0 - 0.35 * atm_risk)
        if product in risk_ctx["long_candidates"] or product in risk_ctx["short_candidates"]:
            size_scale *= min(1.2, 0.95 + 0.10 * abs(rv_score))
        else:
            size_scale *= 0.72
