...
class Trader:
    ...
    - Step 6: split Velvet into a hedge book and a capped standalone alpha book
    - Step 7: force flatten-before-flip so the hedge book cannot turn into a large directional swing
    - Step 8: rebalance Hydrogel and Velvet fair construction toward the
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        return {
            ...
            "hedge_feasible_scale": clamp(1.0 - max(0.0, abs(strip_delta) - 70.0) / 180.0, 0.60, 1.0),
        }

    def _build_velvet_targets(
        self,
        state: TradingState,
        velvet_fair: float,
        overlay_bias: float,
        ...
        hedge_ratio = 0.50
        excess_delta = 0.0
        if strip_delta > delta_band:
            excess_delta = strip_delta - delta_band
        elif strip_delta < -delta_band:
            excess_delta = strip_delta + delta_band
        hedge_target = int(round(clamp(-hedge_ratio * excess_delta, -120.0, 120.0)))
        ...
        good_book = (
            ...
            and spread <= 6.0
            and top_depth >= 40.0
            and abs(float(stable) - float(mid)) <= 0.9
        )

        alpha_signal = 0.0
        if good_book:
            alpha_signal = (
                0.85 * (velvet_fair - float(mid))
                + 0.50 * book_imbalance(od, levels=2) * max(1.0, 0.5 * spread)
                + 0.10 * (float(micro) - float(mid))
                + 0.25 * overlay_bias
            )

        alpha_scale = clamp(1.0 - max(0.0, abs(strip_delta) - 20.0) / 60.0, 0.0, 1.0)
        alpha_cap = 40.0
        alpha_target = int(round(clamp(alpha_signal * 8.0, -alpha_cap, alpha_cap) * alpha_scale))

        raw_target = hedge_target + alpha_target
        current_pos = int(state.position.get(VELVET, 0))
        if current_pos * raw_target < 0 and abs(current_pos) > 60:
            final_target = 0
            flatten_before_flip = True
        else:
            final_target = int(clamp(float(raw_target), -200.0, 200.0))
            flatten_before_flip = False

        quote_bias = -0.12 * float(risk_ctx["delta_pressure"])
        if good_book and abs(strip_delta) < 60.0:
            quote_bias += 0.02 * alpha_target

        size_mult = 1.0
        if abs(strip_delta) > 120.0:
            size_mult = 0.85
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        hedge_feasible_scale = float(risk_ctx["hedge_feasible_scale"])
        base_soft_limit = 85 if middle_band else 125
        base_per_strike_cap = 100 if not middle_band else 80
        soft_limit = max(60, int(round(base_soft_limit * hedge_feasible_scale)))
        per_strike_cap = max(55, int(round(base_per_strike_cap * hedge_feasible_scale)))
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            if pair_bias > 0:
                edge += 0.20
            if edge >= take_edge and not delta_block_buy:
        ...
        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            ...
            if pair_bias < 0:
                edge += 0.20
            if edge >= take_edge and not delta_block_sell:
        ...
        if abs(residual_rank) >= 3:
            size_scale *= 0.90
        size_scale *= hedge_feasible_scale
    ...
    def run(self, state: TradingState):
        ...
        if velvet_fair is not None:
            try:
                ...
                velvet_targets = self._build_velvet_targets(state, velvet_fair, velvet_overlay_bias, risk_ctx)
                ...
                memory["velvet_book"] = {
                    "mid": round(float(velvet_targets["mid"]), 3),
                    "strip_delta": round(float(velvet_targets["strip_delta"]), 3),
                    "hedge_target": int(velvet_targets["hedge_target"]),
                    "alpha_target": int(velvet_targets["alpha_target"]),
                    "final_target": int(velvet_targets["final_target"]),
                    "good_book": bool(velvet_targets["good_book"]),
                    "alpha_scale": round(float(velvet_targets["alpha_scale"]), 3),
                    "flatten_before_flip": bool(velvet_targets["flatten_before_flip"]),
                }
                result[VELVET] = self._trade_underlying(
                    VELVET,
                    state,
                    velvet_fair,
                    position_target=float(velvet_targets["final_target"]),
                    quote_bias=float(velvet_targets["quote_bias"]),
                    size_mult=float(velvet_targets["size_mult"]),
                )
