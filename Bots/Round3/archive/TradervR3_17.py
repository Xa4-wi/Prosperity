...
class Trader:
    ...
    - Step 8: selectively reopen the low-strike voucher lane without touching
      the protected 5100+ middle and upper wing controls
    """
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        low_alpha_zone = strike in {4500, 5000}
        ...
        if low_alpha_zone:
            resid_threshold *= 0.72
            extreme_resid_threshold *= 0.82
        cheap_signal = max(0.0, -iv_residual)
        rich_signal = max(0.0, iv_residual)
        low_zone_buy_bias = low_alpha_zone and bs_gap <= -max(1.0, 0.30 * spread) and cheap_signal >= 0.55 * resid_threshold
        low_zone_sell_bias = low_alpha_zone and bs_gap >= max(1.0, 0.30 * spread) and rich_signal >= 0.55 * resid_threshold
        buy_confirmed = cheap_signal >= resid_threshold or pair_bias > 0 or low_zone_buy_bias
        sell_confirmed = rich_signal >= resid_threshold or pair_bias < 0 or low_zone_sell_bias
        ...
        if abs(residual_rank) >= 3:
            size_scale *= 0.90
        if low_alpha_zone and (buy_confirmed or sell_confirmed):
            size_scale *= 1.25
        ...
        if fair >= 0.5:
            ...
            if low_alpha_zone:
                can_bid = mgr.buy_cap > 0 and pos < per_strike_cap and (buy_confirmed or cheap_signal >= 0.45 * resid_threshold or pos < -18)
                can_ask = mgr.sell_cap > 0 and pos > -per_strike_cap and (sell_confirmed or rich_signal >= 0.45 * resid_threshold or pos > 18)
