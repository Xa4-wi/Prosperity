...
class Trader:
    ...
    - Step 8: add a strip shock / IV scalp mode that only activates when the
      smile is broadly dislocated but strip risk is still under control
    """
    ...
    def _build_voucher_risk_context(self, state: TradingState, surface: Dict[str, dict]) -> dict:
        ...
        avg_abs_residual = (
            sum(abs(float(surface[product]["iv_residual"])) for product in liquid_products) / max(1, len(liquid_products))
        )
        pair_agreement = sum(1 for product in liquid_products if abs(int(pair_bias.get(product, 0))) > 0)
        total_delta_cap = 150.0
        middle_cap = 170
        broad_dislocation = (
            len(liquid_products) >= 5
            and avg_abs_residual > max(0.055, 1.20 * resid_scale)
            and pair_agreement >= 4
            and abs(strip_delta) < 0.80 * total_delta_cap
            and middle_abs < 0.80 * middle_cap
        )
        hedge_band = 24.0 if broad_dislocation else 12.0
        hedge_excess = 0.0
        if abs(strip_delta) > hedge_band:
            hedge_excess = math.copysign(abs(strip_delta) - hedge_band, strip_delta)
        hedge_ratio = 0.45 if broad_dislocation else 0.50
        ...
        return {
            ...
            "delta_pressure": clamp(hedge_excess / 140.0, -2.0, 2.0),
            "hedge_band": hedge_band,
            "target_velvet_pos": int(round(clamp(-hedge_ratio * hedge_excess, -100.0, 100.0))),
    ...
    def _trade_voucher(self, product: str, state: TradingState, voucher_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        broad_dislocation = bool(risk_ctx.get("broad_dislocation", False))
        ...
        if broad_dislocation:
            take_edge -= 0.22
            take_max += 4 if middle_band else 2
            pair_bonus = 0.35
        ...
        reservation = fair - inv_penalty + 0.30 * signal_shift + (0.18 if broad_dislocation else 0.12) * pair_bias
        ...
        if abs(bs_gap) > 1.0:
            quote_edge += 0.10
        if broad_dislocation:
            quote_edge = max(0.85, quote_edge - 0.15)
        ...
        if abs(residual_rank) >= 3:
            size_scale *= 0.90
        if broad_dislocation and confirmed:
            size_scale *= 1.10
