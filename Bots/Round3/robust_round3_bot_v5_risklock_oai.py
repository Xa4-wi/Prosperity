...
class Trader:
    """
    Round 3 robust v5: volatility-smile voucher model with profit-lock risk control.
    ...
    The voucher model is Black-Scholes based:
    ...
    def run(self, state: TradingState):
        ...
        # Estimate live mark-to-market PnL from own trades + current mids.
        # Used only as a risk governor: lock gains and stop adding voucher risk after a strong run-up.
    ...
    def _update_voucher_sigma(self, data: dict, order_depths: Dict[str, OrderDepth], underlying_fair: float, timestamp: int) -> None:
        """Update only a small residual around the fixed cross-day smile.

        This avoids relearning the full smile from a single day, while still allowing
        the bot to adapt if the final day is slightly richer/cheaper in vol.
        """
    ...
    def _trade_voucher(self, product: str, depth: OrderDepth, pos: int, fair: float, underlying_fair: float, delta: float, data: dict) -> List[Order]:
        ...
        # Strike-specific controls from the v3 log: 5400 was toxic; far OTM vouchers
        # are fragile and need larger edge before trading.
        ...
        if lock_only_unwind:
            # Only reduce existing voucher exposure. Do not add new positions after peak PnL lock.
        ...
        if spread >= 2 and not (fair <= 1.05 and spread < 3):
            ...
            if bid_px < ask_px:
                ...
                if lock_only_unwind:
                    # Quote only the flattening side once gains are locked.
                    if pos > 0:
                        buy_size = 0
