...
SMALL_VELVET_LIMIT = 90
SMALL_VOUCHER_STRIKES: Dict[str, int] = {
    "VEV_4000": 4000,
    "VEV_4500": 4500,
    "VEV_5000": 5000,
    "VEV_5100": 5100,
}
LOW_VOUCHER_STRIKES = ("VEV_4000", "VEV_4500")
MID_VOUCHER_STRIKES = ("VEV_5000", "VEV_5100")
SMALL_VOUCHER_LIMITS: Dict[str, int] = {
    "VEV_4000": 16,
    "VEV_4500": 16,
    "VEV_5000": 18,
    "VEV_5100": 18,
}
SMALL_VOUCHER_HALF_SPREAD: Dict[str, float] = {
    "VEV_4000": 10.0,
    "VEV_4500": 8.0,
    "VEV_5000": 4.0,
    "VEV_5100": 3.0,
...
class Trader:
    ...
    def _build_small_velvet_context(self, state: TradingState, memory: dict) -> Optional[dict]:
        od = state.order_depths.get(VELVET)
        if od is None:
            return None
        saved = memory.setdefault("velvet_small_state", {})
        mid = raw_mid(od)
        if mid is None:
            mid = stable_mid(od)
        if mid is None:
        ...
        bb = best_bid(od)
        ba = best_ask(od)
        spread = float(max(1, ba - bb)) if bb is not None and ba is not None else 6.0
        ...
        bull_peak = z > 1.45 and micro_gap <= 0.0 and ret_ema <= 0.2
        bear_trough = z < -1.45 and micro_gap >= 0.0 and ret_ema >= -0.2
    ...
    def _trade_targeted(
        ...
        od = state.order_depths[product]
        limit = int(limit_override if limit_override is not None else LIMITS[product])
        mgr = OrderManager(product, state.position.get(product, 0), limit)
        ...
        for ask, volume in sorted(od.sell_orders.items()):
            ...
            if edge < take_edge:
                break
            desired = max(0, int(round(position_target - mgr.projected())))
        ...
        for bid, volume in sorted(od.buy_orders.items(), reverse=True):
            ...
            if edge < take_edge:
                break
            desired = max(0, int(round(mgr.projected() - position_target)))
        ...
        bb = best_bid(od)
        ba = best_ask(od)
        relative = mgr.projected() - position_target
        if relative > soft_limit and bb is not None and bb >= fair - clear_edge:
            mgr.sell(bb, min(int(math.ceil(relative - soft_limit)), take_max + 6))
        elif relative < -soft_limit and ba is not None and ba <= fair + clear_edge:
            mgr.buy(ba, min(int(math.ceil((-soft_limit) - relative)), take_max + 6))

        relative = mgr.projected() - position_target
        reservation = fair + quote_bias - inv_skew * (relative / max(1.0, float(limit)))
        bid_px = round(reservation - quote_edge)
        ask_px = round(reservation + quote_edge)
        if bb is not None and ba is not None and bb < ba:
            spread = ba - bb
            if spread >= 3:
        ...
        if not bid_block and mgr.buy_cap > 0 and (ba is None or bid_px < ba):
            mgr.buy(bid_px, min(quote_size, mgr.buy_cap))
        if not ask_block and mgr.sell_cap > 0 and (bb is None or ask_px > bb):
    ...
    def run(self, state: TradingState):
        ...
        if VELVET in state.order_depths:
            try:
                velvet_ctx = self._build_small_velvet_context(state, memory)
                if velvet_ctx is not None:
                    memory["velvet_small"] = {
                        "fair": round(float(velvet_ctx["fair"]), 3),
                        "mid": round(float(velvet_ctx["mid"]), 3),
                        "z": round(float(velvet_ctx["z"]), 3),
                        "vol_ema": round(float(velvet_ctx["vol_ema"]), 3),
                        "target": int(round(float(velvet_ctx["target"]))),
                        "mode": str(velvet_ctx["mode"]),
                    }
                    result[VELVET] = self._trade_targeted(
                        VELVET,
                        state,
                        float(velvet_ctx["fair"]),
                        float(velvet_ctx["target"]),
                        take_edge=max(1.0, 0.45 * float(velvet_ctx["vol_ema"])),
                        clear_edge=max(1.0, 0.28 * float(velvet_ctx["vol_ema"])),
                        quote_edge=max(1.5, 3.0 + 0.18 * float(velvet_ctx["vol_ema"])),
                        ...
                        limit_override=SMALL_VELVET_LIMIT,
                    )
                    for product in VOUCHER_STRIKES:
                        if product in state.order_depths and product not in SMALL_VOUCHER_STRIKES:
                            result[product] = []

                    sigma_bs = clamp(0.17 + 0.04 * min(2.5, abs(float(velvet_ctx["z"]))), 0.12, 0.30)
                    regime = str(velvet_ctx["mode"])
                    voucher_diag: Dict[str, dict] = {}
                    low_rows: Dict[str, dict] = {}
                    mid_rows: Dict[str, dict] = {}
                    for product, strike in SMALL_VOUCHER_STRIKES.items():
                        od = state.order_depths.get(product)
                        if od is None:
                            continue
                        opt_mid = raw_mid(od)
                        if opt_mid is None:
                            opt_mid = stable_mid(od)
                        if opt_mid is None:
                            result[product] = []
                            continue
                        limit = SMALL_VOUCHER_LIMITS[product]
                        fair = black_scholes_call(float(velvet_ctx["fair"]), strike, TTE_YEARS, sigma_bs)
                        delta = black_scholes_delta_call(float(velvet_ctx["fair"]), strike, TTE_YEARS, sigma_bs)
                        edge_scale = SMALL_VOUCHER_HALF_SPREAD[product]
                        ...
                        row = {
                            ...
                            "score": mispricing / max(2.0, edge_scale),
                        }
                        if product in LOW_VOUCHER_STRIKES:
                            low_rows[product] = row
                        else:
                            mid_rows[product] = row

                    mid_pair_adjust: Dict[str, float] = {product: 0.0 for product in MID_VOUCHER_STRIKES}
                    if all(product in mid_rows for product in MID_VOUCHER_STRIKES):
                        spread_score = mid_rows["VEV_5000"]["score"] - mid_rows["VEV_5100"]["score"]
                        if abs(spread_score) > 0.45:
                            pair_size = 3.0 + (2.0 if regime != "neutral" else 0.0)
                            if spread_score > 0.0:
                                mid_pair_adjust["VEV_5000"] += pair_size
                                mid_pair_adjust["VEV_5100"] -= pair_size
                            else:
                                mid_pair_adjust["VEV_5000"] -= pair_size
                                mid_pair_adjust["VEV_5100"] += pair_size

                    ordered_rows = [*(low_rows[p] for p in LOW_VOUCHER_STRIKES if p in low_rows), *(mid_rows[p] for p in MID_VOUCHER_STRIKES if p in mid_rows)]
                    for row in ordered_rows:
                        product = str(row["product"])
                        strike = int(row["strike"])
                        limit = int(row["limit"])
                        fair = float(row["fair"])
                        opt_mid = float(row["mid"])
                        delta = float(row["delta"])
                        edge_scale = float(row["edge_scale"])
                        mispricing = float(row["mispricing"])
                        if product in LOW_VOUCHER_STRIKES:
                            target = clamp(0.45 * limit * math.tanh(mispricing / max(2.0, edge_scale)), -float(limit), float(limit))
                            take_edge = max(1.5, 1.05 * edge_scale)
                            clear_edge = max(1.25, 0.85 * edge_scale)
                            quote_edge = max(1.4, edge_scale + 0.8)
                            quote_size = 2
                            take_max = 3
                            soft_limit = max(4, int(0.55 * limit))
                            inv_skew = 3.3
                            take_bias = 0.08 * math.tanh(mispricing / max(2.0, edge_scale)) * edge_scale
                            quote_bias = 0.06 * math.tanh(mispricing / max(2.0, edge_scale)) * edge_scale
                            bid_block = False
                            ask_block = False
                            lane = "low_bs"
                        else:
                            bs_target = 0.30 * limit * math.tanh(mispricing / max(2.0, edge_scale))
                            dir_weight = 0.52 if strike == 5000 else 0.72
                            dir_target = 0.0
                            if regime == "bull_peak":
                                dir_target = -limit * dir_weight
                            elif regime == "bear_trough":
                                dir_target = limit * dir_weight
                            target = clamp(dir_target + bs_target + mid_pair_adjust.get(product, 0.0), -float(limit), float(limit))
                            take_edge = max(1.0, 0.92 * edge_scale)
                            clear_edge = max(1.0, 0.72 * edge_scale)
                            quote_edge = max(1.0, edge_scale + 0.35)
                            quote_size = 3
                            take_max = 5
                            soft_limit = max(5, int(0.62 * limit))
                            inv_skew = 2.4
                            take_bias = 0.12 * math.tanh(mispricing / max(2.0, edge_scale)) * edge_scale
                            quote_bias = 0.10 * math.tanh(mispricing / max(2.0, edge_scale)) * edge_scale
                        ...
                        voucher_diag[product] = {
                            "lane": lane,
                            "fair": round(float(fair), 3),
                            "mid": round(float(opt_mid), 3),
                            "mispricing": round(float(mispricing), 3),
                            "target": int(round(float(target))),
                            "delta": round(float(delta), 4),
                            "pair_adjust": round(float(mid_pair_adjust.get(product, 0.0)), 3),
                        }
                    memory["voucher_small"] = voucher_diag
            except Exception as exc:
                memory["engine_errors"]["velvet_small"] = type(exc).__name__
