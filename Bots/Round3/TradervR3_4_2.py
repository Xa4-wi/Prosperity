...
class VoucherEngine:
    ...
    def build_surface(self, state: TradingState, velvet_ctx: dict) -> Dict[str, dict]:
        ...
        for product, strike in VOUCHER_STRIKES.items():
            ...
            m = math.log(strike / spot_fair) / math.sqrt(TTE_YEARS)
            sigma = clamp(float(polyval(coeffs, m)), 1e-4, 3.0)
            fair = black_scholes_call(spot_fair, strike, TTE_YEARS, sigma)
            delta = black_scholes_delta_call(spot_fair, strike, TTE_YEARS, sigma)
    ...
    def build_risk_context(self, state: TradingState, surface: Dict[str, dict], velvet_ctx: dict) -> dict:
        residual_pairs = [
            (product, float(ctx["iv_residual"]))
            for product, ctx in surface.items()
            if ctx.get("market_mid") is not None and float(ctx["market_mid"]) > 0.5
        ]
        ...
        long_candidates = {product for product, _ in sorted(residual_pairs, key=lambda item: item[1])[:1]}
        short_candidates = {product for product, _ in sorted(residual_pairs, key=lambda item: item[1], reverse=True)[:1]}
        ...
        hedge_ratio = 0.18
        hedge_target = int(round(clamp(-hedge_ratio * strip_delta, -35.0, 35.0)))
        delta_pressure = clamp(strip_delta / 240.0, -1.5, 1.5)
        return {
            ...
            "target_velvet_pos": hedge_target,
            "spot_signal": float(velvet_ctx.get("directional_signal", 0.0)),
        }

    def trade_product(self, product: str, state: TradingState, voucher_ctx: dict, velvet_ctx: dict, risk_ctx: dict) -> List[Order]:
        ...
        spot_signal = float(risk_ctx["spot_signal"])
        delta_pressure = float(risk_ctx["delta_pressure"])
        rv_score = clamp(-iv_residual / float(risk_ctx["resid_scale"]), -1.5, 1.5)
        atm_risk = clamp(1.0 - abs(delta - 0.5) / 0.5, 0.0, 1.0)
        linked_shift = 0.18 * delta * spot_signal - 0.10 * delta * delta_pressure + 0.15 * rv_score

        buy_take_edge = max(
            0.60,
            max(0.85, 0.38 * spread)
            - max(0.0, 0.08 * rv_score)
            + max(0.0, 0.14 * delta * delta_pressure)
            + 0.16 * atm_risk,
        )
        sell_take_edge = max(
            0.60,
            max(0.85, 0.38 * spread)
            - max(0.0, -0.08 * rv_score)
            - min(0.0, 0.14 * delta * delta_pressure)
            + 0.16 * atm_risk,
        )
        clear_edge = max(0.35, 0.15 * spread)
        soft_limit = max(80, int(round(128 - 58 * atm_risk)))
        take_max = 16 if atm_risk > 0.55 else 22
        ...
        quote_edge += 0.38 * atm_risk + 0.24 * abs(delta_pressure) * delta
        if product not in risk_ctx["long_candidates"] and product not in risk_ctx["short_candidates"]:
            quote_edge += 0.45
        else:
            quote_edge = max(0.9, quote_edge - 0.10 * abs(rv_score))
        ...
        size_scale *= max(0.45, 1.0 - 0.45 * atm_risk)
        if product in risk_ctx["long_candidates"] or product in risk_ctx["short_candidates"]:
            size_scale *= min(1.1, 0.90 + 0.08 * abs(rv_score))
        else:
            size_scale *= 0.58
    ...
    def trade_all(self, state: TradingState, velvet_ctx: dict, surface: Dict[str, dict], risk_ctx: dict) -> Dict[str, List[Order]]:
        result: Dict[str, List[Order]] = {}
        for product in VOUCHER_STRIKES:
            if product in state.order_depths:
                result[product] = self.trade_product(product, state, surface[product], velvet_ctx, risk_ctx)
        return result
...
class Trader:
    ...
    - VoucherEngine: option surface + light strip-risk-aware execution for all VEV_* products

    The strip context is intentionally light-touch:
    - better size-based wall fair construction
    - engine isolation so one module failure does not kill the tick
    - voucher smile residual ranking and strip-delta awareness
    - stronger residual-aware throttling on the voucher strip
    - no strong cross-strike target positions
    """
    ...
    def run(self, state: TradingState):
        ...
        if velvet_ctx is not None:
            ...
            try:
                ...
                risk_ctx = self.voucher_engine.build_risk_context(state, surface, velvet_ctx)
            except Exception as exc:
                memory["engine_errors"]["voucher_context"] = type(exc).__name__

            try:
                velvet_target = float(risk_ctx["target_velvet_pos"]) if risk_ctx is not None else 0.0
                velvet_quote_bias = -0.12 * float(risk_ctx["delta_pressure"]) if risk_ctx is not None else 0.0
            ...
            if surface is not None and risk_ctx is not None:
                try:
                    result.update(self.voucher_engine.trade_all(state, velvet_ctx, surface, risk_ctx))
                except Exception as exc:
