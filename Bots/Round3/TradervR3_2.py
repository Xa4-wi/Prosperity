...
class AnchoredUnderlyingEngine:
    ...
    def trade(self, state: TradingState, fair: float) -> List[Order]:
...
class VoucherEngine:
    ...
    def trade_product(self, product: str, state: TradingState, voucher_ctx: dict, velvet_ctx: dict) -> List[Order]:
        ...
        # Explicit connection to the Velvet engine: carry its directional signal into
        # call-option execution and quoting. Bullish Velvet -> slightly happier to own calls.
        spot_signal = float(velvet_ctx.get("directional_signal", 0.0))
        linked_shift = 0.35 * delta * spot_signal

        buy_take_edge = max(0.50, max(0.75, 0.35 * spread) - max(0.0, 0.15 * delta * spot_signal))
        sell_take_edge = max(0.50, max(0.75, 0.35 * spread) - max(0.0, -0.15 * delta * spot_signal))
    ...
    def trade_all(self, state: TradingState, velvet_ctx: dict) -> Dict[str, List[Order]]:
        surface = self.build_surface(state, velvet_ctx)
        result: Dict[str, List[Order]] = {}
        for product in VOUCHER_STRIKES:
            if product in state.order_depths:
                result[product] = self.trade_product(product, state, surface[product], velvet_ctx)
        return result
...
class Trader:
    ...
    - VoucherEngine: option surface + execution for all VEV_* products

    The VoucherEngine receives an explicit Velvet context so the option layer is
    connected to the underlying through both spot fair and directional signal.
    """
    ...
    def run(self, state: TradingState):
        ...
        if velvet_ctx is not None:
            result[VELVET] = self.velvet_engine.trade(state, float(velvet_ctx["fair"]))
            result.update(self.voucher_engine.trade_all(state, velvet_ctx))
