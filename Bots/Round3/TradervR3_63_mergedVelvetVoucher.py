...
# ============================================================
# Clean Round 3 base bot
# - Hydrogel: state-aware trend/local-fair engine with discrete
#   target optimization, unwind lock, and inventory danger control.
# - Velvet + vouchers: Black-Scholes / IV-slice engine with
#   quote repair, bid/ask execution bands, hybrid local IV fair,
#   pair-first strip logic, and buffered dynamic hedge.
# ============================================================
...
# Default per-voucher exchange limit from the prompt.
DEFAULT_VOUCHER_LIMIT = 300

# Round 3 expiry assumption (starting from round 1 with TTE=7d,
# round 3 means TTE=5d). Used only as a stable coordinate system.
TTE_DAYS = 5.0
YEAR_DAYS = 365.0
TTE_YEARS = TTE_DAYS / YEAR_DAYS

# Stronger safety than exchange limits: smaller working caps help hedgeability.
...
def bs_vanna(s: float, k: float, t: float, sigma: float) -> float:
    ...
    d1 = (math.log(s / k) + 0.5 * sigma * sigma * t) / st
    d2 = d1 - st
    # Common Black-Scholes vanna form for a call.
...
def weighted_quadratic_fit(xs: List[float], ys: List[float], ws: List[float]) -> Optional[Tuple[float, float, float, float]]:
    ...
    if sol is None:
        return None
    a, b, c = sol
    # Weighted residual RMSE as confidence proxy.
    err_num = 0.0
    err_den = 0.0
    for x, y, w in zip(xs, ys, ws):
        yhat = a + b * x + c * x * x
...
class Book:
    ...
    def stable_mid(self, top_n: int = 3) -> float:
        ...
        # True wall price: largest visible queue among the sampled levels.
...
class Trader:
    ...
    def _hydro_state(self, memory: dict, book: Book, position: int, timestamp: int) -> Tuple[dict, dict]:
        ...
        # Re-entry block after unwind: prevents churn.
        ...
        # Approximate dynamic programming over discrete targets.
        ...
        for q in candidates:
            if abs(q) > entry_cap:
                continue
            # Strongly shrink hold target when already large or late.
            if abs(position) > 100 and abs(q) > hold_cap and sign(q) == sign(position):
                continue
            # Block immediate refill after unwind.
        ...
        # Small state-aware fair shift only in trend states.
        if state == "UP_TREND":
    ...
    def _trade_hydrogel(self, state: TradingState, memory: dict) -> List[Order]:
        ...
        # Absolute danger clearing independent of target gap.
        ...
        # Clear / recycle around target and absolute danger.
    ...
    def _repair_call_slice(self, strikes: List[int], prices: List[Optional[float]], spot_fair: float) -> List[Optional[float]]:
        ...
        for i in range(len(strikes)):
            ...
            prev = repaired[i]
        # Simple convexity repair: middle strike should not exceed chord.
        for _ in range(2):
    ...
    def _build_strip_context(self, state: TradingState, memory: dict, velvet_fair: float) -> Tuple[dict, dict]:
        ...
        # Low-strike lane is where the strip most often expresses directional edge cleanly.
        # We summarize it once here so Velvet can act as a hedge-first instrument with a
        # small strip-aware alpha overlay rather than trading its own local signal in a silo.
        low_lane_scores: List[float] = []
        low_lane_cheap = 0
        low_lane_rich = 0

        # Pair support boosts pair-first behavior.
        ...
        for product, strike in vouchers:
            ...
            if strike <= 5000:
                score = -float(residuals.get(product, 0.0)) + 0.6 * float(pair_support.get(product, 0.0))
                low_lane_scores.append(score)
                if float(residuals.get(product, 0.0)) < -0.04:
                    low_lane_cheap += 1
                elif float(residuals.get(product, 0.0)) > 0.04:
                    low_lane_rich += 1
        ...
        context = {
            ...
            "low_lane_bias": low_lane_bias,
            "low_lane_cheap": low_lane_cheap,
            "low_lane_rich": low_lane_rich,
            "cheap_rich_balance": cheap_rich_balance,
        }
    ...
    def _trade_velvet(self, state: TradingState, vev_ctx: dict, strip_ctx: dict) -> List[Order]:
        ...
        low_lane_bias = float(strip_ctx.get("low_lane_bias", 0.0))
        cheap_rich_balance = int(strip_ctx.get("cheap_rich_balance", 0))

        # Blend local Velvet alpha with a small strip-aware overlay so Velvet is no longer
        # acting independently from the voucher book. The hedge book still dominates.
        alpha_signal = (fair - book.mid) / max(1.0, book.spread / 2.0)
        alpha_target = 0.0
        if abs(strip_delta) < 60 and vev_ctx["book_health"] > 0.45:
            local_alpha = 26.0 * math.tanh(0.7 * alpha_signal)
            strip_alpha = 14.0 * math.tanh(0.9 * low_lane_bias) + 6.0 * clamp(float(cheap_rich_balance), -2.0, 2.0)
            alpha_target = clamp(local_alpha + strip_alpha, -45.0, 45.0)
        if broad_dislocation and abs(strip_delta) >= 60:
            alpha_target = clamp(alpha_target, -18.0, 18.0)
        if resid_compression > 0.015:
            alpha_target *= 0.65
        ...
        hedge_gap = position - hedge_target
        ...
        if rel_gap > 18 and book.best_bid >= reservation - 0.8:
            mgr.sell(book.best_bid, min(book.best_bid_vol, 16, max(6, rel_gap)))
        if rel_gap < -18 and book.best_ask <= reservation + 0.8:
        ...
        if broad_dislocation:
            ...
            sell_qe -= 0.1
        if abs(hedge_gap) > 30:
            buy_qe += 0.15 if hedge_gap > 0 else -0.10
            sell_qe += 0.15 if hedge_gap < 0 else -0.10
        front_buy = int(math.floor(reservation - buy_qe))
        front_sell = int(math.ceil(reservation + sell_qe))
        front_buy = min(front_buy, book.best_ask - 1)
        front_sell = max(front_sell, book.best_bid + 1)
        buy_q = 12 if abs(hedge_gap) < 28 else 8
        sell_q = 12 if abs(hedge_gap) < 28 else 8
        if mgr.projected_pos() < final_target + 55:
            mgr.buy(front_buy, buy_q)
        if mgr.projected_pos() > final_target - 55:
            mgr.sell(front_sell, sell_q)
        return mgr.orders
    ...
    def _trade_vouchers(self, state: TradingState, memory: dict, strip_ctx: dict) -> Dict[str, List[Order]]:
        ...
        hedge_gap = velvet_pos - hedge_target
        hedge_ready = abs(hedge_gap) <= 28

        # Track adjacent same-side gross for risk control.
        ...
        for idx, (product, strike) in enumerate(vouchers):
            ...
            # If Velvet is still behind its hedge target, slow down any voucher adds that
            # worsen strip delta. This ties realized strip risk to actual hedge readiness.
            worsens_delta = (target > position and strip_delta > 0.0) or (target < position and strip_delta < 0.0)
            if not hedge_ready and worsens_delta:
                target = int(round(position + 0.45 * (target - position)))

            # Adjacent same-side cap.
            if idx > 0:
            ...
            if not hedge_ready and strip_delta > 0.0:
                buy_confirmed = buy_confirmed and (pair_confirmed or score < -1.6)
            if not hedge_ready and strip_delta < 0.0:
                sell_confirmed = sell_confirmed and (pair_confirmed or score > 1.6)

            # Pair-first: require confirmation for larger actions.
            ...
            # Clear toward target once residual normalizes.
            ...
            # Small passive maker around fair execution bands.
            buy_qe = 1.0
            sell_qe = 1.0
    ...
    def run(self, state: TradingState):
        ...
        # Hydrogel first: separate state engine, mostly independent.
        if HYDRO in state.order_depths:
            result[HYDRO] = self._trade_hydrogel(state, memory)

        # Velvet + voucher strip as one connected portfolio.
