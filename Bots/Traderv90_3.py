...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "TREND_FAIR_BONUS": 0.15,
    "TREND_ENTRY_TAKE_BONUS": 1.5,
    "TREND_HOLD_EXIT_BONUS": 0.34,
    ...
    "WALL_SIZE_FLOOR": 4.0,
    "WALL_ALPHA_WEIGHT": 0.08,
    "WALL_FAIR_WEIGHT": 0.12,
    ...
    "MR_TREND_DAMP": 0.82,
    "MR_RANGE_MIN_STRENGTH": 0.24,
    "MR_RANGE_MIN_SIGNAL": 0.32,
    "MR_RANGE_TARGET_DEADZONE": 2.0,
    "MR_RANGE_CONTINUATION_VETO": 0.18,
    "MR_RANGE_PASSIVE_SIZE_SCALE": 0.72,
    "MR_RANGE_WEAK_EDGE_PENALTY": 0.10,
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.14,
    "ANCHOR_CAP_RATIO": 0.24,
    "ANCHOR_TARGET_BLEND": 0.00,
    "ANCHOR_RESERVATION_WEIGHT": 0.00,
    "WRONG_WAY_RESERVATION_MULT": 1.55,
    "ALIGNED_RESERVATION_MULT": 0.72,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def gate_range_mean_reversion(
        ...
        if mr_signal * lob_signal < -veto:
            opposing_votes += 1
        if opposing_votes >= 2:
            return mr_signal * 0.55, mr_strength * 0.55
        return mr_signal, mr_strength
    ...
    def take_edge(
        ...
        if regime == "range" and mr_strength > 0.0:
            ...
            edge += self.MR_RANGE_WEAK_EDGE_PENALTY
    ...
    def passive_size(self, side: str, regime: str, volatility: float, queue_bias: float) -> int:
        ...
        if regime == "range":
            size = max(1, int(round(size * self.MR_RANGE_PASSIVE_SIZE_SCALE)))
    ...
    def in_warmup(self) -> bool:
        return len(self.mr_price_window) < int(self.MR_LONG_WINDOW) or len(self.mid_history.get(self.product, [])) < int(self.REGRESSION_WINDOW)
    ...
    def run(self) -> List[Order]:
        ...
        if (
            ...
            if regime == "range":
                ...
                if desired > 0:
                    quantity = min(self.passive_size("BUY", regime, volatility, queue_bias), self.buy_capacity, desired)
        ...
        if (
            ...
            if regime == "range":
                ...
                if desired > 0:
                    quantity = min(self.passive_size("SELL", regime, volatility, queue_bias), self.sell_capacity, desired)
