...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "BASE_TAKE_EDGE": 0.78,
    "BASE_QUOTE_EDGE": 2.42,
    "MAX_QUOTE_EDGE": 9.0,
    "PASSIVE_SIZE": 9,
    "MAX_TAKE_SIZE": 12,
    "REGRESSION_WINDOW": 8,
    "REGRESSION_HORIZON": 0.5,
    "TREND_EDGE_THRESHOLD": 0.82,
    ...
    "TREND_FAIR_BONUS": 0.12,
    "TREND_ENTRY_TAKE_BONUS": 1.0,
    "TREND_HOLD_EXIT_BONUS": 0.30,
    "STRONG_TREND_HOLD_EXIT_BONUS": 0.55,
    "TREND_PASSIVE_PUSH": 0.0,
    "TREND_PASSIVE_SIZE_BONUS": 1.0,
    ...
    "WALL_SIZE_FLOOR": 4.0,
    "WALL_ALPHA_WEIGHT": 0.06,
    "WALL_FAIR_WEIGHT": 0.10,
    "WALL_EWMA_ALPHA": 0.22,
    "WALL_PERSISTENCE_FLOOR": 0.35,
    "GAP_CURVE_POWER": 1.60,
    "GAP_FAIR_WEIGHT": 0.00,
    "GAP_RESERVATION_WEIGHT": 0.00,
    "GAP_QUOTE_EDGE_WEIGHT": 0.00,
    "GAP_PASSIVE_SIZE_WEIGHT": 0.00,
    "QUEUE_ALPHA_WEIGHT": 0.08,
    "QUEUE_FAIR_WEIGHT": 0.03,
    ...
    "MR_RSI_WINDOW": 10,
    "MR_SIGNAL_CAP": 2.4,
    "MR_RANGE_TARGET_SCALE": 16.0,
    "MR_FAIR_WEIGHT": 0.34,
    "MR_TAKE_SHIFT": 0.16,
    "MR_QUOTE_SHIFT": 0.10,
    "MR_ZSCORE_ACTIVATION": 0.95,
    "MR_RSI_ACTIVATION": 6.0,
    "MR_CONFLICT_DAMP": 0.58,
    "MR_TREND_DAMP": 0.70,
    "ANCHOR_PHI": 0.48,
    "ANCHOR_BETA": 0.14,
    "ANCHOR_CAP_RATIO": 0.24,
    "ANCHOR_TARGET_BLEND": 0.00,
    "ANCHOR_RESERVATION_WEIGHT": 0.00,
    "WRONG_WAY_RESERVATION_MULT": 1.40,
    "ALIGNED_RESERVATION_MULT": 0.78,

    "EARLY_READINESS_MIN": 0.40,
    "INV_WEAK_CAP": 16,
    "INV_MEDIUM_CAP": 28,
    "INV_STRONG_CAP": 40,
    "INV_EXCEPTIONAL_CAP": 54,
    "INV_MAX_CAP": 68,
    "INV_LEAN_ZONE": 24,
    "INV_UNWIND_ZONE": 38,
    "INV_EMERGENCY_ZONE": 56,
    "INV_UNWIND_TARGET": 16,
    "INV_ADD_CONFIDENCE": 0.55,
    "INV_HOLD_CONFIDENCE": 0.66,
    "INV_UNWIND_FLATTEN_BIAS": 0.65,
    "INV_EMERGENCY_FLATTEN_BIAS": 1.25,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def queue_reactive_signal(self) -> float:
        if not self.has_book():
            return 0.0
    ...
    def anchor_target(
        ...
        confidence = self.signal_confidence(predicted_edge, fit_quality, queue_bias, lob_signal)
        scale = max(1.0, float(self.spread) / 2.0)
    ...
    def readiness_scale(self) -> float:
        history_len = len(self.mid_history.get(self.product, []))
        mr_len = len(self.mr_price_window)
        required = max(int(self.MR_LONG_WINDOW), int(self.REGRESSION_WINDOW))
        ready = min(history_len, mr_len)
        return clamp(ready / max(1.0, float(required)), self.EARLY_READINESS_MIN, 1.0)
    ...
    def directional_confidence(
        ...
        confidence = self.signal_confidence(predicted_edge, fit_quality, queue_bias, lob_signal)
        half_spread = max(1.0, float(self.spread) / 2.0)
        trend_score = abs(predicted_edge) / max(1.0, 1.8 * half_spread)
        mr_score = abs(mr_signal) * max(0.25, mr_strength) / max(1.0, self.MR_SIGNAL_CAP)
        score = confidence * (0.58 * min(1.0, trend_score) + 0.42 * min(1.0, mr_score))
        direction = 0
        if regime == "range":
            if mr_strength >= 0.18 and mr_signal > 0.10 and queue_bias > -0.18 and lob_signal > -0.18:
                direction = 1
            elif mr_strength >= 0.18 and mr_signal < -0.10 and queue_bias < 0.18 and lob_signal < 0.18:
                direction = -1
        if direction == 0:
            if predicted_edge > 0.18:
                direction = 1
            elif predicted_edge < -0.18:
                direction = -1
        ...
        return direction, clamp(score, 0.0, 1.0)

    def target_cap(self, regime: str, directional_score: float) -> int:
        if directional_score < 0.24:
            cap = self.INV_WEAK_CAP
        elif directional_score < 0.42:
            cap = self.INV_MEDIUM_CAP
        elif directional_score < 0.64:
            cap = self.INV_STRONG_CAP
        elif directional_score < 0.84:
            cap = self.INV_EXCEPTIONAL_CAP
        else:
            cap = self.INV_MAX_CAP
        if regime == "range":
            cap = min(cap, self.INV_STRONG_CAP if directional_score < 0.70 else self.INV_EXCEPTIONAL_CAP)
        return int(min(self.position_limit, cap))

    def inventory_mode(self) -> str:
        abs_pos = abs(self.projected_position())
        if abs_pos >= self.INV_EMERGENCY_ZONE:
            return "emergency"
        if abs_pos >= self.INV_UNWIND_ZONE:
            return "unwind"
        if abs_pos >= self.INV_LEAN_ZONE:
            return "lean"
        return "neutral"
    ...
    def apply_inventory_policy(
        ...
        queue_bias: float,
        lob_signal: float,
    ) -> Tuple[int, bool, bool, float, str, float]:
        direction, directional_score = self.directional_confidence(
            regime, predicted_edge, fit_quality, mr_signal, mr_strength, queue_bias, lob_signal
        )
        cap = self.target_cap(regime, directional_score)
        target_position = int(max(-cap, min(cap, target_position)))
        ...
        if mode == "neutral":
            return target_position, block_buy, block_sell, flatten_bias, mode, directional_score

        if mode == "lean":
            if position > 0 and not (direction > 0 and directional_score >= self.INV_ADD_CONFIDENCE):
                block_buy = True
                if direction < 0:
                    target_position = min(target_position, max(0, int(round(0.55 * position))))
                    flatten_bias = max(flatten_bias, 0.18 * half_spread)
            if position < 0 and not (direction < 0 and directional_score >= self.INV_ADD_CONFIDENCE):
                block_sell = True
                if direction > 0:
                    target_position = max(target_position, min(0, int(round(0.55 * position))))
                    flatten_bias = min(flatten_bias, -0.18 * half_spread)
            return target_position, block_buy, block_sell, flatten_bias, mode, directional_score

        unwind_target = int(self.INV_UNWIND_TARGET)
        if mode == "unwind":
            if position > 0:
                block_buy = True
                target_position = min(target_position, unwind_target)
                flatten_bias = self.INV_UNWIND_FLATTEN_BIAS * half_spread
            elif position < 0:
                block_sell = True
                target_position = max(target_position, -unwind_target)
                flatten_bias = -self.INV_UNWIND_FLATTEN_BIAS * half_spread
            return target_position, block_buy, block_sell, flatten_bias, mode, directional_score
        ...
        if position > 0:
            ...
            flatten_bias = self.INV_EMERGENCY_FLATTEN_BIAS * half_spread
        elif position < 0:
            block_sell = True
    ...
    def target_position(
        ...
        if regime == "range":
            align = 1.0
            if mr_signal * queue_bias > 0:
                align += 0.10
            elif mr_signal * queue_bias < 0:
                align -= 0.15
            if mr_signal * lob_signal > 0:
                align += 0.10
            elif mr_signal * lob_signal < 0:
                align -= 0.15
            base_target = int(round(base_target * max(0.60, min(1.25, align))))
            return max(lower, min(upper, base_target))
        anchor_target = self.anchor_target(regime, predicted_edge, fit_quality, queue_bias, lob_signal)
    ...
    def fair_value(
        ...
        queue_fair_weight = 0.0 if regime == "range" else self.QUEUE_FAIR_WEIGHT
        lob_fair_weight = 0.0 if regime == "range" else self.LOB_FAIR_WEIGHT
        regression_weight = 0.08 if regime == "range" else self.REGRESSION_WEIGHT
        fair_alpha_weight = 0.18 if regime == "range" else self.FAIR_ALPHA_WEIGHT
        fair = (
            self.MID_WEIGHT * float(self.mid)
            + self.MICRO_WEIGHT * float(self.micro)
            ...
            + queue_fair_weight * half_spread * queue_bias
        )
        fair += fair_alpha_weight * hybrid_alpha
        ...
        fair += lob_fair_weight * half_spread * lob_signal
        if regime == "range":
    ...
    def take_edge(
        ...
        queue_take_shift = self.QUEUE_TAKE_SHIFT if regime != "range" else 0.30 * self.QUEUE_TAKE_SHIFT
        if side == "BUY":
            edge -= queue_take_shift * max(0.0, queue_bias)
            edge += queue_take_shift * max(0.0, -queue_bias)
        else:
            edge -= queue_take_shift * max(0.0, -queue_bias)
            edge += queue_take_shift * max(0.0, queue_bias)
        ...
        if lob_signal != 0.0:
            ...
            lob_take_shift = self.LOB_TAKE_SHIFT if regime != "range" else 0.35 * self.LOB_TAKE_SHIFT
            if aligned:
                edge -= lob_take_shift * min(1.0, abs(lob_signal))
            else:
                edge += (0.05 if regime != "range" else 0.02) * min(1.0, abs(lob_signal))
    ...
    def quote_edge(
        ...
        if regime == "trend_up":
            ...
            edge += 0.02 * abs(queue_bias)
            edge -= self.MR_QUOTE_SHIFT * mr_strength
        if lob_signal != 0.0 and regime != "volatile":
            lob_quote_shift = self.LOB_QUOTE_SHIFT if regime != "range" else 0.35 * self.LOB_QUOTE_SHIFT
            edge -= lob_quote_shift * min(1.0, abs(lob_signal))
    ...
    def passive_quotes(
        ...
        edge = self.quote_edge(regime, volatility, fit_quality, target_position, mr_strength, queue_bias, lob_signal)
    ...
    def passive_size(self, side: str, regime: str, volatility: float, queue_bias: float, directional_score: float) -> int:
        ...
        readiness = self.readiness_scale()
        size = max(1, int(round(size * readiness)))
        if directional_score < self.INV_HOLD_CONFIDENCE:
            size = max(1, int(round(0.75 * size)))
        mode = self.inventory_mode()
        if mode == "lean":
            size = max(1, int(round(0.70 * size)))
        elif mode == "unwind":
    ...
    def allow_passive(self, side: str, regime: str, directional_score: float) -> bool:
        position = self.projected_position()
        ...
        if mode == "emergency":
            ...
            if side == "SELL" and position < 0:
                return False
        if mode == "lean" and directional_score < self.INV_ADD_CONFIDENCE:
            if side == "BUY" and position > 0:
                return False
            if side == "SELL" and position < 0:
                return False
    ...
    def take_orders(
        ...
        if (
            int(self.best_ask) <= adjusted_fair - self.take_edge("BUY", regime, predicted_edge, fit_quality, volatility, mr_signal, mr_strength, queue_bias, lob_signal)
            and self.buy_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() >= target_position:
                ...
                if regime == "range":
                    quantity = min(quantity, max(0, target_position - self.projected_position()))
                else:
                    quantity = min(quantity, max(1, target_position - self.projected_position()))
                if quantity > 0:
                    qty_scale = self.readiness_scale()
                    if regime == "range" and mr_signal > 0 and queue_bias > 0 and lob_signal > 0:
                        qty_scale = min(1.15, qty_scale + 0.10)
                    quantity = max(0, int(round(quantity * qty_scale)))
                    before = self.buy_capacity
                    self.add_buy(int(self.best_ask), quantity)
                    took_buy = self.buy_capacity < before

        if (
            int(self.best_bid) >= adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, mr_signal, mr_strength, queue_bias, lob_signal)
            and self.sell_capacity > 0
        ):
            ...
            if regime != "range" and self.projected_position() <= target_position:
                pass
            elif int(self.best_bid) < adjusted_fair + self.take_edge("SELL", regime, predicted_edge, fit_quality, volatility, mr_signal, mr_strength, queue_bias, lob_signal) + required_bonus:
                pass
            else:
                quantity = min(self.best_bid_volume, take_limit)
                if regime == "range":
                    quantity = min(quantity, max(0, self.projected_position() - target_position))
                else:
    ...
    def in_warmup(self) -> bool:
        return len(self.mr_price_window) < max(6, int(self.MR_SHORT_WINDOW)) or len(self.mid_history.get(self.product, [])) < 4

    def adverse_range_brake(
        self,
        regime: str,
        target_position: int,
        predicted_edge: float,
        ...
        if regime != "range":
            return target_position, False, False, 0.0
        ...
        if long_adverse:
            target_position = min(target_position, max(0, int(round(0.35 * position))))
            block_buy = True
            flatten_bias = 0.45 * half_spread * max(0.35, mr_strength)
        elif short_adverse:
            target_position = max(target_position, min(0, int(round(0.35 * position))))
            block_sell = True
            flatten_bias = -0.45 * half_spread * max(0.35, mr_strength)
    ...
    def run(self) -> List[Order]:
        ...
        queue_bias = self.queue_reactive_signal()
        ...
        _hybrid_fair, hybrid_alpha = self.hybrid_alpha(wall_mid, wall_strength, queue_bias, lob_signal)
        ...
        target_position = self.target_position(regime, predicted_edge, fit_quality, mr_signal, mr_strength, queue_bias, lob_signal)
        target_position, block_buy, block_sell, flatten_bias, inventory_mode, directional_score = self.apply_inventory_policy(
            regime,
            target_position,
            predicted_edge,
            fit_quality,
        ...
        target_position, range_block_buy, range_block_sell, range_flatten_bias = self.adverse_range_brake(
            regime,
            target_position,
            predicted_edge,
            queue_bias,
        ...
        block_buy = block_buy or range_block_buy
        block_sell = block_sell or range_block_sell
        flatten_bias += range_flatten_bias
        adjusted_fair = self.adjusted_fair_value(
            regime,
            target_position,
            predicted_now,
            ...
            queue_bias,
            lob_signal,
        ) - self.reservation_adjustment(regime, target_position, predicted_edge, volatility) + flatten_bias
        ...
        if (
            ...
            and buy_quote is not None
            and self.buy_capacity > 0
            and self.allow_passive("BUY", regime, directional_score)
            and not block_buy
        ):
            if regime == "range":
                desired = max(0, target_position - position)
                if desired > 0:
                    quantity = min(self.passive_size("BUY", regime, volatility, queue_bias, directional_score), self.buy_capacity, desired)
                    self.add_buy(buy_quote, quantity)
            elif position < target_position:
                quantity = min(self.passive_size("BUY", regime, volatility, queue_bias, directional_score), self.buy_capacity)
        ...
        if (
            ...
            and sell_quote is not None
            and self.sell_capacity > 0
            and self.allow_passive("SELL", regime, directional_score)
            and not block_sell
        ):
            if regime == "range":
                desired = max(0, position - target_position)
                if desired > 0:
                    quantity = min(self.passive_size("SELL", regime, volatility, queue_bias, directional_score), self.sell_capacity, desired)
                    self.add_sell(sell_quote, quantity)
            elif position > target_position:
                quantity = min(self.passive_size("SELL", regime, volatility, queue_bias, directional_score), self.sell_capacity)
