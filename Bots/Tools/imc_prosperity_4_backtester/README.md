...
This clone is wired into the Prosperity workspace with a small launcher:

```bash
python3 Bots/Tools/imc_prosperity_4_backtester/run_repo_backtest.py \
  Bots/Round3/TradervR3_47.py \
  3-2 \
  --no-vis \
  --no-progress
```
...
Notes:
- The launcher injects the right `PYTHONPATH` entries for both `prosperity4bt` and `datamodel.py`.
- If you do not pass `--out` or `--no-out`, output logs are saved to:
  - `Bots/Tools/imc_prosperity_4_backtester/output/`
- You can still use the upstream CLI directly if you want.
