...
DEFAULT_ASH_PARAMS = {
    ...
    "SHORT_COVER_POS": -12,
    "SHORT_COVER_SIGNAL_MAX": -0.12,
    "SHORT_COVER_TAKE_RELIEF": 0.20,
    "SHORT_COVER_QE_BONUS": 0.15,
    "SHORT_COVER_FRONT_BONUS": 2,
    "BASELINE_SIGNAL_MAX": 0.18,
    "BASELINE_QE_BONUS": 0.08,
    "BASELINE_FRONT_MIN": 3,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if ask_level == 2 and pos < -6:
            can_take_sell = False
        short_cover = (
            pos <= int(self.p["SHORT_COVER_POS"])
            and quote_signal > float(self.p["SHORT_COVER_SIGNAL_MAX"])
            and take_signal > (float(self.p["SHORT_COVER_SIGNAL_MAX"]) - 0.08)
            and ask_level <= 1
        )
        baseline_active = (
            mode == "normal"
            ...
            and abs(quote_signal) <= float(self.p["BASELINE_SIGNAL_MAX"])
            and abs(pos) <= max(14, soft - 6)
        )
        ...
        if short_cover:
            buy_need = max(0.70, buy_need - float(self.p["SHORT_COVER_TAKE_RELIEF"]))
        ...
        if access_buy:
            ...
            sell_qe -= 0.08
        if short_cover:
            buy_qe -= float(self.p["SHORT_COVER_QE_BONUS"])
        if baseline_active:
            buy_qe -= float(self.p["BASELINE_QE_BONUS"])
            sell_qe -= float(self.p["BASELINE_QE_BONUS"])
        ...
        if access_buy and book.best_bid + 1 < book.best_ask:
            ...
            front_sell = min(front_sell, book.best_ask - 1)
        if short_cover and book.best_bid + 1 < book.best_ask:
            front_buy = max(front_buy, book.best_bid + 1)
        ...
        if sell_markout < float(self.p["HARD_BAD_MARKOUT"]) and book.imbalance >= 0.0 and ask_level >= 2:
            allow_ask = False
        if baseline_active and max(bid_level, ask_level) <= 1:
            allow_bid = True
            allow_ask = True
        ...
        if short_cover:
            buy_front_sz += int(self.p["SHORT_COVER_FRONT_BONUS"])
            sell_front_sz = max(2, sell_front_sz - 1)
        if baseline_active:
            buy_front_sz = max(buy_front_sz, int(self.p["BASELINE_FRONT_MIN"]))
            sell_front_sz = max(sell_front_sz, int(self.p["BASELINE_FRONT_MIN"]))
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
