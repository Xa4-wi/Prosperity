...
PORTFOLIO_RISK_PARAMS = {
    "ENABLED": True,
    "EQUITY_SHOCK_ALPHA": 0.10,
    "DRAWDOWN_CAUTION": 140.0,
    "DRAWDOWN_DEFENSE": 260.0,
    "DRAWDOWN_KILL": 420.0,
    "SHOCK_CAUTION": 70.0,
    "SHOCK_DEFENSE": 120.0,
    "CAUTION_SCALE": 0.88,
    "DEFENSE_SCALE": 0.62,
    "DOMINANCE_SHARE": 0.72,
    "DOMINANCE_SCALE": 0.72,
    "LATE_ASH_PROGRESS": 0.92,
    "LATE_ASH_POS": 18,
    "LATE_ASH_SCALE": 0.78,
    "LATE_ASH_KILL_PROGRESS": 0.975,
    "LATE_ASH_KILL_POS": 28,
}
...
class Trader:
    ...
    def _mark_price(self, product: str, state: TradingState, risk_state: dict) -> float:
        book = Book(state.order_depths.get(product))
        last_mark = risk_state.get(f"last_mark_{product}")
        default_mark = 10000.0 if product == "ASH_COATED_OSMIUM" else 13000.0
        if book.valid:
            mark = book.mid
        elif book.has_bid or book.has_ask:
            mark = float(book.best_bid if book.has_bid else book.best_ask)
        elif last_mark is not None:
            mark = float(last_mark)
        else:
            mark = default_mark
        risk_state[f"last_mark_{product}"] = round(mark, 4)
        return mark

    def _update_portfolio_risk(self, state: TradingState, memory: dict) -> Tuple[dict, dict]:
        risk_state = memory.get("PORTFOLIO_RISK", {})
        if not isinstance(risk_state, dict):
            risk_state = {}

        if not PORTFOLIO_RISK_PARAMS.get("ENABLED", True):
            risk_state["mode"] = "normal"
            memory["PORTFOLIO_RISK"] = risk_state
            return risk_state, memory

        timestamp = int(getattr(state, "timestamp", 0))
        last_ts = int(risk_state.get("last_ts", -1))
        if last_ts >= 0 and timestamp < last_ts:
            risk_state = {}

        total_cash = 0.0
        marks: Dict[str, float] = {}
        gross_by_product: Dict[str, float] = {}
        for product in ("ASH_COATED_OSMIUM", "INTARIAN_PEPPER_ROOT"):
            last_seen_trade_ts = int(risk_state.get(f"last_seen_trade_ts_{product}", -1))
            cash = float(risk_state.get(f"cash_{product}", 0.0))
            ...
            risk_state[f"cash_{product}"] = round(cash, 4)
            risk_state[f"last_seen_trade_ts_{product}"] = int(latest_seen)
            total_cash += cash

            mark = self._mark_price(product, state, risk_state)
            marks[product] = mark
            pos = int(state.position.get(product, 0))
            gross_by_product[product] = abs(pos * mark)

        equity = total_cash + sum(int(state.position.get(product, 0)) * mark for product, mark in marks.items())
        prev_equity = risk_state.get("equity")
        shock_ema = float(risk_state.get("equity_shock_ema", 0.0))
        if prev_equity is not None:
            shock_ema = ema(shock_ema, abs(equity - float(prev_equity)), float(PORTFOLIO_RISK_PARAMS["EQUITY_SHOCK_ALPHA"]))
        high_water = max(float(risk_state.get("equity_high", equity)), equity)
        drawdown = high_water - equity

        mode = "normal"
        if (
            drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_KILL"])
        ):
            mode = "kill"
        elif (
            drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_DEFENSE"])
            or shock_ema >= float(PORTFOLIO_RISK_PARAMS["SHOCK_DEFENSE"])
        ):
            mode = "defense"
        elif (
            drawdown >= float(PORTFOLIO_RISK_PARAMS["DRAWDOWN_CAUTION"])
            or shock_ema >= float(PORTFOLIO_RISK_PARAMS["SHOCK_CAUTION"])
        ):
            mode = "caution"

        product_scales = {
            "ASH_COATED_OSMIUM": 1.0,
            "INTARIAN_PEPPER_ROOT": 1.0,
        }
        gross_total = sum(gross_by_product.values())
        if gross_total > 0.0:
            dominant_product = max(gross_by_product, key=gross_by_product.get)
            dominant_share = gross_by_product[dominant_product] / gross_total
            if dominant_share >= float(PORTFOLIO_RISK_PARAMS["DOMINANCE_SHARE"]) and mode in ("caution", "defense", "kill"):
                product_scales[dominant_product] *= float(PORTFOLIO_RISK_PARAMS["DOMINANCE_SCALE"])

        progress = clamp(timestamp / 999900.0 if timestamp > 0 else 0.0, 0.0, 1.0)
        ash_pos = abs(int(state.position.get("ASH_COATED_OSMIUM", 0)))
        if progress >= float(PORTFOLIO_RISK_PARAMS["LATE_ASH_PROGRESS"]) and ash_pos >= int(PORTFOLIO_RISK_PARAMS["LATE_ASH_POS"]):
            product_scales["ASH_COATED_OSMIUM"] *= float(PORTFOLIO_RISK_PARAMS["LATE_ASH_SCALE"])
        if progress >= float(PORTFOLIO_RISK_PARAMS["LATE_ASH_KILL_PROGRESS"]) and ash_pos >= int(PORTFOLIO_RISK_PARAMS["LATE_ASH_KILL_POS"]):
            if mode == "normal":
                mode = "defense"
            elif mode == "caution":
                mode = "defense"
        ...
        risk_state["equity"] = round(equity, 4)
        risk_state["equity_high"] = round(high_water, 4)
        risk_state["drawdown"] = round(drawdown, 4)
        risk_state["equity_shock_ema"] = round(shock_ema, 4)
        risk_state["mode"] = mode
        risk_state["product_scales"] = {k: round(v, 4) for k, v in product_scales.items()}
        risk_state["last_ts"] = timestamp
        memory["PORTFOLIO_RISK"] = risk_state
        return risk_state, memory

    def _scaled_qty(self, qty: int, factor: float) -> int:
        if factor >= 0.999:
            return int(qty)
        scaled = int(round(abs(qty) * factor))
        if scaled <= 0:
            return 0
        return scaled if qty > 0 else -scaled

    def _apply_portfolio_overlay(
        self,
        result: Dict[str, List[Order]],
        state: TradingState,
        risk_state: dict,
    ) -> Dict[str, List[Order]]:
        mode = str(risk_state.get("mode", "normal"))
        product_scales = risk_state.get("product_scales", {})
        if mode == "normal" and all(abs(float(product_scales.get(p, 1.0)) - 1.0) < 1e-9 for p in result):
            return result

        base_scale = 1.0
        if mode == "caution":
            base_scale = float(PORTFOLIO_RISK_PARAMS["CAUTION_SCALE"])
        elif mode == "defense":
            base_scale = float(PORTFOLIO_RISK_PARAMS["DEFENSE_SCALE"])
        elif mode == "kill":
            base_scale = 0.0
        ...
        for product, orders in result.items():
            book = Book(state.order_depths.get(product))
            projected = int(state.position.get(product, 0))
            scale = base_scale * float(product_scales.get(product, 1.0))
            kept: List[Order] = []
            for order in orders:
                qty = int(order.quantity)
                if qty == 0:
                    continue
                new_pos = projected + qty
                increases_abs = abs(new_pos) > abs(projected)
                reduces_abs = abs(new_pos) < abs(projected)
                aggressive = (
                    qty > 0 and book.has_ask and int(order.price) >= int(book.best_ask)
                ) or (
                    qty < 0 and book.has_bid and int(order.price) <= int(book.best_bid)
                )

                keep = True
                out_qty = qty
                if mode == "caution":
                    if aggressive and increases_abs:
                        keep = False
                    else:
                        out_qty = self._scaled_qty(qty, scale if increases_abs else max(scale, 0.95))
                elif mode == "defense":
                    if increases_abs:
                        keep = False if aggressive or scale < 0.9 else True
                        out_qty = self._scaled_qty(qty, min(scale, 0.55))
                    else:
                        out_qty = self._scaled_qty(qty, max(scale, 0.85))
                elif mode == "kill":
                    if not reduces_abs:
                        keep = False
                    else:
                        out_qty = self._scaled_qty(qty, 1.0)

                if not keep or out_qty == 0:
                    continue
                kept.append(Order(order.symbol, int(order.price), int(out_qty)))
                projected += int(out_qty)
            filtered[product] = kept
        return filtered

    def run(self, state: TradingState):
        memory = self._load_memory(state.traderData if hasattr(state, "traderData") else "")
        risk_state, memory = self._update_portfolio_risk(state, memory)
        result: Dict[str, List[Order]] = {}
        ...
        result = self._apply_portfolio_overlay(result, state, risk_state)
