...
class AshLocalFairTrader:
    ...
    MICRO_WEIGHT = 0.20
    WALL_BLEND = 0.25
    LOCAL_IMBALANCE_BIAS = 0.10
    DEEP_LEVELS = 3
    DEEP_MICRO_WEIGHT = 0.45
    DEEP_IMBALANCE_WEIGHT = 0.28
    DEPTH_IMPACT_SCALE = 26.0
    DEPTH_FLOOR = 8.0
    INVENTORY_SKEW = 0.08
    SOFT_LIMIT = 60
    TAKE_EDGE = 1.00
    ...
    MICRO_DOMINANCE_FLOOR = 0.20
    STABLE_MAGNET_EDGE = 1.00
    STABLE_STRONG_EDGE = 1.25
    AGREE_SIGNAL_BONUS = 0.45
    MAGNET_SIGNAL_BONUS = 0.16
    TRADE_CONFIRM_BONUS = 0.18
    FAST_SIGNAL_CLIP = 2.00
    FAST_TAKE_WEIGHT = 0.78
    FAST_QUOTE_WEIGHT = 0.38
    ...
    ACCESS_SIGNAL_MIN = 0.22
    ACCESS_CONVICTION_MIN = 0.40
    ACCESS_MAGNET_MIN = 0.65
    ACCESS_SWEEP_LEVELS = 2
    ACCESS_SWEEP_EDGE_STEP = 0.15
    ACCESS_TAKE_RELIEF = 0.10
    ACCESS_JOIN_BONUS = 0.18
    ...
    def _stable_mid(self, book: Book) -> float:
        popular_mid = book.stable_mid(self.DEEP_LEVELS)
        bid_levels = book.buy_levels[: self.DEEP_LEVELS]
        ask_levels = book.sell_levels[: self.DEEP_LEVELS]
        if not bid_levels or not ask_levels:
            return popular_mid

        wall_bid = max(bid_levels, key=lambda level: (level[1], level[0]))[0]
        wall_ask = min(ask_levels, key=lambda level: (-level[1], level[0]))[0]
        wall_mid = (wall_bid + wall_ask) / 2.0
        return (1.0 - self.WALL_BLEND) * popular_mid + self.WALL_BLEND * wall_mid

    def _deep_micro_signal(self, book: Book) -> Tuple[float, float]:
        bid_levels = book.buy_levels[: self.DEEP_LEVELS]
        ask_levels = book.sell_levels[: self.DEEP_LEVELS]
        bid_vol = sum(volume for _, volume in bid_levels)
        ask_vol = sum(volume for _, volume in ask_levels)
        if bid_vol <= 0 or ask_vol <= 0:
            return 0.0, 0.0

        bid_vwap = sum(price * volume for price, volume in bid_levels) / bid_vol
        ask_vwap = sum(price * volume for price, volume in ask_levels) / ask_vol
    ...
    def _signal_components(
        ...
        half_spread = clamp(book.spread / 2.0, 6.0, 10.0)
        depth = max(self.DEPTH_FLOOR, float(book.best_bid_vol + book.best_ask_vol))
        depth_beta = self.DEPTH_IMPACT_SCALE / depth
        imbalance_signal = (
            half_spread * book.imbalance
            + depth_beta * book.imbalance
            + self.DEEP_IMBALANCE_WEIGHT * half_spread * deep_imbalance
        )
        ...
        local_fair = (
            ...
            + self.LOCAL_IMBALANCE_BIAS * imbalance_signal
        )

        tight_book = book.spread <= self.TIGHT_SPREAD_MAX
        fast_signal = (
            0.90 * imbalance_signal
            + 0.35 * micro_gap
            + self.DEEP_MICRO_WEIGHT * deep_micro_gap
        )
        if tight_book and abs(micro_gap) >= self.MICRO_DOMINANCE_FLOOR:
            fast_signal = (
                0.90 * imbalance_signal
                + 0.70 * micro_gap
                + 0.75 * deep_micro_gap
        ...
        if agree:
            fast_signal += self.AGREE_SIGNAL_BONUS * stable_gap
            conviction += 0.35
            if abs(stable_gap) >= self.STABLE_MAGNET_EDGE:
                fast_signal += self.MAGNET_SIGNAL_BONUS * stable_gap
                conviction += 0.25
            if abs(stable_gap) >= self.STABLE_STRONG_EDGE:
                conviction += 0.15
        elif disagree:
            conviction += 0.10
        elif abs(stable_gap) >= self.STABLE_MAGNET_EDGE:
            fast_signal += 0.18 * stable_gap
            conviction += 0.15

        trade_confirm = self._trade_confirmation(state, book)
        if trade_confirm != 0.0 and sign(trade_confirm, 0.05) == imbalance_sign:
            fast_signal += self.TRADE_CONFIRM_BONUS * trade_confirm
            conviction += 0.15

        clipped = clamp(fast_signal, -self.FAST_SIGNAL_CLIP, self.FAST_SIGNAL_CLIP)
        take_alpha = self.FAST_TAKE_WEIGHT * clipped
        quote_alpha = self.FAST_QUOTE_WEIGHT * clipped
        return (
            local_fair,
            take_alpha,
            quote_alpha,
            clamp(conviction, 0.0, 1.10),
            stable_gap,
            trade_confirm,
        )
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        local_fair, take_alpha, quote_alpha, signal_conviction, stable_gap, trade_confirm = (
            self._signal_components(state, book)
        )
        ...
        if signal_conviction > 0.0:
            if quote_alpha > 0.0:
                buy_need = max(0.75, buy_need - 0.12 * signal_conviction)
            elif quote_alpha < 0.0:
                sell_need = max(0.75, sell_need - 0.12 * signal_conviction)
        ...
        access_buy = (
            bid_level == 0
            and ask_level <= 1
            and abs(mgr.projected()) <= max(10, self.SOFT_LIMIT - 10)
            and quote_alpha >= self.ACCESS_SIGNAL_MIN
            and signal_conviction >= self.ACCESS_CONVICTION_MIN
            and stable_gap >= self.ACCESS_MAGNET_MIN
        ...
        if access_buy and can_take_buy:
            remaining = min(mgr.buy_cap, 12)
            base_need = max(0.95, buy_need - self.ACCESS_TAKE_RELIEF)
            for level_ix, (price, volume) in enumerate(book.sell_levels[: self.ACCESS_SWEEP_LEVELS]):
                if remaining <= 0:
                    break
                level_edge = take_fair - price + min(0.0, buy_markout)
                level_need = base_need + level_ix * self.ACCESS_SWEEP_EDGE_STEP
                if level_edge < level_need:
                    break
                clip = min(int(volume), remaining)
                if clip <= 0:
                    continue
                mgr.buy(price, clip)
                remaining -= clip
        ...
        if access_sell and can_take_sell:
            remaining = min(mgr.sell_cap, 12)
            base_need = max(0.95, sell_need - self.ACCESS_TAKE_RELIEF)
            for level_ix, (price, volume) in enumerate(book.buy_levels[: self.ACCESS_SWEEP_LEVELS]):
                if remaining <= 0:
                    break
                level_edge = price - take_fair + min(0.0, sell_markout)
                level_need = base_need + level_ix * self.ACCESS_SWEEP_EDGE_STEP
                if level_edge < level_need:
                    break
                clip = min(int(volume), remaining)
        ...
        if (
            can_take_buy
            and not access_bought
            and mgr.projected() < self.LIMIT
            and book.best_ask <= take_fair - buy_need
        ):
        ...
        if (
            ...
            and not access_sold
            and mgr.projected() > -self.LIMIT
            and book.best_bid >= take_fair + sell_need
        ):
        ...
        if signal_conviction > 0.0:
            if quote_alpha > 0.0:
                buy_edge -= 0.10 * signal_conviction
            elif quote_alpha < 0.0:
                sell_edge -= 0.10 * signal_conviction
        if access_buy:
            buy_edge -= 0.08
            buy_join_edge += self.ACCESS_JOIN_BONUS
        elif access_sell:
            sell_edge -= 0.08
            sell_join_edge += self.ACCESS_JOIN_BONUS
        ...
        if signal_conviction >= 0.95:
            if quote_alpha > 0.0:
                ...
                sell_size += 1
        if access_buy:
            buy_size += self.ACCESS_FRONT_BONUS
        elif access_sell:
            sell_size += self.ACCESS_FRONT_BONUS
        if buy_markout < self.SOFT_BAD_MARKOUT and book.imbalance <= 0.0:
        ...
        if allow_bid and 0 < front_buy < book.best_ask:
            ...
            if back_buy < book.best_ask and mgr.projected() < self.SOFT_LIMIT + 12:
                mgr.buy(back_buy, self.BACK_SIZE + (self.ACCESS_BACK_BONUS if access_buy else 0))
        if allow_ask and front_sell > book.best_bid:
            ...
            if back_sell > book.best_bid and mgr.projected() > -(self.SOFT_LIMIT + 12):
                mgr.sell(back_sell, self.BACK_SIZE + (self.ACCESS_BACK_BONUS if access_sell else 0))
        ...
        astate["last_conviction"] = round(signal_conviction, 4)
        astate["last_access_side"] = "buy" if access_buy else ("sell" if access_sell else "")
        astate["last_position"] = int(position)
        astate["prev_mid"] = float(book.mid)
        memory["ASH"] = astate
        return mgr.orders, memory
