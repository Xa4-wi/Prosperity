...
class AshCoatedOsmiumTrader:
    ...
    def _markout_bucket(
        ...
        book: Book,
        stable_gap: float,
        book_imbalance_signal: float,
        side_toxic_level: int,
    ) -> str:
        spread_regime = "tight" if book.spread_val <= float(self.p["TIGHT_SPREAD_MAX"]) else "wide"
        stable_sign = sign(stable_gap, 0.20)
        imbalance_sign = sign(book_imbalance_signal, 0.12)
        if stable_sign != 0 and stable_sign == imbalance_sign:
            ...
            alignment = "neutral"
        toxicity = "safe" if side_toxic_level == 0 else "toxic"
        return f"{side}:{spread_regime}:{alignment}:{toxicity}"
    ...
    def _infer_markout_quality(
        ...
        buy_buckets = ash_state.get("buy_markout_buckets", {})
        if not isinstance(buy_buckets, dict):
            buy_buckets = {}
        sell_buckets = ash_state.get("sell_markout_buckets", {})
        if not isinstance(sell_buckets, dict):
            sell_buckets = {}
        if prev_mid is not None:
            ...
            if delta > 0:
                ...
                prev_buy_bucket = ash_state.get("last_buy_bucket")
                if isinstance(prev_buy_bucket, str) and prev_buy_bucket:
                    prev_bucket_val = float(buy_buckets.get(prev_buy_bucket, buy_markout))
                    buy_buckets[prev_buy_bucket] = round(
                        ema(prev_bucket_val, move, float(self.p["MARKOUT_BUCKET_ALPHA"])),
                        4,
                    )
            elif delta < 0:
                sell_markout = ema(sell_markout, -move, float(self.p["MARKOUT_ALPHA"]))
                prev_sell_bucket = ash_state.get("last_sell_bucket")
                if isinstance(prev_sell_bucket, str) and prev_sell_bucket:
                    prev_bucket_val = float(sell_buckets.get(prev_sell_bucket, sell_markout))
                    sell_buckets[prev_sell_bucket] = round(
                        ema(prev_bucket_val, -move, float(self.p["MARKOUT_BUCKET_ALPHA"])),
                        4,
                    )
        ash_state["buy_markout_buckets"] = buy_buckets
        ash_state["sell_markout_buckets"] = sell_buckets
        current_buy_markout = float(buy_buckets.get(buy_bucket, buy_markout))
        current_sell_markout = float(sell_buckets.get(sell_bucket, sell_markout))
        return current_buy_markout, current_sell_markout
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_bucket = self._markout_bucket("buy", book, stable_gap, book.imbalance, raw_bid_level)
        sell_bucket = self._markout_bucket("sell", book, stable_gap, book.imbalance, raw_ask_level)
        buy_markout, sell_markout = self._infer_markout_quality(
            book,
            position,
            ash_state,
            buy_bucket,
        ...
        ash_state["last_buy_bucket"] = buy_bucket
        ash_state["last_sell_bucket"] = sell_bucket
        ash_state["last_fill_ts"] = int(last_fill_ts)
        ash_state["no_fill_ticks"] = int(no_fill_ticks)
        ash_state["last_buy_fill_ts"] = int(last_buy_fill_ts)
        ash_state["last_sell_fill_ts"] = int(last_sell_fill_ts)
