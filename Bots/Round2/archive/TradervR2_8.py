...
DEFAULT_ASH_PARAMS = {
    ...
    "ACCESS_JOIN_EDGE_BONUS": 0.20,
    "ACCESS_FRONT_BONUS": 2,
    "ACCESS_BACK_BONUS": 1,
    "ACCESS_TAKE_RELIEF": 0.10,
    "ACCESS_SWEEP_LEVELS": 3,
    "ACCESS_SWEEP_EDGE_STEP": 0.15,
    "ACCESS_SIGNAL_MIN": 0.25,
    "ACCESS_CONVICTION_MIN": 0.45,
...
DEFAULT_IPR_PARAMS = {
    ...
    "TARGET_PROGRESS_END": 0.60,
    "TARGET_PROGRESS_MIN_FRAC": 0.18,
    "TARGET_EDGE_WEIGHT": 0.72,
    "TARGET_MID_DECAY_START": 0.45,
    "TARGET_MID_DECAY_STRENGTH": 18.0,
    "TARGET_LATE_NEUTRAL_START": 0.84,
    "TARGET_LATE_NEUTRAL_LEVEL": 8.0,
    "EXIT_ZSCORE": 0.90,
    "EXIT_SELL_RELIEF": 0.40,
    "EXIT_FRONT_IMPROVE": 1,
    "LATE_NEUTRAL_SELL_RELIEF": 0.25,
    "VACUUM_GAP": 4,
    "VACUUM_SIZE": 3,
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        target += float(self.p["EDGE_TARGET_SCALE"]) * float(self.p["TARGET_EDGE_WEIGHT"]) * edge
        ...
        if progress > 0.85 and zscore > 1.25:
            target -= 8.0
        if progress >= float(self.p["TARGET_MID_DECAY_START"]):
            decay_frac = clamp(
                (progress - float(self.p["TARGET_MID_DECAY_START"]))
                / (1.0 - float(self.p["TARGET_MID_DECAY_START"])),
                0.0,
                1.0,
            )
            target -= float(self.p["TARGET_MID_DECAY_STRENGTH"]) * decay_frac
        if progress >= float(self.p["TARGET_LATE_NEUTRAL_START"]):
            neutral_frac = clamp(
                (progress - float(self.p["TARGET_LATE_NEUTRAL_START"]))
                / (1.0 - float(self.p["TARGET_LATE_NEUTRAL_START"])),
                0.0,
                1.0,
            )
            target = (1.0 - neutral_frac) * target + neutral_frac * float(self.p["TARGET_LATE_NEUTRAL_LEVEL"])

        progress_cap = float(self.p["MAX_LONG_TARGET"]) * (
            float(self.p["TARGET_PROGRESS_MIN_FRAC"])
            + (1.0 - float(self.p["TARGET_PROGRESS_MIN_FRAC"]))
            * clamp(progress / float(self.p["TARGET_PROGRESS_END"]), 0.0, 1.0)
        )
        hard_long_cap = int(min(float(self.p["MAX_LONG_TARGET"]), progress_cap))
        target_int = int(clamp(target, -int(self.p["MAX_SHORT_TARGET"]), hard_long_cap))
        reservation = fair - (mgr.projected() - target_int) * float(self.p["INVENTORY_SKEW"])
        over_target = mgr.projected() > target_int
        ...
        if over_target:
            buy_te += 0.80
        if over_target and progress >= float(self.p["TARGET_MID_DECAY_START"]):
            sell_te -= float(self.p["EXIT_SELL_RELIEF"])
        if strong_up_extension and pos > max(int(float(self.p["TARGET_LATE_NEUTRAL_LEVEL"])), target_int - 4):
            sell_te -= float(self.p["EXIT_SELL_RELIEF"])
        if progress >= float(self.p["TARGET_LATE_NEUTRAL_START"]) and pos > target_int:
            sell_te -= float(self.p["LATE_NEUTRAL_SELL_RELIEF"])
        ...
        if book.best_bid - reservation >= sell_te and mgr.sell_cap > 0:
            pos = mgr.projected()
            qty = min(book.best_bid_vol, 16, max(0, pos - max(0, target_int - 4)))
            if bullish and pos < max(26, target_int - 6):
                qty = 0
            elif bullish and pos > 0:
                qty = min(qty, 4)
            if strong_up_extension and pos > max(int(float(self.p["TARGET_LATE_NEUTRAL_LEVEL"])), target_int - 2):
                qty = max(qty, min(book.best_bid_vol, 8, pos - max(0, target_int - 2)))
