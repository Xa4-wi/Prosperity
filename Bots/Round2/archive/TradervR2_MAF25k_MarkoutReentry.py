...
DEFAULT_ASH_PARAMS = {
    ...
    "MARKOUT_EDGE_PENALTY": 0.18,
    "MARKOUT_SIZE_PENALTY": 0.12,
    "SIDE_REENTRY_BARS": 8,
    "NEUTRAL_DRIP_BARS": 10,
    "DEEP_SWEEP_MAX_LEVELS": 2,
    "DEEP_SWEEP_MIN_EDGE": 1.80,
    "DEEP_SWEEP_SIZE": 3,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _smoothed_toxicity_levels(
        ...
        if raw_bid_level == 0 and raw_ask_level == 0 and quiet_non_toxic_bars >= 6:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        # Own-trade timers for side-specific reentry.
        ...
        own_ash_trades = state.own_trades.get("ASH_COATED_OSMIUM", []) if hasattr(state, "own_trades") else []
        if own_ash_trades:
            for tr in own_ash_trades:
                tr_ts = int(getattr(tr, "timestamp", timestamp))
        ...
        bars_since_fill = max(0, (timestamp - last_fill_ts) // 100) if last_fill_ts >= 0 else 0
        bars_since_buy_fill = (
            max(0, (timestamp - last_buy_fill_ts) // 100)
            if last_buy_fill_ts >= 0
            else bars_since_fill
        )
        ...
        quiet_non_toxic_bars = bars_since_fill if raw_bid_level == 0 and raw_ask_level == 0 else 0
        bid_level, ask_level, memory = self._smoothed_toxicity_levels(
            ...
            memory,
            timestamp,
            quiet_non_toxic_bars=quiet_non_toxic_bars,
        )
        attack = clamp(
        ...
        # Markout-aware net edge: apply softly to taking and quoting, but do not let it dominate.
        if buy_markout < float(self.p["HARD_BAD_MARKOUT"]) and book.imbalance <= 0.0:
        ...
        recent_vacuum_recovery = prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks <= 2
        stale_side_base = (
            mode == "normal"
            and abs(pos) <= max(8, soft - 8)
            and book.spread_val <= 16
            and (prev_vacuum_side not in ("bid_only", "ask_only") or recent_vacuum_recovery)
        )
        side_reentry_bars = int(self.p["SIDE_REENTRY_BARS"])
        buy_reactivation = stale_side_base and quote_signal > 0.10 and bid_level <= 1 and bars_since_buy_fill >= side_reentry_bars
        sell_reactivation = stale_side_base and quote_signal < -0.10 and ask_level <= 1 and bars_since_sell_fill >= side_reentry_bars
        neutral_drip = (
            stale_side_base
            and max(bid_level, ask_level) == 0
            and abs(quote_signal) < 0.10
            and bars_since_fill >= int(self.p["NEUTRAL_DRIP_BARS"])
            and abs(pos) <= 8
        )
        micro_nibble = False
        if buy_reactivation and abs(quote_signal) >= 0.28 and signal_conviction >= 0.35:
            micro_nibble = True
            if can_take_buy and buy_edge >= max(0.80, buy_need - 0.25):
                mgr.buy(book.best_ask, min(book.best_ask_vol, 2))
        elif sell_reactivation and abs(quote_signal) >= 0.28 and signal_conviction >= 0.35:
            micro_nibble = True
        ...
        # Selective deeper sweep only in very high-quality states.
        if mode == "normal" and max(bid_level, ask_level) == 0 and signal_conviction >= 0.85:
            max_levels = int(self.p["DEEP_SWEEP_MAX_LEVELS"])
            deep_sz = int(self.p["DEEP_SWEEP_SIZE"])
            deep_min_edge = float(self.p["DEEP_SWEEP_MIN_EDGE"])
            if take_signal > 0.0 and mgr.buy_cap > 0:
                for ask_price, ask_vol in book.sell_levels[1:1 + max_levels]:
                    if (take_reservation - ask_price) >= deep_min_edge:
                        qty = min(deep_sz, ask_vol, mgr.buy_cap)
                        mgr.buy(ask_price, qty)
                    else:
                        break
            elif take_signal < 0.0 and mgr.sell_cap > 0:
                for bid_price, bid_vol in book.buy_levels[1:1 + max_levels]:
                    if (bid_price - take_reservation) >= deep_min_edge:
                        qty = min(deep_sz, bid_vol, mgr.sell_cap)
                        mgr.sell(bid_price, qty)
                    else:
                        break
        ...
        # Markout-aware passive penalty on the side that recently underperformed.
        ...
        if buy_reactivation:
            extra = min(0.30, 0.05 * max(0, bars_since_buy_fill - side_reentry_bars))
            buy_qe -= 0.15 + extra
            if micro_nibble:
                buy_qe -= 0.05
        elif sell_reactivation:
            extra = min(0.30, 0.05 * max(0, bars_since_sell_fill - side_reentry_bars))
            sell_qe -= 0.15 + extra
            if micro_nibble:
                sell_qe -= 0.05
        elif neutral_drip:
        ...
        if buy_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance <= 0.0:
            buy_mult *= max(0.60, 1.0 - float(self.p["MARKOUT_SIZE_PENALTY"]))
        if sell_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance >= 0.0:
            sell_mult *= max(0.60, 1.0 - float(self.p["MARKOUT_SIZE_PENALTY"]))
        buy_mult *= size_bonus
        ...
        if signal_conviction >= 0.95:
            if quote_signal > 0.0:
                ...
                sell_front_sz += 1
        if buy_reactivation:
            buy_front_sz += 1 + int(bars_since_buy_fill >= side_reentry_bars + 4)
        elif sell_reactivation:
            sell_front_sz += 1 + int(bars_since_sell_fill >= side_reentry_bars + 4)
        elif neutral_drip:
            buy_front_sz = max(buy_front_sz, 2)
            sell_front_sz = max(sell_front_sz, 2)
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
        elif pos < -6 and ask_level >= 1:
            buy_front_sz += 1
