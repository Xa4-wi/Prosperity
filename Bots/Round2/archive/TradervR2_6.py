...
DEFAULT_ASH_PARAMS = {
    ...
    "THROUGHPUT_CONVICTION_MIN": 0.75,
    "THROUGHPUT_CONFIRM_MIN": 0.25,
    "THROUGHPUT_JOIN_BONUS": 0.15,
    "THROUGHPUT_FRONT_BONUS": 2,
    "THROUGHPUT_BACK_BONUS": 1,
    "THROUGHPUT_TAKE_RELIEF": 0.12,
    "THROUGHPUT_EXTRA_SWEEP_LEVELS": 1,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        strong_align_buy = (
            access_buy
            and signal_conviction >= float(self.p["THROUGHPUT_CONVICTION_MIN"])
            and stable_gap >= float(self.p["STABLE_STRONG_EDGE"])
            and trade_confirm >= float(self.p["THROUGHPUT_CONFIRM_MIN"])
        )
        strong_align_sell = (
            access_sell
            and signal_conviction >= float(self.p["THROUGHPUT_CONVICTION_MIN"])
        ...
        if access_buy and can_take_buy:
            ...
            base_need = max(
                float(self.p["TAKE_L1_EDGE"]),
                buy_need
                - float(self.p["ACCESS_TAKE_RELIEF"])
                - (float(self.p["THROUGHPUT_TAKE_RELIEF"]) if strong_align_buy else 0.0),
            )
            sweep_levels = int(self.p["ACCESS_SWEEP_LEVELS"]) + (
                int(self.p["THROUGHPUT_EXTRA_SWEEP_LEVELS"]) if strong_align_buy else 0
            )
            for level_ix, (price, vol) in enumerate(book.sell_levels[:sweep_levels]):
                if remaining <= 0:
                    break
        ...
        if access_sell and can_take_sell:
            ...
            base_need = max(
                ...
                sell_need
                - float(self.p["ACCESS_TAKE_RELIEF"])
                - (float(self.p["THROUGHPUT_TAKE_RELIEF"]) if strong_align_sell else 0.0),
            )
            sweep_levels = int(self.p["ACCESS_SWEEP_LEVELS"]) + (
                int(self.p["THROUGHPUT_EXTRA_SWEEP_LEVELS"]) if strong_align_sell else 0
            )
            for level_ix, (price, vol) in enumerate(book.buy_levels[:sweep_levels]):
                if remaining <= 0:
                    break
        ...
        if strong_align_buy or strong_align_sell:
            join_edge += float(self.p["THROUGHPUT_JOIN_BONUS"])
        ...
        if access_buy and book.best_bid + 1 < book.best_ask:
            ...
            front_sell = min(front_sell, book.best_ask - 1)
        if strong_align_buy and book.best_bid + 1 < book.best_ask:
            front_buy = max(front_buy, book.best_bid + 1)
        elif strong_align_sell and book.best_ask - 1 > book.best_bid:
            front_sell = min(front_sell, book.best_ask - 1)
        ...
        if strong_align_buy:
            buy_front_sz += int(self.p["THROUGHPUT_FRONT_BONUS"])
            buy_back_sz += int(self.p["THROUGHPUT_BACK_BONUS"])
            sell_front_sz = max(2, sell_front_sz - 1)
        elif strong_align_sell:
            sell_front_sz += int(self.p["THROUGHPUT_FRONT_BONUS"])
            sell_back_sz += int(self.p["THROUGHPUT_BACK_BONUS"])
