...
ROUND2_MAF_BID = 15000


DEFAULT_ASH_PARAMS = {
    # Feature toggles [mixed: core live + research]
    "ENABLED": True,
    "FAIR_STYLE": "median_low_health",
    ...
    # Fair model and anchor shaping [core live]
    ...
    # Inventory and regime control [core live]
    "SOFT_LIMIT": 62,
    "INVENTORY_SKEW": 0.07,
    ...
    # Core quoting and taking [core live]
    "BASE_EDGE": -0.55,
    "MIN_QUOTE_EDGE": 1.95,
    "JOIN_EDGE": 1.45,
    "FRONT_SIZE": 17,
    ...
    # Signal thresholds and conviction shaping [core live]
    ...
    # Passive fill quality [robustness live]
    "MARKOUT_ALPHA": 0.18,
    ...
    # Access / strong-state expansion [selective live]
    ...
    # Vacuum and refill handling [core live]
    "VACUUM_GAP": 4,
    "VACUUM_SIZE": 2,
    "VACUUM_VISIBLE_EDGE": 2.5,
    "VACUUM_FAIR_BLEND": 0.65,
    "VACUUM_RECOVERY_STABLE_BARS": 3,
    "REFILL_TAKE_RELIEF": 0.10,
    "REFILL_EDGE_BONUS": 0.10,
    "REFILL_JOIN_BONUS": 0.10,
    "REFILL_FRONT_BONUS": 1,
    "REFILL_STICKY_TICKS": 2,

    # Noisy-book and robustness overlays [robustness live]
    "NOISY_TOUCH_GAP": 1.20,
    "NOISY_STABLE_EMA_GAP": 0.90,
    "NOISY_JOIN_PENALTY": 0.25,
    "NOISY_SIGNAL_DAMP": 0.82,
    "NOISY_EDGE_PENALTY": 0.10,
    "ALTMM_GEOMETRY_BLEND": 0.22,
    "ALTMM_GEOMETRY_HEALTH_MAX": 0.72,
    "ALTMM_VAR_ALPHA": 0.10,

    # Book-health controller [robustness live]
    ...
    "BOOK_HEALTH_EDGE_PENALTY_LOW": 0.1,
    "BOOK_HEALTH_EDGE_PENALTY_MED": 0.04,
    "BOOK_HEALTH_JOIN_PENALTY_LOW": 0.22,
    "BOOK_HEALTH_JOIN_PENALTY_MED": 0.1,
    "BOOK_HEALTH_SIZE_MULT_LOW": 0.9,
    "BOOK_HEALTH_SIZE_MULT_MED": 0.96,

    # Side-specific reentry [robustness live]
    "STARVATION_BARS": 6,
    ...
    # Legacy terminal recycler path [legacy / research-only]
    ...
    # Rebuilt Ash risk context [robustness live]
    "RISK_VOL_NORMAL": 1.65,
    "RISK_FAIR_DISAGREE_SOFT": 1.35,
    "RISK_FAIR_DISAGREE_HARD": 2.80,
    "RISK_DRAWDOWN_SOFT": 180.0,
    "RISK_DRAWDOWN_HARD": 340.0,
    "RISK_DRAWDOWN_ALPHA": 0.10,
    "RISK_DRAWDOWN_EDGE_SOFT": 0.03,
    "RISK_DRAWDOWN_EDGE_HARD": 0.08,
    "RISK_TERMINAL_START": 0.78,
    "RISK_TERMINAL_MID": 0.90,
    "RISK_TERMINAL_LATE": 0.97,
    "RISK_TERMINAL_STEP1": 0.10,
    "RISK_TERMINAL_STEP2": 0.22,
    "RISK_TERMINAL_STEP3": 0.45,
    "RISK_CONF_ATTACK": 0.80,
    "RISK_CONF_PRESS": 0.62,
    "RISK_CONF_BALANCED": 0.42,
}


DEFAULT_IPR_PARAMS = {
    # Feature toggles [minimal live]
    "ENABLED": True,

    # Core carry model [core live]
    ...
    # Residual and spread memory [core live]
    "RESIDUAL_ALPHA": 0.10,
    "SPREAD_ALPHA": 0.08,
    ...
    # Inventory and reservation [core live]
    "SOFT_LIMIT": 52,
    "INVENTORY_SKEW": 0.078,

    # Core execution [core live]
    ...
    # Cheap accumulation and early schedule [core live]
    ...
    # Rich / cheap residual response [core live]
    ...
    # Exit and late-session trimming [core live]
    "EXIT_ZSCORE": 1.05,
    ...
    # One-sided-book handling [robustness live]
    "VACUUM_GAP": 4,
    "VACUUM_SIZE": 3,
    "VACUUM_FAIR_BLEND": 0.60,

    # Rebuilt Pepper risk context [robustness live]
    "RISK_CHASE_Z": 1.15,
    "RISK_ENTRY_PENALTY": 0.12,
    "RISK_LONG_INV_PENALTY": 0.02,
    "RISK_SHORT_INV_PENALTY": 0.12,
    "RISK_DRAWDOWN_CATA": 800.0,
    "RISK_DRAWDOWN_ALPHA": 0.08,
    "RISK_CATA_BUY_RELIEF": 0.10,
    "RISK_CATA_SELL_RELIEF": 0.04,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _slow_fair(self, stable_mid: float, ash_state: Optional[dict], book_health: float) -> float:
        ...
        if style == "median_low_health" and book_health < float(self.p["BOOK_HEALTH_MED"]):
    ...
    def _sigma_state(self, book: Book, ash_state: dict) -> Tuple[float, float]:
        prev_mid = ash_state.get("prev_mid")
        prev_var = float(ash_state.get("var_ema", 1.5))
        if prev_mid is None:
            var_ema = prev_var
        else:
            move = book.mid - float(prev_mid)
            var_ema = ema(prev_var, move * move, float(self.p["ALTMM_VAR_ALPHA"]))
            var_ema = clamp(var_ema, 0.20, 30.0)
        return math.sqrt(var_ema), var_ema

    def _altmm_quote_geometry(
        self,
        slow_fair: float,
        quote_signal: float,
        book_health: float,
        ...
        inv_ratio = projected_pos / float(PRODUCT_LIMITS["ASH_COATED_OSMIUM"])
        gamma = 0.020 + 0.030 * abs(inv_ratio) + 0.015 * (1.0 - book_health)
        reservation = slow_fair - projected_pos * gamma * max(0.6, sigma) * horizon
        reservation += clamp(quote_signal * 0.70, -0.90, 0.90)

        k = 0.90 + 1.40 * book_health
        half_spread = (1.0 / k) + gamma * sigma * sigma * horizon + 1.10 * (1.0 - book_health)
        return reservation, clamp(half_spread, 1.70, 4.80)
    ...
    def _equity_drawdown(
        ...
        last_seen_trade_ts = int(ash_state.get("risk_last_trade_ts", -1))
        cash = float(ash_state.get("risk_cash", 0.0))
        latest_seen = last_seen_trade_ts
        for trade in state.own_trades.get("ASH_COATED_OSMIUM", []):
            trade_ts = int(getattr(trade, "timestamp", -1))
            if trade_ts <= last_seen_trade_ts:
                continue
            ...
            if getattr(trade, "buyer", None) == "SUBMISSION":
                ...
                cash += px * qty
            latest_seen = max(latest_seen, trade_ts)

        last_mark = float(ash_state.get("risk_last_mark", self.p["REFERENCE_PRICE"]))
        if book.valid:
            mark = book.mid
        elif book.has_bid or book.has_ask:
            mark = float(book.best_bid if book.has_bid else book.best_ask)
        else:
            mark = last_mark
        equity = cash + position * mark
        peak = max(float(ash_state.get("risk_equity_peak", equity)), equity)
        drawdown = peak - equity
        dd_ema = ema(float(ash_state.get("risk_drawdown_ema", 0.0)), drawdown, float(self.p["RISK_DRAWDOWN_ALPHA"]))

        ash_state["risk_cash"] = round(cash, 4)
        ash_state["risk_last_trade_ts"] = int(latest_seen)
        ash_state["risk_last_mark"] = round(mark, 4)
        ash_state["risk_equity"] = round(equity, 4)
        ash_state["risk_equity_peak"] = round(peak, 4)
        ash_state["risk_drawdown_ema"] = round(dd_ema, 4)
        return drawdown, dd_ema, equity
    ...
    def _risk_context(
        ...
        anchor = float(self.p["REFERENCE_PRICE"])
        last_good_fair = float(ash_state.get("last_good_fair", anchor))
        fair_disagreement = max(anchor, stable_mid, last_good_fair, slow_fair) - min(anchor, stable_mid, last_good_fair, slow_fair)
        inv_ratio = abs(position) / float(PRODUCT_LIMITS["ASH_COATED_OSMIUM"])
        toxicity_level = max(bid_level, ask_level)
        low_toxicity = 1.0 - 0.45 * min(2, toxicity_level)

        normal_vol = float(self.p["RISK_VOL_NORMAL"])
        vol_excess = max(0.0, sigma - normal_vol)
        vol_mult = 1.0 / (1.0 + vol_excess / max(0.6, normal_vol))

        fair_soft = float(self.p["RISK_FAIR_DISAGREE_SOFT"])
        fair_hard = float(self.p["RISK_FAIR_DISAGREE_HARD"])
        disagreement_mult = 1.0
        if fair_disagreement > fair_soft:
            disagreement_mult = 1.0 / (1.0 + (fair_disagreement - fair_soft) / max(0.5, fair_soft))
        severe_disagreement = fair_disagreement >= fair_hard

        progress = clamp(timestamp / 999900.0 if timestamp > 0 else 0.0, 0.0, 1.0)
        terminal_penalty = 0.0
        if progress >= float(self.p["RISK_TERMINAL_START"]):
            terminal_penalty += float(self.p["RISK_TERMINAL_STEP1"]) * inv_ratio
        if progress >= float(self.p["RISK_TERMINAL_MID"]):
            terminal_penalty += float(self.p["RISK_TERMINAL_STEP2"]) * inv_ratio
        if progress >= float(self.p["RISK_TERMINAL_LATE"]):
            terminal_penalty += float(self.p["RISK_TERMINAL_STEP3"]) * inv_ratio

        drawdown_edge = 0.0
        drawdown_mult = 1.0
        drawdown_soft = float(self.p["RISK_DRAWDOWN_SOFT"])
        drawdown_hard = float(self.p["RISK_DRAWDOWN_HARD"])
        if drawdown_ema >= drawdown_hard:
            drawdown_edge = float(self.p["RISK_DRAWDOWN_EDGE_HARD"])
            drawdown_mult = 0.90
        elif drawdown_ema >= drawdown_soft:
            drawdown_edge = float(self.p["RISK_DRAWDOWN_EDGE_SOFT"])
            drawdown_mult = 0.96

        inventory_mult = max(0.72, 1.0 - 0.45 * inv_ratio)
        book_mult = 0.55 + 0.45 * book_health
        size_mult = clamp(book_mult * inventory_mult * vol_mult * disagreement_mult * drawdown_mult, 0.60, 1.18)

        agreement_score = clamp(signal_conviction / 1.0, 0.0, 1.0)
        low_inventory = 1.0 - inv_ratio
        low_volatility = clamp(vol_mult, 0.0, 1.0)
        confidence = clamp(
            0.35 * book_health
            + 0.25 * agreement_score
            + 0.20 * clamp(low_toxicity, 0.0, 1.0)
            + 0.10 * low_inventory
            + 0.10 * low_volatility,
            0.0,
            1.0,
        ...
        if confidence >= float(self.p["RISK_CONF_ATTACK"]):
            mode = "attack"
        elif confidence >= float(self.p["RISK_CONF_PRESS"]):
            mode = "press"
        elif confidence >= float(self.p["RISK_CONF_BALANCED"]):
            mode = "balanced"
        else:
            mode = "defend"

        allow_access = book_health >= float(self.p["ACCESS_BOOK_HEALTH_MIN"]) and not severe_disagreement and drawdown_ema < drawdown_hard
        return {
            "fair_disagreement": fair_disagreement,
            "vol_mult": vol_mult,
            "disagreement_mult": disagreement_mult,
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        sigma, var_ema = self._sigma_state(book, ash_state)
        _drawdown, drawdown_ema, _equity = self._equity_drawdown(state, book, ash_state, position)
        progress = clamp(timestamp / 999900.0 if timestamp > 0 else 0.0, 0.0, 1.0)
        (
            slow_fair,
            take_signal,
        ...
        if risk_ctx["mode"] == "attack":
            attack = max(attack, 0.78)
        elif risk_ctx["mode"] == "press":
            attack = max(attack, 0.52)
        elif risk_ctx["mode"] == "balanced":
            attack = min(max(attack, 0.24), 0.48)
        else:
            attack = min(attack, 0.18)
        ...
        if pos > 0:
            buy_need += 0.45 * float(risk_ctx["terminal_penalty"])
            sell_need = max(0.70, sell_need - 0.35 * float(risk_ctx["terminal_penalty"]))
        elif pos < 0:
            sell_need += 0.45 * float(risk_ctx["terminal_penalty"])
            buy_need = max(0.70, buy_need - 0.35 * float(risk_ctx["terminal_penalty"]))
        if refill_bias > 0:
        ...
        altmm_blend = 0.0
        altmm_quote_mid = 0.0
        altmm_half_spread = 0.0
        if self.p.get("ALTMM_GEOMETRY_BLEND", 0.0) > 0.0:
            health_cap = float(self.p["ALTMM_GEOMETRY_HEALTH_MAX"])
            if touch_noisy or book_health < health_cap:
                health_weight = clamp((health_cap - book_health) / max(0.05, health_cap), 0.0, 1.0)
                noisy_bonus = 0.35 if touch_noisy else 0.0
                altmm_blend = clamp(
                    float(self.p["ALTMM_GEOMETRY_BLEND"]) * (0.45 + 0.55 * health_weight + noisy_bonus),
                    0.0,
                    0.38,
                )
                altmm_quote_mid, altmm_half_spread = self._altmm_quote_geometry(
                    slow_fair,
                    quote_signal,
                    book_health,
                    sigma,
        ...
        if pos > 0:
            buy_qe += float(risk_ctx["terminal_penalty"])
            sell_qe -= 0.35 * float(risk_ctx["terminal_penalty"])
        elif pos < 0:
            sell_qe += float(risk_ctx["terminal_penalty"])
            buy_qe -= 0.35 * float(risk_ctx["terminal_penalty"])
        if terminal_short_recycle:
        ...
        if altmm_blend > 0.0:
            buy_qe = (1.0 - altmm_blend) * buy_qe + altmm_blend * altmm_half_spread
            sell_qe = (1.0 - altmm_blend) * sell_qe + altmm_blend * altmm_half_spread
        ...
        if altmm_blend > 0.0:
            quote_mid = (1.0 - altmm_blend) * quote_mid + altmm_blend * altmm_quote_mid
        ...
        if risk_ctx["mode"] == "defend":
            join_edge = max(0.25, join_edge - 0.10)
        ...
        buy_mult *= float(risk_ctx["size_mult"])
        sell_mult *= float(risk_ctx["size_mult"])
        if pos > 0:
            buy_mult *= max(0.70, 1.0 - 0.25 * float(risk_ctx["terminal_penalty"]))
            sell_mult *= min(1.12, 1.0 + 0.12 * float(risk_ctx["terminal_penalty"]))
        elif pos < 0:
            sell_mult *= max(0.70, 1.0 - 0.25 * float(risk_ctx["terminal_penalty"]))
            buy_mult *= min(1.12, 1.0 + 0.12 * float(risk_ctx["terminal_penalty"]))
        ...
        ash_state["var_ema"] = round(var_ema, 4)
        ash_state["risk_mode"] = str(risk_ctx["mode"])
        ash_state["risk_confidence"] = round(float(risk_ctx["confidence"]), 4)
        ash_state["risk_fair_disagreement"] = round(float(risk_ctx["fair_disagreement"]), 4)
        ash_state["last_fill_ts"] = int(last_fill_ts)
        ash_state["no_fill_ticks"] = int(no_fill_ticks)
        ash_state["last_buy_fill_ts"] = int(last_buy_fill_ts)
        ash_state["last_sell_fill_ts"] = int(last_sell_fill_ts)
...
class IntarianPepperRootTrader:
    ...
    def _equity_drawdown(
        ...
        last_seen_trade_ts = int(ps.get("risk_last_trade_ts", -1))
        cash = float(ps.get("risk_cash", 0.0))
        latest_seen = last_seen_trade_ts
        for trade in state.own_trades.get("INTARIAN_PEPPER_ROOT", []):
            trade_ts = int(getattr(trade, "timestamp", -1))
            if trade_ts <= last_seen_trade_ts:
                continue
            qty = abs(int(getattr(trade, "quantity", 0)))
        ...
        last_mark = float(ps.get("risk_last_mark", 13000.0))
        if book.valid:
            mark = book.mid
        elif book.has_bid or book.has_ask:
            mark = float(book.best_bid if book.has_bid else book.best_ask)
            ...
            mark = last_mark
        equity = cash + position * mark
        peak = max(float(ps.get("risk_equity_peak", equity)), equity)
        drawdown = peak - equity
        dd_ema = ema(float(ps.get("risk_drawdown_ema", 0.0)), drawdown, float(self.p["RISK_DRAWDOWN_ALPHA"]))

        ps["risk_cash"] = round(cash, 4)
        ps["risk_last_trade_ts"] = int(latest_seen)
        ps["risk_last_mark"] = round(mark, 4)
        ps["risk_equity"] = round(equity, 4)
        ps["risk_equity_peak"] = round(peak, 4)
        ps["risk_drawdown_ema"] = round(dd_ema, 4)
        return drawdown, dd_ema, equity
    ...
    def _risk_context(
        ...
        long_ratio = max(0.0, position) / float(PRODUCT_LIMITS["INTARIAN_PEPPER_ROOT"])
        short_ratio = max(0.0, -position) / float(PRODUCT_LIMITS["INTARIAN_PEPPER_ROOT"])
        inventory_penalty = (
            float(self.p["RISK_LONG_INV_PENALTY"]) * long_ratio
            + float(self.p["RISK_SHORT_INV_PENALTY"]) * short_ratio
        )
        chase_penalty = max(0.0, zscore - float(self.p["RISK_CHASE_Z"])) * float(self.p["RISK_ENTRY_PENALTY"])
        catastrophic = drawdown_ema >= float(self.p["RISK_DRAWDOWN_CATA"])
        buy_relief = float(self.p["RISK_CATA_BUY_RELIEF"]) if catastrophic else 0.0
        sell_relief = float(self.p["RISK_CATA_SELL_RELIEF"]) if catastrophic else 0.0
        short_cover_bias = 0.14 * short_ratio
        if position < 0 and position < target_int:
            short_cover_bias += 0.10
        return {
            "inventory_penalty": inventory_penalty,
            "chase_penalty": chase_penalty,
            "catastrophic": catastrophic,
            "buy_relief": buy_relief,
            "sell_relief": sell_relief,
            "short_cover_bias": short_cover_bias,
        }
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        _drawdown, drawdown_ema, _equity = self._equity_drawdown(state, book, ps, position)
        ...
        risk_ctx = self._risk_context(position, target_int, zscore, drawdown_ema)
        ...
        if pos < max(20, target_int - 10):
            sell_te += 0.55
        buy_te += float(risk_ctx["inventory_penalty"]) + float(risk_ctx["chase_penalty"])
        buy_te = max(0.35, buy_te - float(risk_ctx["short_cover_bias"]) - float(risk_ctx["buy_relief"]))
        sell_te = max(1.05, sell_te - float(risk_ctx["sell_relief"]))
        if (
        ...
        if zscore < -0.55:
            ...
            sell_qe -= 0.15
        buy_qe += float(risk_ctx["inventory_penalty"]) + 0.80 * float(risk_ctx["chase_penalty"])
        buy_qe = max(0.75, buy_qe - 0.60 * float(risk_ctx["short_cover_bias"]))
        sell_qe = max(1.50, sell_qe - 0.50 * float(risk_ctx["sell_relief"]))
        ...
        if risk_ctx["catastrophic"]:
            buy_front_sz = max(3, buy_front_sz - 2)
            buy_back_sz = max(2, buy_back_sz - 1)
        ...
        memory["IPR_STATE"] = {
            ...
            "risk_drawdown_ema": ps.get("risk_drawdown_ema", 0.0),
            "risk_equity_peak": ps.get("risk_equity_peak", 0.0),
            "risk_cash": ps.get("risk_cash", 0.0),
            "risk_last_trade_ts": ps.get("risk_last_trade_ts", -1),
            "risk_last_mark": ps.get("risk_last_mark", fair),
        }
        return mgr.orders, memory
