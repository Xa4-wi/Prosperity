...
DEFAULT_ASH_PARAMS = {
    ...
    "RISK_DRAWDOWN_SOFT": 140.0,
    "RISK_DRAWDOWN_HARD": 260.0,
    "RISK_DRAWDOWN_ALPHA": 0.10,
    "RISK_DRAWDOWN_EDGE_SOFT": 0.05,
    "RISK_DRAWDOWN_EDGE_HARD": 0.12,
    "RISK_TERMINAL_START": 0.70,
    "RISK_TERMINAL_MID": 0.85,
    "RISK_TERMINAL_LATE": 0.95,
    "RISK_TERMINAL_STEP1": 0.18,
    "RISK_TERMINAL_STEP2": 0.40,
    "RISK_TERMINAL_STEP3": 0.80,
...
DEFAULT_IPR_PARAMS = {
    ...
    "RISK_CHASE_Z": 1.15,
    "RISK_ENTRY_PENALTY": 0.22,
    "RISK_LONG_INV_PENALTY": 0.04,
    "RISK_SHORT_INV_PENALTY": 0.20,
    "RISK_DRAWDOWN_CATA": 520.0,
    "RISK_DRAWDOWN_ALPHA": 0.08,
    "RISK_CATA_BUY_RELIEF": 0.20,
    "RISK_CATA_SELL_RELIEF": 0.08,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _risk_context(
        ...
        inventory_mult = max(0.62, 1.0 - 0.55 * inv_ratio)
        book_mult = 0.55 + 0.45 * book_health
        size_mult = clamp(book_mult * inventory_mult * vol_mult * disagreement_mult * drawdown_mult, 0.45, 1.18)
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_need += float(risk_ctx["drawdown_edge"])
        sell_need += float(risk_ctx["drawdown_edge"])
        if pos > 0:
            buy_need += float(risk_ctx["terminal_penalty"])
            sell_need = max(0.70, sell_need - 0.55 * float(risk_ctx["terminal_penalty"]))
        elif pos < 0:
            sell_need += float(risk_ctx["terminal_penalty"])
            buy_need = max(0.70, buy_need - 0.55 * float(risk_ctx["terminal_penalty"]))
        if refill_bias > 0:
        ...
        buy_qe += float(risk_ctx["drawdown_edge"])
        sell_qe += float(risk_ctx["drawdown_edge"])
        if book_health < float(self.p["BOOK_HEALTH_LOW"]):
        ...
        if pos > 0:
            ...
            sell_qe -= 0.55 * float(risk_ctx["terminal_penalty"])
        elif pos < 0:
            sell_qe += float(risk_ctx["terminal_penalty"])
            buy_qe -= 0.55 * float(risk_ctx["terminal_penalty"])
        if terminal_short_recycle:
        ...
        if pos > 0:
            buy_mult *= max(0.55, 1.0 - 0.45 * float(risk_ctx["terminal_penalty"]))
            sell_mult *= min(1.18, 1.0 + 0.20 * float(risk_ctx["terminal_penalty"]))
        elif pos < 0:
            sell_mult *= max(0.55, 1.0 - 0.45 * float(risk_ctx["terminal_penalty"]))
            buy_mult *= min(1.18, 1.0 + 0.20 * float(risk_ctx["terminal_penalty"]))
