...
DEFAULT_TOMATOES_PARAMS = {
    ...
    "QUEUE_REACTIVE_WEIGHT": 0.18,
    "QUEUE_REACTIVE_FAIR_WEIGHT": 0.12,
    "QUEUE_REACTIVE_EDGE_SHIFT": 0.12,
    "QUEUE_REACTIVE_ALPHA_SHIFT": 0.10,
}
...
class TomatoesTrader(BaseProductTrader):
    ...
    def queue_reactive_signal(self) -> float:
        previous = self.previous_book_state()
        if not previous or not self.has_book():
            return 0.0
        scale = max(1.0, float(self.spread) / 2.0, float(previous["spread"]) / 2.0)
        ...
        prev_bid_volume = max(1.0, float(previous["best_bid_volume"]))
        prev_ask_volume = max(1.0, float(previous["best_ask_volume"]))

        bid_depletion = 0.0
        bid_refill = 0.0
        if float(self.best_bid) == float(previous["best_bid"]):
            bid_depletion = max(0.0, float(previous["best_bid_volume"]) - float(self.best_bid_volume)) / prev_bid_volume
            bid_refill = max(0.0, float(self.best_bid_volume) - float(previous["best_bid_volume"])) / prev_bid_volume

        ask_depletion = 0.0
        ask_refill = 0.0
        if float(self.best_ask) == float(previous["best_ask"]):
            ask_depletion = max(0.0, float(previous["best_ask_volume"]) - float(self.best_ask_volume)) / prev_ask_volume
            ask_refill = max(0.0, float(self.best_ask_volume) - float(previous["best_ask_volume"])) / prev_ask_volume

        depth_skew = 0.45 * self.depth_imbalance_short + 0.35 * self.depth_imbalance_long + 0.20 * self.depth_concentration
        ...
        return (buy_pressure - sell_pressure) + (0.35 * depth_skew) + (0.25 * trade_pressure)
    ...
    def hybrid_alpha(self) -> Tuple[float, float]:
        ...
        queue_signal = self.queue_reactive_signal()
        hybrid_fair = (
            ...
            + self.QUEUE_REACTIVE_ALPHA_SHIFT * half_spread * queue_signal
        )
    ...
    def fair_value(
        ...
        fair = (
            ...
            + self.QUEUE_REACTIVE_FAIR_WEIGHT * half_spread * queue_signal
        )
    ...
    def take_edge(
        ...
        if side == "BUY":
            edge -= self.QUEUE_REACTIVE_EDGE_SHIFT * max(0.0, queue_signal)
            edge += self.QUEUE_REACTIVE_EDGE_SHIFT * max(0.0, -queue_signal)
        else:
            edge -= self.QUEUE_REACTIVE_EDGE_SHIFT * max(0.0, -queue_signal)
            edge += self.QUEUE_REACTIVE_EDGE_SHIFT * max(0.0, queue_signal)
    ...
    def passive_size(self, side: str, regime: str, volatility: float) -> int:
        ...
        if side == "BUY" and queue_signal > 0:
            size += 1
        elif side == "SELL" and queue_signal < 0:
            size += 1
