...
class TomatoesBot:
    ...
    LOB_ALPHA_WEIGHT = 0.08
    LOB_FAIR_WEIGHT = 0.08
    LOB_EDGE_WEIGHT = 0.16
    LOB_TAKE_SHIFT = 0.10
    LOB_QUOTE_SHIFT = 0.10
    ...
    def load_product_state(self) -> Dict[str, object]:
        ...
        return {
            ...
            "lob_signal_ema": float(raw.get("lob_signal_ema", 0.0)),
    ...
    def save_product_state(self) -> None:
        self.memory["tomatoes"] = {
            ...
            "lob_signal_ema": float(self.product_state["lob_signal_ema"]),
    ...
    def filtered_lob_signal(self) -> float:
        ...
        for index, (price, volume) in enumerate(bid_levels):
            ...
            bid_weight += weight
            bid_distance += weight * (float(self.book.best_bid) - float(price))
        for index, (price, volume) in enumerate(ask_levels):
            weight = float(max(0, volume)) / (index + 1.0)
            ask_weight += weight
            ask_distance += weight * (float(price) - float(self.book.best_ask))
        ...
        micro_premium = (float(self.book.micro) - float(self.book.mid)) / half_spread
        raw = 0.45 * self.book.imbalance + 0.30 * depth_skew + 0.20 * micro_premium + 0.05 * shape_skew
        raw = clamp(raw, -1.5, 1.5)
        self.product_state["lob_signal_ema"] = ema(float(self.product_state["lob_signal_ema"]), raw, self.LOB_SIGNAL_ALPHA)
        return float(self.product_state["lob_signal_ema"])
    ...
    def hybrid_alpha(self, lob_signal: float) -> float:
    ...
    def take_threshold(
        ...
        if lob_signal != 0.0:
            ...
            if aligned:
                threshold -= self.LOB_TAKE_SHIFT * min(1.0, abs(lob_signal))
            else:
                threshold += 0.05 * min(1.0, abs(lob_signal))
    ...
    def take_orders(
        ...
        buy_threshold = self.take_threshold("BUY", regime, target, predicted_edge, fit_quality, volatility, lob_signal)
        sell_threshold = self.take_threshold("SELL", regime, target, predicted_edge, fit_quality, volatility, lob_signal)
    ...
    def quote_edge(self, side: str, regime: str, target: int, volatility: float, fit_quality: float, lob_signal: float) -> float:
        ...
        if lob_signal != 0.0 and regime != "toxic":
    ...
    def passive_quotes(
        ...
        buy_edge = self.quote_edge("BUY", regime, target, volatility, fit_quality, lob_signal)
        sell_edge = self.quote_edge("SELL", regime, target, volatility, fit_quality, lob_signal)
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        hybrid_alpha = self.hybrid_alpha(lob_signal)
        ...
        predicted_edge += self.LOB_EDGE_WEIGHT * max(1.0, float(self.book.spread) / 2.0) * lob_signal
        ...
        fair = self.fair_value(regime, target, predicted_now, predicted_next, guarded_alpha, lob_signal)
        reservation = self.reservation(fair, target)

        self.take_orders(reservation, regime, target, predicted_edge, fit_quality, volatility, lob_signal)
        self.clear_inventory(self.reservation(fair, target), regime, target)
        reservation = self.reservation(fair, target)
        buy_quote, sell_quote = self.passive_quotes(
            reservation, regime, target, predicted_edge, fit_quality, volatility, lob_signal
        )
