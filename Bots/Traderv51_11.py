...
class TomatoesBot:
    ...
    QUOTE_ONE_SIDE_EDGE = 0.95
    QUOTE_NONE_EDGE = 0.40
    QUOTE_ONE_SIDE_IMBALANCE = 0.10
    QUOTE_NONE_VOL_THRESHOLD = 2.9
    ...
    def quote_action(
        ...
        if regime == "toxic":
            return "none"
        if (
            volatility >= self.QUOTE_NONE_VOL_THRESHOLD
            and abs(predicted_edge) <= self.QUOTE_NONE_EDGE
            and abs(self.manager.projected_position() - target) <= 6
        ):
            return "none"

        directional = (
            fit_quality >= self.FIT_THRESHOLD
            and abs(predicted_edge) >= self.QUOTE_ONE_SIDE_EDGE
            and abs(self.book.imbalance) >= self.QUOTE_ONE_SIDE_IMBALANCE
        )
        if regime in {"trend_up", "strong_up"} and directional:
            return "buy_only" if self.manager.projected_position() < target else "both"
        if regime in {"trend_down", "strong_down"} and directional:
            return "sell_only" if self.manager.projected_position() > target else "both"
        return "both"
    ...
    def passive_quotes(
        ...
        action = self.quote_action(regime, target, predicted_edge, fit_quality, volatility)
        if action == "none":
            return None, None
