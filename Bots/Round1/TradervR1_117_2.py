...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if not book.has_bid and not book.has_ask:
            ...
            ash_state["bars_since_buy_fill"] = int(bars_since_buy_fill)
            ash_state["bars_since_sell_fill"] = int(bars_since_sell_fill)
            memory["ASH_STATE"] = ash_state
            return [], memory
        ...
        buy_reactivation = reactivation_base and bars_since_buy_fill >= 8
        sell_reactivation = reactivation_base and bars_since_sell_fill >= 8
        react_side = sign(quote_signal, 0.10)
        micro_nibble = (
            ((buy_reactivation and react_side > 0) or (sell_reactivation and react_side < 0))
        ...
        if buy_reactivation and react_side > 0:
            extra = min(0.30, 0.05 * max(0, bars_since_buy_fill - 8))
            buy_qe -= 0.15 + extra
            if micro_nibble:
                buy_qe -= 0.05
        elif sell_reactivation and react_side < 0:
