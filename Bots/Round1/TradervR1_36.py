...
DEFAULT_ASH_PARAMS = {
    ...
    # Passive fill-quality memory
    "FILL_QUALITY_ALPHA": 0.22,
    "FILL_QUALITY_DECAY": 0.08,
    "FILL_QUALITY_EDGE_SHIFT": 0.85,
    "FILL_QUALITY_STOP": 1.05,
    "FILL_QUALITY_TOXIC_BOOST": 0.30,
    "FILL_QUALITY_SIZE_CUT": 0.45,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _load_state(self, memory: dict) -> dict:
        ...
        return {
            ...
            "buy_bad_ema": float(raw.get("buy_bad_ema", 0.0)),
            "sell_bad_ema": float(raw.get("sell_bad_ema", 0.0)),
        }
    ...
    def _update_fill_quality(self, book: Book, position: int, st: dict) -> dict:
        ...
        if delta_pos > 0:
            # Net buy fills followed by downward movement are low-quality.
            st["buy_bad_ema"] = ema(float(st["buy_bad_ema"]), max(0.0, -move), alpha)
        else:
            st["buy_bad_ema"] = ema(float(st["buy_bad_ema"]), 0.0, decay)

        if delta_pos < 0:
            # Net sell fills followed by upward movement are low-quality.
            st["sell_bad_ema"] = ema(float(st["sell_bad_ema"]), max(0.0, move), alpha)
        else:
            st["sell_bad_ema"] = ema(float(st["sell_bad_ema"]), 0.0, decay)
        return st
    ...
    def _fair_value(self, book: Book) -> float:
        ...
        return local_conf * local_fair + (1.0 - local_conf) * legacy_fair
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if ask_toxic:
            sell_qe += 1.0
        buy_bad = float(st["buy_bad_ema"]) + (float(self.p["FILL_QUALITY_TOXIC_BOOST"]) if bid_toxic else 0.0)
        sell_bad = float(st["sell_bad_ema"]) + (float(self.p["FILL_QUALITY_TOXIC_BOOST"]) if ask_toxic else 0.0)
        buy_qe += float(self.p["FILL_QUALITY_EDGE_SHIFT"]) * buy_bad
        sell_qe += float(self.p["FILL_QUALITY_EDGE_SHIFT"]) * sell_bad
        pos = mgr.projected()
        ...
        if buy_bad >= float(self.p["FILL_QUALITY_STOP"]) and pos >= -12:
            allow_bid = False
        if sell_bad >= float(self.p["FILL_QUALITY_STOP"]) and pos <= 12:
            allow_ask = False

        front_sz = int(self.p["FRONT_SIZE"])
        back_sz = int(self.p["BACK_SIZE"])
        if buy_bad > 0.0:
            front_sz_buy = max(8, int(round(front_sz * (1.0 - float(self.p["FILL_QUALITY_SIZE_CUT"]) * min(1.0, buy_bad / 2.0)))))
            back_sz_buy = max(0, int(round(back_sz * (1.0 - 0.60 * min(1.0, buy_bad / 2.0)))))
        else:
            front_sz_buy = front_sz
            back_sz_buy = back_sz
        if sell_bad > 0.0:
            front_sz_sell = max(8, int(round(front_sz * (1.0 - float(self.p["FILL_QUALITY_SIZE_CUT"]) * min(1.0, sell_bad / 2.0)))))
            back_sz_sell = max(0, int(round(back_sz * (1.0 - 0.60 * min(1.0, sell_bad / 2.0)))))
        else:
