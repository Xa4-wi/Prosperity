...
DEFAULT_IPR_PARAMS = {
    ...
    "RARE_OVERLAY_Z": 1.35,
    "RARE_OVERLAY_BUFFER": 10,
    "RARE_OVERLAY_LATE": 0.88,
}
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            ...
            rare_sell_ok = (
                progress > float(self.p["RARE_OVERLAY_LATE"])
                or (pos > target_int + int(self.p["RARE_OVERLAY_BUFFER"]) and zscore > float(self.p["RARE_OVERLAY_Z"]))
            )
            if not rare_sell_ok:
                qty = 0
