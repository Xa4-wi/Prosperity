...
# Archetype: lighter drift-style Osmium experiment
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_take = float(self.p["BASE_TAKE_EDGE"])
        sell_take = float(self.p["BASE_TAKE_EDGE"])
        quote_edge = float(self.p["BASE_QUOTE_EDGE"])
        if regime == "trend_up":
            buy_take -= 0.20
            sell_take += 0.40
            quote_edge += 0.15
        ...
        if book.best_ask <= reservation - max(0.8, buy_take) and regime != "trend_down":
            mgr.buy(book.best_ask, min(book.best_ask_vol, int(self.p["MAX_TAKE_SIZE"])))
        if book.best_bid >= reservation + max(0.8, sell_take) and regime != "trend_up":
            mgr.sell(book.best_bid, min(book.best_bid_vol, int(self.p["MAX_TAKE_SIZE"])))
        ...
        if regime == "toxic" and abs(mgr.projected()) <= 6:
            buy_quote = None
            sell_quote = None

        passive_size = int(self.p["PASSIVE_SIZE"])
        if regime == "toxic":
            passive_size = max(2, passive_size - 4)
        elif regime in {"trend_up", "trend_down"}:
            passive_size = max(3, passive_size - 1)

        if buy_quote is not None and mgr.projected() < target + 8 and mgr.projected() < int(self.p["SOFT_LIMIT"]):
            mgr.buy(buy_quote, passive_size)
        if sell_quote is not None and mgr.projected() > target - 8 and mgr.projected() > -int(self.p["SOFT_LIMIT"]):
