...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        react_side = sign(quote_signal, 0.08)
        react_strength = 0.0
        if reactivation:
            extra = min(0.30, 0.05 * (no_fill_ticks - 8))
            react_strength = clamp(0.30 + extra + 0.20 * signal_conviction, 0.0, 1.0)
            if react_side > 0:
                buy_qe -= 0.15 + extra
                sell_qe += 0.10 + 0.05 * react_strength
            elif react_side < 0:
                sell_qe -= 0.15 + extra
                buy_qe += 0.10 + 0.05 * react_strength
        ...
        if reactivation:
            if react_side > 0:
                if react_strength >= 0.55 and book.best_bid + 1 < book.best_ask:
                    front_buy = max(front_buy, book.best_bid + 1)
                else:
                    front_buy = max(front_buy, book.best_bid)
                front_sell = max(front_sell, book.best_bid + 2)
            elif react_side < 0:
                if react_strength >= 0.55 and book.best_ask - 1 > book.best_bid:
                    front_sell = min(front_sell, book.best_ask - 1)
                else:
                    front_sell = min(front_sell, book.best_ask)
                front_buy = min(front_buy, book.best_ask - 2)
