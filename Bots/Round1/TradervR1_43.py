...
DEFAULT_ASH_PARAMS = {
    ...
    # Lightweight fill-quality memory from the best surviving Osmium donor.
    "MARKOUT_ALPHA": 0.22,
    "SOFT_BAD_MARKOUT": -0.55,
    "HARD_BAD_MARKOUT": -1.00,
...
DEFAULT_IPR_PARAMS = {
    ...
    # Drift-adjusted anti-chasing gate: only suppress clearly positive shock
    # buys; do not slow normal or negative-shock accumulation.
    "BUY_INNOVATION_POS": 0.55,
    "BUY_INNOVATION_TAKE_PENALTY": 0.14,
    "BUY_INNOVATION_QUOTE_PENALTY": 0.18,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if ask_toxic:
            sell_qe += 1.0
        # Soft adverse-selection brake: slightly back off a side after bad recent
        # passive fills, and only fully veto it when markout is clearly bad and
        # current imbalance still points the wrong way.
        ...
        allow_bid = (
            ...
            and pos < soft + 6
            and not (buy_markout < float(self.p["HARD_BAD_MARKOUT"]) and book.imbalance <= 0.0)
        )
        allow_ask = (
            not (ask_toxic and pos < -8)
            and pos > -(soft + 6)
            and not (sell_markout < float(self.p["HARD_BAD_MARKOUT"]) and book.imbalance >= 0.0)
        )

        front_sz = int(self.p["FRONT_SIZE"])
        back_sz = int(self.p["BACK_SIZE"])
        if buy_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance <= 0.0:
            front_sz_buy = max(10, int(round(front_sz * (1.0 - float(self.p["MARKOUT_SIZE_PENALTY"])))))
            back_sz_buy = max(2, int(round(back_sz * (1.0 - float(self.p["MARKOUT_SIZE_PENALTY"])))))
        else:
            front_sz_buy = front_sz
            back_sz_buy = back_sz
        if sell_markout < float(self.p["SOFT_BAD_MARKOUT"]) and book.imbalance >= 0.0:
            front_sz_sell = max(10, int(round(front_sz * (1.0 - float(self.p["MARKOUT_SIZE_PENALTY"])))))
            back_sz_sell = max(2, int(round(back_sz * (1.0 - float(self.p["MARKOUT_SIZE_PENALTY"])))))
        else:
        ...
        memory["ASH_STATE"] = {
            ...
            "buy_markout_ema": float(buy_markout),
            "sell_markout_ema": float(sell_markout),
        }
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        innovation = 0.0
        if prev_mid > 0.0 and last_ts >= 0 and timestamp > last_ts:
            expected_move = drift * (timestamp - last_ts)
            innovation = (book.mid - prev_mid) - expected_move
        ...
        if pos < target_int:
            buy_te -= 0.35
        if innovation > float(self.p["BUY_INNOVATION_POS"]):
            buy_te += float(self.p["BUY_INNOVATION_TAKE_PENALTY"])
        if pos > target_int:
            sell_te -= 0.05
        if zscore < -0.45:
            buy_te -= 0.30
        ...
        if zscore < -0.55:
            ...
            sell_qe -= 0.15
        if innovation > float(self.p["BUY_INNOVATION_POS"]):
            buy_qe += float(self.p["BUY_INNOVATION_QUOTE_PENALTY"])
        cheap_accum = (
            progress < float(self.p["CHEAP_ACCUM_END"])
            and pos < target_int
            and zscore > float(self.p["CHEAP_ACCUM_Z_RELAX"])
