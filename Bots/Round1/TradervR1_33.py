...
DEFAULT_ASH_PARAMS = {
    ...
    "ANCHOR_WEIGHT": 0.38,
    "STABLE_MID_WEIGHT": 0.62,
    "MICRO_WEIGHT": 0.62,
    "IMBALANCE_BIAS": 0.26,
    "DEPTH_IMPACT_SCALE": 88.0,
    "DEPTH_FLOOR": 8.0,
    "LOCAL_FAIR_ALPHA": 0.22,
    "BOOK_LEVELS": 3,
    "WALL_MID_BLEND": 0.45,
    ...
    "TAKE_EDGE": 1.55,
    "DISLOCATION_EDGE": 3.90,
    "DISLOCATION_LEVELS": 2,
    "DISLOCATION_CLIP": 10,
    # Toxicity / fill-quality
    "TOXIC_IMBALANCE": 0.22,
    "STRONG_IMBALANCE": 0.14,
    "THIN_DEPTH_FLOOR": 18.0,
    "PENALTY_TOXIC": 1.18,
    "PENALTY_ALPHA": 0.18,
    "SPREAD_ALPHA": 0.10,
    "VOL_ALPHA": 0.10,
...
class AshCoatedOsmiumTrader:
    ...
    def _toxicity(self, book: Book, st: dict) -> tuple[bool, bool, float, float, bool, bool]:
        ...
        bid_thin = bid_support < thin_floor or bid_support < 0.70 * ask_support
        ask_thin = ask_support < thin_floor or ask_support < 0.70 * bid_support
        ...
        bid_penalty = (
            ...
            + (0.22 if bid_thin else 0.0)
            + 0.12 * float(st["vol_ema"])
        )
        ask_penalty = (
            float(st["sell_penalty_ema"])
            + float(self.p["PENALTY_TOXIC"]) * max(0.0, book.imbalance)
            + max(0.0, book.micro - book.mid)
            + (0.22 if ask_thin else 0.0)
            + 0.12 * float(st["vol_ema"])
        )
    ...
    def _quote_half_spreads(
        ...
        if inv_ratio > 0.0:
            buy_half += 2.1 * inv_ratio + 4.1 * (inv_ratio ** 3)
            sell_half -= 0.9 * inv_ratio
        elif inv_ratio < 0.0:
            sell_half += 2.1 * (-inv_ratio) + 4.1 * ((-inv_ratio) ** 3)
            buy_half -= 0.9 * (-inv_ratio)
        ...
        if fair_stability < 0.75 and abs(inv_ratio) < 0.20 and not bid_toxic and not ask_toxic:
            buy_half -= 0.25
            sell_half -= 0.25
    ...
    def _dislocation_take(
        ...
        if not bid_toxic:
            for idx, (ask_px, ask_vol) in enumerate(book.sell_levels[:max_levels]):
                edge = reservation - ask_px - 0.55 * bid_penalty
                threshold = disloc_edge if idx > 0 else take_edge
                if edge < threshold:
                    break
                qty = min(ask_vol, clip if idx > 0 else min(clip, 7))
        ...
        if not ask_toxic:
            for idx, (bid_px, bid_vol) in enumerate(book.buy_levels[:max_levels]):
                edge = bid_px - reservation - 0.55 * ask_penalty
                threshold = disloc_edge if idx > 0 else take_edge
                if edge < threshold:
                    break
                qty = min(bid_vol, clip if idx > 0 else min(clip, 7))
                if mgr.projected() <= -soft:
                    qty = max(0, qty - 3)
                mgr.sell(bid_px, qty)
