...
# Archetype: passive-first anchored MM with reduced churn
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    "REFERENCE_PRICE": 10000.0,
    "IMBALANCE_WEIGHT": 0.55,   # fair += imbalance_weight * book_imbalance
    "MICRO_WEIGHT": 0.18,       # fair += micro_weight * (micro - mid)
    "INVENTORY_SKEW": 0.1197820182,    # reservation -= skew * position
    "BASE_EDGE": 2.6,           # default passive quote edge
    "JOIN_EDGE": 2.0,           # snap quote to existing order when within this
    "FRONT_SIZE": 14.0,
    "BACK_SIZE": 4,
    "SOFT_LIMIT": 70.0,           # reduce aggressiveness above this inventory
    "NARROW_SPREAD_LEVEL": 14.0,
...
class AshCoatedOsmiumTrader:
    """
    Passive-first anchored Osmium engine.
    - Keep the anchor at 10000
    - Reduce fair twitchiness and taker aggression
    - Size up only when spread is wide and non-toxic
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        for edge_thr, clip in [
            ...
            if buy_edge >= max(float(edge_thr), buy_edge_needed):
                take_buy = int(clip)
        if book.spread_val <= float(self.p["NARROW_SPREAD_LEVEL"]):
        ...
        if book.spread_val <= float(self.p["NARROW_SPREAD_LEVEL"]):
            buy_qe += 0.35; sell_qe += 0.35
        elif book.spread_val >= float(self.p["WIDE_SPREAD_LEVEL"]):
            buy_qe -= 0.4; sell_qe -= 0.4
        if book.imbalance > strong:
            buy_qe -= 0.2; sell_qe += 0.15
        elif book.imbalance < -strong:
            buy_qe += 0.15; sell_qe -= 0.2
        if bid_toxic:
            buy_qe += float(self.p["TOXIC_RETREAT"])
        if ask_toxic:
            sell_qe += float(self.p["TOXIC_RETREAT"])
        pos = mgr.projected()
        ...
        allow_bid = not (bid_toxic and pos > 4) and pos < soft + 6
        allow_ask = not (ask_toxic and pos < -4) and pos > -(soft + 6)

        front_sz = int(self.p["FRONT_SIZE"])
        back_sz = int(self.p["BACK_SIZE"])
        if book.spread_val >= float(self.p["WIDE_SPREAD_LEVEL"]) and not bid_toxic and not ask_toxic:
            front_sz += 2
            back_sz += 2
        elif book.spread_val <= float(self.p["NARROW_SPREAD_LEVEL"]):
            front_sz = max(8, front_sz - 4)
            back_sz = 0
        if bid_toxic:
            front_sz = max(6, front_sz - 3)
            back_sz = 0
        if ask_toxic:
            front_sz = max(6, front_sz - 3)
            back_sz = 0
