...
DEFAULT_IPR_PARAMS = {
    ...
    # Extreme carry-hold test
    "MIN_HOLD_UNTIL": 0.88,
    "MIN_HOLD_POSITION": 56,
    "MIN_HOLD_Z": 1.55,
    "MIN_HOLD_TARGET_FLOOR": 52,
    "LATE_ONLY_TRIM_START": 0.94,
    "LATE_ONLY_TRIM_Z": 1.10,
    "HOLD_SELL_EDGE_BONUS": 1.25,
    "HOLD_SELL_QUOTE_LIFT": 1.80,
}
...
class IntarianPepperRootTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if progress < float(self.p["MIN_HOLD_UNTIL"]):
            target = max(target, float(self.p["MIN_HOLD_TARGET_FLOOR"]))
        if progress > float(self.p["LATE_ONLY_TRIM_START"]) and zscore > float(self.p["LATE_ONLY_TRIM_Z"]):
            target -= 8.0
        ...
        if bullish:
            buy_te -= 0.10
            sell_te += 0.90
        if progress < float(self.p["MIN_HOLD_UNTIL"]) and pos < int(self.p["MIN_HOLD_POSITION"]):
            sell_te += float(self.p["HOLD_SELL_EDGE_BONUS"])
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            ...
            if bullish and pos < max(26, target_int - 6):
                ...
                qty = min(qty, 4)
            if (
                progress < float(self.p["MIN_HOLD_UNTIL"])
                and pos < int(self.p["MIN_HOLD_POSITION"])
                and zscore < float(self.p["MIN_HOLD_Z"])
            ):
                qty = 0
        ...
        if zscore < -0.55:
            ...
            sell_qe -= 0.15
        if progress < float(self.p["MIN_HOLD_UNTIL"]) and pos < int(self.p["MIN_HOLD_POSITION"]):
            sell_qe += float(self.p["HOLD_SELL_QUOTE_LIFT"])
