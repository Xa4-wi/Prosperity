...
DEFAULT_ASH_PARAMS = {
    ...
    "STATE_ALPHA": 0.18,
    "MARKOUT_ALPHA": 0.28,
    "FILL_PENALTY_SCALE": 0.35,
    "TOXIC_EDGE_WIDEN": 0.35,
    "CALM_EDGE_TIGHTEN": 0.0,
    "TOXIC_SIZE_MULT": 0.85,
    "CALM_SIZE_MULT": 1.03,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_edge_needed = 1.30 + (0.45 * p_toxic if bid_toxic else 0.10 * p_toxic)
        sell_edge_needed = 1.30 + (0.45 * p_toxic if ask_toxic else 0.10 * p_toxic)
        ...
        allow_bid = (
            ...
            and pos < soft + 6
            and (buy_qe - bid_penalty) > 0.20
            and not (p_toxic > 0.88 and book.imbalance < 0.0)
        )
        allow_ask = (
            not (ask_toxic and pos < -8)
            and pos > -(soft + 6)
            and (sell_qe - ask_penalty) > 0.20
            and not (p_toxic > 0.88 and book.imbalance > 0.0)
        )
