...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        high_fill_state = (
            mode == "normal"
            and signal_conviction >= float(self.p["HIGH_FILL_CONVICTION"])
            and abs(pos) <= max(8, soft - 10)
        )
        if high_fill_state:
            if quote_signal > 0.10 and bid_level == 0:
                buy_front_sz += 2
                buy_back_sz += 1
            elif quote_signal < -0.10 and ask_level == 0:
                sell_front_sz += 2
                sell_back_sz += 1
        if buy_reactivation:
