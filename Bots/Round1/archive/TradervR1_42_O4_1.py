...
DEFAULT_ASH_PARAMS = {
    ...
    "INVENTORY_CLEAR_EDGE": 0.90,
    "INVENTORY_CLEAR_CLIP": 3,
}
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        # Explicit low-edge inventory clearing to free capacity.
        pos = mgr.projected()
        clear_edge = float(self.p["INVENTORY_CLEAR_EDGE"])
        clear_clip = int(self.p["INVENTORY_CLEAR_CLIP"])
        if pos > self.p["SOFT_LIMIT"] and abs(book.best_bid - reservation) <= clear_edge and not ask_toxic:
            mgr.sell(book.best_bid, min(clear_clip, book.best_bid_vol))
        elif pos < -self.p["SOFT_LIMIT"] and abs(reservation - book.best_ask) <= clear_edge and not bid_toxic:
