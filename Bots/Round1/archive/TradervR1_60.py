...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        if mode == "normal":
            buy_need = max(0.85, buy_need - 0.20)
            sell_need = max(0.85, sell_need - 0.20)
        elif mode == "dislocation_take":
            buy_need = max(0.70, buy_need - 0.55)
            sell_need = max(0.70, sell_need - 0.55)
