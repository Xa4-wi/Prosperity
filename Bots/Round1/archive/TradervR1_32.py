...
# Archetype: tomatoes-style drift trader adapted to a much more anchored product
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    "REFERENCE_PRICE": 10000.0,
    "SHORT_MID_ALPHA": 0.24,
    "LONG_MID_ALPHA": 0.06,
    "RESIDUAL_ALPHA": 0.14,
    "SPREAD_ALPHA": 0.10,
    "MID_WEIGHT": 0.45,
    ...
    "TOXIC_SPREAD_THRESHOLD": 18.0,
    "TOXIC_IMBALANCE_THRESHOLD": 0.24,
    "BASE_TAKE_EDGE": 2.2,
    "BASE_QUOTE_EDGE": 4.8,
    "MAX_QUOTE_EDGE": 8.0,
    "PASSIVE_SIZE": 12,
...
class AshCoatedOsmiumTrader:
    """
    Drift-style Osmium experiment inspired by the old TOMATOES architecture.
    The adaptation is intentionally smaller and more anchored than TOMATOES.
    """
    ...
    def _load_state(self, memory: dict) -> dict:
        raw = memory.get("ASH_DRIFT_STATE", {})
        if not isinstance(raw, dict):
            raw = {}
        return {
            "anchor": float(raw.get("anchor", 0.0)),
            "short_mid_ema": raw.get("short_mid_ema"),
            "long_mid_ema": raw.get("long_mid_ema"),
    ...
    def _save_state(self, memory: dict, st: dict, timestamp: int) -> dict:
        memory["ASH_DRIFT_STATE"] = {
            "anchor": st["anchor"],
            "short_mid_ema": st["short_mid_ema"],
            "long_mid_ema": st["long_mid_ema"],
            "residual_ema": st["residual_ema"],
            "spread_ema": st["spread_ema"],
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if st["last_ts"] >= 0 and timestamp < st["last_ts"]:
            st["initialized"] = False
            st["anchor"] = 0.0
            st["short_mid_ema"] = None
            st["long_mid_ema"] = None
            st["residual_ema"] = 0.0
            st["spread_ema"] = 14.0

        if not st["initialized"]:
            st["anchor"] = float(self.p["REFERENCE_PRICE"])
            st["short_mid_ema"] = book.mid
            st["long_mid_ema"] = book.mid
            st["residual_ema"] = 0.0
            st["spread_ema"] = book.spread_val
            st["initialized"] = True
        else:
            st["short_mid_ema"] = ema(st["short_mid_ema"], book.mid, float(self.p["SHORT_MID_ALPHA"]))
            st["long_mid_ema"] = ema(st["long_mid_ema"], book.mid, float(self.p["LONG_MID_ALPHA"]))
            st["spread_ema"] = ema(float(st["spread_ema"]), book.spread_val, float(self.p["SPREAD_ALPHA"]))

        trend = float(st["short_mid_ema"]) - float(st["long_mid_ema"])
        residual = book.mid - float(st["anchor"])
        st["residual_ema"] = ema(float(st["residual_ema"]), residual, float(self.p["RESIDUAL_ALPHA"]))
        residual_z = residual / max(2.5, 0.5 * float(st["spread_ema"]))

        toxic = (
            book.spread_val >= float(self.p["TOXIC_SPREAD_THRESHOLD"])
            and abs(book.imbalance) >= float(self.p["TOXIC_IMBALANCE_THRESHOLD"])
        )
        if toxic:
            regime = "toxic"
        elif trend >= float(self.p["STRONG_TREND_THRESHOLD"]):
            regime = "trend_up"
        elif trend <= -float(self.p["STRONG_TREND_THRESHOLD"]):
            regime = "trend_down"
        else:
        ...
        fair = (
            ...
            + float(self.p["MID_WEIGHT"]) * (book.mid - float(self.p["REFERENCE_PRICE"]))
            + float(self.p["MICRO_WEIGHT"]) * (book.micro - book.mid)
            + float(self.p["TREND_WEIGHT"]) * trend
            + float(self.p["RESIDUAL_WEIGHT"]) * float(st["residual_ema"])
            + float(self.p["IMBALANCE_WEIGHT"]) * book.imbalance
        )

        target = 0
        if regime == "trend_up":
            target = 18 if trend >= float(self.p["STRONG_TREND_THRESHOLD"]) else 10
            ...
            if residual_z <= -1.0:
                ...
                target = -10
        reservation = fair - (mgr.projected() - target) * float(self.p["INVENTORY_SKEW"])

        base_take = float(self.p["BASE_TAKE_EDGE"])
        base_quote = float(self.p["BASE_QUOTE_EDGE"])
        buy_take = base_take
        sell_take = base_take
        quote_edge = base_quote
        ...
        if position >= int(self.p["SOFT_LIMIT"]):
            buy_take += 0.5
            quote_edge += 0.5
        elif position <= -int(self.p["SOFT_LIMIT"]):
            sell_take += 0.5
            quote_edge += 0.5

        if book.best_ask <= reservation - max(0.5, buy_take):
            qty = min(book.best_ask_vol, int(self.p["MAX_TAKE_SIZE"]))
            if regime != "trend_down":
                if regime == "trend_up":
                    qty = min(qty, max(1, target - mgr.projected()))
                mgr.buy(book.best_ask, qty)

        if book.best_bid >= reservation + max(0.5, sell_take):
            qty = min(book.best_bid_vol, int(self.p["MAX_TAKE_SIZE"]))
            if regime != "trend_up":
                if regime == "trend_down":
                    qty = min(qty, max(1, mgr.projected() - target))
                mgr.sell(book.best_bid, qty)

        buy_quote = math.floor(reservation - min(float(self.p["MAX_QUOTE_EDGE"]), quote_edge))
        sell_quote = math.ceil(reservation + min(float(self.p["MAX_QUOTE_EDGE"]), quote_edge))
        buy_quote = min(buy_quote, book.best_ask - 1)
        sell_quote = max(sell_quote, book.best_bid + 1)

        if regime == "trend_up":
            buy_quote = max(book.best_bid + 1, buy_quote + 1)
            if mgr.projected() < 8:
                sell_quote = None
        elif regime == "trend_down":
            sell_quote = min(book.best_ask - 1, sell_quote - 1)
            if mgr.projected() > -8:
                buy_quote = None
        elif regime == "toxic" and abs(mgr.projected()) <= 6:
            buy_quote = None
            sell_quote = None

        passive_size = int(self.p["PASSIVE_SIZE"])
        if regime == "toxic":
            passive_size = max(2, passive_size - 5)
        elif regime in {"trend_up", "trend_down"}:
            passive_size = max(3, passive_size - 2)

        if buy_quote is not None and mgr.projected() < target + 10 and mgr.projected() < int(self.p["SOFT_LIMIT"]):
            mgr.buy(buy_quote, passive_size)
        if sell_quote is not None and mgr.projected() > target - 10 and mgr.projected() > -int(self.p["SOFT_LIMIT"]):
            mgr.sell(sell_quote, passive_size)

        memory = self._save_state(memory, st, timestamp)
