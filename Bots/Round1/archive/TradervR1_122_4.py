...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        buy_reactivation = (
            stale_side_base
            and quote_signal > 0.12
            and bid_level <= 1
            and bars_since_buy_fill >= 10
        )
        sell_reactivation = (
            stale_side_base
            and quote_signal < -0.12
            and ask_level <= 1
            and bars_since_sell_fill >= 10
        )
        micro_nibble = False
        if (
            buy_reactivation
            and abs(pos) <= max(8, soft - 8)
            and abs(quote_signal) >= 0.32
        ...
        if buy_reactivation:
            extra = min(0.24, 0.04 * max(0, bars_since_buy_fill - 10))
            buy_qe -= 0.15 + extra
            if micro_nibble:
                buy_qe -= 0.05
        elif sell_reactivation:
            extra = min(0.24, 0.04 * max(0, bars_since_sell_fill - 10))
            sell_qe -= 0.15 + extra
            if micro_nibble:
                sell_qe -= 0.05
