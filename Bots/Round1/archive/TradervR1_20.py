...
DEFAULT_IPR_PARAMS = {
    ...
    "BASE_CARRY": 10.0,
    "EARLY_LONG_BIAS": 18.0,
    "EARLY_ACCUM_END": 0.48,
    "STRONG_TARGET_BONUS": 14.0,
    "STEADY_TARGET_BONUS": 6.0,
    "NOISY_TARGET_CAP": 9.0,
    "ZSCORE_BUY_BONUS": 10.0,
    "ZSCORE_SELL_PENALTY": 4.0,
    "MAX_LONG_TARGET": 76.0,
    "MAX_SHORT_TARGET": 20,
    "OVERLAY_BAND": 15.0,
    "LATE_UNWIND_START": 0.955,
    "LATE_UNWIND_Z": 1.45,
...
class IntarianPepperRootTrader:
    ...
    def _trend_features(self, book: Book, timestamp: int, progress: float, state: dict) -> dict:
        ...
        lookahead = float(self.p["LOOKAHEAD_BONUS"]) * (1.0 - 0.58 * progress) * (0.78 + 0.42 * p_strong)
    ...
    def _target_inventory(self, progress: float, book: Book, features: dict) -> Tuple[int, bool]:
        ...
        early_factor = max(0.0, 1.0 - progress / 0.88)
        target = max(core_target, float(self.p["BASE_CARRY"]) + float(self.p["EARLY_LONG_BIAS"]) * early_factor)
        ...
        if p_noisy < 0.35 and p_strong + p_steady > 0.70:
            target = max(target, core_target + 8.0)
        if p_noisy >= 0.60 and progress < float(self.p["LATE_UNWIND_START"]):
            target = max(target, core_target)
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        very_late_unwind = (
            ...
            and float(features["p_noisy"]) >= 0.62
        ...
        if action == "overlay_sell":
            ...
            if sell_edge >= float(self.p["OVERLAY_SELL_EDGE"]) and mgr.sell_cap > 0:
                qty = min(book.best_bid_vol, 3, max(0, mgr.projected() - overlay_top))
        ...
        if progress < float(self.p["EARLY_ACCUM_END"]) and mgr.projected() <= max(target_int - 6, core_target + 2):
            allow_overlay_sell = False
