...
# Archetype: combined research engine using depth-aware fair, nonlinear inventory,
# toxicity retreat, and passive fill-penalty gating.
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    ...
    "TOXIC_REINSERT_TICKS": 2,
    "FILL_PENALTY_BASE": 0.45,
    "FILL_PENALTY_IMB": 1.55,
    "FILL_PENALTY_TOXIC": 1.00,
    "FILL_PENALTY_DEPTH": 10.0,
    "MIN_NET_EDGE": 0.0,
}
...
class AshCoatedOsmiumTrader:
    """
    Combined Osmium research engine.
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        beta = float(self.p["DEPTH_IMPACT"]) / max(float(self.p["DEPTH_FLOOR"]), depth)
        fair = ref + beta * book.imbalance + self.p["MICRO_WEIGHT"] * (book.micro - book.mid)
        inv_ratio = clamp(mgr.projected() / float(PRODUCT_LIMITS["ASH_COATED_OSMIUM"]), -1.0, 1.0)
        inv_pressure = float(self.p["INV_A1"]) * inv_ratio + float(self.p["INV_A3"]) * (inv_ratio ** 3)
        reservation = fair - inv_pressure
        ...
        # Dynamic quote edges with asymmetric inventory and toxicity retreat.
        ...
        if pos >= soft:
            buy_qe += 1.8; sell_qe -= 0.8
        elif pos <= -soft:
            buy_qe -= 0.8; sell_qe += 1.8
        elif pos > 0:
            buy_qe += 0.8 * inv_ratio
            sell_qe -= 0.3 * inv_ratio
        elif pos < 0:
            buy_qe += 0.3 * inv_ratio
            sell_qe -= 0.8 * inv_ratio
        ...
        reinsert = int(self.p["TOXIC_REINSERT_TICKS"])
        if bid_toxic:
            front_buy -= reinsert
        if ask_toxic:
            front_sell += reinsert
        ...
        penalty_base = float(self.p["FILL_PENALTY_BASE"])
        depth_penalty = float(self.p["FILL_PENALTY_DEPTH"]) / max(1.0, depth)
        penalty_bid = penalty_base + float(self.p["FILL_PENALTY_IMB"]) * abs(min(0.0, book.imbalance)) + depth_penalty
        penalty_ask = penalty_base + float(self.p["FILL_PENALTY_IMB"]) * abs(max(0.0, book.imbalance)) + depth_penalty
        if bid_toxic:
            penalty_bid += float(self.p["FILL_PENALTY_TOXIC"])
        if ask_toxic:
            penalty_ask += float(self.p["FILL_PENALTY_TOXIC"])
        net_edge_bid = reservation - front_buy - penalty_bid
        net_edge_ask = front_sell - reservation - penalty_ask
        allow_bid = not (bid_toxic and pos > 8) and pos < soft + 6 and net_edge_bid > float(self.p["MIN_NET_EDGE"])
        allow_ask = not (ask_toxic and pos < -8) and pos > -(soft + 6) and net_edge_ask > float(self.p["MIN_NET_EDGE"])
