...
DEFAULT_ASH_PARAMS = {
    ...
    "MARKOUT_ALPHA": 0.28,
    "BAD_MARKOUT": -0.75,
    "TOXIC_STATE_THRESHOLD": 0.76,
    "TOXIC_TAKE_EDGE_BONUS": 0.20,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if p_toxic > float(self.p["TOXIC_STATE_THRESHOLD"]) and book.imbalance < 0.0:
            buy_edge_needed += float(self.p["TOXIC_TAKE_EDGE_BONUS"])
        if p_toxic > float(self.p["TOXIC_STATE_THRESHOLD"]) and book.imbalance > 0.0:
            sell_edge_needed += float(self.p["TOXIC_TAKE_EDGE_BONUS"])
        ...
        allow_bid = (
            ...
            and pos < soft + 6
            and not (p_toxic > float(self.p["TOXIC_STATE_THRESHOLD"]) and book.imbalance < 0.0)
