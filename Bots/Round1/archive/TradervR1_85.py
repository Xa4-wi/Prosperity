...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        safe_take_jump = (
            ...
            and book.spread_val <= 14.0
            and abs(book.imbalance) <= 0.18
        )
        if safe_take_jump:
            buy_need = max(0.70, buy_need - 0.18)
            sell_need = max(0.70, sell_need - 0.18)
            if pos > 8:
                buy_need += 0.10
                sell_need = max(0.60, sell_need - 0.22)
            elif pos < -8:
                buy_need = max(0.60, buy_need - 0.22)
                sell_need += 0.10
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_buy:
            ...
            for edge_thr, clip in take_levels:
                if buy_edge >= max(edge_thr, buy_need):
                    take_buy = clip
            if safe_take_jump and pos < -8 and take_signal >= 0.65 and buy_edge >= max(0.60, buy_need - 0.15):
                take_buy = max(take_buy, 3)
            elif safe_take_jump and abs(pos) <= 6 and take_signal >= 0.85 and buy_edge >= max(0.70, buy_need - 0.10):
                take_buy = max(take_buy, 2)
            if take_buy > 0:
                if pos >= soft:
        ...
        if self.p.get("ENABLE_TAKE_LADDER", True) and can_take_sell:
            ...
            for edge_thr, clip in take_levels:
                if sell_edge >= max(edge_thr, sell_need):
                    take_sell = clip
            if safe_take_jump and pos > 8 and take_signal <= -0.65 and sell_edge >= max(0.60, sell_need - 0.15):
                take_sell = max(take_sell, 3)
            elif safe_take_jump and abs(pos) <= 6 and take_signal <= -0.85 and sell_edge >= max(0.70, sell_need - 0.10):
                take_sell = max(take_sell, 2)
            if take_sell > 0:
                if pos <= -soft:
        ...
        if safe_take_jump:
            pos = mgr.projected()
            if pos > 8 and sell_edge >= 0.40:
                mgr.sell(book.best_bid, min(book.best_bid_vol, 4, max(0, pos - 6)))
            elif pos < -8 and buy_edge >= 0.40:
                mgr.buy(book.best_ask, min(book.best_ask_vol, 4, max(0, 6 - pos)))
