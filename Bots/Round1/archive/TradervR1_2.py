...
class AshCoatedOsmiumBot:
    ...
    MID_WEIGHT = 0.12
    INVENTORY_SKEW = 0.08196997950186136
    DEFAULT_EDGE = 7.946659153660129
    JOIN_EDGE = 2.0
    BASE_ORDER_SIZE = 12
    SOFT_LIMIT = 28
    TAKE_LEVELS = (
...
class IntarianPepperRootBot:
    DRIFT_PER_TIMESTAMP = 0.0011402091923482594
    RESIDUAL_ALPHA = 0.11062621308397591
    SPREAD_ALPHA = 0.08
    BASE_CARRY = 8.243887688442305
    EDGE_TARGET_SCALE = 9.653407692241805
    ZSCORE_BUY_BONUS = 9.696484461852018
    ZSCORE_SELL_PENALTY = 10.372821052960441
    MAX_LONG_TARGET = 56
    MAX_SHORT_TARGET = 24
    INVENTORY_SKEW = 0.10353181664929359
    BASE_TAKE_EDGE = 1.2423622794825406
    BASE_QUOTE_EDGE = 3.0661455259070096
    SOFT_LIMIT = 45
    PASSIVE_SIZE = 10
    MAX_TAKE_SIZE = 14
    BULLISH_IMBALANCE = 0.07173131264646221
    OVEREXTENSION_Z = 0.9121418651205282
    ...
    def fair_value(self) -> float:
        ...
        return (
            ...
            + 0.08 * flow_fair
            + 0.08 * (trend + float(self.product_state["residual_ema"]))
        )
    ...
    def target_position(self, fair: float) -> int:
        ...
        target = self.BASE_CARRY + int(round(self.EDGE_TARGET_SCALE * edge))
        if zscore < 0.0:
            target += int(round(self.ZSCORE_BUY_BONUS * min(1.0, abs(zscore) / 2.0)))
        if zscore > 0.35:
            target -= int(round(self.ZSCORE_SELL_PENALTY * min(1.0, (zscore - 0.35) / 1.4)))
        if self.book.imbalance < -0.18 and self.book.micro < self.book.mid:
            target -= 6
        if self.bullish_state(zscore):
            target += 5
        return int(clamp(float(target), -self.MAX_SHORT_TARGET, self.MAX_LONG_TARGET))
    ...
    def take_edges(self, target: int, zscore: float) -> Tuple[float, float]:
        ...
        sell_edge = self.BASE_TAKE_EDGE + 0.20

        if self.manager.projected_position() < target:
            buy_edge -= 0.20
        if self.manager.projected_position() > target:
            sell_edge -= 0.10
        ...
        if self.bullish_state(zscore) and self.manager.projected_position() > 0:
            sell_edge += 0.35
        return max(0.50, buy_edge), max(0.60, sell_edge)
    ...
    def take_orders(self, reservation: float, target: int, zscore: float) -> None:
        ...
        if buy_edge >= buy_edge_needed and self.manager.buy_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, target + 10 - self.manager.projected_position()),
            )
        ...
        if sell_edge >= sell_edge_needed and self.manager.sell_capacity > 0:
            quantity = min(
                ...
                self.MAX_TAKE_SIZE,
                max(0, self.manager.projected_position() - (target - 8)),
            )
            if self.bullish_state(zscore) and self.manager.projected_position() > 0:
    ...
    def passive_quotes(self, reservation: float, target: int, zscore: float) -> Tuple[Optional[int], Optional[int]]:
        ...
        if self.bullish_state(zscore) and self.manager.projected_position() < target:
        ...
        if self.bullish_state(zscore) and self.manager.projected_position() < target - 6:
    ...
    def run(self) -> Tuple[List[Order], Dict[str, object]]:
        ...
        position = self.manager.projected_position()
        if buy_quote is not None and self.manager.buy_capacity > 0 and position < target + 12:
            quantity = min(buy_size, self.manager.buy_capacity, max(0, target + 12 - position))
            self.manager.add_buy(buy_quote, quantity)

        position = self.manager.projected_position()
        if sell_quote is not None and self.manager.sell_capacity > 0 and position > target - 12:
            quantity = min(sell_size, self.manager.sell_capacity, max(0, position - (target - 12)))
