...
# Archetype: anchored MM with nonlinear inventory pressure (GLFT-inspired)
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        if pos >= soft:
            ...
            buy_qe += 1.0 * inv_ratio
            sell_qe -= 0.4 * inv_ratio
        elif pos < 0:
            buy_qe += 0.4 * inv_ratio
            sell_qe -= 1.0 * inv_ratio
