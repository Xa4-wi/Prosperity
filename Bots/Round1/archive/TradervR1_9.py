...
class AshCoatedOsmiumBot:
    ...
    IMBALANCE_FAIR_WEIGHT = 3.425625169991709
    MICRO_FAIR_WEIGHT = 0.35
    INVENTORY_SKEW = 0.09626693426587984
    BASE_EDGE = 6.963193155612019
    JOIN_EDGE = 2.0
    FRONT_ORDER_SIZE = 8
    BACK_ORDER_SIZE = 5
    SOFT_LIMIT = 28
    ...
    def quote_edges(self, reservation: float) -> Tuple[float, float]:
        del reservation
        ...
        return max(4.8, buy_edge), max(4.8, sell_edge)
    ...
    def passive_quotes(self, reservation: float) -> List[Tuple[str, int, int]]:
        if not self.book.valid:
            return []

        buy_edge, sell_edge = self.quote_edges(reservation)
        ...
        allow_bid = not (self.bid_toxic() and position > 8)
        allow_ask = not (self.ask_toxic() and position < -8)
        ...
        if allow_bid and front_buy < self.book.best_ask and front_buy > 0:
            quotes.append(("buy", front_buy, self.FRONT_ORDER_SIZE))
            if back_buy > 0 and back_buy < self.book.best_ask:
                quotes.append(("buy", back_buy, self.BACK_ORDER_SIZE))
        if allow_ask and front_sell > self.book.best_bid:
            quotes.append(("sell", front_sell, self.FRONT_ORDER_SIZE))
            if back_sell > self.book.best_bid:
                quotes.append(("sell", back_sell, self.BACK_ORDER_SIZE))
        return quotes
    ...
    def run(self) -> List[Order]:
        ...
        for side, price, size in self.passive_quotes(reservation):
            if side == "buy":
