...
DEFAULT_ASH_PARAMS = {
    ...
    "TAKE_L3_EDGE": 8.0,
    "TAKE_L3_SIZE": 16,
    # Fast recycle / low-hold Osmium experiment
    "RECYCLE_TARGET": 0.0,
    "FAST_SOFT_LIMIT": 22.0,
    "RECYCLE_FRONT_SIZE": 13,
    "RECYCLE_BACK_SIZE": 3,
    "OPPOSITE_TAKE_BONUS": 0.9,
    "SAME_TAKE_PENALTY": 0.55,
    "RECYCLE_RESERVATION_SKEW": 0.18,
    "AGE_ALPHA": 0.18,
    "AGE_PENALTY": 1.0,
    "AGE_ONE_SIDED": 5.5,
    "AGE_EXIT_BONUS": 0.75,
...
class AshCoatedOsmiumTrader:
    """
    Fast-paced Osmium recycle branch.
    Keeps the strong local-fair / reservation structure from the trunk, but
    treats inventory as something to recycle quickly instead of carry. The
    engine uses:
    - lower effective soft limits
    - stronger zero-target reservation pull
    - cheaper opposite-side takes
    - smaller same-side passive quoting
    - inventory-age memory so stale inventory gets pushed out faster
    """
    ...
    def _load_state(self, memory: dict) -> dict:
        raw = memory.get("ASH_FAST_STATE", {})
        if not isinstance(raw, dict):
            raw = {}
        return {
            "last_pos": int(raw.get("last_pos", 0)),
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        fast_state = self._load_state(memory)
        if fast_state["last_ts"] >= 0 and timestamp < fast_state["last_ts"]:
            fast_state = {"last_pos": 0, "age": 0.0, "last_ts": -1.0}

        age = float(fast_state["age"])
        last_pos = int(fast_state["last_pos"])
        if position == 0 or last_pos == 0 or position * last_pos <= 0:
            age = 0.0
        else:
            age = ema(age, age + 1.0, float(self.p["AGE_ALPHA"]))

        fair = self._fair_value(book)
        projected = mgr.projected()
        reservation = self._reservation(fair, projected)
        recycle_target = float(self.p["RECYCLE_TARGET"])
        reservation -= (projected - recycle_target) * float(self.p["RECYCLE_RESERVATION_SKEW"])
        if projected > 0:
            reservation -= age * float(self.p["AGE_PENALTY"])
        elif projected < 0:
            reservation += age * float(self.p["AGE_PENALTY"])
        ...
        for edge_thr, clip in [
            ...
            need = max(float(edge_thr), buy_edge_needed)
            if flattening_short:
                need -= float(self.p["OPPOSITE_TAKE_BONUS"]) + min(1.0, age * 0.18) * float(self.p["AGE_EXIT_BONUS"])
            elif pos > 0:
                need += float(self.p["SAME_TAKE_PENALTY"])
            if buy_edge >= need:
                take_buy = int(clip)
        if take_buy > 0:
            pos = mgr.projected()
            if pos >= self.p["FAST_SOFT_LIMIT"]:
        ...
        take_sell = 0
        pos = mgr.projected()
        flattening_long = pos > 0
        for edge_thr, clip in [
            (self.p["TAKE_L1_EDGE"], self.p["TAKE_L1_SIZE"]),
            (self.p["TAKE_L2_EDGE"], self.p["TAKE_L2_SIZE"]),
            (self.p["TAKE_L3_EDGE"], self.p["TAKE_L3_SIZE"]),
        ]:
            need = max(float(edge_thr), sell_edge_needed)
            if flattening_long:
                need -= float(self.p["OPPOSITE_TAKE_BONUS"]) + min(1.0, age * 0.18) * float(self.p["AGE_EXIT_BONUS"])
            elif pos < 0:
                need += float(self.p["SAME_TAKE_PENALTY"])
        ...
        if ask_toxic:
            sell_qe += 1.0
        pos = mgr.projected()
        soft = int(self.p["FAST_SOFT_LIMIT"])
        if pos >= soft:
            buy_qe += 1.4; sell_qe -= 1.0
        elif pos <= -soft:
            buy_qe -= 1.0; sell_qe += 1.4

        # Inventory recycle bias: quote more eagerly on the exit side, less on
        # the carry side, and push harder if we've been stuck in inventory.
        if pos > 0:
            buy_qe += 0.55 + 0.12 * min(age, 6.0)
            sell_qe -= 0.75 + 0.18 * min(age, 6.0)
        elif pos < 0:
            buy_qe -= 0.75 + 0.18 * min(age, 6.0)
            sell_qe += 0.55 + 0.12 * min(age, 6.0)
        ...
        if pos > 0 and age >= float(self.p["AGE_ONE_SIDED"]):
            allow_bid = False
        elif pos < 0 and age >= float(self.p["AGE_ONE_SIDED"]):
            allow_ask = False

        front_sz = int(self.p["RECYCLE_FRONT_SIZE"])
        back_sz = int(self.p["RECYCLE_BACK_SIZE"])
        if pos > 0:
            buy_front_sz = max(4, front_sz - 5)
            sell_front_sz = front_sz + 3
            buy_back_sz = max(0, back_sz - 2)
            sell_back_sz = back_sz + 1
        elif pos < 0:
            buy_front_sz = front_sz + 3
            sell_front_sz = max(4, front_sz - 5)
            buy_back_sz = back_sz + 1
            sell_back_sz = max(0, back_sz - 2)
        else:
            buy_front_sz = sell_front_sz = front_sz
            buy_back_sz = sell_back_sz = back_sz
        if allow_bid and front_buy < book.best_ask and front_buy > 0:
            mgr.buy(front_buy, buy_front_sz)
            if back_buy > 0 and back_buy < book.best_ask and buy_back_sz > 0:
                mgr.buy(back_buy, buy_back_sz)
        if allow_ask and front_sell > book.best_bid:
            mgr.sell(front_sell, sell_front_sz)
            if back_sell > book.best_bid and sell_back_sz > 0:
                mgr.sell(back_sell, sell_back_sz)

        memory["ASH_FAST_STATE"] = {
            "last_pos": int(state.position.get("ASH_COATED_OSMIUM", 0)),
            "age": age,
