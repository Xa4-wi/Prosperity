...
class IntarianPepperRootTrader:
    ...
    def _overlay_top(self, progress: float) -> int:
        return int(round(self._core_target(progress))) + int(self.p["OVERLAY_BAND"])
    ...
    def _trend_features(self, book: Book, timestamp: int, progress: float, state: dict) -> dict:
        ...
        raw_return = 0.0 if prev_mid <= 0.0 else (book.mid - prev_mid)
        base_return = drift * dt
        observed_return = raw_return - base_return
        ...
        steady_mean = float(self.p["STATE_STEADY_MEAN_BONUS"]) + 0.03 * short_gap
        strong_mean = float(self.p["STATE_STRONG_MEAN_BONUS"]) + 0.06 * max(0.0, short_gap)
        noisy_mean = -float(self.p["STATE_NOISY_MEAN_SCALE"]) * max(0.0, zscore - 0.55) * spread_scale * 0.25
        ...
        lookahead_scale = clamp(1.0 + 0.06 * p_steady - 0.18 * p_noisy, 0.75, 1.10)
        lookahead = float(self.p["LOOKAHEAD_BONUS"]) * (1.0 - 0.65 * progress) * lookahead_scale
    ...
    def _execution_mode(
        ...
        if position < target - 12:
            ...
            if cheap and trend_prob >= 0.62 and float(features["p_noisy"]) <= 0.32 and float(features["reload_ema"]) <= 0.0:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if not initialized:
            ...
            initialized = True
        ps["anchor"] = anchor
        ps["reload_ema"] = max(0.0, float(ps["reload_ema"]) - float(self.p["RELOAD_PAUSE_ALPHA"]))
        ...
        overlay_top = self._overlay_top(progress)
        ...
        if action == "aggressive_buy":
            ...
            ps["reload_ema"] = float(self.p["RELOAD_COOLDOWN"])
        ...
        if action == "passive_buy":
            buy_qe = min(buy_qe - 0.35, float(self.p["PASSIVE_BUY_EDGE"]))
        elif action == "hold":
