    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if (
            ...
            if can_take_buy and buy_edge >= max(0.80, buy_need - 0.25):
                mgr.buy(book.best_ask, min(book.best_ask_vol, 3))
        elif (
            sell_reactivation
            and abs(pos) <= max(8, soft - 8)
            and abs(quote_signal) >= 0.28
            ...
            if can_take_sell and sell_edge >= max(0.80, sell_need - 0.25):
                mgr.sell(book.best_bid, min(book.best_bid_vol, 3))
        ...
        if signal_conviction >= 0.95:
            if quote_signal > 0.0:
                ...
                sell_front_sz += 1
        if buy_reactivation:
            buy_front_sz += 2 + int(bars_since_buy_fill >= 12)
            if micro_nibble:
                buy_front_sz += 2
        elif sell_reactivation:
            sell_front_sz += 2 + int(bars_since_sell_fill >= 12)
            if micro_nibble:
                sell_front_sz += 2
        if pos > 6 and bid_level >= 1:
            sell_front_sz += 1
