...
DEFAULT_ASH_PARAMS = {
    ...
    "MARKOUT_ALPHA": 0.20,
    "SOFT_BAD_MARKOUT": -0.48,
    "HARD_BAD_MARKOUT": -0.95,
    "MARKOUT_EDGE_PENALTY": 0.20,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        if buy_reactivation:
            extra = min(0.32, 0.05 * max(0, bars_since_buy_fill - 8))
            buy_qe -= 0.15 + extra
            if micro_nibble:
                buy_qe -= 0.05
        elif sell_reactivation:
            extra = min(0.32, 0.05 * max(0, bars_since_sell_fill - 8))
            sell_qe -= 0.15 + extra
            if micro_nibble:
                sell_qe -= 0.05
