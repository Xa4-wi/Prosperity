...
DEFAULT_ASH_PARAMS = {
    ...
    "TAKE_L3_EDGE": 8.0,
    "TAKE_L3_SIZE": 16,
    # Mild fast-recycle overlay
    "FAST_SOFT_LIMIT": 42.0,
    "RECYCLE_RESERVATION_SKEW": 0.06,
    "OPPOSITE_TAKE_BONUS": 0.45,
    "SAME_TAKE_PENALTY": 0.18,
    "RECYCLE_FRONT_SIZE": 15,
    "RECYCLE_BACK_SIZE": 4,
    "RECYCLE_EXIT_EDGE_BONUS": 0.35,
    "RECYCLE_CARRY_EDGE_PENALTY": 0.20,
    "RECYCLE_CLEAR_CLIP": 2,
    "RECYCLE_CLEAR_EDGE": 0.45,
...
class AshCoatedOsmiumTrader:
    """
    Mild fast-recycle Osmium branch.
    Keeps the trunk fair-value and general market-making behavior, but makes
    inventory slightly cheaper to recycle than to carry.
    """
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        reservation -= mgr.projected() * float(self.p["RECYCLE_RESERVATION_SKEW"])
        ...
        for edge_thr, clip in [
            ...
            if pos < 0:
                need -= float(self.p["OPPOSITE_TAKE_BONUS"])
            elif pos > 0:
        ...
        if pos > 0:
            buy_qe += float(self.p["RECYCLE_CARRY_EDGE_PENALTY"])
            sell_qe -= float(self.p["RECYCLE_EXIT_EDGE_BONUS"])
        elif pos < 0:
            buy_qe -= float(self.p["RECYCLE_EXIT_EDGE_BONUS"])
            sell_qe += float(self.p["RECYCLE_CARRY_EDGE_PENALTY"])
        ...
        buy_front_sz = front_sz
        sell_front_sz = front_sz
        buy_back_sz = back_sz
        sell_back_sz = back_sz
        if pos > 0:
            buy_front_sz = max(6, front_sz - 3)
            buy_back_sz = max(1, back_sz - 2)
            sell_front_sz = front_sz + 1
        elif pos < 0:
            sell_front_sz = max(6, front_sz - 3)
            sell_back_sz = max(1, back_sz - 2)
            buy_front_sz = front_sz + 1
        ...
        # Tiny inventory-clearing cross only when stretched and local edge is
        # basically flat; this frees capacity without turning the engine into a
        # dedicated flatten-at-all-costs trader.
        clear_clip = int(self.p["RECYCLE_CLEAR_CLIP"])
        clear_edge = float(self.p["RECYCLE_CLEAR_EDGE"])
        pos = mgr.projected()
        if pos > soft and abs(book.best_bid - reservation) <= clear_edge and not ask_toxic:
            mgr.sell(book.best_bid, min(clear_clip, book.best_bid_vol))
        elif pos < -soft and abs(reservation - book.best_ask) <= clear_edge and not bid_toxic:
            mgr.buy(book.best_ask, min(clear_clip, book.best_ask_vol))
