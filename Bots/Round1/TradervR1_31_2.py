...
# Archetype: trunk-like anchored MM with smarter bad-state retreat
DEFAULT_ASH_PARAMS = {
    "ENABLED": True,
    "REFERENCE_PRICE": 10000.0,
    "IMBALANCE_WEIGHT": 0.90,
    "MICRO_WEIGHT": 0.30,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        if book.spread_val >= float(self.p["WIDE_SPREAD_LEVEL"]) and not bid_toxic and not ask_toxic:
            ...
            front_sz = max(11, front_sz - 2)
            back_sz = max(0, back_sz - 1)
        if bid_toxic or ask_toxic:
            front_sz = max(10, front_sz - 2)
            back_sz = max(0, back_sz - 2)
