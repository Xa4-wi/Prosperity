...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        # Level-1 sniper only: take tiny stale top-of-book opportunities, do
        # not escalate into larger aggression.
        buy_edge = reservation - book.best_ask
        take_buy = int(self.p["TAKE_L1_SIZE"]) if buy_edge >= max(float(self.p["TAKE_L1_EDGE"]) + 0.4, buy_edge_needed) else 0
        if take_buy > 0:
            pos = mgr.projected()
            if pos >= self.p["SOFT_LIMIT"]:
                take_buy = max(0, take_buy - 4)
            mgr.buy(book.best_ask, min(book.best_ask_vol, take_buy, 4))

        sell_edge = book.best_bid - reservation
        take_sell = int(self.p["TAKE_L1_SIZE"]) if sell_edge >= max(float(self.p["TAKE_L1_EDGE"]) + 0.4, sell_edge_needed) else 0
        if take_sell > 0:
            pos = mgr.projected()
            if pos <= -self.p["SOFT_LIMIT"]:
                take_sell = max(0, take_sell - 4)
            mgr.sell(book.best_bid, min(book.best_bid_vol, take_sell, 4))
