...
class AshCoatedOsmiumTrader:
    ...
    def _inventory_band(self, pos: int) -> str:
        size = abs(pos)
        if size <= 5:
            return "neutral"
        if size <= 20:
    ...
    def _feasible_build_scores(
        ...
        spread_score = 1.0 if 14.0 <= book.spread_val <= 18.0 else (0.5 if book.spread_val <= 20.0 else 0.0)

        long_score = 0.0
        if book.mid <= slow_fair - 0.25:
            long_score += 1.0
        if bid_level == 0:
            long_score += 1.0
        elif bid_level == 1:
        ...
        if book.imbalance > -0.18:
            long_score += 1.0
        if book.micro >= book.mid - 0.25:
            long_score += 1.0
        long_score += spread_score
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        inventory_band = self._inventory_band(pos)
        long_build_score, short_build_score = self._feasible_build_scores(book, slow_fair, bid_level, ask_level)
        long_feasible = long_build_score >= 4.0
        short_feasible = short_build_score >= 4.0
        prev_reserve_side = int(ash_state.get("reserve_side", 0))
        prev_reserve_age = int(ash_state.get("reserve_age", 0))
        reserve_side = sign(pos, 20.0) if abs(pos) > 20 else 0
        if reserve_side == 0:
            reserve_age = 0
        elif reserve_side == prev_reserve_side:
            reserve_age = prev_reserve_age + 1
        else:
            reserve_age = 1
        ...
        if (
            ...
            and inventory_band in ("working", "reserve", "danger")
            and 14.0 <= book.spread_val <= 18.0
        ):
            if (
                pos > 5
                and book.mid >= slow_fair - 0.10
                and ask_level <= 1
                and book.imbalance > -0.22
                and book.micro >= book.mid - 0.35
            ):
                harvest_side = 1
            elif (
                pos < -5
                ...
                harvest_side = -1
        harvest_strength = clamp(
            0.25
            + 0.03 * max(0, no_fill_ticks - 10)
            + 0.04 * min(8, reserve_age),
            0.0,
            0.80,
        ) if harvest_side != 0 else 0.0
        ...
        if inventory_band in ("reserve", "danger"):
            if pos > 0 and not long_feasible:
                can_take_buy = False
                buy_need += 0.35
            elif pos < 0 and not short_feasible:
                can_take_sell = False
                sell_need += 0.35
        if inventory_band == "danger":
            if pos > 0:
                can_take_buy = False
                buy_need += 0.40
            elif pos < 0:
        ...
        if inventory_band in ("reserve", "danger"):
            if pos > 0:
                buy_qe += 0.25
                if not long_feasible:
                    buy_qe += 0.35
                sell_qe -= 0.10
            elif pos < 0:
                sell_qe += 0.25
                if not short_feasible:
                    sell_qe += 0.35
                buy_qe -= 0.10
        if reserve_age >= 8:
            age_pen = min(0.60, 0.05 * (reserve_age - 8))
            if pos > 0:
                buy_qe += 0.15 + age_pen
                sell_qe -= 0.10 + 0.30 * age_pen
            elif pos < 0:
                sell_qe += 0.15 + age_pen
                buy_qe -= 0.10 + 0.30 * age_pen
        if harvest_side > 0:
            sell_qe -= 0.15 + harvest_strength
            buy_qe += 0.10 + 0.20 * harvest_strength
        elif harvest_side < 0:
            buy_qe -= 0.15 + harvest_strength
            sell_qe += 0.10 + 0.20 * harvest_strength
        ...
        if inventory_band == "danger":
            if pos > 0:
                ...
                allow_ask = False
        elif inventory_band == "reserve":
            if pos > 0 and (not long_feasible or reserve_age >= 10 or harvest_side > 0):
                allow_bid = False
            elif pos < 0 and (not short_feasible or reserve_age >= 10 or harvest_side < 0):
                allow_ask = False
        ...
        if signal_conviction >= 0.95:
            if quote_signal > 0.0:
                ...
                sell_front_sz += 1
        if harvest_side > 0:
            sell_front_sz += 1 + int(harvest_strength >= 0.55)
            buy_front_sz = max(2, buy_front_sz - 1)
        elif harvest_side < 0:
            buy_front_sz += 1 + int(harvest_strength >= 0.55)
        ...
        ash_state["last_fill_ts"] = int(last_fill_ts)
        ash_state["no_fill_ticks"] = int(no_fill_ticks)
        ash_state["reserve_side"] = int(reserve_side)
        ash_state["reserve_age"] = int(reserve_age)
        ash_state["inventory_band"] = inventory_band
        ash_state["harvest_side"] = int(harvest_side)
        if prev_vacuum_side in ("bid_only", "ask_only") and prev_vacuum_ticks > 0:
