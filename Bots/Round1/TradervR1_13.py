...
DEFAULT_IPR_PARAMS = {
    ...
    "CORE_TARGET_EARLY": 34.0,
    "CORE_TARGET_LATE": 12.0,
    "LATE_UNWIND_START": 0.82,
    "LATE_UNWIND_Z": 1.05,
    "LATE_UNWIND_EDGE_BONUS": 0.35,
}
...
class IntarianPepperRootTrader:
    ...
    def core_target(self, progress: float) -> int:
        start = float(self.p["CORE_TARGET_EARLY"])
        end = float(self.p["CORE_TARGET_LATE"])
        fade = clamp(progress / 0.95, 0.0, 1.0)
        return int(round((1.0 - fade) * start + fade * end))
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        core_target = self.core_target(progress)
        if zscore <= float(self.p["LATE_UNWIND_Z"]) or progress < float(self.p["LATE_UNWIND_START"]):
            target = max(target, float(core_target))
        ...
        if zscore < -0.45:
            ...
            sell_te -= 0.15
        late_unwind = progress >= float(self.p["LATE_UNWIND_START"]) and zscore >= float(self.p["LATE_UNWIND_Z"])
        if late_unwind:
            sell_te -= float(self.p["LATE_UNWIND_EDGE_BONUS"])
        if bullish:
            buy_te -= 0.10
            sell_te += 0.90
        ...
        if sell_edge_avail >= sell_te and mgr.sell_cap > 0:
            pos = mgr.projected()
            overlay_floor = core_target if late_unwind else (target_int - 4)
            qty = min(book.best_bid_vol, 16, max(0, pos - overlay_floor))
        ...
        if bullish and pos < max(24, target_int - 4):
            allow_sell = False
        if late_unwind and pos > core_target and zscore > float(self.p["LATE_UNWIND_Z"]):
            allow_sell = True
        ...
        if allow_sell and front_sell > book.best_bid:
            if late_unwind and pos > core_target:
                front_sell = max(book.best_bid + 1, front_sell - 1)
