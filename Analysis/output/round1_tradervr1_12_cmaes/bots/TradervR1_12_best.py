...
DEFAULT_ASH_PARAMS = {
    ...
    "IMBALANCE_WEIGHT": 1.0336006878,   # fair += imbalance_weight * book_imbalance
    "MICRO_WEIGHT": 0.35,       # fair += micro_weight * (micro - mid)
    "INVENTORY_SKEW": 0.1204227425,    # reservation -= skew * position
    "BASE_EDGE": 2.0867952982,           # default passive quote edge
    "JOIN_EDGE": 2.0,           # snap quote to existing order when within this
    "FRONT_SIZE": 16.3712535884,
    "BACK_SIZE": 5,
    "SOFT_LIMIT": 70.9548974294,           # reduce aggressiveness above this inventory
    ...
    "ADVERSE_IMBALANCE": 0.2050115168,
    "STRONG_IMBALANCE": 0.1571758778,
    "TOXIC_RETREAT_SIZE": 0.676248181,
    "TOXIC_RETREAT_EDGE": 0.518627298,
    "DISLOCATION_EDGE": 6.2396470008,
    "DISLOCATION_BONUS_SIZE": 2.6119641159,
}
...
DEFAULT_IPR_PARAMS = {
    ...
    # Position targeting
    "BASE_CARRY": 8.102874535,              # always want at least this many units
    "EARLY_LONG_BIAS": 42.9033191096,         # extra target early in day (fades to 0 by 85%)
    ...
    "PASSIVE_SELL_BUFFER": 7.9376559951,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        toxic_edge_bump = float(self.p["TOXIC_RETREAT_EDGE"])
        dislocation_edge = float(self.p["DISLOCATION_EDGE"])
        ...
        for edge_thr, clip in [
            ...
            if buy_edge >= max(float(edge_thr), buy_edge_needed):
                take_buy = int(clip)
        if buy_edge >= dislocation_edge:
            take_buy += int(self.p["DISLOCATION_BONUS_SIZE"])
        if take_buy > 0:
            pos = mgr.projected()
        ...
        for edge_thr, clip in [
            ...
            if sell_edge >= max(float(edge_thr), sell_edge_needed):
                take_sell = int(clip)
        if sell_edge >= dislocation_edge:
            take_sell += int(self.p["DISLOCATION_BONUS_SIZE"])
        if take_sell > 0:
            pos = mgr.projected()
            if pos <= -self.p["SOFT_LIMIT"]:
        ...
        if bid_toxic:
            buy_qe += 1.0 + toxic_edge_bump
        if ask_toxic:
            sell_qe += 1.0 + toxic_edge_bump
        pos = mgr.projected()
        ...
        if bid_toxic or ask_toxic:
            front_sz = max(8, int(round(front_sz * float(self.p["TOXIC_RETREAT_SIZE"]))))
            back_sz = 0
        if bid_toxic and pos > 0:
            allow_bid = False
        if ask_toxic and pos < 0:
            allow_ask = False
        if allow_bid and front_buy < book.best_ask and front_buy > 0:
            mgr.buy(front_buy, front_sz)
            if back_sz > 0 and back_buy > 0 and back_buy < book.best_ask:
                mgr.buy(back_buy, back_sz)
        if allow_ask and front_sell > book.best_bid:
            mgr.sell(front_sell, front_sz)
            if back_sz > 0 and back_sell > book.best_bid:
