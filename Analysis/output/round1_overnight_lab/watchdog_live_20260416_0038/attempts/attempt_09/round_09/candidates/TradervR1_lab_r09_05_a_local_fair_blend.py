...
DEFAULT_ASH_PARAMS = {
    ...
    "ANCHOR_WEIGHT": 0.4124781192,
    "STABLE_MID_WEIGHT": 0.5875218808,
    "WALL_MID_BLEND": 0.3451923368,
    "LOCAL_MICRO_WEIGHT": 0.2389850971,
    "LOCAL_IMBALANCE_BIAS": 0.2988285123,
    "DEPTH_IMPACT_SCALE": 62.1656350334,
    "DEPTH_FLOOR": 8.0,
    "INVENTORY_SKEW": 0.095750567,
    "INVENTORY_CURVE": 3.0768043864,
    "BASE_EDGE": -1.2384326885,          # strongest Osmium aggression tested so far
    "JOIN_EDGE": 1.9371566836,           # snap quote to existing order when within this
    "FRONT_SIZE": 20.5135380295,
    "BACK_SIZE": 4,
    "SOFT_LIMIT": 65.6367888275,           # reduce aggressiveness above this inventory
    # Tiered take: [(min_edge, qty_clip), ...]
    "TAKE_L1_EDGE": -0.3618733755,
    "TAKE_L1_SIZE": 6,
    "TAKE_L2_EDGE": 1.5508016073,
    "TAKE_L2_SIZE": 10,
    "TAKE_L3_EDGE": 5.8556271815,
    "MIN_QUOTE_EDGE": 1.9896143984,
    "TAKE_L3_SIZE": 16,
    # Toxic-book thresholds
    "ADVERSE_IMBALANCE": 0.1907403545,
    "STRONG_IMBALANCE": 0.1574962348,
    # Confidence-gated mode switching
    "NORMAL_BASE_EDGE": 1.0,
    "DEFENSE_BASE_EDGE": 2.1,
    "NORMAL_MIN_QUOTE_EDGE": 3.8,
    "DEFENSE_MIN_QUOTE_EDGE": 4.8,
    "NORMAL_TAKE_L1_EDGE": 1.0,
    "NORMAL_TAKE_L2_EDGE": 4.0,
    "NORMAL_TAKE_L3_EDGE": 7.0,
    "DEFENSE_TAKE_L1_EDGE": 2.0,
    "DEFENSE_TAKE_L2_EDGE": 5.5,
    "DEFENSE_TAKE_L3_EDGE": 8.5,
    "DEFENSE_FRONT_SIZE": 13,
    "DEFENSE_BACK_SIZE": 3,
    "ATTACK_CONFIDENCE_THRESHOLD": 0.64,
    "DEFENSE_CONFIDENCE_THRESHOLD": 0.34,
}
...
class AshCoatedOsmiumTrader:
    ...
    def _confidence(self, book: Book, fair: float) -> float:
        stable_mid = self._stable_mid(book)
        depth = float(book.best_bid_vol + book.best_ask_vol)
        spread = float(book.spread_val)
        micro_gap = abs(book.micro - book.mid)
        anchor_gap = abs(stable_mid - float(self.p["REFERENCE_PRICE"]))
        fair_gap = abs(fair - book.mid)
        ...
        if abs(book.imbalance) <= 0.12:
            score += 0.18
        elif abs(book.imbalance) <= 0.22:
            score += 0.10
        elif abs(book.imbalance) <= 0.30:
            score += 0.04
    ...
    def build_orders(self, state: TradingState) -> List[Order]:
        ...
        confidence = self._confidence(book, fair)
        ...
        calm_book = (
            not bid_toxic
            and not ask_toxic
            and abs(book.imbalance) <= 0.18
            and book.spread_val <= 16
        )
        attack_mode = confidence >= float(self.p["ATTACK_CONFIDENCE_THRESHOLD"]) and calm_book
        defense_mode = (
            confidence <= float(self.p["DEFENSE_CONFIDENCE_THRESHOLD"])
            or (bid_toxic and ask_toxic)
            or book.spread_val >= 19
        )
        attack_w = clamp((confidence - 0.46) / 0.36, 0.0, 1.0)
        defense_w = clamp((0.48 - confidence) / 0.28, 0.0, 1.0)
        if attack_mode:
            attack_w = max(attack_w, 0.9)
        if defense_mode:
            defense_w = max(defense_w, 0.65)
            attack_w *= 0.25
        ...
        take_levels = [
            (
                float(self.p["TAKE_L1_EDGE"]) * attack_w
                + float(self.p["NORMAL_TAKE_L1_EDGE"]) * (1.0 - attack_w) * (1.0 - defense_w)
                + float(self.p["DEFENSE_TAKE_L1_EDGE"]) * defense_w,
                self.p["TAKE_L1_SIZE"],
            ),
            (
                float(self.p["TAKE_L2_EDGE"]) * attack_w
                + float(self.p["NORMAL_TAKE_L2_EDGE"]) * (1.0 - attack_w) * (1.0 - defense_w)
                + float(self.p["DEFENSE_TAKE_L2_EDGE"]) * defense_w,
                self.p["TAKE_L2_SIZE"],
            ),
            (
                float(self.p["TAKE_L3_EDGE"]) * attack_w
                + float(self.p["NORMAL_TAKE_L3_EDGE"]) * (1.0 - attack_w) * (1.0 - defense_w)
                + float(self.p["DEFENSE_TAKE_L3_EDGE"]) * defense_w,
                self.p["TAKE_L3_SIZE"],
            ),
        ]
        ...
        normal_base = float(self.p["NORMAL_BASE_EDGE"])
        defense_base = float(self.p["DEFENSE_BASE_EDGE"])
        base_edge = (
            float(self.p["BASE_EDGE"]) * attack_w
            + normal_base * (1.0 - attack_w) * (1.0 - defense_w)
            + defense_base * defense_w
        )
        ...
        min_quote_edge = (
            float(self.p["MIN_QUOTE_EDGE"]) * attack_w
            + float(self.p["NORMAL_MIN_QUOTE_EDGE"]) * (1.0 - attack_w) * (1.0 - defense_w)
            + float(self.p["DEFENSE_MIN_QUOTE_EDGE"]) * defense_w
        )
        buy_qe = max(min_quote_edge, buy_qe)
        sell_qe = max(min_quote_edge, sell_qe)
        ...
        allow_bid = not (bid_toxic and pos > (4 if defense_mode else 8)) and pos < soft + (3 if attack_mode else 6)
        allow_ask = not (ask_toxic and pos < (-4 if defense_mode else -8)) and pos > -(soft + (3 if attack_mode else 6))

        front_sz = int(round(
            float(self.p["FRONT_SIZE"]) * attack_w
            + float(self.p["FRONT_SIZE"]) * 0.92 * (1.0 - attack_w) * (1.0 - defense_w)
            + float(self.p["DEFENSE_FRONT_SIZE"]) * defense_w
        ))
        back_sz = int(round(
            float(self.p["BACK_SIZE"]) * attack_w
            + float(self.p["BACK_SIZE"]) * 0.9 * (1.0 - attack_w) * (1.0 - defense_w)
            + float(self.p["DEFENSE_BACK_SIZE"]) * defense_w
        ))
