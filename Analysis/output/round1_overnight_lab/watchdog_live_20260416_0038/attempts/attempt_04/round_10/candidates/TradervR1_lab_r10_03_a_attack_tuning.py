...
DEFAULT_ASH_PARAMS = {
    ...
    "INVENTORY_CURVE": 2.2,
    "BASE_EDGE": 2.5,           # default passive quote edge
    "JOIN_EDGE": 2.5406051961,           # snap quote to existing order when within this
    "FRONT_SIZE": 18.7118179601,
    "BACK_SIZE": 4,
    "SOFT_LIMIT": 68.4481743883,           # reduce aggressiveness above this inventory
    # Tiered take: [(min_edge, qty_clip), ...]
    "TAKE_L1_EDGE": 2.4837410115,
    "TAKE_L1_SIZE": 6,
    "TAKE_L2_EDGE": 5.5490623324,
    "TAKE_L2_SIZE": 10,
    "TAKE_L3_EDGE": 7.3027327436,
    "TAKE_L3_SIZE": 16,
    # Toxic-book thresholds
    "ADVERSE_IMBALANCE": 0.184322297,
    "STRONG_IMBALANCE": 0.1686224016,
...
class AshCoatedOsmiumTrader:
    ...
    def build_orders(self, state: TradingState, memory: dict) -> Tuple[List[Order], dict]:
        ...
        p_calm, p_normal, p_toxic = 0.0, 1.0, 0.0
        spread_ema = ema(float(ash_state.get("spread_ema", book.spread_val)), book.spread_val, float(self.p["STATE_ALPHA"]))
        ...
        allow_bid = (
            ...
            and pos < soft + 6
            and not (buy_markout < float(self.p["BAD_MARKOUT"]) and book.imbalance <= 0.0)
        )
        allow_ask = (
            not (ask_toxic and pos < -8)
            and pos > -(soft + 6)
            and not (sell_markout < float(self.p["BAD_MARKOUT"]) and book.imbalance >= 0.0)
        )
